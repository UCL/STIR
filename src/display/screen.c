...
struct color_info sm4[7]={{0,0,0,32},{0,64,94,32},{0,127,0,32},
 {127,0,0,32},{127,127,127,0},{127,127,0,0}};

struct color_info sm4test[7]={{0,0,0,32},{0,0,127,32},{0,127,0,32},
 {127,0,0,32},{127,127,127,0},{127,127,0,0}};

/* change November 1997
   .F values added only up to 126, added 2 to the second entry
   */
struct color_info sm4green[7]={{0,0,0,32},{0,0,127,64},{0,127,0,31},
...
#ifdef TEST
  nmap = XCopyColormapAndFree (mydisplay, XDefaultColormap (mydisplay,
        DefaultScreen (mydisplay)));
#else
  nmap = XCreateColormap(mydisplay,mywindow,
        XDefaultVisual (mydisplay, DefaultScreen (mydisplay)),
        AllocAll);
#endif
  cc.flags = DoRed | DoGreen |DoBlue;
/* XWindows defines RGB values as unsigned 16-bit ints, the color scales
   assume values between 0-127, we have to convert these
*/
#define SCX_Color_conv 60535. / 127
  ...
  for (i=0,k=0;i<size-1;i++)
  {
    ...
    for (j=0;j<x[i].F;j++)
    {
      ...
      cc.pixel = (int)(k++ * (float)(SC_C_MAX-SC_C_BACKGROUND+1)/128)
                 + SC_C_BACKGROUND;
#ifdef TEST
      printf("%d, %d %d:%d:%d\n",k,cc.pixel,cc.red,cc.green,cc.blue);
      if (XAllocColor (mydisplay, nmap, &cc) == 0)
      {
        message ("%s: Error in color alloc %d, value %d\n",procn, k, cc.pixel);
    /*    return (1);*/
      }
#else
      XStoreColor (mydisplay, nmap, &cc);
#endif
    }
  }
  ...
  XSetWindowColormap (mydisplay, mywindow, nmap);
  XSetWindowBorder (mydisplay, mywindow, SC_C_ANNOTATE);
  return (0);
#undef SCX_Color_conv
}
...
/*  XSynchronize (SCX_display, 1);*/
  ...
  XChangeWindowAttributes(SCX_display, SCX_window,
  ...
  /* change November 1997, choose default colormap */
  /*
...
/* change November 1997, moved SCX_SCALE to the bottom and 
   renamed to SC_SCALE */
...
/*  XSynchronize (SCX_display, 1);*/
  ...
  SetColormap(SCX_display, SCX_window,
	      ...
	      all_color_scales[CurrentColormap].size);
/* scale */
  /*  SC_SCALE(SC_X_MAX-20,
            SC_Y_MAX-20,20,SC_Y_MAX-40);*/
  SC_COLOR(SC_C_ANNOTATE);
  SC_MASK(SC_M_ANNOTATE);
}
...
/* change November 1997, added a message here, and cycle through color maps

...
#  if 1
    message("Found pixmap format with depth %d, bits_per_pixel %d, scanline_pad %d.", SCX_pixmap_format.depth,
            SCX_pixmap_format.bits_per_pixel, SCX_pixmap_format.scanline_pad);
#  endif
    XFree(pixmapformats);
  } /* end ZPixmap format */
  ...
  /* create window */
  {
    ...
    SCX_window = XCreateWindow(SCX_display, DefaultRootWindow(SCX_display), SCX_hint->x, SCX_hint->y, (unsigned)SCX_hint->width,
                               (unsigned)SCX_hint->height, 5, SCX_visual_info.depth, InputOutput, SCX_visual_info.visual,
                               CWBackingStore | CWColormap | CWBorderPixel | CWBackingPixel, &attrib);
...
*/
int
SCX_WRITE(int* Xpos_x, int* Xpos_y, char* text) { /* when deleting characters, pieces of the image could disappear ?? */
  SC_MASK(SC_M_ANNOTATE);
...
void SCX_STOP(stop) int stop;
{
  ...
  while (!stop) {
    ...
    case ButtonPress:
      ...
      SetColormap(SCX_display, SCX_window, SCX_visual_info, all_color_scales[CurrentColormap].p,
...
int x_begin, y_begin, lengthX, lengthY;
{
  ...
  myimage = XCreateImage(SCX_display, SCX_visual_info.visual, SCX_pixmap_format.depth, ZPixmap, /*offset */ 0, (char*)local_image,
                         (unsigned)lengthX, (unsigned)lengthY, SCX_pixmap_format.scanline_pad, bytes_per_line);
  if (myimage == NULL) {

...
void
assign_to_subregion(Array<3, elemT>& input_array, const BasicCoordinate<3, int>& mask_location,
                    const BasicCoordinate<3, int>& half_mask_size, const elemT& value) {
  ...
  for (int k = std::max(mask_location[1] - half_mask_size[1], min_k_index);
       k <= std::min(mask_location[1] + half_mask_size[1], max_k_index); ++k) {
    ...
    for (int j = std::max(mask_location[2] - half_mask_size[2], min_j_index);
         j <= std::min(mask_location[2] + half_mask_size[2], max_j_index); ++j) {
      ...
      for (int i = std::max(mask_location[3] - half_mask_size[3], min_i_index);
           i <= std::min(mask_location[3] + half_mask_size[3], max_i_index); ++i)

...
IndexRange<num_dimensions>::IndexRange() : base_type(), is_regular_range(regular_true) {}

template <int num_dimensions>
IndexRange<num_dimensions>::IndexRange(const IndexRange<num_dimensions>& range)
    : base_type(range), is_regular_range(range.is_regular_range) {}

template <int num_dimensions>
IndexRange<num_dimensions>::IndexRange(const base_type& range) : base_type(range), is_regular_range(regular_to_do) {}
...
IndexRange<num_dimensions>::IndexRange(const BasicCoordinate<num_dimensions, int>& min_v,
    ...
    : base_type(min_v[1], max_v[1]), is_regular_range(regular_true) {
...
IndexRange<num_dimensions>::IndexRange(const BasicCoordinate<num_dimensions, int>& sizes)
    : base_type(sizes[1]), is_regular_range(regular_true) {
...
template <int num_dimensions>
bool
IndexRange<num_dimensions>::is_regular() {
  switch (is_regular_range) {
  case regular_true:
    return true;
  case regular_false:
...
IndexRange<1>::IndexRange() : min(0), max(0) {}

IndexRange<1>::IndexRange(const int min_v, const int max_v) : min(min_v), max(max_v) {}

IndexRange<1>::IndexRange(const BasicCoordinate<1, int>& min_v, const BasicCoordinate<1, int>& max_v)
    : min(min_v[1]), max(max_v[1]) {}

IndexRange<1>::IndexRange(const int length) : min(0), max(length - 1) {}

IndexRange<1>::IndexRange(const BasicCoordinate<1, int>& size) : min(0), max(size[1] - 1) {}

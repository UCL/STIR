...
public:
  ...
  /// Set CUDA device
  void set_cuda_device(const char cuda_device) { _cuda_device = cuda_device; }

  /// Set CUDA verbosity
  void set_cuda_verbosity(const bool cuda_verbosity) { _cuda_verbosity = cuda_verbosity; }

  /// Set start time
  void set_start_time(const int start_time) { _start_time = start_time; }

  /// Set stop time
  void set_stop_time(const int stop_time) { _stop_time = stop_time; }

  /// Set listmode binary file
  void set_listmode_binary_file(const std::string& listmode_binary_file) { _listmode_binary_file = listmode_binary_file; }

  /// Set norm binary file
  void set_norm_binary_file(const std::string& norm_binary_file) { _norm_binary_file = norm_binary_file; }

  /// This function does the actual work
  virtual void process_data();

  /// Get prompts
  shared_ptr<const ProjData> get_prompts_sptr() const { return _prompts_sptr; }

  /// Get delayeds
  shared_ptr<const ProjData> get_delayeds_sptr() const { return _delayeds_sptr; }

  /// Get randoms
  shared_ptr<const ProjData> get_randoms_sptr() const { return _randoms_sptr; }

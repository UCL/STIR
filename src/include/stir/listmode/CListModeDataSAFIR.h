...
class CListModeDataSAFIR : public CListModeData {
public:
  /*! Constructor
  \par
  ...
  file
  */
  CListModeDataSAFIR(const std::string& listmode_filename, const std::string& crystal_map_filename,
                     const std::string& template_proj_data_filename, const double lor_randomization_sigma = 0.0);
  ...
  */
  virtual SavedPosition save_get_position() { return static_cast<SavedPosition>(current_lm_data_ptr->save_get_position()); }
  virtual Succeeded set_get_position(const SavedPosition& pos) { return current_lm_data_ptr->set_get_position(pos); }

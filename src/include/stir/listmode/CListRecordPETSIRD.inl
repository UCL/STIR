  ...
  if (it == petsird_to_stir->end())
    {
      ...
      error("get_stir_det_pos_from_PETSIRD_id: PETSIRD id not found in petsird_to_stir map",
            exp_det_bin.module_index,
            exp_det_bin.element_index,
            exp_det_bin.energy_index);
    }
...
CListEventPETSIRD::get_bin(Bin& bin, const ProjDataInfo& proj_data_info) const
{
  ...
  if (scanner_sptr->get_scanner_geometry() == "Cylindrical")
  ...
  else if (scanner_sptr->get_scanner_geometry() == "Generic")
    {
      ...
      //           det_pos_pair.pos1() = get_stir_det_pos_from_PETSIRD_id(exp_det_0);
      //           det_pos_pair.pos2() = get_stir_det_pos_from_PETSIRD_id(exp_det_1);
      //           // std::cout<< exp_det_0.module_index << ", " << exp_det_0.element_index << " ---- " << exp_det_1.module_index

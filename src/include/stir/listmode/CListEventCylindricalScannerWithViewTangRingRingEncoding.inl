...
void
CListEventCylindricalScannerWithViewTangRingRingEncoding<Derived>::get_detection_position(
    DetectionPositionPair<>& det_pos) const {
  ...
  this->get_uncompressed_proj_data_info_sptr()->get_det_num_pair_for_view_tangential_pos_num(det_num_1, det_num_2, view_num,
                                                                                             tangential_pos_num);
...
template <class Derived>
void
CListEventCylindricalScannerWithViewTangRingRingEncoding<Derived>::set_detection_position(
    const DetectionPositionPair<>& det_pos) {
  int tangential_pos_num;
  int view_num;
...
static void
sinogram_coordinates_to_bin(Bin& bin, const int view_num, const int tang_pos_num, const int ring_a, const int ring_b,
...
template <class Derived>
void
CListEventCylindricalScannerWithViewTangRingRingEncoding<Derived>::get_uncompressed_bin(Bin& bin) const {
  unsigned int ring_a;
  unsigned int ring_b;
  this->get_sinogram_and_ring_coordinates(bin.view_num(), bin.tangential_pos_num(), ring_a, ring_b);
  this->get_uncompressed_proj_data_info_sptr()->get_segment_axial_pos_num_for_ring_pair(bin.segment_num(), bin.axial_pos_num(),
                                                                                        ring_a, ring_b);
}

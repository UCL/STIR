...
public:
  ...
  void set_sinogram_and_ring_coordinates(const int view_num, const int tangential_pos_num, const unsigned int ring_a,
...
public:
  ...
  inline unsigned int get_gating() const { return gating; }
  inline Succeeded set_gating(unsigned int g) {
    gating = g & 0xf;
    return gating == g ? Succeeded::yes : Succeeded::no;
  } // TODONK check
private:
...
public:
  CListRecordECAT962()
      : CListEventCylindricalScannerWithViewTangRingRingEncoding<CListRecordECAT962>(
            shared_ptr<Scanner>(new Scanner(Scanner::E962))) {}

  bool is_time() const { return time_data.type == 1U; }
  bool is_gating_input() const { return this->is_time(); }
  bool is_event() const { return time_data.type == 0U; }
  ...
  virtual ListGatingInput& gating_input() { return *this; }
  virtual const ListGatingInput& gating_input() const { return *this; }

  bool operator==(const CListRecord& e2) const {
    return dynamic_cast<CListRecordECAT962 const*>(&e2) != 0 && raw == static_cast<CListRecordECAT962 const&>(e2).raw;
  }

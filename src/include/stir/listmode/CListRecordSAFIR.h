...
/*!
Provides interface of the record class to STIR by implementing get_LOR(). It uses a map from detector indices to coordinates to
specify LORAs2Points from given detection pair indices.
...
public:
  ...
  inline bool is_prompt() const { return !(static_cast<const Derived*>(this)->is_prompt()); }
...
private:
  ...
  detector indices and space coordinates. Always use other constructor with a map pointer. Or use set_map(
  shared_ptr<DetectorCoordinateMapFromFile> new_map ) after default construction.
  */
...
public:
  ...
  //! Returns 0 if event is prompt and 1 if random/delayed
  inline bool is_prompt() const { return !isRandom; }
...
public:
  ...
  virtual bool is_time() const { return time_data.type == 1; }

  virtual bool is_event() const { return time_data.type == 0; }

  virtual CListEvent& event() { return *this; }

  virtual const CListEvent& event() const { return *this; }

  virtual CListEventSAFIR<CListRecordSAFIR>& event_SAFIR() { return *this; }

  virtual const CListEventSAFIR<CListRecordSAFIR>& event_SAFIR() const { return *this; }
  ...
  virtual bool operator==(const CListRecord& e2) const {
    return dynamic_cast<CListRecordSAFIR const*>(&e2) != 0 && raw == static_cast<CListRecordSAFIR const&>(e2).raw;
  }

  inline unsigned long get_time_in_millisecs() const { return time_data.get_time_in_millisecs(); }
  ...
  std::size_t size_of_record_at_ptr(const char* const /*data_ptr*/, const std::size_t /*size*/,

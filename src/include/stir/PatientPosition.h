...
public:
  ...
  PatientPosition() : orientation(unknown_orientation), rotation(unknown_rotation) {
    assert(rotation >= 0);
  ...
  PatientPosition(OrientationValue orientation, RotationValue rotation) : orientation(orientation), rotation(rotation) {
    assert(rotation >= 0);
    assert(rotation <= unknown_rotation);
    assert(orientation >= 0);
    assert(orientation <= unknown_orientation);
  ...
  bool operator==(const PatientPosition& p1) const {
    return this->get_orientation() == p1.get_orientation() && this->get_position() == p1.get_position() &&
  ...
  RotationValue get_rotation() const { return rotation; }
  ...
  OrientationValue get_orientation() const { return orientation; }

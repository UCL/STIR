...
//! default constructor
PlasmaData::PlasmaData() { this->set_is_decay_corrected(false); }
...
PlasmaData::read_plasma_data(const std::string input_string) {
  ...
  if (!data_stream)
    ...
    if (std::getline(data_stream, first_line)) {
      ...
      first_line.erase(std::find_if(first_line.rbegin(), first_line.rend(), std::bind1st(std::not_equal_to<char>(), ' ')).base(),
                       first_line.end());
      first_line.erase(first_line.begin(),
                       std::find_if(first_line.begin(), first_line.end(), std::bind1st(std::not_equal_to<char>(), ' ')));

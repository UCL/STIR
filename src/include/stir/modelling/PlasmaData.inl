...
PlasmaData::get_sample_data_in_frames(TimeFrameDefinitions time_frame_def)
{
  ...
  for (cur_plasma_frame_iter = plasma_data_in_frames.begin(); cur_plasma_frame_iter != plasma_data_in_frames.end();
       ++cur_plasma_frame_iter)
    std::cout << cur_plasma_frame_iter->get_time_in_s() << "		" << cur_plasma_frame_iter->get_plasma_counts_in_kBq()

...
template <int num_conv_points>
void GeneralizedPatlakMatrix<num_conv_points>::read_from_file(const std::string input_string, int num_conv_params) 
{ 
  ...
  max_range[1]=num_conv_params;  max_range[2]=last_frame;
...
template <int num_conv_points>
Succeeded GeneralizedPatlakMatrix<num_conv_points>::write_to_file(const std::string output_string, int num_conv_params) 
{ 
...
template <int num_conv_points> 
void GeneralizedPatlakMatrix<num_conv_points>::
set_model_array(const Array<2,float>& model_array)
{ this->_model_array=model_array ; }

template <int num_conv_points>
Array<2,float> 
GeneralizedPatlakMatrix<num_conv_points>::
...
Array<2,float> 
GeneralizedPatlakMatrix<num_conv_points>::
get_Hfunction_array() const
{  return this->_Hfunction_array ; }

template <int num_conv_points>
void GeneralizedPatlakMatrix<num_conv_points>::
set_Ki_array(const Array<2,float>& Ki_array)
...
template <int num_conv_points>
void GeneralizedPatlakMatrix<num_conv_points>::
set_conv_sample_interval(const unsigned int conv_sampling_interval)
{ 
...
template <int num_conv_points>
void GeneralizedPatlakMatrix<num_conv_points>::
set_prefetched_sampling(const float kloss_start,
...
template <int num_conv_points>
void GeneralizedPatlakMatrix<num_conv_points>::
estimate_inverse_Hfunction(float & kloss_estimate,
						   float & Hfunction_val) const
{
  ...
  for ( unsigned int kloss_index = 1; kloss_index<= this->_kloss_nsamples; ++kloss_index )
	...
	if (Hfunction_val < this->_Hfunction_array[2][kloss_index])
      ...
      if (kloss_index>1)
		...
		// Linear interpolation, if second column of this->_Hfunction_array and Hfunction_val are in linear scale (not implemented now)
        // Effectively Linear-log interpolation if second column of this->_Hfunction_array and Hfunction_val are in log scale	(deafult now)	  
		kloss_estimate=this->_Hfunction_array[1][kloss_index-1] + ((this->_Hfunction_array[1][kloss_index] - this->_Hfunction_array[1][kloss_index-1])/(this->_Hfunction_array[2][kloss_index] - this->_Hfunction_array[2][kloss_index-1]))*(Hfunction_val - this->_Hfunction_array[2][kloss_index-1]);
		}
	  else
		{
		 kloss_estimate=this->_Hfunction_array[1][kloss_index];
...
template <int num_conv_points>
void GeneralizedPatlakMatrix<num_conv_points>::
estimate_denominator_Kifunction(float & Ki_denominator,
						        float & kloss_val) const
{
  ...
  for ( unsigned int kloss_index = 1; kloss_index<= this->_kloss_nsamples; ++kloss_index )
	...
	if (kloss_val == this->_Ki_array[1][kloss_index])
	  ...
	  if (kloss_index>1) 
		...
		Ki_denominator=this->_Ki_array[2][kloss_index-1] + ((this->_Ki_array[2][kloss_index] - this->_Ki_array[2][kloss_index-1])/(this->_Ki_array[1][kloss_index] - this->_Ki_array[1][kloss_index-1]))*(kloss_val - this->_Ki_array[1][kloss_index-1]);
		
		// Fast linear-log interpolation assuming column: this->_Ki_array[2][kloss_index] is in log scale already
		//float log_Ki_denominator=this->_Ki_array[2][kloss_index-1] + ((this->_Ki_array[2][kloss_index] - this->_Ki_array[2][kloss_index-1])/(this->_Ki_array[1][kloss_index] - this->_Ki_array[1][kloss_index-1]))*(kloss_val - this->_Ki_array[1][kloss_index-1]);
		//Ki_denominator=exp(log_Ki_denominator);
		
		// Linear-Log Interpolation
		//float log_Ki_denominator=log(this->_Ki_array[2][kloss_index-1]) + ((log(this->_Ki_array[2][kloss_index]) - log(this->_Ki_array[2][kloss_index-1]))/(this->_Ki_array[1][kloss_index] - this->_Ki_array[1][kloss_index-1]))*(kloss_val - this->_Ki_array[1][kloss_index-1]);
		//Ki_denominator=exp(log_Ki_denominator);
		}
	  else
	    {
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
multiply_dynamic_image_with_model_and_add_to_input(DynamicDiscretisedDensity & impulse_response_image,
      ...
      for ( int j = min_j_index; j<= max_j_index; ++j)
        {
          ...
          for ( int i = min_i_index; i<= max_i_index; ++i)
			...
			for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1] ; ++conv_param_num)
			  for(int frame_num = model_array_min[2];frame_num<=model_array_max[2] ; ++frame_num)				  
				impulse_response_image[conv_param_num][k][j][i]+=this->_model_array[conv_param_num][frame_num]*dynamic_image[frame_num][k][j][i];
			}		  
        }
    }
}
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
multiply_dynamic_image_with_model(DynamicDiscretisedDensity & impulse_response_image,
                                  const DynamicDiscretisedDensity & dynamic_image,
								  int num_conv_params) const
{
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
synthesize_impulse_response_from_parametric_image(DynamicDiscretisedDensity & impulse_response_image,  
      ...
      for ( int j = min_j_index; j<= max_j_index; ++j)
        {
          ...
          for ( int i = min_i_index; i<= max_i_index; ++i)
			...
			for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1]-1 ; ++conv_param_num)
			  ...
			  impulse_response_image[conv_param_num][k][j][i]=parametric_image[k][j][i][1]*exp(-parametric_image[k][j][i][2]*actual_time_point);
			  }
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
multiply_impulse_response_with_model_and_add_to_input(DynamicDiscretisedDensity & dynamic_image,  
      ...
      for ( int j = min_j_index; j<= max_j_index; ++j)
        {
          ...
          for ( int i = min_i_index; i<= max_i_index; ++i)
			for(int frame_num = model_array_min[2];frame_num<=model_array_max[2] ; ++frame_num)  
			  for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1] ; ++conv_param_num)
				dynamic_image[frame_num][k][j][i]+=this->_model_array[conv_param_num][frame_num]*impulse_response_image[conv_param_num][k][j][i];
        }
    }
}
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
multiply_impulse_response_with_model(DynamicDiscretisedDensity & dynamic_image,  
                                     const DynamicDiscretisedDensity & impulse_response_image,
									 int num_conv_params) const
{
      ...
      for ( int j = min_j_index; j<= max_j_index; ++j)
        {
          ...
          for ( int i = min_i_index; i<= max_i_index; ++i)
			...
			for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1]-1 ; ++conv_param_num)
			  {
			  int actual_time_point=(conv_param_num-1)*this->_conv_sampling_interval+1;
			  impulse_response_vector[conv_param_num]=parametric_image[k][j][i][1]*exp(-parametric_image[k][j][i][2]*actual_time_point);
			  }
			impulse_response_vector[num_conv_params]=parametric_image[k][j][i][3];
			for(int frame_num = model_array_min[2];frame_num<=model_array_max[2] ; ++frame_num)  
			  for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1] ; ++conv_param_num)
				dynamic_image[frame_num][k][j][i]+=this->_model_array[conv_param_num][frame_num]*impulse_response_vector[conv_param_num];
			}
        }
    }
   
  //Print out the min and max values of the last voxel impulse response vector
  const float current_min_imp_response =
  *std::min_element(impulse_response_vector.begin(),
   				    impulse_response_vector.end()); 
  const float current_max_imp_response = 
  *std::max_element(impulse_response_vector.begin(),
				    impulse_response_vector.end());
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
estimate_generalized_patlak_parameters_with_impulse_response_and_add_to_input(GeneralizedPatlakVoxelsOnCartesianGrid & parametric_image,
																			  const DynamicDiscretisedDensity & impulse_response_image, 
																			  int num_conv_params ) const
{
  //Initialization
															...
															int num_conv_params ) const
{
  ...
  this->estimate_generalized_patlak_parameters_with_impulse_response_and_add_to_input(parametric_image, impulse_response_image, num_conv_params);
}

template <int num_conv_points>
void 
GeneralizedPatlakMatrix<num_conv_points>::
normalise_parametric_image_with_model_sum( GeneralizedPatlakVoxelsOnCartesianGrid & parametric_image_out,
										   const GeneralizedPatlakVoxelsOnCartesianGrid & parametric_image,
										   int num_conv_params) const
{
  BasicCoordinate<2,int> model_array_min, model_array_max;
...
void 
GeneralizedPatlakMatrix<num_conv_points>::
estimate_nested_loop_parameters_with_model(GeneralizedPatlakVoxelsOnCartesianGrid & parametric_image,  
										  ...
										  int num_conv_params) const
{
  ...
  assert(dynamic_image_nested_loop_estimate.get_time_frame_definitions().get_num_frames()==static_cast<unsigned int> (model_array_max[2]));
  assert(model_array_max[1]-model_array_min[1]+1==num_conv_params);
  ...
  for(int nested_subiterations_num=1; nested_subiterations_num<=num_nested_subiterations; nested_subiterations_num++)
	{
		  ...
		  for ( int j = min_j_index; j<= max_j_index; ++j)
			{
			  ...
			  for ( int i = min_i_index; i<= max_i_index; ++i)
				...
				for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1] ; ++conv_param_num)
				  {
				   if (conv_param_num<model_array_max[1]) 
					 impulse_response_vector[conv_param_num]=parametric_image[k][j][i][1]*exp(-parametric_image[k][j][i][2]*conv_param_num);
				  else
				...
				for(int frame_num = model_array_min[2];frame_num<=model_array_max[2] ; ++frame_num)  
				  ...
				  for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1] ; ++conv_param_num)
					sum_over_conv_param+=impulse_response_vector[conv_param_num]*this->_model_array[conv_param_num][frame_num];
		  ...
		  for ( int j = min_j_index; j<= max_j_index; ++j)
			{
			  ...
			  for ( int i = min_i_index; i<= max_i_index; ++i)
				...
				for(int conv_param_num = model_array_min[1];conv_param_num<=model_array_max[1] ; ++conv_param_num)
				  ...
				  for(int frame_num = model_array_min[2];frame_num<=model_array_max[2] ; ++frame_num)				  
					{
					sum_over_frames+=this->_model_array[conv_param_num][frame_num]*dynamic_image_update_factor[frame_num][k][j][i];
					
					//Also here we calculate the sensitivity of the GeneralizedPatlakMatrix
					//TODO: No need to repeat sensitivity calculation for its voxel. This should be done elsewhere once and then passed to the method
	 ...
	 if (nested_subiterations_num != 1)
	  ...
	  const float current_min_nested_gradient =
	    *std::min_element(impulse_response_update_factor.begin(),
						  impulse_response_update_factor.end()); 
	  const float current_max_nested_gradient = 
        *std::max_element(impulse_response_update_factor.begin(),
						  impulse_response_update_factor.end()); 
	  const float new_min_nested_gradient = 
	    static_cast<float>(min_nested_rel_change);
	  const float new_max_nested_gradient = 
	    static_cast<float>(max_nested_rel_change);
	 ...
	 *std::min_element(impulse_response_estimate.begin(),
					  impulse_response_estimate.end()); 
	 const float current_max_nested_updated_impulse_response = 
	 *std::max_element(impulse_response_estimate.begin(),
					  impulse_response_estimate.end());
	 cerr << "Nested iteration: " << nested_subiterations_num 
	      << " Updated impulse reponse value (min, max) ("
		  << current_min_nested_updated_impulse_response << ", " << current_max_nested_updated_impulse_response << ")" << endl << endl;	
	  ...
	  *std::min_element(parametric_image.begin_all(),
					  parametric_image.end_all()); 
	  const float current_max_nested_updated_image = 
	  *std::max_element(parametric_image.begin_all(),
					  parametric_image.end_all());
	  cerr << "Nested iteration: " << nested_subiterations_num 
	       << " Updated parametric image value (min, max) ("

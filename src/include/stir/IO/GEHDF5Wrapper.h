...
public:
  ...
  */
  Succeeded read_list_data(char* output, std::streampos& current_offset, const hsize_t size) const;
  ...
  Succeeded read_sinogram(Array<3, unsigned char>& output, const std::array<hsize_t, 3>& offset = {0, 0, 0},
  ...
  Succeeded read_geometric_factors(Array<1, unsigned int>& output, const std::array<hsize_t, 2>& offset = {0, 0},
                                   const std::array<hsize_t, 2>& count = {0, 0}, const std::array<hsize_t, 2>& stride = {1, 1});

  Succeeded read_efficiency_factors(Array<1, float>& output, const std::array<hsize_t, 2>& offset = {0, 0},

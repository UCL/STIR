...
It reads a parameter file, which refers to
  - crystal map containing the mapping between detector index triple and cartesian coordinates of the crystal surfaces (see
...
and "SAFIR CListModeData\0". If either is successfull, the class claims it can read the file format. The rest of the file is read
as records as specified as template parameter, e.g. CListRecordSAFIR.
*/
...
public:
  ...
  virtual const std::string get_name() const { return "SAFIR Coincidence Listmode File Format"; }
  ...
  virtual std::unique_ptr<data_type> read_from_file(std::istream& input) const {
  ...
  virtual std::unique_ptr<data_type> read_from_file(const std::string& filename) const {
    info("SAFIRCListmodeInputFileFormat: read_from_file(" + std::string(filename) + ")");
    actual_do_parsing(filename);
    return std::unique_ptr<data_type>(new CListModeDataSAFIR<CListRecordSAFIR>(
...
protected:
  ...
  bool post_processing() {
    if (!file_exists(listmode_filename))
      return true;
    else if (!file_exists(crystal_map_filename))
      return true;

...
public InputFileFormat<CListModeData >
{
 ...
 protected:
  virtual 
    ...
    actual_can_read(const FileSignature& signature,
		    std::istream& input) const
  {
    warning("can_read for ECAT962 listmode data with istream not implemented %s:%s. Sorry",
	  __FILE__, __LINE__);
    return false;
 ...
 public:
    ...
    read_from_file(std::istream& input) const
  {
    ...
    warning("read_from_file for ECAT962 listmode data with istream not implemented %s:%s. Sorry",
	  __FILE__, __LINE__);
    return

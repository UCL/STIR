 ...
 protected:
    ...
    read_from_file(const std::string& filename) const
  {
    ...
    for (int i=1; i<=header.get_num_data_sets(); ++i) {
        ...
        if (i==1)
          {
            _exam_info_sptr = dyn_disc_den_ptr->get_exam_info_sptr()->create_shared_clone();
            _exam_info_sptr->time_frame_definitions.set_num_time_frames(header.get_num_data_sets());
        ...
        _exam_info_sptr->time_frame_definitions.set_time_frame(i, start, end);
        dyn_disc_den_ptr->set_density(*t,i);
    }
    dyn_disc_den_ptr->set_exam_info(*_exam_info_sptr);

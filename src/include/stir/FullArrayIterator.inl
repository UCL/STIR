...
FullArrayIterator<topleveliterT, restiterT, elemT, _Ref, _Ptr>::FullArrayIterator()
    : current_top_level_iter(0), last_top_level_iter(0), current_rest_iter(), last_rest_iter() {}
...
FullArrayIterator<topleveliterT, restiterT, elemT, _Ref, _Ptr>::FullArrayIterator(
    ...
    : current_top_level_iter(0), last_top_level_iter(0), current_rest_iter(0), last_rest_iter(0) {
  assert(param == 0);
}
...
FullArrayIterator<topleveliterT, restiterT, elemT, _Ref, _Ptr>::FullArrayIterator(const topleveliterT& top_level_iter,
    ...
    : current_top_level_iter(top_level_iter), last_top_level_iter(last_top_level_iter), current_rest_iter(rest_iter),
      last_rest_iter(last_rest_iter) {}

template <class topleveliterT, class restiterT, class elemT, class _Ref, class _Ptr>
FullArrayIterator<topleveliterT, restiterT, elemT, _Ref, _Ptr>::FullArrayIterator(
    const FullArrayIterator<topleveliterT, restiterT, elemT, _Ref, _Ptr>& iter2)
    : current_top_level_iter(iter2.current_top_level_iter), last_top_level_iter(iter2.last_top_level_iter),
      current_rest_iter(iter2.current_rest_iter), last_rest_iter(iter2.last_rest_iter) {}

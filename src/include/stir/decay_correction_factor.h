...
decay_correction_factor(const double isotope_halflife, const double start_time, const double end_time) {
  ...
  return std::fabs(lambda * (end_time - start_time)) < .01
             ? std::exp(-start_time * lambda) // if very short frame, we can ignore the duration
             : lambda * (end_time - start_time) / (std::exp(-start_time * lambda) - std::exp(-end_time * lambda));
}
...
*/
inline double
decay_correction_factor(const double isotope_halflife, const double rel_time) {

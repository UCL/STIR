...
/*!
  ...
  The algorithm, its reference, and comments on its implementation are described in Chapter 5 of Dimitra Kyriakopoulou's doctoral
    ...
    : public RegisteredParsingObject<GRD2DReconstruction, Reconstruction<DiscretisedDensity<3, float>>, AnalyticReconstruction>
{
  ...
  typedef RegisteredParsingObject<GRD2DReconstruction, Reconstruction<DiscretisedDensity<3, float>>, AnalyticReconstruction>
      base_type;
...
public:
  ...
  */
  explicit GRD2DReconstruction(const std::string& parameter_filename);

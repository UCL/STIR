...
/*!
 ...
 In 2001 IEEE Nuclear Science Symposium Conference Record, 4:2065–2069. San Diego, CA, USA: IEEE.
 ...
 In 2008 IEEE Nuclear Science Symposium and Medical Imaging Conference (2008 NSS/MIC), 4580–4586. IEEE.
 https://doi.org/10.1109/nssmic.2008.4774352.

 3.  Tahereh Niknejad, Stefaan Tavernier, Joao Varela, and Kris Thielemans,
    <cite>Validation of 3D Model-Based Maximum-Likelihood Estimation of Normalisation Factors for Partial Ring Positron Emission
 Tomography</cite> In 2016 IEEE Nuclear Science Symposium, Medical Imaging Conference and Room-Temperature Semiconductor Detector
 Workshop (NSS/MIC/RTSD), 1–5. <a href="https://doi.org/10.1109/NSSMIC.2016.8069577">DOI: 10.1109/NSSMIC.2016.8069577</a>.
...
void make_det_pair_data(DetPairData& det_pair_data, const ProjDataInfo& proj_data_info, const int segment_num,
...
public:
  GeoData3D();
  GeoData3D(const int num_axial_crystals_per_block, const int half_num_transaxial_crystals_per_block, const int num_rings,
...
shared_ptr<const ProjDataInfoCylindricalNoArcCorr> get_fan_info(int& num_rings, int& num_detectors_per_ring, int& max_ring_diff,
                                                                int& fan_size, const ProjDataInfo& proj_data_info);

void make_fan_data(FanProjData& fan_data, const ProjData& proj_data);

void make_fan_data_remove_gaps(FanProjData& fan_data, const ProjData& proj_data);

void set_fan_data(ProjData& proj_data, const FanProjData& fan_data);

void set_fan_data_add_gaps(ProjData& proj_data, const FanProjData& fan_data);
...
void make_fan_sum_data(Array<2, float>& data_fan_sums, const DetectorEfficiencies& efficiencies, const int max_ring_diff,
...
void iterate_efficiencies(DetectorEfficiencies& efficiencies, const Array<2, float>& data_fan_sums, const int max_ring_diff,
                          const int half_fan_size);

...
template <int num_dimensions, typename elemT>
DiscretisedDensityOnCartesianGrid<num_dimensions, elemT>::DiscretisedDensityOnCartesianGrid(
    const IndexRange<num_dimensions>& range_v, const CartesianCoordinate3D<float>& origin_v,
    const BasicCoordinate<num_dimensions, float>& grid_spacing_v)
    : DiscretisedDensity<num_dimensions, elemT>(range_v, origin_v), grid_spacing(grid_spacing_v) {}

template <int num_dimensions, typename elemT>
DiscretisedDensityOnCartesianGrid<num_dimensions, elemT>::DiscretisedDensityOnCartesianGrid(
    const shared_ptr<const ExamInfo>& exam_info_sptr, const IndexRange<num_dimensions>& range_v,
    const CartesianCoordinate3D<float>& origin_v, const BasicCoordinate<num_dimensions, float>& grid_spacing_v)
    : DiscretisedDensity<num_dimensions, elemT>(exam_info_sptr, range_v, origin_v), grid_spacing(grid_spacing_v) {}
...
DiscretisedDensityOnCartesianGrid<num_dimensions, elemT>::actual_has_same_characteristics(
  ...
  if (norm(other.get_grid_spacing() - this->get_grid_spacing()) > 1.E-4F * norm(this->get_grid_spacing())) {
    char tmp[2000];
    sprintf(tmp, "Not the same grid spacing: (%g,%g,%g) and (%g,%g,%g)", other.get_grid_spacing()[1],
            num_dimensions > 1 ? other.get_grid_spacing()[2] : 0., num_dimensions > 2 ? other.get_grid_spacing()[3] : 0.,
            this->get_grid_spacing()[1], num_dimensions > 1 ? this->get_grid_spacing()[2] : 0.,
...
DiscretisedDensityOnCartesianGrid<num_dimensions, elemT>::actual_get_relative_coordinates_for_indices(
  ...
  return CartesianCoordinate3D<float>(num_dimensions > 2 ? coord[num_dimensions - 2] : 0,
                                      num_dimensions > 1 ? coord[num_dimensions - 1] : 0, coord[num_dimensions]);
}

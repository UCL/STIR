...
class ArrayFunctionObject_1ArgumentImplementation : public ArrayFunctionObject<num_dimensions, elemT> {
public:
  virtual void operator()(Array<num_dimensions, elemT>& array) const { do_it(array); }

  //! result stored in another array, implemented inline
  virtual void inline operator()(Array<num_dimensions, elemT>& out_array, const Array<num_dimensions, elemT>& in_array) const {

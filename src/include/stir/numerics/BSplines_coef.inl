...
#endif
    cplus0(const IterT input_begin_iterator, const IterT input_end_iterator, const constantsT z1, const constantsT precision,
...
void
BSplines_coef(RandIterOut c_begin_iterator, RandIterOut c_end_iterator, IterT input_begin_iterator, IterT input_end_iterator,
  ...
  */
  ...
  if (z1 == 0 && z2 == 0) // Linear and Nearest Neighbour coefficients are the same to the input data
  {
    ...
    *(cplus.begin()) =
        detail::cplus0(input_begin_iterator, input_end_iterator, z1, static_cast<constantsT>(.00001), 0); // k or Nmax_precision

    IIR_filter(cplus.begin(), cplus.end(), input_begin_iterator, input_end_iterator, input_factor_for_cplus.begin(),
               input_factor_for_cplus.end(), pole_for_cplus.begin(), pole_for_cplus.end(), 1);

    *(cminus.end() - 1) = static_cast<out_elemT>((*(cplus.end() - 1) + (*(cplus.end() - 2)) * z1) * z1 / (z1 * z1 - 1));
    IIR_filter(cminus.rbegin(), cminus.rend(), cplus.rbegin(), cplus.rend(), input_factor_for_cminus.begin(),
               input_factor_for_cminus.end(), pole_for_cminus.begin(), pole_for_cminus.end(), 1);

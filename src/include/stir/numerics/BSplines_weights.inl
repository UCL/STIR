...
public:
  ...
  posT derivative_piece(const posT, int) const { return 0; }
  ...
  int find_highest_piece() const { return 0; }
};
...
public:
  ...
  int find_piece(const posT x) const { return static_cast<int>(floor(x)); }
...
public:
  ...
  int find_highest_piece() const { return 1; }
...
BSplines_weights(const posT relative_position, const BSplineType spline_type) {
  ...
  case quintic:
    return (pow(std::max(0., -3. + relative_position), 5) - 6 * pow(std::max(0., -2. + relative_position), 5) +
            ...
            pow(std::max(0., 3. + relative_position), 5)) /
           120.;
  case oMoms:

...
private:
  ...
  inline out_elemT BSplines_product(const int index, const pos_type relative_position, const bool if_deriv) const;

  inline void set_private_values(BSplineType);

  inline out_elemT compute_BSplines_value(const pos_type relative_position, const bool if_deriv) const;

  // sadly,VC6.0 needs definition of template members in the class definition
  template <class IterT>
  inline void set_coef(IterT input_begin_iterator, IterT input_end_iterator) {
    ...
    BSplines_coef(BSplines_coef_vector.begin(), BSplines_coef_vector.end(), input_begin_iterator, input_end_iterator, z1, z2,
                  lambda);
  }
...
public:
  ...
  template <class IterT>
  inline BSplines1DRegularGrid(const IterT input_begin_iterator, const IterT input_end_iterator) {
    set_private_values(cubic);
    set_coef(input_begin_iterator, input_end_iterator);
  }
  //! constructor given a begin_ and end_ iterator as input, estimates the Coefficients
  template <class IterT>
  inline BSplines1DRegularGrid(const IterT input_begin_iterator, const IterT input_end_iterator, const BSplineType this_type) {
    set_private_values(this_type);
    set_coef(input_begin_iterator, input_end_iterator);
  }

  inline BSplines1DRegularGrid(const std::vector<in_elemT>& input_vector, const BSplineType this_type);

  //! destructor
  inline ~BSplines1DRegularGrid();

  inline out_elemT BSplines(const pos_type relative_position) const;

  inline out_elemT BSplines_1st_der(const pos_type relative_position) const;

  //! same as BSplines()
  inline const out_elemT operator()(const pos_type relative_position) const;

  inline const std::vector<out_elemT>
  ...
  inline const std::vector<out_elemT> BSplines_output_sequence(std::vector<pos_type> output_relative_position);
};
} // namespace BSpline

...
inline Succeeded
absolute_max_eigenvector_using_power_method(elemT& max_eigenvalue, Array<1, elemT>& max_eigenvector, const Array<2, elemT>& m,
                                            const Array<1, elemT>& start, const double tolerance = .01,
...
template <class elemT>
inline Succeeded
absolute_max_eigenvector_using_shifted_power_method(elemT& max_eigenvalue, Array<1, elemT>& max_eigenvector,
                                                    const Array<2, elemT>& m, const Array<1, elemT>& start, const elemT shift,
  ...
  Succeeded success = absolute_max_eigenvector_using_power_method(
      max_eigenvalue, max_eigenvector,
      // sadly need to explicitly convert result of subtraction back to Array
      Array<2, elemT>(m - diagonal_matrix(static_cast<unsigned>(m.size()), shift)), start, tolerance, max_num_iterations);
...
template <class elemT>
inline Succeeded
max_eigenvector_using_power_method(elemT& max_eigenvalue, Array<1, elemT>& max_eigenvector, const Array<2, elemT>& m,
                                   const Array<1, elemT>& start, const double tolerance = .03,
                                   const unsigned long max_num_iterations = 10000UL) {
  ...
  success = absolute_max_eigenvector_using_shifted_power_method(max_eigenvalue, max_eigenvector, m, start, max_eigenvalue,
                                                                tolerance, max_num_iterations);
  if (success == Succeeded::no)
    return Succeeded::no;

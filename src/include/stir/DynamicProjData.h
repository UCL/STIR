...
*/
class DynamicProjData : public MultipleProjData {
public:
  static unique_ptr<DynamicProjData> read_from_file(const std::string& filename);

  DynamicProjData() {}

  DynamicProjData(const MultipleProjData& m) : MultipleProjData(m) {}

  DynamicProjData(const shared_ptr<const ExamInfo>& exam_info_sptr) : MultipleProjData(exam_info_sptr) {}

  DynamicProjData(const shared_ptr<const ExamInfo>& exam_info_sptr, const int num_gates)
      : MultipleProjData(exam_info_sptr, num_gates) {}
  ...
  */
  ...
  unsigned int get_num_frames() const { return this->get_num_proj_data(); }
    ...
    for (unsigned int frame_num = 1; frame_num <= this->get_time_frame_definitions().get_num_frames(); ++frame_num)
      for (int segment_num = (this->_proj_datas[frame_num - 1])->get_min_segment_num();
        ...
        for (int timing_pos_num = (this->_proj_datas[frame_num - 1])->get_min_tof_pos_num();
             timing_pos_num <= (this->_proj_datas[frame_num - 1])->get_max_tof_pos_num(); ++timing_pos_num) {
          ...
          if ((*(this->_proj_datas[frame_num - 1])).set_segment(segment_by_view) == Succeeded::no) {
...
template <typename iterT>
void
fill_from(DynamicProjData& stir_object, iterT iter, iterT /*iter_end*/) {

...
template <class T>
VectorWithOffset<T>::VectorWithOffset() : _owns_memory_for_data(true) {
  pointer_access = false;
  this->init();
}

template <class T>
VectorWithOffset<T>::VectorWithOffset(const int hsz) : length(hsz), start(0), pointer_access(false), _owns_memory_for_data(true) {
  if ((hsz > 0)) {
    num = new T[hsz];
...
VectorWithOffset<T>::VectorWithOffset(const int min_index, const int max_index)
    : length(static_cast<unsigned>(max_index - min_index) + 1), start(min_index), pointer_access(false),
...
VectorWithOffset<T>::VectorWithOffset(const int sz, T* const data_ptr, T* const end_of_data_ptr)
    : length(static_cast<unsigned>(sz)), start(0), pointer_access(false), _owns_memory_for_data(false) {
...
template <class T>
VectorWithOffset<T>::VectorWithOffset(const VectorWithOffset& il) : pointer_access(false), _owns_memory_for_data(true) {
  this->init();

...
public:
  ...
  explicit inline ReconstructionTests(const std::string& proj_data_filename = "", const std::string& density_filename = "");

  ~ReconstructionTests() override {}

  //! default proj_data_info
  virtual inline std::unique_ptr<ProjDataInfo> construct_default_proj_data_info_uptr() const;
...
template <class TargetT>
ReconstructionTests<TargetT>::ReconstructionTests(const std::string& proj_data_filename, const std::string& density_filename)
...
ReconstructionTests<TargetT>::construct_default_proj_data_info_uptr() const
{
...
template <class TargetT>
void
ReconstructionTests<TargetT>::construct_input_data()
{
  Verbosity::set(1);
  if (this->_proj_data_filename.empty())
    ...
    {
      shared_ptr<ProjData> proj_data_sptr = ProjData::read_from_file(this->_proj_data_filename);
  ...
  if (this->_input_density_filename.empty())
    {
      ...
      shared_ptr<VoxelsOnCartesianGrid<float>> vox_sptr(new VoxelsOnCartesianGrid<float>(
          this->_proj_data_sptr->get_exam_info_sptr(), *this->_proj_data_sptr->get_proj_data_info_sptr(), zoom, origin));
  ...
  // forward project
  {
    ...
    fwd_proj_sptr->set_up(this->_proj_data_sptr->get_proj_data_info_sptr(), this->_input_density_sptr);
...
template <class TargetT>
void
ReconstructionTests<TargetT>::reconstruct(shared_ptr<TargetT> target_sptr)
{
...
template <class TargetT>
void
ReconstructionTests<TargetT>::compare(const shared_ptr<const TargetT> output_sptr)
{
  if (!check(this->_input_density_sptr->has_same_characteristics(*output_sptr), "output image has wrong characteristics"))
    return;

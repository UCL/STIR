...
*/
class BackProjectorByBin : public TimedObject, public RegisteredObject<BackProjectorByBin> {
public:
  ...
  */
  ...
  void back_project(DiscretisedDensity<3, float>&, const RelatedViewgrams<float>&);

  /*! \brief projects the specified range of the viewgrams into the volume
   it adds to the data already present in the volume.*/
  void back_project(DiscretisedDensity<3, float>&, const RelatedViewgrams<float>&, const int min_axial_pos_num,
  ...
  /*! \brief Get output
   This will overwrite the array-content of the argument with the result of all backprojections since calling
   `start_accumulating_in_new_target()`. Note that the argument has to have the same characteristics as what was used when calling
   `set_up()`.
  */
...
protected:
  ...
  */
  virtual void actual_back_project(DiscretisedDensity<3, float>&, const RelatedViewgrams<float>&, const int min_axial_pos_num,
...
private:
  ...
  void do_segments(DiscretisedDensity<3, float>& image, const ProjData& proj_data_org, const int start_segment_num,
                   const int end_segment_num, const int start_axial_pos_num, const int end_axial_pos_num,

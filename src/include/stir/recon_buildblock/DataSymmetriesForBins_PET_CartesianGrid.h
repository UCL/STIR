...
public:
  ...
  */
  DataSymmetriesForBins_PET_CartesianGrid(const shared_ptr<const ProjDataInfo>& proj_data_info_ptr,
                                          ...
                                          const bool do_symmetry_swap_segment = true, const bool do_symmetry_swap_s = true,
  ...
  inline void get_related_bins_factorised(std::vector<AxTangPosNumbers>&, const Bin& b, const int min_axial_pos_num,
                                          const int max_axial_pos_num, const int min_tangential_pos_num,
                                          const int max_tangential_pos_num) const;

  inline int num_related_bins(const Bin& b) const;

  inline unique_ptr<SymmetryOperation> find_symmetry_operation_from_basic_bin(Bin&) const;

  inline bool find_basic_bin(Bin& b) const;

  inline int num_related_view_segment_numbers(const ViewSegmentNumbers& vs) const;

  inline void get_related_view_segment_numbers(std::vector<ViewSegmentNumbers>& rel_vs, const ViewSegmentNumbers& vs) const;

  inline bool find_basic_view_segment_numbers(ViewSegmentNumbers& v_s) const;
...
private:
  ...
  inline bool find_basic_bin(int& segment_num, int& view_num, int& axial_pos_num, int& tangential_pos_num) const;

...
public:
  /// Default constructor
  NiftyPETHelper() : _already_set_up(false), _span(-1), _devid(0), _att(-1), _scanner_type(Scanner::Unknown_scanner) {}
  ...
  void lm_to_proj_data(shared_ptr<ProjData>& prompts_sptr, shared_ptr<ProjData>& delayeds_sptr,
                       shared_ptr<ProjData>& randoms_sptr, shared_ptr<ProjData>& norm_sptr, const int tstart, const int tstop,
...
private:
  ...
  /// Permute the data
  void permute(std::vector<float>& output_array, const std::vector<float>& orig_array, const unsigned output_dims[3],

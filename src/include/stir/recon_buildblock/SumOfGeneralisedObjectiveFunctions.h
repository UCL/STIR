...
class SumOfGeneralisedObjectiveFunctions : public ParentT
{
...
public:
  ...
  inline SumOfGeneralisedObjectiveFunctions(IterT begin, IterT end);

  inline virtual ~SumOfGeneralisedObjectiveFunctions();

  template <typename IterT>
  inline void set_functions(IterT begin, IterT end);
  ...
  /*! Will call set_up() for all terms in the sum, but will stop as soon as
      one set_up() fails.
  */
  inline virtual Succeeded set_up(shared_ptr<TargetT> const& target_sptr);
   ...
   */
  inline virtual void
  compute_sub_gradient_without_penalty(TargetT& gradient, const TargetT& current_estimate, const int subset_num);

  inline virtual double actual_compute_objective_function_without_penalty(const TargetT& current_estimate, const int subset_num);
  ...
  inline virtual int set_num_subsets(const int num_subsets);
...
protected:
  ...
  /*!
     \todo doc subset
  */
  inline virtual bool actual_subsets_are_approximately_balanced(std::string& warning_message) const;
};

...
float
DataSymmetriesForBins_PET_CartesianGrid::get_num_planes_per_axial_pos(const int segment_num) const
{
  return static_cast<float>(num_planes_per_axial_pos[segment_num]);
}

float
DataSymmetriesForBins_PET_CartesianGrid::get_num_planes_per_scanner_ring() const
{
  return static_cast<float>(num_planes_per_scanner_ring);
}

float
DataSymmetriesForBins_PET_CartesianGrid::get_axial_pos_to_z_offset(const int segment_num) const
{
  return axial_pos_to_z_offset[segment_num];
}

int
DataSymmetriesForBins_PET_CartesianGrid::find_transform_z(const int segment_num, const int axial_pos_num) const
{
  ...
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "Cylindrical")
    {
      ...
      // Z+Q = 2*centre_of_LOR_in_image_coordinates == transform_z
      {
        ...
        const float transform_z_float = (2 * num_planes_per_axial_pos[segment_num] * (axial_pos_num)
                                         + num_planes_per_scanner_ring * delta + 2 * axial_pos_to_z_offset[segment_num]);
...
SymmetryOperation*
DataSymmetriesForBins_PET_CartesianGrid::find_sym_op_bin0(int segment_num, int view_num, int axial_pos_num) const
{
  // cylindrical implementaion
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "Cylindrical")
    {
      // note: if do_symmetry_shift_z==true, then basic axial_pos_num will be 0
      const int transform_z = find_transform_z(abs(segment_num), do_symmetry_shift_z ? 0 : axial_pos_num);
      ...
      const int z_shift = do_symmetry_shift_z ? num_planes_per_axial_pos[segment_num] * axial_pos_num : 0;
      ...
      if (do_symmetry_90degrees_min_phi && view_num > view90 && view_num <= view135)
        { //(90, 135 ]
          if (!do_symmetry_swap_segment || segment_num >= 0)
            ...
            return new SymmetryOperation_PET_CartesianGrid_swap_xmy_yx_zq(
                view180, axial_pos_shift, z_shift, transform_z); // seg < 0
        }
      else if (do_symmetry_90degrees_min_phi && view_num > view45 && view_num <= view90)
        { // [ 45,  90]
          if (!do_symmetry_swap_segment || segment_num >= 0)
            return new SymmetryOperation_PET_CartesianGrid_swap_xy_yx_zq(view180, axial_pos_shift, z_shift, transform_z);
          else
            return new SymmetryOperation_PET_CartesianGrid_swap_xy_yx(
                view180, axial_pos_shift, z_shift); // seg < 0   //KT????????????  different for view90, TODO
        }
      else if (do_symmetry_180degrees_min_phi && view_num > view90 /* && view_num <= view180 */)
        { // (135, 180) but (90,180) for reduced symmetry case
          if (!do_symmetry_swap_segment || segment_num >= 0)
  ...
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "BlocksOnCylindrical")
    {
      if (do_symmetry_90degrees_min_phi || do_symmetry_swap_segment || do_symmetry_swap_s || do_symmetry_180degrees_min_phi)
        {
      ...
      if (do_symmetry_shift_z)
        {
          ...
          const int z_shift = num_planes_per_axial_pos[segment_num] * (axial_pos_num - basic_bin.axial_pos_num());
...
// from symmetries
SymmetryOperation*
DataSymmetriesForBins_PET_CartesianGrid::find_sym_op_general_bin(int s, int segment_num, int view_num, int axial_pos_num) const
{
  // cylindrical implementaion
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "Cylindrical")
    {
          ...
          if (!do_symmetry_swap_segment || segment_num > 0)
            { // pos_plus90
              if (!do_symmetry_swap_s || s > 0)
                ...
                return new SymmetryOperation_PET_CartesianGrid_swap_xy_ymx_zq(
          ...
          if (!do_symmetry_swap_segment || segment_num > 0)
          ...
          else // segment_num == 0
            {
              if (!do_symmetry_swap_s || s > 0)
               ...
               && view_num > view90 /* && view_num <= view180 */) // (135, 180) but (90,180) for reduced symmetry case
        {
          if (!do_symmetry_swap_segment || segment_num > 0)
            {
              if (!do_symmetry_swap_s || s > 0)
...
bool
DataSymmetriesForBins_PET_CartesianGrid::find_basic_view_segment_numbers(ViewSegmentNumbers& v_s) const
{
  bool change = false;
  // TODO get rid of next 2 restrictions
  ...
  if (do_symmetry_swap_segment && v_s.segment_num() < 0)
    {
      v_s.segment_num() = -v_s.segment_num();
      change = true;
    }
  ...
  if (do_symmetry_90degrees_min_phi)
    {
      ...
      if (v_s.view_num() >= view135)
        {
          v_s.view_num() = num_views - v_s.view_num();
          return true;
        }
      else if (v_s.view_num() >= view90)
        {
          v_s.view_num() = v_s.view_num() - view90;
          return true;
        }
      else if (v_s.view_num() > view45)
        {
          v_s.view_num() = view90 - v_s.view_num();
          return true;
        }
    }
  else if (do_symmetry_180degrees_min_phi)
...
bool
DataSymmetriesForBins_PET_CartesianGrid::find_basic_bin(
    int& segment_num, int& view_num, int& axial_pos_num, int& tangential_pos_num, int& timing_pos_num) const
{
  bool change = false;
  // cylindrical implementaion
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "Cylindrical")
          ...
          if (axial_crys_diff >= 0)
            { // seg_num > =0
              if (axial_crys_diff % num_axial_crys_per_block == 0)
                { // In this case, axial block difference can be only equal to axial_crys_diff/num_axial_crys_per_block. So we
                  // only have one type of related bins
  ...
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "Generic")
    { // same procedure as for the block implementation above
      const ProjDataInfoGeneric* proj_data_info_gen_ptr = static_cast<const ProjDataInfoGeneric*>(proj_data_info_ptr.get());
      if (do_symmetry_shift_z)
        {
          int ring1, ring2;
          ...
          if (axial_crys_diff >= 0)
            { // seg_num > 0
              if (axial_crys_diff % num_axial_crys_per_block == 0)
                { // axial block difference can be only axial_crys_diff/num_axial_crys_per_block. So we only have one type of
                  // related bins
                  // basic bin will be the first lor from the corresponding group
                  ring1 = (ring1 / num_axial_crys_per_block) * num_axial_crys_per_block;
                  ring2 = ring1 + axial_crys_diff;
...
bool
DataSymmetriesForBins_PET_CartesianGrid::find_basic_bin(Bin& b) const
{
...
DataSymmetriesForBins_PET_CartesianGrid::find_symmetry_operation_from_basic_bin(Bin& b) const
{
...
int
DataSymmetriesForBins_PET_CartesianGrid::num_related_view_segment_numbers(const ViewSegmentNumbers& vs) const
{
  ...
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "BlocksOnCylindrical")
    {
      ...
      if (do_symmetry_shift_z)
        {
          int ring1, ring2;
          proj_data_info_blk_ptr->get_ring_pair_for_segment_axial_pos_num(ring1, ring2, b.segment_num(), b.axial_pos_num());
          ...
          if (axial_blk_diff == axial_crys_diff / num_axial_crys_per_block)
            {
              num = num_axial_crys_per_block - abs(axial_crys_diff) % num_axial_crys_per_block;
            }
          else if (axial_blk_diff > axial_crys_diff / num_axial_crys_per_block)
            {
...
void
DataSymmetriesForBins_PET_CartesianGrid::get_related_bins_factorised(std::vector<AxTangPosNumbers>& ax_tang_poss,
                                                                     const Bin& b,
  ...
  if (proj_data_info_ptr->get_scanner_ptr()->get_scanner_geometry() == "Cylindrical")
    {
      for (int axial_pos_num = do_symmetry_shift_z ? min_axial_pos_num : b.axial_pos_num();
           ...
           ++axial_pos_num)
        {
          if (b.tangential_pos_num() >= min_tangential_pos_num && b.tangential_pos_num() <= max_tangential_pos_num)
            ax_tang_poss.push_back(AxTangPosNumbers(axial_pos_num, b.tangential_pos_num()));
          if (do_symmetry_swap_s && b.tangential_pos_num() != 0 && -b.tangential_pos_num() >= min_tangential_pos_num
...
void
DataSymmetriesForBins_PET_CartesianGrid::get_related_view_segment_numbers(std::vector<ViewSegmentNumbers>& rel_vs,
                                                                          const ViewSegmentNumbers& vs) const
{
#ifndef NDEBUG
  {
  ...
  const bool symz = do_symmetry_swap_segment && (segment_num != 0);
  ...
  if (do_symmetry_180degrees_min_phi && do_symmetry_90degrees_min_phi && (view_num % (num_views / 2)) != num_views / 4)
    {
      const int related_view_num = view_num < num_views / 2 ? view_num + num_views / 2 : view_num - num_views / 2;
  ...
  if (do_symmetry_90degrees_min_phi && (view_num % (num_views / 4)) != 0)
    {
      ...
      const int related_view_num = (num_views / 2 - view_num + num_views) % num_views;
      rel_vs.push_back(ViewSegmentNumbers(related_view_num, segment_num));
      if (symz)
        rel_vs.push_back(ViewSegmentNumbers(related_view_num, -segment_num));
    }

  assert(rel_vs.size() == static_cast<unsigned>(num_related_view_segment_numbers(vs)));
}

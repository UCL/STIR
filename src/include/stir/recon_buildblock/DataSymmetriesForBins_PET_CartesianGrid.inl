...
DataSymmetriesForBins_PET_CartesianGrid::find_transform_z(const int segment_num, const int axial_pos_num) const {
  const ProjDataInfoCylindrical* proj_data_info_cyl_ptr = static_cast<const ProjDataInfoCylindrical*>(proj_data_info_ptr.get());
  ...
  if (do_symmetry_90degrees_min_phi && view_num > view90 && view_num <= view135) { //(90, 135 ]
    ...
    if (!do_symmetry_swap_segment || segment_num >= 0)
      ...
      return new SymmetryOperation_PET_CartesianGrid_swap_xy_yx(
          view180, axial_pos_num, z_shift); // seg < 0   //KT????????????  different for view90, TODO
...
DataSymmetriesForBins_PET_CartesianGrid::find_sym_op_general_bin(int s, int segment_num, int view_num, int axial_pos_num) const {
             ...
             view_num > view90 /* && view_num <= view180 */) // (135, 180) but (90,180) for reduced symmetry case
  {
    ...
    // /*else{   if ( !do_symmetry_swap_s || s > 0 ) return new SymmetryOperation_PET_CartesianGrid_swap_xmx();	else 	return new
    // SymmetryOperation_PET_CartesianGrid_swap_ymy(view180, axial_pos_num, z_shift);}*/
  } else {
    assert(!do_symmetry_90degrees_min_phi || (view_num >= view0 && view_num <= view45));
    assert(!do_symmetry_180degrees_min_phi || (view_num >= view0 && view_num <= view90));
    if (!do_symmetry_swap_segment || segment_num > 0) {
...
bool
DataSymmetriesForBins_PET_CartesianGrid::find_basic_bin(int& segment_num, int& view_num, int& axial_pos_num,
  ...
  if (do_symmetry_swap_s && tangential_pos_num < 0) {
    tangential_pos_num = -tangential_pos_num;
    change = true;
  };
...
DataSymmetriesForBins_PET_CartesianGrid::find_basic_bin(Bin& b) const {
  return find_basic_bin(b.segment_num(), b.view_num(), b.axial_pos_num(), b.tangential_pos_num());
}
...
void
DataSymmetriesForBins_PET_CartesianGrid::get_related_bins_factorised(std::vector<AxTangPosNumbers>& ax_tang_poss, const Bin& b,
  ...
  for (int axial_pos_num = do_symmetry_shift_z ? min_axial_pos_num : b.axial_pos_num();
       axial_pos_num <= (do_symmetry_shift_z ? max_axial_pos_num : b.axial_pos_num()); ++axial_pos_num) {

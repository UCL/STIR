...
initialise_keymap()
{
  ...
  this->parser.add_start_key("PoissonNestedLogLikelihoodWithLinearKineticModelAndDynamicProjectionData Parameters");
  this->parser.add_stop_key("End PoissonNestedLogLikelihoodWithLinearKineticModelAndDynamicProjectionData Parameters");
...
{  // call actual_subsets_are_approximately_balanced( for first single_frame_obj_func )
  if (this->_patlak_plot_sptr->get_time_frame_definitions().get_num_frames() == 0 || this->_single_frame_obj_funcs.size() == 0)
    error("PoissonNestedLogLikelihoodWithLinearKineticModelAndDynamicProjectionData:\n"
    ...
    warning("Something strange happened in PoissonNestedLogLikelihoodWithLinearKineticModelAndDynamicProjectionData:\n"
...
set_up_before_sensitivity(shared_ptr<const TargetT > const& target_sptr)
{
	...
	
  {
	...
	//Computes model sensitivity image by utilizing Patlak plot model matrix	
                               ...
                               const bool add_sensitivity)
{
  ...
  for(unsigned int frame_num=this->_patlak_plot_sptr->get_starting_frame();frame_num<=this->_patlak_plot_sptr->get_time_frame_definitions().get_num_frames();++frame_num)
    std::fill(dyn_image_estimate[frame_num].begin_all(),
  	...
  	//Print out the min and max values of the initial parametric estimate
										   ...
										   DynamicDiscretisedDensity &dyn_image_nested_loop_estimate)											 
{
  ...
  for(nested_subiterations_num=1; nested_subiterations_num<=this->num_nested_subiterations; nested_subiterations_num++)
  {
	...
	this->_patlak_plot_sptr->get_dynamic_image_from_parametric_image(dyn_image_nested_loop_estimate,
	                                                                 current_estimate); 
...
compute_model_sensitivity_image(const TargetT& param_image)
{
  ...
  this->model_sensitivity_image_sptr=param_image_sptr;
  
  //Initialize model sensitivity image
  std::fill(model_sensitivity_image_sptr->begin_all(),
  ...
  this->_patlak_plot_sptr->multiply_dynamic_image_with_model_gradient(*this->model_sensitivity_image_sptr,
...
add_subset_sensitivity(TargetT& model_sensitivity, const int subset_num) const
{
  ...
  this->_patlak_plot_sptr->multiply_dynamic_image_with_model_gradient_and_add_to_input(model_sensitivity,
																						dyn_image_of_all_ones);
...
{
  {
    ...
    if (!input.has_same_characteristics(this->get_sensitivity(), 
                                        explanation))
      {
        warning("PoissonNestedLogLikelihoodWithLinearKineticModelAndDynamicProjectionData:\n"

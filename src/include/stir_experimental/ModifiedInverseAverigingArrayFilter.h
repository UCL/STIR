...
#include "stir_experimental/SeparableLowPassArrayFilter2.h"
...
public:
  ...
  ModifiedInverseAverigingArrayFilter(const VectorWithOffset<elemT>& filter_coefficients, const float kapa0_over_kapa1);

  // temporary (?) member
  IndexRange<num_dimensions> get_kernel_index_range() const { return kernel_index_range; }

...
#ifndef __stir_NonseparableSpatiallyVaryingFilters3D_H__
...
class NonseparableSpatiallyVaryingFilters3D
    : public RegisteredParsingObject<NonseparableSpatiallyVaryingFilters3D<elemT>,
                                     ...
                                     DataProcessor<DiscretisedDensity<num_dimensions, elemT>>> {
private:
  typedef RegisteredParsingObject<NonseparableSpatiallyVaryingFilters3D<elemT>,
                                  DataProcessor<DiscretisedDensity<num_dimensions, elemT>>,
                                  DataProcessor<DiscretisedDensity<num_dimensions, elemT>>>
      base_type;
...
public:
  ...
  NonseparableSpatiallyVaryingFilters3D(string proj_data_filename, string attenuation_proj_data_filename,
                                        const Array<3, float>& filter_coefficients, shared_ptr<ProjData> proj_data_ptr,
                                        ...
                                        DiscretisedDensity<3, float>* normalised_bck_image, int mask_size,
                                        float rescaling_coefficient);

private:
  int mask_size;
  Array<3, float> filter_coefficients_for_parsing;
  mutable Array<3, float> filter_coefficients;
  ...
  void precalculate_filter_coefficients_3D(

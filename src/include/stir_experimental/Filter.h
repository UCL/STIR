...
public:
  Filter1D(const int length_v) : filter(1, length_v) {}
  Filter1D(const Array<1, T>& filter) : filter(filter) {}
  virtual ~Filter1D() {}
...
public:
  Filter2D(int height_v, int width_v) : height(height_v), width(width_v), filter(1, width_v * height_v) {}

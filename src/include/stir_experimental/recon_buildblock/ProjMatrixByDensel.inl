...
void
ProjMatrixByDensel::get_proj_matrix_elems_for_one_densel(ProjMatrixElemsForOneDensel& probabilities, const Densel& densel) const
{
  // set to empty
  probabilities.erase();
  ...
  unique_ptr<SymmetryOperation> symm_ptr = get_symmetries_ptr()->find_symmetry_operation_from_basic_densel(basic_densel);

  probabilities.set_densel(basic_densel);

  // check if in cache
  if (get_cached_proj_matrix_elems_for_one_densel(probabilities) == Succeeded::no)
...
ProjMatrixByDensel::cache_key(const Densel& densel)
{
  ...
  return (CacheKey)((static_cast<unsigned int>(densel[1]) << 20) | (static_cast<unsigned int>(densel[2]) << 10)
                    | (static_cast<unsigned int>(densel[3])));
}

//! insert matrix elements for one densel into the cache collection
void
ProjMatrixByDensel::cache_proj_matrix_elems_for_one_densel(const ProjMatrixElemsForOneDensel& probabilities) const
{
  if (cache_disabled)
    return;

...
public:
  ...
  inline void get_related_densels(vector<Densel>&, const Densel& b) const;

  inline int num_related_densels(const Densel& b) const;

  inline unique_ptr<SymmetryOperation> find_symmetry_operation_from_basic_densel(Densel&) const;

  inline bool find_basic_densel(Densel& b) const;

...
*/
class RigidObject3DTransformation : public RegisteredParsingObject<RigidObject3DTransformation, ObjectTransformation<3, float>,
...
public:
  ...
  */
  ...
  static Succeeded find_closest_transformation(RigidObject3DTransformation& result, Iter1T start_orig_points,
  ...
  */
  template <class Iter1T, class Iter2T>
  static double RMSE(const RigidObject3DTransformation& transformation, Iter1T start_orig_points, Iter1T end_orig_points,
  ...
  virtual BasicCoordinate<3, float> transform_point(const BasicCoordinate<3, float>& point) const;

  //! Computes the jacobian for the transformation (which is always 1)
  float jacobian(const BasicCoordinate<3, float>& point) const { return 1; }

...
class RigidObject3DMotionFromPolaris
    : public RegisteredParsingObject<RigidObject3DMotionFromPolaris, RigidObject3DMotion, RigidObject3DMotion>
...
public:
  ...
  virtual RigidObject3DTransformation get_motion_in_tracker_coords_rel_time(const double time) const;

  virtual std::vector<double> get_rel_time_of_samples(const double start_time, const double end_time) const;
  ...
  virtual const RigidObject3DTransformation& get_transformation_to_scanner_coords() const;
  virtual const RigidObject3DTransformation& get_transformation_from_scanner_coords() const;
  virtual void set_transformation_from_scanner_coords(const RigidObject3DTransformation&);
  ...
  double get_max_time_offset_deviation() const { return max_time_offset_deviation; }
  //! Sets boundaries to determine when the time offset is out of bounds
  void set_max_time_offset_deviation(const double v) { max_time_offset_deviation = v; }
  //! Gets boundaries to determine when the time drift is too large
  /*! deviation is measured as fabs(time_drift-1) */
  double get_max_time_drift_deviation() const { return max_time_drift_deviation; }
  //! Sets boundaries to determine when the time drift is too large
  void set_max_time_drift_deviation(const double v) { max_time_drift_deviation = v; }
...
private:
  ...
  RigidObject3DTransformation compute_average_motion_polaris_time(const double start_time, const double end_time) const;

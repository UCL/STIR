...
BloodFrameData::BloodFrameData(const std::vector<BloodFrame>& blood_plot) { this->_blood_plot = blood_plot; }
...
void
BloodFrameData::read_blood_frame_data(const std::string input_string) {
...
void
BloodFrameData::shift_time(const float time_shift) {
  _time_shift = time_shift;
  for (std::vector<BloodFrame>::iterator cur_iter = this->_blood_plot.begin(); cur_iter != this->_blood_plot.end(); ++cur_iter) {
...
float
BloodFrameData::get_time_shift() {
  return BloodFrameData::_time_shift;
}

void
BloodFrameData::set_isotope_halflife(const float isotope_halflife) {
  _isotope_halflife = isotope_halflife;
}

void
BloodFrameData::set_if_decay_corrected(const bool is_decay_corrected) {
  this->_is_decay_corrected = is_decay_corrected;
}

bool
BloodFrameData::get_if_decay_corrected() {
  return this->_is_decay_corrected;
}

void
BloodFrameData::decay_correct_BloodFrameData() {
  if (BloodFrameData::_is_decay_corrected == true)
    warning("BloodFrameData are already decay corrected");
  else {
    for (std::vector<BloodFrame>::iterator cur_iter = this->_blood_plot.begin(); cur_iter != this->_blood_plot.end(); ++cur_iter)
      cur_iter->set_blood_counts_in_kBq(
          cur_iter->get_blood_counts_in_kBq() *
...
void
BloodFrameData::set_plot(const std::vector<BloodFrame>& blood_plot) {

...
pull_linear_interpolate(const Array<3, elemT>& in, const BasicCoordinate<3, positionT>& point_in_input_coords) {
  // find left neighbour
  const Coordinate3D<int> left_neighbour(round(std::floor(point_in_input_coords[1])), round(std::floor(point_in_input_coords[2])),
...
void
push_transpose_linear_interpolate(Array<3, elemT>& out, const BasicCoordinate<3, positionT>& point_in_output_coords,
                                  valueT value) {
  if (value == 0)
    return;
  // find left neighbour

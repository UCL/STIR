...
public:
  ROOTconsistency_Tests(const std::string& in, const std::string& image) : root_header_filename(in), image_filename(image) {}
  void run_tests();
...
private:
  ...
  void construct_list_of_LOR_max(const shared_ptr<DiscretisedDensity<3, float>>& test_discretised_density_sptr);
...
void
ROOTconsistency_Tests::construct_list_of_LOR_max(const shared_ptr<DiscretisedDensity<3, float>>& discretised_density_sptr) {
  ...
  proj_matrix_sptr.get()->set_up(lm_data_sptr->get_proj_data_info_sptr(), discretised_density_sptr);
...
void
ROOTconsistency_Tests::get_LOR_of_max(const ProjMatrixElemsForOneBin& probabilities,
                                      const shared_ptr<DiscretisedDensity<3, float>>& test_discretised_density_sptr) {
  ...
  while (element_ptr != probabilities.end()) {
    if (element_ptr->get_value() >= maxLOR) {
      ...
      voxel_centre = test_discretised_density_sptr->get_physical_coordinates_for_indices(element_ptr->get_coords());
      LORMax tmp;
...
ROOTconsistency_Tests::compute_centre_of_mass() {
  ...
  for (std::vector<LORMax>::iterator lor_element_ptr = max_lor.begin(); lor_element_ptr != max_lor.end(); ++lor_element_ptr) {
    ...
    myfile << lor_element_ptr->voxel_centre.x() << " " << lor_element_ptr->voxel_centre.y() << " "
           << lor_element_ptr->voxel_centre.z() << " " << lor_element_ptr->value << std::endl;
  }
  ...
  cerr << "Centre of gravity coordinates: " << centreofmass.voxel_centre.x() << " " << centreofmass.voxel_centre.y() << " "
       << centreofmass.voxel_centre.z() << std::endl;
...
// TODO change this
void
ROOTconsistency_Tests::compare_original_and_calculated_coordinates(const CartesianCoordinate3D<float>& original_coords,
                                                                   const CartesianCoordinate3D<float>& centre_of_mass,
                                                                   const BasicCoordinate<3, float>& grid_spacing) {
  ...
  cerr << "Original coordinates: " << original_coords.x() << " " << original_coords.y() << " " << original_coords.z()
       << std::endl;
}
...
main(int argc, char** argv) {
  ...
  if (argc != 3) {
    cerr << "Usage : " << argv[1] << " filename " << argv[2] << "original image \n"
  ...
  ifstream in(argv[1]);
  if (!in) {
    cerr << argv[0] << ": Error opening root file " << argv[1] << "\nExiting.\n";

    return EXIT_FAILURE;
  }
  ifstream in2(argv[2]);
  if (!in2) {
    cerr << argv[0] << ": Error opening original image " << argv[2] << "\nExiting.\n";

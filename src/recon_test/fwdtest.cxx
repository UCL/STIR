...
static void do_segments(const VoxelsOnCartesianGrid<float>& image,
                        ProjData& s3d,
                        const int start_timing_pos_num,
                        const int end_timing_pos_num,
                        const int start_segment_num,
                        const int end_segment_num,
...
main(int argc, char* argv[])
{
  ...
  if (argc < 4)
    {
      switch (int choice = ask_num("Start image is cuboid (1) or cylinder (2) or on file (3)", 1, 3, 2))
        ...
        case 1:
          case 2: {
            dispstart = ask_num("Display start image ? no (0), yes (1)", 0, 1, 0);

            save = ask_num("Save  start images ? no (0), yes (1)", 0, 1, 0);
            ...
            vox_image_ptr = new VoxelsOnCartesianGrid<float>(*(proj_data_ptr->get_proj_data_info_sptr()),
                                                             zoom,
                                                             CartesianCoordinate3D<float>(0, 0, 0),
                                                             Coordinate3D<int>(z_size, xy_size, xy_size));
            image_sptr.reset(vox_image_ptr);
            vox_image_ptr->set_grid_spacing(vox_image_ptr->get_grid_spacing() / CartesianCoordinate3D<float>(zoom_z, 1.F, 1.F));
            if (choice == 1)
          ...
          case 3: {
            ...
            image_sptr = read_from_file<DiscretisedDensity<3, float>>(filename);
  ...
  forw_projector_ptr->set_up(proj_data_ptr->get_proj_data_info_sptr()->create_shared_clone(), image_sptr);
  ...
  if (save)
    {
      ...
      OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file("test_image", *image_sptr);
    }
  ...
  if (ask("Do full forward projection ?", true))
    ...
    {
      const bool disp = ask("Display projected related viewgrams ? ", false);
      ...
      do
        {
          ...
          const int timing_pos_num = ask_num("Timing position index to forward project",
                                             proj_data_ptr->get_min_tof_pos_num(),
                                             proj_data_ptr->get_max_tof_pos_num(),
                                             0);
          ...
          const int start_view = ask_num("Start view  (related views will be done as well)", min_view, max_view, min_view);
          const int end_view = ask_num("End   view  (related views will be done as well)", start_view, max_view, max_view);
          ...
          do_segments(*vox_image_ptr,
                      *proj_data_ptr,
                      ...
                      start_view,
                      end_view,
                      start_tangential_pos_num,
                      end_tangential_pos_num,
                      *forw_projector_ptr,
                      disp);
...
fill_cylinder(VoxelsOnCartesianGrid<float>& image)
{
  ...
  const double xc = ask_num(
      "Centre X coordinate", (double)image.get_min_x(), (double)image.get_max_x(), (image.get_min_x() + image.get_max_x()) / 2.);

  const double yc = ask_num(
      "Centre Y coordinate", (double)image.get_min_y(), (double)image.get_max_y(), (image.get_min_y() + image.get_max_y()) / 2.);

  const float zc = ask_num(
      "Centre Z coordinate", (float)image.get_min_z(), (float)image.get_max_z(), (image.get_min_z() + image.get_max_z()) / 2.F);

  const double Rcyl = ask_num(
      "Radius (pixels)", .5, (image.get_max_x() - image.get_min_x()) / 2., (image.get_max_x() - image.get_min_x()) / 4.);

  // Max length is num_planes+1 because of edges of voxels
  const float Lcyl = ask_num(
      "Length (planes)", 1.F, (image.get_max_z() - image.get_min_z()) + 1.F, (image.get_max_z() - image.get_min_z()) + 1.F);

...
main(int argc, char* argv[]) {
  ...
  if (argc < 4) {
    ...
    case 2: {
      ...
      int z_size = stir::round((2 * proj_data_ptr->get_proj_data_info_sptr()->get_scanner_ptr()->get_num_rings() - 1) * zoom_z);
      z_size = ask_num("Number of z pixels", 1, 1000, z_size);
      vox_image_ptr =
          new VoxelsOnCartesianGrid<float>(*(proj_data_ptr->get_proj_data_info_sptr()), zoom,
                                           CartesianCoordinate3D<float>(0, 0, 0), Coordinate3D<int>(z_size, xy_size, xy_size));
...
fill_cylinder(VoxelsOnCartesianGrid<float>& image) {
  ...
  const float zc = ask_num("Centre Z coordinate", (float)image.get_min_z(), (float)image.get_max_z(),
  ...
  const float Lcyl = ask_num("Length (planes)", 1.F, (image.get_max_z() - image.get_min_z()) + 1.F,

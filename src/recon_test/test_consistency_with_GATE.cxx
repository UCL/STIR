...
private:
  ...
  // Test results storage. Records if each test failed or not. E.g.`test_index` result is stored in
  // `test_results_nonTOF[test_index -1]`
...
GATEConsistencyTests::process_list_data()
{
  ...
  proj_matrix_sptr->set_up(lm_data_sptr->get_proj_data_info_sptr(), discretised_density_sptr);
...
void
GATEConsistencyTests::post_processing_nonTOF()
{
...
//// TOF TESTING METHODS ////
bool
GATEConsistencyTests::test_TOF_max_lor_voxel(const ProjMatrixElemsForOneBin& probabilities)
{
  ...
  // At the end of the loop, divide the coordinates by `num_max_value_elements` and add to the list of max_value_voxels, 1 value
  // per event.
...
void
GATEConsistencyTests::post_processing_TOF()
{
  cerr << "\nTOF number of failed events: " << num_failed_TOF_lor_events << "\tNumber of tested events: = " << num_events_tested
       << std::endl;

  test_results_TOF[test_index - 1] = check_if_less(num_failed_TOF_lor_events,
                                                   failure_tolerance_TOF * num_events_tested,
                                                   "The number of failed TOF events is more than the tolerance("
                                                       + std::to_string(100 * failure_tolerance_nonTOF) + "%)");
...
void
GATEConsistencyTests::log_results_to_console()
{
  ...
  for (int j = 0; j < num_tests; ++j)
    cerr << "\t\t" << std::to_string(j + 1) << "\t\t|\t" << ((test_results_nonTOF[j]) ? "Pass" : "Fail") << "\t|\t"

...
struct ForwardBackProjectorPair
{
  ...
  ForwardBackProjectorPair(const std::string forward_name,
                           const std::string back_name,
                           const bool matched_projectors,
                           const shared_ptr<ForwardProjectorByBin> forward_projector_sptr,
                           const shared_ptr<BackProjectorByBin> back_projector_sptr,
...
struct OneForwardBackTest
{
  ...
  OneForwardBackTest(const shared_ptr<ProjDataInfo> proj_data_info_sptr,
                     const shared_ptr<ExamInfo> exam_info_sptr,
                     const shared_ptr<VoxelsOnCartesianGrid<float>> image_sptr,
                     const ForwardBackProjectorPair projector_pair,
                     const std::string name = "")
  ...
  {
    projection_sptr.reset(new ProjDataInMemory(exam_info_sptr, proj_data_info_sptr));
...
public:
  ...
  template <class T>
  bool check_if_correlated(const VectorWithOffset<T>& t1, const VectorWithOffset<T>& t2, const std::string& str = "");
...
private:
  ...
  Succeeded run_tests_for_1_projdatainfo(const shared_ptr<ProjDataInfo> proj_data_info_sptr,
                                         const shared_ptr<ExamInfo> exam_info_sptr);

  Succeeded run_tests_for_1_projdata_extended_axial_fov(const shared_ptr<ProjDataInfo> proj_data_info_sptr,
                                                        const shared_ptr<ExamInfo> exam_info_sptr,
...
CoordinateSystemMappingTests::CoordinateSystemMappingTests(char const* template_proj_data_filename)
    : template_proj_data_filename(template_proj_data_filename)
{}

bool
recursive_calculate_correlation(
    float& sum_x, float& sum_y, float& sum_xx, float& sum_yy, float& sum_xy, int& n, const float x, const float y)
{
  sum_x += x;
  sum_y += y;
  sum_xx += x * x;
...
template <class T>
bool
recursive_calculate_correlation(float& sum_x,
                                float& sum_y,
                                float& sum_xx,
                                float& sum_yy,
                                float& sum_xy,
                                int& n,
                                const VectorWithOffset<T>& x,
                                const VectorWithOffset<T>& y)
{
  if (x.get_min_index() != y.get_min_index() or x.get_max_index() != y.get_max_index())
    {
      return false;
    }
  for (int i = x.get_min_index(); i <= x.get_max_index(); i++)
    {
      bool still_going = recursive_calculate_correlation(sum_x, sum_y, sum_xx, sum_yy, sum_xy, n, x[i], y[i]);
      if (not still_going)
        {
          return false;
        }
...
template <class T>
bool
CoordinateSystemMappingTests::check_if_correlated(const VectorWithOffset<T>& t1,
                                                  const VectorWithOffset<T>& t2,
                                                  const std::string& str)
{
  ...
  float sum_xy = 0;
  int n = 0;
  if (recursive_calculate_correlation(sum_x, sum_y, sum_xx, sum_yy, sum_xy, n, t1, t2))
    {
      if (n == 0)
        {
      ...
      const float correlation = (n * sum_xy - sum_x * sum_y) / sqrt((n * sum_xx - sum_x * sum_x) * (n * sum_yy - sum_y * sum_y));
      if (correlation < correlation_tolerance)
        {
          std::cerr << "Error: Uncorrelated vectors, correlation was only " << correlation << ". " << str << std::endl;
          return everything_ok = false;
        }
      else
        {
    ...
    {
      std::cerr << "Error: unequal ranges: (" << t1.get_min_index() << "," << t1.get_max_index() << ") (" << t2.get_min_index()
                << "," << t2.get_max_index() << "). " << str << std::endl;
      return everything_ok = false;
    }
}

void
CoordinateSystemMappingTests::set_correlation_tolerance(const double correlation_tolerance_v)
{
...
fill_in_cylinder_test_image(VoxelsOnCartesianGrid<float>& image)
{
  ...
  const CartesianCoordinate3D<float> cyl_orig
      = CartesianCoordinate3D<float>(cyl_offset_z, cyl_offset_y, cyl_offset_x) + image.get_image_centre_in_physical_coordinates();
...
void
do_forward_then_back_projections(ProjData& projection,
                                 shared_ptr<VoxelsOnCartesianGrid<float>> backprojection_sptr,
                                 const shared_ptr<ProjDataInfo> proj_data_info_sptr,
                                 const shared_ptr<VoxelsOnCartesianGrid<float>> standard_image_sptr,
                                 const ForwardBackProjectorPair& projector_pair)
...
Succeeded
CoordinateSystemMappingTests::one_forward_backward_test(const OneForwardBackTest& test)
{
  std::cerr << "##" << std::endl;
  ...
  do_forward_then_back_projections(
      *test.projection_sptr, test.backprojection_sptr, test.proj_data_info_sptr, test.image_sptr, test.projector_pair);

  // round-trip test
  if (not check_if_correlated(*test.image_sptr, *test.backprojection_sptr, "Checking backprojection matches original."))
    {
      std::cerr << "Saving last failed image comparison to " << test.name << "_failed_{ref,oth}.hv. "
  ...
  for (ForwardBackProjectorPair projector_pair : projector_pairs)
    {
      ...
      OneForwardBackTest standard_fov_test
          = OneForwardBackTest(proj_data_info_sptr, exam_info_sptr, standard_image_sptr, projector_pair, "standard-FOV");

      std::vector<OneForwardBackTest> extended_fov_tests;

      {
        shared_ptr<VoxelsOnCartesianGrid<float>> image_sptr(standard_image_sptr->clone());
        image_sptr->grow_z_range(standard_image_sptr->get_min_z() - EXTEND_BY, standard_image_sptr->get_max_z() + EXTEND_BY);
        extended_fov_tests.push_back(
            OneForwardBackTest(proj_data_info_sptr, exam_info_sptr, image_sptr, projector_pair, "extended-FOV-both"));
      }

      {
        std::cerr << "... extend in positive z direction" << std::endl;
        shared_ptr<VoxelsOnCartesianGrid<float>> image_sptr(standard_image_sptr->clone());
        image_sptr->grow_z_range(standard_image_sptr->get_min_z(), standard_image_sptr->get_max_z() + EXTEND_BY);
        extended_fov_tests.push_back(
            OneForwardBackTest(proj_data_info_sptr, exam_info_sptr, image_sptr, projector_pair, "extended-FOV-positive"));
      }

      {
        std::cerr << "... extend in negative z direction" << std::endl;
        shared_ptr<VoxelsOnCartesianGrid<float>> image_sptr(standard_image_sptr->clone());
        image_sptr->grow_z_range(standard_image_sptr->get_min_z() - EXTEND_BY, standard_image_sptr->get_max_z());
        extended_fov_tests.push_back(
            OneForwardBackTest(proj_data_info_sptr, exam_info_sptr, image_sptr, projector_pair, "extended-FOV-negative"));
      }
      ...
      for (OneForwardBackTest extended_fov_test : extended_fov_tests)
        {
          ...
          if (not check_if_equal<float>(standard_fov_test.projection_sptr->begin_all(),
                                        ...
                                        "Checking extended FOV projection matches standard."))
            {
              ...
              std::cerr << "Saving last failed image comparison to " << extended_fov_test.name << "_fovdiff_failed_{ref,oth}.hs. "
                        << "(may be overwritten)" << std::endl;
...
Succeeded
CoordinateSystemMappingTests::run_tests_for_1_projdatainfo(const shared_ptr<ProjDataInfo> proj_data_info_sptr,
                                                           const shared_ptr<ExamInfo> exam_info_sptr)
{
  Succeeded succeeded = Succeeded::yes;
  ...
  shared_ptr<VoxelsOnCartesianGrid<float>> standard_image_sptr(
  ...
  shared_ptr<ProjMatrixByBin> matrix_raytrace_sptr(new ProjMatrixByBinUsingRayTracing());
  shared_ptr<ProjectorByBinPairUsingProjMatrixByBin> projector_pair_matrix_raytrace_sptr(
      new ProjectorByBinPairUsingProjMatrixByBin(matrix_raytrace_sptr));
  shared_ptr<ProjMatrixByBin> matrix_interp_sptr(new ProjMatrixByBinUsingInterpolation());
  shared_ptr<ProjectorByBinPairUsingProjMatrixByBin> projector_pair_matrix_interp_sptr(
      new ProjectorByBinPairUsingProjMatrixByBin(matrix_interp_sptr));

  // Define forward projectors
  using NameAndFwdProjPair = std::pair<std::string, shared_ptr<ForwardProjectorByBin>>;
  std::vector<NameAndFwdProjPair> fwd_projectors;
  NameAndFwdProjPair reference_fwd_projector
      = NameAndFwdProjPair("RayTracingMatrix", projector_pair_matrix_raytrace_sptr->get_forward_projector_sptr());
  ...
  fwd_projectors.push_back(
      NameAndFwdProjPair("InterpolationMatrix", projector_pair_matrix_interp_sptr->get_forward_projector_sptr()));

  // Define backward projectors
  using NameAndBckProjPair = std::pair<std::string, shared_ptr<BackProjectorByBin>>;
  std::vector<NameAndBckProjPair> bck_projectors;
  NameAndBckProjPair reference_bck_projector
      = NameAndBckProjPair("RayTracingMatrix", projector_pair_matrix_raytrace_sptr->get_back_projector_sptr());
  // don't add the RT, all combos already covered
  ...
  bck_projectors.push_back(
      NameAndBckProjPair("InterpolationMatrix", projector_pair_matrix_interp_sptr->get_back_projector_sptr()));

  // Set up pairs to test
  std::vector<ForwardBackProjectorPair> projector_pairs;
  // Test the correctly paired projectors first
  projector_pairs.push_back(ForwardBackProjectorPair("RayTraceMatrix",
                                                     "RayTraceMatrix",
                                                     true,
                                                     projector_pair_matrix_raytrace_sptr->get_forward_projector_sptr(),
                                                     projector_pair_matrix_raytrace_sptr->get_back_projector_sptr()));
  ...
  projector_pairs.push_back(ForwardBackProjectorPair("InterpolationMatrix",
                                                     "InterpolationMatrix",
                                                     true,
  ...
  for (NameAndBckProjPair bck_projector : bck_projectors)
    {
      projector_pairs.push_back(ForwardBackProjectorPair(
          reference_fwd_projector.first, bck_projector.first, false, reference_fwd_projector.second, bck_projector.second));
    }
  // each forward projector against the reference back projector
  for (NameAndFwdProjPair fwd_projector : fwd_projectors)
    {
      projector_pairs.push_back(ForwardBackProjectorPair(
          fwd_projector.first, reference_bck_projector.first, false, fwd_projector.second, reference_bck_projector.second));
    }

  // Now test each of the pairs
  succeeded
      &= run_tests_for_1_projdata_extended_axial_fov(proj_data_info_sptr, exam_info_sptr, standard_image_sptr, projector_pairs);
...
void
CoordinateSystemMappingTests::run_tests()
{

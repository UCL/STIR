...
CListModeDataPETSIRD::isCylindricalConfiguration(const std::vector<petsird::ReplicatedDetectorModule>& replicated_module_list)
{
  ...
  vector_utils::find_unique_values_2D(unique_tof_values, scanner_info->tof_resolution);
    ...
    }
{
  ...
  {
    std::cout << "TOF bin edge " << i << ": " << tof_bin_edges.edges[i] << "\n";
  }
}
  ...
  if (std::abs(expected_circle_area - polygon_area) / expected_circle_area < 0.1f)
    {
    ...
    : use_hdf5(use_hdf5)
{
  ...
  bool is_cylindrical = isCylindricalConfiguration(replicated_module_list);
  // if (is_cylindrical)
  //   {
  ...
  for (uint32_t module = 0; module < numberOfModules; module++)
    for (uint32_t elem = 0; elem < numberOfElementsIndices; elem++)
      //            for (uint32_t ener = 0; ener < num_event_energy_bins; ener++) //energy not supported yet
      {
        ...
        if (this_scanner_sptr->get_scanner_geometry() == "Generic")
          {
            auto box_shape = petsird_helpers::geometry::get_detecting_box(*scanner_info, type_of_module, expanded_detection_bin);
            ...
            std::cout << ind << " : " << detpos.radial_coord() << ", " << detpos.axial_coord() << ", "
                      << detpos.tangential_coord() << ", " << mean_coord.x() << ", " << mean_coord.y() << ", " << mean_coord.z()
                      << "\n";
            ++ind;
          }
        else if (this_scanner_sptr->get_scanner_geometry() == "BlocksOnCylindrical" || is_cylindrical)
          {
  ...
  if (this_scanner_sptr->get_scanner_geometry() == "Generic")
    {
      std::dynamic_pointer_cast<CListRecordPETSIRD>(sptr)->event().set_map_sptr(this->map);
          ...
          this_scanner_sptr->get_scanner_geometry() == "Cylindrical")
    {
      std::dynamic_pointer_cast<CListRecordPETSIRD>(sptr)->event().set_petsird_to_stir_map(petsird_to_stir);
    }

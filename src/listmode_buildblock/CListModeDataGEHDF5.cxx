  ...
  : listmode_filename(listmode_filename)
{
  if (open_lm_file() == Succeeded::no)
    error(boost::format("CListModeDataGEHDF5: error opening the listmode file for filename %s") %
      listmode_filename);
}
...
//  input_sptr.reset( new GEHDF5Wrapper(listmode_filename));
  ...
  this->first_time_stamp =
    inputFile.read_dataset_uint32("/HeaderData/ListHeader/firstTmAbsTimeStamp");
  const std::uint32_t last_time_stamp =
    inputFile.read_dataset_uint32("/HeaderData/ListHeader/lastTmAbsTimeStamp");
  this->lm_duration_in_millisecs = last_time_stamp - this->first_time_stamp;
  info(boost::format("First/last time-stamp: %1%/%2%. Duration %3% ms.")
       % this->first_time_stamp % last_time_stamp % this->lm_duration_in_millisecs,
       2);

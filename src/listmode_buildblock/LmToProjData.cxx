...
static void allocate_segments(VectorWithOffset<VectorWithOffset<segment_type*>>& segments, const int start_timing_pos_index,
                              const int end_timing_pos_index, const int start_segment_index, const int end_segment_index,
 ...
 */
static void save_and_delete_segments(shared_ptr<iostream>& output, VectorWithOffset<VectorWithOffset<segment_type*>>& segments,
                                     const int start_timing_pos_index, const int end_timing_pos_index,
                                     const int start_segment_index, const int end_segment_index, ProjData& proj_data);
static shared_ptr<ProjData> construct_proj_data(shared_ptr<iostream>& output, const string& output_filename,
...
LmToProjData::post_processing() {
  ...
  template_proj_data_info_ptr->set_bed_position_horizontal(lm_data_ptr->get_proj_data_info_sptr()->get_bed_position_horizontal());
  template_proj_data_info_ptr->set_bed_position_vertical(lm_data_ptr->get_proj_data_info_sptr()->get_bed_position_vertical());
  ...
  if (num_segments_in_memory == -1 || interactive)
    ...
    num_segments_in_memory = min(num_segments_in_memory, num_segments);
  if (num_segments == 0) {
    warning("LmToProjData: num_segments_in_memory cannot be 0");
    return true;
  }

  Scanner const* const scanner_ptr = template_proj_data_info_ptr->get_scanner_ptr();
  ...
  if (*scanner_ptr != *lm_data_ptr->get_scanner_ptr()) {
    warning("LmToProjData:\nScanner from list mode data (%s) is different from\n"
            ...
            lm_data_ptr->get_scanner_ptr()->get_name().c_str(), scanner_ptr->get_name().c_str(),
            lm_data_ptr->get_scanner_ptr()->parameter_info().c_str(), scanner_ptr->parameter_info().c_str());
    return true;
  }
  ...
  if (do_pre_normalisation) {
    ...
    proj_data_info_cyl_uncompressed_ptr.reset(ProjDataInfo::ProjDataInfoCTI(
        scanner_sptr, 1, scanner_ptr->get_num_rings() - 1, scanner_ptr->get_num_detectors_per_ring() / 2,
        scanner_ptr->get_default_num_arccorrected_bins(), false, 1));

    if (normalisation_ptr->set_up(lm_data_ptr->get_exam_info_sptr(), proj_data_info_cyl_uncompressed_ptr) != Succeeded::yes)
      error("LmToProjData: set-up of pre-normalisation failed\n");
  } else {
    if (post_normalisation_ptr->set_up(lm_data_ptr->get_exam_info_sptr(), template_proj_data_info_ptr) != Succeeded::yes)
...
LmToProjData::LmToProjData() { set_defaults(); }

LmToProjData::LmToProjData(const char* const par_filename) {
  set_defaults();
  if (par_filename != 0) {
...
***************************************************************/
void
LmToProjData::get_bin_from_event(Bin& bin, const ListEvent& event) const {
  if (do_pre_normalisation) {
    ...
    const float bin_efficiency = normalisation_ptr->get_bin_efficiency(uncompressed_bin, start_time, end_time);
...
***************************************************************/
void
LmToProjData::do_post_normalisation(Bin& bin) const {
...
int
LmToProjData::get_compression_count(const Bin& bin) const {
  // TODO this currently works ONLY for cylindrical PET scanners

  const ProjDataInfoCylindrical& proj_data_info = dynamic_cast<const ProjDataInfoCylindrical&>(*template_proj_data_info_ptr);
...
***************************************************************/
void
LmToProjData::process_new_time_event(const ListTime&) {}

void
LmToProjData::start_new_time_frame(const unsigned int) {}
...
LmToProjData::process_data() {
  ...
  VectorWithOffset<segment_type*> segments(template_proj_data_info_ptr->get_min_segment_num(),
                                           template_proj_data_info_ptr->get_max_segment_num());

  VectorWithOffset<ListModeData::SavedPosition> frame_start_positions(1, static_cast<int>(frame_defs.get_num_frames()));
  ...
  for (current_frame_num = 1; current_frame_num <= frame_defs.get_num_frames(); ++current_frame_num) {
...

    {
      ...
      proj_data_ptr = construct_proj_data(output, output_filename, this_frame_exam_info, template_proj_data_info_ptr);
    }
    ...
    VectorWithOffset<VectorWithOffset<segment_type*>> segments(template_proj_data_info_ptr->get_min_tof_pos_num(),
    ...
    for (int start_timing_pos_index = proj_data_ptr->get_min_tof_pos_num();
         start_timing_pos_index <= proj_data_ptr->get_max_tof_pos_num(); start_timing_pos_index += num_timing_poss_in_memory) {
      const int end_timing_pos_index =
          min(proj_data_ptr->get_max_tof_pos_num() + 1, start_timing_pos_index + num_timing_poss_in_memory) - 1;
      ...
      */
      for (int start_segment_index = proj_data_ptr->get_min_segment_num();
           start_segment_index <= proj_data_ptr->get_max_segment_num(); start_segment_index += num_segments_in_memory) {

        const int end_segment_index =
            min(proj_data_ptr->get_max_segment_num() + 1, start_segment_index + num_segments_in_memory) - 1;

        if (!interactive)
          allocate_segments(segments, start_timing_pos_index, end_timing_pos_index, start_segment_index, end_segment_index,
        ...
        unsigned long int more_events = do_time_frame ? 1 : num_events_to_store;

        if (start_segment_index != proj_data_ptr->get_min_segment_num() ||
            start_timing_pos_index > proj_data_ptr->get_min_tof_pos_num()) {
          ...
          while (current_time < start_time && lm_data_ptr->get_next_record(record) == Succeeded::yes) {
            if (record.is_time())
              current_time = record.time().get_time_in_secs();
          }
          // now save position such that we can go back
          frame_start_positions[current_frame_num] = lm_data_ptr->save_get_position();
        }
        {
          ...
          while (more_events) {
            ...
            if (record.is_event()) {
              ...
              if (bin.get_bin_value() > 0 && bin.tangential_pos_num() >= proj_data_ptr->get_min_tangential_pos_num() &&
                ...
                const int event_increment = record.event().is_prompt() ? (store_prompts ? 1 : 0) // it's a prompt
                ...
                if (bin.timing_pos_num() >= start_timing_pos_index && bin.timing_pos_num() <= end_timing_pos_index) {
                  ...
                  if (bin.segment_num() >= start_segment_index && bin.segment_num() <= end_segment_index) {
                    ...
                    if (num_stored_events % 500000L == 0)
                    ...
                    if (interactive)
                      printf("TOFbin %4d Seg %4d view %4d ax_pos %4d tang_pos %4d time %8g stored with incr %d \n",
                             bin.timing_pos_num(), bin.segment_num(), bin.view_num(), bin.axial_pos_num(),
                             bin.tangential_pos_num(), current_time, event_increment);
                    else
                      (*segments[bin.timing_pos_num()][bin.segment_num()])[bin.view_num()][bin.axial_pos_num()]
                                                                          [bin.tangential_pos_num()] +=
                          bin.get_bin_value() * event_increment;
                  }
                }
        ...
        if (!interactive)
          save_and_delete_segments(output, segments, start_timing_pos_index, end_timing_pos_index, start_segment_index,
                                   end_segment_index, *proj_data_ptr);
  ...
  if (!do_time_frame && (num_stored_events <= 0 ||
...
#else
<<<<<<< HEAD
  VectorWithOffset<VectorWithOffset<segment_type*>> segments(template_proj_data_info_ptr->get_min_segment_num(),
                                                             template_proj_data_info_ptr->get_max_segment_num());
...
=======
    ...
    proj_data_sptr = construct_proj_data(output, output_filename, this_frame_exam_info, template_proj_data_info_ptr);
  }
  ...
  for (int start_segment_index = proj_data_sptr->get_min_segment_num();
       start_segment_index <= proj_data_sptr->get_max_segment_num(); start_segment_index += num_segments_in_memory) {
    ...
    // When do_time_frame=true, the number of events is irrelevant, so we
    // just set more_events to 1, and never change it
    long more_events = do_time_frame ? 1 : num_events_to_store;
    ...
    }
    {
      ...
      while (more_events) {
        ...
        if (record.is_event()) {
          ...
          // check if it's inside the range we want to store
          if (bin.get_bin_value() > 0 && bin.tangential_pos_num() >= proj_data_sptr->get_min_tangential_pos_num() &&
            ...
            if (bin.segment_num() >= start_segment_index && bin.segment_num() <= end_segment_index) {
              ...
              if (interactive)
                printf("Seg %4d view %4d ax_pos %4d tang_pos %4d time %8g stored with incr %d \n", bin.segment_num(),
                       bin.view_num(), bin.axial_pos_num(), bin.tangential_pos_num(), current_time, event_increment);
              else
    ...
    if (!interactive)
      save_and_delete_segments(output, segments, start_segment_index, end_segment_index, *proj_data_sptr);
  } // end of for loop for segment range
  cerr << "\nNumber of prompts stored in this time period : " << num_prompts_in_frame
       << "\nNumber of delayeds stored in this time period: " << num_delayeds_in_frame << '\n';
} // end of loop over frames
...
>>>>>>> master
  ...
  for (int current_timing_pos_index = proj_data_ptr->get_min_tof_pos_num();
       current_timing_pos_index <= proj_data_ptr->get_max_tof_pos_num(); current_timing_pos_index += 1) {
    for (int start_segment_index = proj_data_ptr->get_min_segment_num();
         start_segment_index <= proj_data_ptr->get_max_segment_num(); start_segment_index += 1) {

      const int end_segment_index =
          min(proj_data_ptr->get_max_segment_num() + 1, start_segment_index + num_segments_in_memory) - 1;

      if (!interactive)
        allocate_segments(segments, start_segment_index, end_segment_index, start_timing_pos_index, end_timing_pos_index,
      ...
      for (int ax_num = proj_data_ptr->get_proj_data_info_ptr()->get_min_axial_pos_num(start_segment_index);
           ax_num <= proj_data_ptr->get_proj_data_info_ptr()->get_max_axial_pos_num(start_segment_index); ++ax_num) {
        for (int view_num = proj_data_ptr->get_proj_data_info_ptr()->get_min_view_num();
          ...
          for (int tang_num = proj_data_ptr->get_proj_data_info_ptr()->get_min_tangential_pos_num();
               tang_num <= proj_data_ptr->get_proj_data_info_ptr()->get_max_tangential_pos_num(); ++tang_num) {
      ...
      if (!interactive)
        save_and_delete_segments(output, segments, start_segment_index, end_segment_index, *proj_data_ptr);
    } // end of for loop for segment range
...
void
allocate_segments(VectorWithOffset<VectorWithOffset<segment_type*>>& segments, const int start_timing_pos_index,
                  const int end_timing_pos_index, const int start_segment_index, const int end_segment_index,
  ...
  for (int timing_pos_num = start_timing_pos_index; timing_pos_num <= end_timing_pos_index; timing_pos_num++)
    for (int seg = start_segment_index; seg <= end_segment_index; seg++) {
      ...
      segments[timing_pos_num][seg] =
          new SegmentByView<elem_type>(proj_data_info_sptr->get_empty_segment_by_view(seg, false, timing_pos_num));
#else
      segments[timing_pos_num][seg] = new Array<3, elem_type>(IndexRange3D(
          0, proj_data_info_sptr->get_num_views() - 1, 0, proj_data_info_sptr->get_num_axial_poss(seg) - 1,
...
void
save_and_delete_segments(shared_ptr<iostream>& output, VectorWithOffset<VectorWithOffset<segment_type*>>& segments,
                         const int start_timing_pos_index, const int end_timing_pos_index, const int start_segment_index,
                         const int end_segment_index, ProjData& proj_data) {
  for (int timing_pos_num = start_timing_pos_index; timing_pos_num <= end_timing_pos_index; timing_pos_num++)
    for (int seg = start_segment_index; seg <= end_segment_index; seg++) {
#ifdef USE_SegmentByView
...
construct_proj_data(shared_ptr<iostream>& output, const string& output_filename, const ExamInfo& exam_info,
                    const shared_ptr<const ProjDataInfo>& proj_data_info_ptr) {
  ...
  if (!proj_data_info_ptr->is_tof_data())
    proj_data_sptr.reset(new ProjDataInterfile(exam_info_sptr, proj_data_info_ptr, output_filename, ios::out,
                                               ProjDataFromStream::Segment_View_AxialPos_TangPos, OUTPUTNumericType));
  else
    proj_data_sptr.reset(new ProjDataInterfile(exam_info_sptr, proj_data_info_ptr, output_filename, ios::out,
                                               ProjDataFromStream::Timing_Segment_View_AxialPos_TangPos, OUTPUTNumericType));
  ...
  {
    std::vector<int>::iterator current_segment_iter = segment_sequence_in_stream.begin();
  ...
  shared_ptr<ProjDataFromStream> proj_data_ptr(new ProjDataFromStream(exam_info_sptr, proj_data_info_ptr, output,
                                                                      /*offset=*/std::streamoff(0), segment_sequence_in_stream,

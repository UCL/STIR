...
/*
    ...
    Copyright (C) 2013, 2021, 2023, 2024 University College London
...
static void allocate_segments(VectorWithOffset<VectorWithOffset<segment_type*>>& segments,
...
**************************************************************/
void
LmToProjData::set_template_proj_data_info_sptr(shared_ptr<const ProjDataInfo> t_sptr)
{
...
void
LmToProjData::set_input_data(const shared_ptr<ExamData>& v)
{
...
void
LmToProjData::set_output_filename_prefix(const std::string& v)
{
  this->output_filename_prefix = v;
}

std::string
LmToProjData::get_output_filename_prefix() const
{
...
void
LmToProjData::set_store_prompts(bool v)
{
  this->store_prompts = v;
}

bool
LmToProjData::get_store_prompts() const
{
  return store_prompts;
}

void
LmToProjData::set_store_delayeds(bool v)
{
  this->store_delayeds = v;
}

bool
LmToProjData::get_store_delayeds() const
{
  return store_delayeds;
}

void
LmToProjData::set_num_segments_in_memory(int v)
{
  this->_already_setup = false;
...
int
LmToProjData::get_num_segments_in_memory() const
{
  return num_segments_in_memory;
}

void
LmToProjData::set_num_events_to_store(long int v)
{
  this->num_events_to_store = v;
}

long int
LmToProjData::get_num_events_to_store() const
{
  return num_events_to_store;
}

void
LmToProjData::set_time_frame_definitions(const TimeFrameDefinitions& v)
{
  this->frame_defs = v;
}

const TimeFrameDefinitions&
LmToProjData::get_time_frame_definitions() const
{
  return frame_defs;
}
...
LmToProjData::post_processing()
{
  ...
  if (template_proj_data_name.size() > 0)
    {
      shared_ptr<ProjData> template_proj_data_sptr = ProjData::read_from_file(template_proj_data_name);
...
LmToProjData::set_up()
{
  ...
  if (is_null_ptr(template_proj_data_info_ptr))
    {
      info("LmToProjData: template not specified. Will use the proj_data_info from the input list-mode (which might be quite "
           "large...).");
  ...
  if (num_timing_poss_in_memory == -1 || interactive)
    ...
    num_timing_poss_in_memory = min(num_timing_poss_in_memory, num_timing_poss);
  ...
  for (current_frame_num = 1; current_frame_num <= frame_defs.get_num_frames(); ++current_frame_num)
    {
      ...
      for (int start_timing_pos_index = output_proj_data_sptr->get_min_tof_pos_num();
        ...
        {
          const int end_timing_pos_index
              = min(output_proj_data_sptr->get_max_tof_pos_num() + 1, start_timing_pos_index + num_timing_poss_in_memory) - 1;
               ...
               start_segment_index += num_segments_in_memory)
            {
              ...
              if (!interactive)
                allocate_segments(segments,
                                  start_timing_pos_index,
                                  end_timing_pos_index,
                                  start_segment_index,
                                  end_segment_index,
                    ...
                    if (record.is_event())
                      {
                        ...
                        try
                          ...
                          {
                            for (int timing_pos_num = start_timing_pos_index; timing_pos_num <= end_timing_pos_index;
                                 timing_pos_num++)
                        ...
                        if (bin.get_bin_value() > 0
                            ...
                            && bin.timing_pos_num() <= output_proj_data_sptr->get_max_tof_pos_num())
                          {
                            ...
                            const int event_increment = record.event().is_prompt()
              ...
              if (!interactive)
                save_and_delete_segments(output,
                                         segments,
                                         start_timing_pos_index,
                                         end_timing_pos_index,
                                         start_segment_index,
    ...
    for (int seg = start_segment_index; seg <= end_segment_index; seg++)
      {
        ...
        segments[timing_pos_num][seg] = new Array<3, elem_type>(IndexRange3D(
            0,
            proj_data_info_sptr->get_num_views() - 1,
            0,
            proj_data_info_sptr->get_num_axial_poss(seg) - 1,
                    ...
                    const shared_ptr<const ProjDataInfo>& proj_data_info_ptr)
{
  ...
  shared_ptr<ProjDataFromStream> proj_data_ptr(new ProjDataFromStream(exam_info_sptr,
                                                                      proj_data_info_ptr,
                                                                      output,

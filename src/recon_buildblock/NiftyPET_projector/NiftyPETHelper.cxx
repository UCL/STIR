...
static void
get_axLUT_sptr(shared_ptr<axialLUT>& axlut_sptr, std::vector<float>& li2rng, std::vector<short>& li2sn_s,
               std::vector<char>& li2nos_c, const Cnst& cnt) {
...
NiftyPETHelper::create_stir_im() {
  ...
  shared_ptr<VoxelsOnCartesianGrid<float>> out_im_stir_sptr = MAKE_SHARED<VoxelsOnCartesianGrid<float>>(
      ...
      CartesianCoordinate3D<float>(0.f, 0.f, 0.f), CartesianCoordinate3D<float>(sz, sy, sx));
...
void
get_stir_indices_and_dims(int stir_dim[3], Coordinate3D<int>& min_indices, Coordinate3D<int>& max_indices,
...
void
NiftyPETHelper::permute(std::vector<float>& output_array, const std::vector<float>& orig_array, const unsigned output_dims[3],
  ...
  for (unsigned old_1d_idx = 0; old_1d_idx < orig_array.size(); ++old_1d_idx) {
    ...
    unsigned old_3d_idx[3] = {old_1d_idx / (old_dims[2] * old_dims[1]), (old_1d_idx / old_dims[2]) % old_dims[1],
    ...
    const unsigned new_1d_idx = new_3d_idx[0] * output_dims[2] * output_dims[1] + new_3d_idx[1] * output_dims[2] + new_3d_idx[2];
...
NiftyPETHelper::back_project(std::vector<float>& image, const std::vector<float>& sino_no_gaps) const {
  ...
  gpu_bprj(unpermuted_image.data(), const_cast<float*>(sino_no_gaps.data()), const_cast<float*>(_li2rng.data()),
           const_cast<short*>(_li2sn.data()), const_cast<char*>(_li2nos.data()), const_cast<short*>(_s2c.data()),
           _txlut_sptr->aw2ali, const_cast<float*>(_crs.data()), const_cast<int*>(_isub.data()), int(_isub.size()), AW,
           4, // n0crs
           nCRS, *_cnt_sptr);
...
NiftyPETHelper::forward_project(std::vector<float>& sino_no_gaps, const std::vector<float>& image) const {
  ...
  gpu_fprj(sino_no_gaps.data(), permuted_image.data(), const_cast<float*>(_li2rng.data()), const_cast<short*>(_li2sn.data()),
           const_cast<char*>(_li2nos.data()), const_cast<short*>(_s2c.data()), _txlut_sptr->aw2ali,
           const_cast<float*>(_crs.data()), const_cast<int*>(_isub.data()), int(_isub.size()), AW,
           4, // n0crs
           nCRS, *_cnt_sptr, _att);
}
...
void
NiftyPETHelper::lm_to_proj_data(shared_ptr<ProjData>& prompts_sptr, shared_ptr<ProjData>& delayeds_sptr,
                                shared_ptr<ProjData>& randoms_sptr, shared_ptr<ProjData>& norm_sptr, const int tstart,
                                const int tstop, const std::string& lm_binary_file, const std::string& norm_binary_file) const {
  check_set_up();
...
void
get_vals_for_proj_data_conversion(std::vector<int>& sizes, std::vector<int>& segment_sequence, int& num_sinograms, int& min_view,
                                  int& max_view, int& min_tang_pos, int& max_tang_pos, const ProjDataInfo& proj_data_info,
...
void
get_stir_segment_and_axial_pos_from_NiftyPET_sino(int& segment, int& axial_pos, const unsigned np_sino,
                                                  const std::vector<int>& sizes, const std::vector<int>& segment_sequence) {
  int z = int(np_sino);
...
void
get_NiftyPET_sino_from_stir_segment_and_axial_pos(unsigned& np_sino, const int segment, const int axial_pos,
                                                  const std::vector<int>& sizes, const std::vector<int>& segment_sequence) {
  np_sino = 0U;
  for (unsigned i = 0; i < segment_sequence.size(); ++i) {
...
NiftyPETHelper::convert_viewgram_stir_to_niftyPET(std::vector<float>& np_vec, const Viewgram<float>& viewgram) const {
  ...
  get_vals_for_proj_data_conversion(sizes, segment_sequence, num_sinograms, min_view, max_view, min_tang_pos, max_tang_pos,
                                    *viewgram.get_proj_data_info_sptr(), np_vec);
...
NiftyPETHelper::convert_proj_data_niftyPET_to_stir(ProjData& stir, const std::vector<float>& np_vec) const {
  ...
  int num_sinograms, min_view, max_view, min_tang_pos, max_tang_pos;
  get_vals_for_proj_data_conversion(sizes, segment_sequence, num_sinograms, min_view, max_view, min_tang_pos, max_tang_pos,
                                    *stir.get_proj_data_info_sptr(), np_vec);

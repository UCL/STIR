                                                             ...
                                                             const int subset_num)
{
  ...
  std::fill(conv_image_estimate.begin_all(),
  ...
  //Print out the min and max values of the initial motion-corrected estimate
  const float current_conv_min_estimate =
	*std::min_element(conv_image_estimate.begin_all(),
					  conv_image_estimate.end_all()); 
  const float current_conv_max_estimate = 
  ...
  const float current_min_system_conv_sensitivity =
	*std::min_element(conv_sensitivity.begin_all(),
					    conv_sensitivity.end_all()); 
  const float current_max_system_conv_sensitivity = 
    *std::max_element(conv_sensitivity.begin_all(),
					    conv_sensitivity.end_all());
  ...
  std::fill(conv_gradient.begin_all(),
            conv_gradient.end_all(),
            0.F);

  // Calculation of the outer loop sub-gradient image in the convolved (motion-contaminated) space using the forward-convolved estimate 			
  ...
  const float current_min_outer_loop_gradient =
    *std::min_element(conv_gradient.begin_all(),
					  conv_gradient.end_all()); 

  const float current_max_outer_loop_gradient = 
	*std::max_element(conv_gradient.begin_all(),
					  conv_gradient.end_all());
  ...
  divide(conv_gradient.begin_all(), 
	 	 conv_gradient.end_all(),
	 	 conv_sensitivity.begin_all(),
		 small_num);
	...
	*std::min_element(conv_image_reference_data.begin_all(),
					  conv_image_reference_data.end_all()); 
	const float current_max_outer_loop_updated_image = 
	*std::max_element(conv_image_reference_data.begin_all(),
					  conv_image_reference_data.end_all());
  ...
  this->estimate_nested_loop_parameters_with_model(gradient,
                                                   current_estimate,
                                                   conv_image_estimate,
											       conv_image_reference_data,
											       conv_image_nested_loop_estimate);
										   ...
										   TargetT &conv_image_nested_loop_estimate)											 
{
  ...
  //nested EM loop
  cerr << endl << "Entering nested loop (" << this->num_nested_subiterations << " Richardson-Lucy EM deconvolution sub-iterations for motion correction)." << endl;
  ...
  for(nested_subiterations_num=1; nested_subiterations_num<=this->num_nested_subiterations; nested_subiterations_num++)
  {
    ...
    cerr << "Richardson-Lucy nested iteration: " << nested_subiterations_num 
	     << " Initial deconvolved image nested estimate , (min, max): (" 
	...
	this->_motion_vectors.average_warp_image(conv_image_nested_loop_estimate,
	                                         current_estimate); 
    ...
    const float conv_min_nested_loop_estimate =
	  *std::min_element(conv_image_nested_loop_estimate.begin_all(),
					    conv_image_nested_loop_estimate.end_all()); 
    const float conv_max_nested_loop_estimate = 
	  *std::max_element(conv_image_nested_loop_estimate.begin_all(),
					    conv_image_nested_loop_estimate.end_all());
    cerr << "Richardson-Lucy nested iteration: " << nested_subiterations_num 
	...
	divide(conv_image_estimate.begin_all(),
	       conv_image_estimate.end_all(),
		   conv_image_nested_loop_estimate.begin_all(),
		   small_num);

    //Print out the min and max values of the convolved image gradient after the division of the reference with the nested estimate in the convolved space
    const float conv_min_nested_estimate =
	  *std::min_element(conv_image_estimate.begin_all(),
					    conv_image_estimate.end_all()); 
    const float conv_max_nested_estimate = 
	  *std::max_element(conv_image_estimate.begin_all(),
					    conv_image_estimate.end_all());
    cerr << "Richardson-Lucy nested iteration: " << nested_subiterations_num 
	     << " Current convolved image gradient after division of reference with the current nested estimate in the convolved space , (min, max): (" 
    ...
    this->_reverse_motion_vectors.average_warp_image(gradient,
	                                                 conv_image_estimate);																		 
	...
	*/
	...
	cerr << "Richardson-Lucy nested iteration: " << nested_subiterations_num 
		 << " Gradient(update image) after sensitivity devision: old value (min, max): (" 
		 << current_min_nested_gradient << ", " << current_max_nested_gradient
		 << "), new value (min, max) (" 
		 << max(current_min_nested_gradient, new_min_nested_gradient) << ", " 
		 << min(current_max_nested_gradient, new_max_nested_gradient) 
	...
	if ( (nested_subiterations_num % this->save_nested_subiterations_interval)==0 )
      {
	   ...
	   Succeeded write_result = OutputFileFormat<DiscretisedDensity<3,float> >::
                                  default_sptr()-> write_to_file(nested_iter_output_filename, current_estimate);
  ...
  cerr << "End of nested reconstruction process of motion-corrected estimates (after " 
       << this->num_nested_subiterations << " nested Richardson-Lucy EM subiterations)" << endl << endl;
...
compute_model_sensitivity_image(TargetT& motion_corrected_image)
{
  ...
  // Nicolas K. : We leave sensitivity image to be all of ones as this is the definition of the sensitivity image in the case of motion transformations
...
void distributable_compute_outer_loop_gradient(const shared_ptr<ForwardProjectorByBin>& forward_projector_sptr,
...
void distributable_accumulate_nested_loglikelihood(

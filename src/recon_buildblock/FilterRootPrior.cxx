...
template <typename DataT>
FilterRootPrior<DataT>::FilterRootPrior(shared_ptr<DataProcessor<DataT>> const& filter_sptr, float penalisation_factor_v)
...
quotient_with_max(const T numerator, const T denominator, const T max) {
  assert(max > 0);
  return my_abs(numerator) < max * my_abs(denominator) ? numerator / denominator : max * sign(numerator) * sign(denominator);
}

template <typename DataT>
double
FilterRootPrior<DataT>::compute_value(const DataT& current_estimate) {
...
template <typename DataT>
void
FilterRootPrior<DataT>::compute_gradient(DataT& prior_gradient, const DataT& current_image_estimate) {
  ...
  typename DataT::full_iterator iter_through_prior_gradient = prior_gradient.begin_all();
  typename DataT::const_full_iterator iter_through_current_image_estimate = current_image_estimate.begin_all();
  while (iter_through_current_image_estimate != current_image_estimate.end_all()) {
    *iter_through_prior_gradient =
        this->penalisation_factor * (quotient_with_max(*iter_through_current_image_estimate, *iter_through_prior_gradient,
...
template <typename DataT>
void
FilterRootPrior<DataT>::check(DataT const& current_image_estimate) const {
  // Do base-class check
  base_type::check(current_image_estimate);
}

template <typename DataT>
const char* const FilterRootPrior<DataT>::registered_name = "FilterRootPrior";

...
MatrixFile* mptr = matrix_open(filename.c_str(),  MAT_READ_ONLY, Norm3d);
 ...
 ignore_key("data format");
  ...
  case Scanner::Siemens_mMR:
    break;
  default:
    error(boost::format("Unknown originating_system '%s', when parsing file '%s'") % /*interfile_parser.*/ originating_system %
          filename);
  }
  ...
  */
  ...
  if (this->_use_gaps) {
    ...
    if (scanner_ptr->get_num_virtual_transaxial_crystals_per_block() > 0) {
      for (int r = 0; r < scanner_ptr->get_num_rings(); ++r)
        for (int c = 0; c < scanner_ptr->get_num_detectors_per_ring(); c += scanner_ptr->get_num_transaxial_crystals_per_block())
    ...
    if (scanner_ptr->get_num_virtual_axial_crystals_per_block() > 0) {
      ...
      for (int r = scanner_ptr->get_num_axial_crystals_per_block() - scanner_ptr->get_num_virtual_axial_crystals_per_block();
           r < scanner_ptr->get_num_rings(); r += scanner_ptr->get_num_axial_crystals_per_block())
...
float
BinNormalisationFromECAT8::get_dead_time_efficiency(const DetectionPosition<>& det_pos, const double start_time,

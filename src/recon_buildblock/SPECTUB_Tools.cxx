...
void
index_calc(int* indexs, SPECTUB::wmh_type& wmh)
{
  if (wmh.prj.NOS == 1)
    {
...
void
read_Rrad(float* Rrad, SPECTUB::wmh_type& wmh)
{
  string line;
  ifstream stream1(wmh.Rrad_fn.c_str());
  if (!stream1)
    error_wmtools_SPECT(114, wmh.Rrad_fn);
  ...
  if (i != wmh.prj.Nang)
    error_wmtools_SPECT(11, wmh.Rrad_fn);
  stream1.close();
...
//	else{
//		getline ( stream1, line );
//		COL->A = atof( line.c_str() );          // linear factor for dependency of sigma on distance (parallel):
// sigma=A*dist+B
//
//		if( !stream1.eof() ) error_wmtools_SPECT( 13, wmh.col_fn	);
//
...
float
calc_sigma_v(voxel_type vox, collim_type COL, SPECTUB::wmh_type& wmh)
{
  float sigma;
  if (COL.do_fb)
    {
...
void
fill_ang(angle_type* ang, SPECTUB::wmh_type& wmh, const float* Rrad)
{
...
void
generate_msk(bool* msk_3d, bool* msk_2d, float* attmap, volume_type* vol, SPECTUB::wmh_type& wmh)
{
...
void
read_msk_file(bool* msk, SPECTUB::wmh_type& wmh)
{
  FILE* fid;
  int* aux;
  ...
  if ((fid = fopen(wmh.msk_fn.c_str(), "rb")) == NULL)
    error_wmtools_SPECT(126, wmh.msk_fn);
...
void
read_att_map(float* attmap, SPECTUB::wmh_type& wmh)
{
  FILE* fid;
  if ((fid = fopen(wmh.att_fn.c_str(), "rb")) == NULL)
    error_wmtools_SPECT(124, wmh.att_fn);
  ...
  if (exist_nan)
    cout << "WARNING: att map contains NaN values. Converted to zero" << endl;
...
int
max_psf_szb(angle_type* ang, SPECTUB::wmh_type& wmh)
{
  int maxszb;
...
void
error_wmtools_SPECT(int nerr, string txt)
{
#if 0
  switch(nerr){
  ...
  switch (nerr)
    ...
    case 11:
      ...
      break;
    case 12:
      error("\n\nError wm_SPECT: first parameter in collimator file should be 'p' or 'f' to indicate parallel or fanbeam "
            "collimator\n");
      break;
    case 13:
      printf("\n\nError wm_SPECT: not enough parameters in collimator file: %s \n", txt.c_str());

...
double CudaRelativeDifferencePrior<elemT>::compute_value(const DiscretisedDensity<3, elemT>& current_image_estimate) {
    ...
    if (do_kappa)
    {
        ...
        cudaMalloc(&d_kappa, current_image_estimate.size_all() * sizeof(float));
        cudaMemcpy(d_kappa, kappa_data.data(), current_image_estimate.size_all() * sizeof(float), cudaMemcpyHostToDevice);
        computeCudaRelativeDifferencePriorValueKernel<<<cuda_grid_dim, cuda_block_dim>>>(
...
//template class CudaRelativeDifferencePrior<float>::CudaRelativeDifferencePrior(const bool only_2D, float penalization_factor, float gamma, float epsilon) : base_type<elemT>(only_2D, penalization_factor, gamma, epsilon) {}

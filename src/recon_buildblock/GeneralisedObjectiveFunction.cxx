...
template <typename TargetT>
void
GeneralisedObjectiveFunction<TargetT>::compute_sub_gradient(TargetT& gradient, const TargetT& current_estimate,
...
template <typename TargetT>
Succeeded
GeneralisedObjectiveFunction<TargetT>::add_multiplication_with_approximate_sub_Hessian_without_penalty(
...
template <typename TargetT>
Succeeded
GeneralisedObjectiveFunction<TargetT>::add_multiplication_with_approximate_sub_Hessian(TargetT& output, const TargetT& input,
  ...
  if (!this->prior_is_zero()) {
    ...
    if (this->prior_sptr->add_multiplication_with_approximate_Hessian(*prior_output_sptr, output) == Succeeded::no)
      return Succeeded::no;
...
template <typename TargetT>
Succeeded
GeneralisedObjectiveFunction<TargetT>::accumulate_Hessian_times_input(TargetT& output, const TargetT& current_image_estimate,
                                                                      const TargetT& input) const {
  for (int subset_num = 0; subset_num < this->get_num_subsets(); ++subset_num) {
...
template <typename TargetT>
Succeeded
GeneralisedObjectiveFunction<TargetT>::accumulate_sub_Hessian_times_input(TargetT& output, const TargetT& current_image_estimate,
  ...
  if (!this->prior_is_zero()) {
    // TODO used boost:scoped_ptr
    shared_ptr<TargetT> prior_output_sptr(output.get_empty_copy());
    if (this->prior_sptr->accumulate_Hessian_times_input(*prior_output_sptr, current_image_estimate, output) == Succeeded::no)
      return Succeeded::no;

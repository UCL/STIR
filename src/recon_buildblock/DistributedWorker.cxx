    ...
    while (true) {
      ...
      } else if (status.MPI_TAG == NEW_VIEWGRAM_TAG) // receive a message with a new viewgram
      {
        ...
        if (add_bin_corr_viewgrams) {
          distributed::receive_and_construct_related_viewgrams(additive_binwise_correction_viewgrams, proj_data_info_sptr,
                                                               symmetries_sptr, 0);
        }
      ...
      RPC_process_related_viewgrams(this->proj_pair_sptr->get_forward_projector_sptr(),
                                    this->proj_pair_sptr->get_back_projector_sptr(), viewgrams, count, count2, log_likelihood_ptr,
                                    additive_binwise_correction_viewgrams, mult_viewgrams_ptr);

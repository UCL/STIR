...
BinNormalisationFromECAT7::initialise_keymap() {
  ...
  this->parser.add_key("normalisation_filename", &this->normalisation_ECAT7_filename);
  ...
  this->parser.add_stop_key("End Bin Normalisation From ECAT7");
}
...
BinNormalisationFromECAT7::BinNormalisationFromECAT7() { set_defaults(); }

BinNormalisationFromECAT7::BinNormalisationFromECAT7(const std::string& filename) { read_norm_data(filename); }
...
#if 1
float
BinNormalisationFromECAT7::get_bin_efficiency(const Bin& bin, const double start_time, const double end_time) const {
    ...
    for (uncompressed_bin.view_num() = start_view; uncompressed_bin.view_num() < end_view; ++uncompressed_bin.view_num()) {
      ...
      */
      for (uncompressed_bin.segment_num() = min_ring_diff + (min_ring_diff + ring1_plus_ring2) % 2;
        ...
        int geo_plane_num = detail::set_detection_axial_coords(proj_data_info_cyl_ptr, ring1_plus_ring2, uncompressed_bin,
...
float
BinNormalisationFromECAT7::get_dead_time_efficiency(const DetectionPosition<>& det_pos, const double start_time,

...
template <>
const char* const LogcoshPrior<float>::registered_name = "Logcosh";
...
template <typename elemT>
LogcoshPrior<elemT>::LogcoshPrior(const bool only_2D_v, float penalisation_factor_v) : only_2D(only_2D_v) {
  set_defaults();
...
template <typename elemT>
Array<3, float>
LogcoshPrior<elemT>::get_weights() const {
  return this->weights;
}
...
template <typename elemT>
void
LogcoshPrior<elemT>::set_weights(const Array<3, float>& w) {
  this->weights = w;
}
...
LogcoshPrior<elemT>::get_kappa_sptr() const {
...
template <typename elemT>
void
LogcoshPrior<elemT>::set_kappa_sptr(const shared_ptr<DiscretisedDensity<3, elemT>>& k) {
  this->kappa_ptr = k;
}
...
template <typename elemT>
float
LogcoshPrior<elemT>::get_scalar() const {
  return this->scalar;
}
...
template <typename elemT>
void
LogcoshPrior<elemT>::set_scalar(const float scalar_v) {
  scalar = scalar_v;
}
    ...
    for (int y = -1; y <= 1; ++y)
      for (int x = -1; x <= 1; ++x) {
        if (z == 0 && y == 0 && x == 0)
          ...
          weights[z][y][x] =
              grid_spacing.x() / sqrt(square(x * grid_spacing.x()) + square(y * grid_spacing.y()) + square(z * grid_spacing.z()));
        }
      }
}
...
template <typename elemT>
double
LogcoshPrior<elemT>::compute_value(const DiscretisedDensity<3, elemT>& current_image_estimate) {
...
template <typename elemT>
void
LogcoshPrior<elemT>::compute_gradient(DiscretisedDensity<3, elemT>& prior_gradient,
...
template <typename elemT>
void
LogcoshPrior<elemT>::compute_Hessian(DiscretisedDensity<3, elemT>& prior_Hessian_for_single_densel,
...
template <typename elemT>
Succeeded
LogcoshPrior<elemT>::accumulate_Hessian_times_input(DiscretisedDensity<3, elemT>& output,

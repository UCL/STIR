...
template <typename TargetT>
int
PoissonLogLikelihoodWithLinearModelForMeanAndListModeDataWithProjMatrixByBin<TargetT>::set_num_subsets(
...
PoissonLogLikelihoodWithLinearModelForMeanAndListModeDataWithProjMatrixByBin<TargetT>::actual_subsets_are_approximately_balanced(
  ...
  for (int subset_num = 0; subset_num < this->num_subsets; ++subset_num) {
    for (int timing_pos_num = proj_data_info_sptr->get_min_tof_pos_num();
         timing_pos_num <= proj_data_info_sptr->get_max_tof_pos_num(); ++timing_pos_num) {
      for (int segment_num = -this->max_ring_difference_num_to_process; segment_num <= this->max_ring_difference_num_to_process;
           ++segment_num) {
        for (int axial_num = proj_data_info_sptr->get_min_axial_pos_num(segment_num);
             axial_num < proj_data_info_sptr->get_max_axial_pos_num(segment_num); axial_num++) {
          // For debugging.
          //                std::cout <<segment_num << " "<<  axial_num  << std::endl;

          for (int tang_num = proj_data_info_sptr->get_min_tangential_pos_num();
               tang_num < proj_data_info_sptr->get_max_tangential_pos_num(); tang_num++) {
            for (int view_num = proj_data_info_sptr->get_min_view_num() + subset_num;
                 view_num <= proj_data_info_sptr->get_max_view_num(); view_num += this->num_subsets) {
  ...
  for (int subset_num = 1; subset_num < this->num_subsets; ++subset_num) {
    if (num_bins_in_subset[subset_num] != num_bins_in_subset[0]) {
      ...
      str << "Number of subsets is such that subsets will be very unbalanced.\n"
          ...
          << proj_data_info_sptr->get_num_views() << "/4 (or if that's not an integer, a divisor of "
          << proj_data_info_sptr->get_num_views() << "/2 or " << proj_data_info_sptr->get_num_views() << ").\n";
...
PoissonLogLikelihoodWithLinearModelForMeanAndListModeDataWithProjMatrixByBin<TargetT>::post_processing() {
  ...
  {
    warning("You need to specify a projection matrix");
    return true;
  }
  ...
  if (this->max_ring_difference_num_to_process == -1) {
    this->max_ring_difference_num_to_process = scanner_sptr->get_num_rings() - 1;
  }
  ...
  if (this->additive_projection_data_filename != "0") {
    ...
    shared_ptr<ProjData> temp_additive_proj_data_sptr = ProjData::read_from_file(this->additive_projection_data_filename);
  ...
  if (max_ring_difference_num_to_process > proj_data_info_sptr->get_max_segment_num()) {
    ...
    proj_data_info_sptr->reduce_segment_range(-max_ring_difference_num_to_process, max_ring_difference_num_to_process);
  }
  ...
  if (this->normalisation_sptr->set_up(this->list_mode_data_sptr->get_exam_info_sptr(), proj_data_info_sptr) == Succeeded::no) {
...
PoissonLogLikelihoodWithLinearModelForMeanAndListModeDataWithProjMatrixByBin<TargetT>::add_subset_sensitivity(
  ...
  for (int segment_num = min_segment_num; segment_num <= max_segment_num; ++segment_num) {
    for (int view = proj_data_info_sptr->get_min_view_num() + subset_num; view <= proj_data_info_sptr->get_max_view_num();
...
template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndListModeDataWithProjMatrixByBin<TargetT>::add_view_seg_to_sensitivity(
    const ViewSegmentNumbers& view_seg_nums) const {
  ...
  for (int timing_pos_num = min_timing_pos_num; timing_pos_num <= max_timing_pos_num; ++timing_pos_num) {
    shared_ptr<DataSymmetriesForViewSegmentNumbers> symmetries_used(this->projector_pair_sptr->get_symmetries_used()->clone());

    RelatedViewgrams<float> viewgrams =
        proj_data_info_sptr->get_empty_related_viewgrams(view_seg_nums, symmetries_used, false, timing_pos_num);
...
PoissonLogLikelihoodWithLinearModelForMeanAndListModeDataWithProjMatrixByBin<
  ...
  long int more_events = this->do_time_frame ? 1 : (this->num_events_to_use / this->num_subsets);
  ...
  while (more_events) // this->list_mode_data_sptr->get_next_record(record) == Succeeded::yes)
  {
    ...
    if (record.is_event() && record.event().is_prompt()) {
      ...
      if (measured_bin.get_bin_value() != 1.0f || measured_bin.segment_num() < proj_data_info_sptr->get_min_segment_num() ||

...
const char* const ForwardProjectorByBinUsingRayTracing::registered_name = "Ray Tracing";

void
ForwardProjectorByBinUsingRayTracing::set_defaults() {
  restrict_to_cylindrical_FOV = true;
}

void
ForwardProjectorByBinUsingRayTracing::initialise_keymap() {
...
ForwardProjectorByBinUsingRayTracing::ForwardProjectorByBinUsingRayTracing() { set_defaults(); }

ForwardProjectorByBinUsingRayTracing::ForwardProjectorByBinUsingRayTracing(
...
void
ForwardProjectorByBinUsingRayTracing::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_sptr,
                                             const shared_ptr<const DiscretisedDensity<3, float>>& image_info_ptr) {
  ...
  const float z_origin_in_planes = image_info_ptr->get_origin().z() / voxel_size.z();
  ...
  for (int segment_num = proj_data_info_sptr->get_min_segment_num(); segment_num <= proj_data_info_sptr->get_max_segment_num();
       ++segment_num) {
    const float num_planes_per_axial_pos = symmetries_ptr->get_num_planes_per_axial_pos(segment_num);
...
void
ForwardProjectorByBinUsingRayTracing::actual_forward_project(RelatedViewgrams<float>& viewgrams,
...

{
  ...
  if (viewgrams.get_basic_segment_num() == 0) {
    if (viewgrams.get_num_viewgrams() == 1) {
      ...
      forward_project_view_2D(pos_view, image, min_axial_pos_num, max_axial_pos_num, min_tangential_pos_num,
                              max_tangential_pos_num);

    } else if (viewgrams.get_num_viewgrams() == 2) {
      ...
      if ((viewgrams.begin() + 1)->get_view_num() == pos_view.get_view_num() + num_views / 2) {
        ...
        forward_project_view_plus_90_2D(pos_view, pos_plus90, image, min_axial_pos_num, max_axial_pos_num, min_tangential_pos_num,
                                        max_tangential_pos_num);
      } else {
        ...
        forward_project_view_min_180_2D(pos_view, pos_min180, image, min_axial_pos_num, max_axial_pos_num, min_tangential_pos_num,
                                        max_tangential_pos_num);
      }
    } else {
      assert(viewgrams.get_basic_view_num() > 0);
      ...
      forward_project_all_symmetries_2D(pos_view, pos_plus90, pos_min180, pos_min90, image, min_axial_pos_num, max_axial_pos_num,
    ...
    if (viewgrams.get_num_viewgrams() == 2) {
      ...
      forward_project_delta(pos_view, neg_view, image, min_axial_pos_num, max_axial_pos_num, min_tangential_pos_num,
                            max_tangential_pos_num);
      ...
      if ((viewgrams.begin() + 2)->get_view_num() == pos_view.get_view_num() + num_views / 2) {
        ...
        if (neg_plus90.get_segment_num() != -pos_view.get_segment_num())
          error("ForwardProjectorUsingRayTracing: error in symmetries. Check 3D case with 4 viewgrams\n");
        forward_project_view_plus_90_and_delta(pos_view, neg_view, pos_plus90, neg_plus90, image, min_axial_pos_num,
                                               max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num);
      } else {
        Viewgram<float>& pos_min180 = *(viewgrams.begin() + 2);
        ...
        forward_project_view_min_180_and_delta(pos_view, neg_view, pos_min180, neg_min180, image, min_axial_pos_num,
                                               max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num);
      }
      ...
      forward_project_all_symmetries(pos_view, neg_view, pos_plus90, neg_plus90, pos_min180, neg_min180, pos_min90, neg_min90,
                                     image, min_axial_pos_num, max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num);
...
void
ForwardProjectorByBinUsingRayTracing::forward_project_all_symmetries(
    Viewgram<float>& pos_view, Viewgram<float>& neg_view, Viewgram<float>& pos_plus90, Viewgram<float>& neg_plus90,
    Viewgram<float>& pos_min180, Viewgram<float>& neg_min180, Viewgram<float>& pos_min90, Viewgram<float>& neg_min90,
    const VoxelsOnCartesianGrid<float>& image, const int min_ax_pos_num, const int max_ax_pos_num,
  ...
  const shared_ptr<const ProjDataInfoCylindrical> proj_data_info_sptr =
      dynamic_pointer_cast<const ProjDataInfoCylindrical>(pos_view.get_proj_data_info_sptr());
  ...
  */
  ...
  const float sphi = sin(proj_data_info_sptr->get_phi(bin));
  ...
  const int num_planes_per_axial_pos = round(symmetries_ptr->get_num_planes_per_axial_pos(segment_num));
  const float axial_pos_to_z_offset = symmetries_ptr->get_axial_pos_to_z_offset(segment_num);

  // KT 20/06/2001 parameters to find 'basic' range of tang_pos_num
  const int max_abs_tangential_pos_num = max(max_tangential_pos_num, -min_tangential_pos_num);
  const int min_abs_tangential_pos_num =
      max_tangential_pos_num < 0 ? -max_tangential_pos_num : (min_tangential_pos_num > 0 ? min_tangential_pos_num : 0);
  // in the loop, the case tang_pos_num==0 will be treated separately (because it's self-symmetric)
  const int min_tang_pos_num_in_loop = min_abs_tangential_pos_num == 0 ? 1 : min_abs_tangential_pos_num;
      ...
      for (D = 0; D < C; D++) {
        if (min_abs_tangential_pos_num == 0) {
          ...
          if (proj_Siddon
              ...
              (2,
#endif
                  Projall, image, proj_data_info_sptr, cphi, sphi, delta + D, 0, R, min_ax_pos_num, max_ax_pos_num, offset,
                  num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / num_lors_per_virtual_ring, restrict_to_cylindrical_FOV))
        ...
        for (tang_pos_num = min_tang_pos_num_in_loop; tang_pos_num <= max_abs_tangential_pos_num; tang_pos_num++) {
          ...
          if (proj_Siddon
              ...
              (1,
#endif
                  Projall, image, proj_data_info_sptr, cphi, sphi, delta + D, s_in_mm, R, min_ax_pos_num, max_ax_pos_num, offset,
                  num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / num_lors_per_virtual_ring, restrict_to_cylindrical_FOV))
            for (ax_pos0 = min_ax_pos_num; ax_pos0 <= max_ax_pos_num; ax_pos0++) {
              my_ax_pos0 = C * ax_pos0 + D;
...
void
ForwardProjectorByBinUsingRayTracing::forward_project_delta(Viewgram<float>& pos_view, Viewgram<float>& neg_view,
  ...
  assert(pos_view.get_view_num() < pos_view.get_proj_data_info_sptr()->get_num_views());

  Viewgram<float> dummy = pos_view;

  forward_project_all_symmetries(pos_view, neg_view, dummy, dummy, dummy, dummy, dummy, dummy, image, min_axial_pos_num,
                                 max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num);
}
...
void
ForwardProjectorByBinUsingRayTracing::forward_project_view_plus_90_and_delta(
    Viewgram<float>& pos_view, Viewgram<float>& neg_view, Viewgram<float>& pos_plus90, Viewgram<float>& neg_plus90,
    const VoxelsOnCartesianGrid<float>& image, const int min_axial_pos_num, const int max_axial_pos_num,
    const int min_tangential_pos_num, const int max_tangential_pos_num) const {
  assert(pos_view.get_segment_num() > 0);
  assert(pos_view.get_view_num() >= 0);
  assert(pos_view.get_view_num() < pos_view.get_proj_data_info_sptr()->get_num_views() / 2);

  Viewgram<float> dummy = pos_view;

  forward_project_all_symmetries(pos_view, neg_view, pos_plus90, neg_plus90, dummy, dummy, dummy, dummy, image, min_axial_pos_num,
                                 max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num);
}

void
ForwardProjectorByBinUsingRayTracing::forward_project_view_min_180_and_delta(
    Viewgram<float>& pos_view, Viewgram<float>& neg_view, Viewgram<float>& pos_min180, Viewgram<float>& neg_min180,
    const VoxelsOnCartesianGrid<float>& image, const int min_axial_pos_num, const int max_axial_pos_num,
    const int min_tangential_pos_num, const int max_tangential_pos_num) const {
  assert(pos_view.get_segment_num() > 0);
  assert(pos_view.get_view_num() >= 0);
  ...
  forward_project_all_symmetries(pos_view, neg_view, dummy, dummy, pos_min180, neg_min180, dummy, dummy, image, min_axial_pos_num,
                                 max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num);
}
			...
			const int view, const int rmin, const int rmax)
{
  ...
  const ProjDataInfoCylindricalArcCorr* proj_data_cyl_ptr =
    dynamic_cast<const ProjDataInfoCylindricalArcCorr*>(sinos.get_proj_data_info_sptr());
  ...
  const float planes_per_virtual_ring = 
    sinos.get_proj_data_info_sptr()->get_scanner_ptr()->ring_spacing / image.get_voxel_size().z();
...
{
  const ProjDataInfoCylindricalArcCorr* proj_data_cyl_ptr =
    dynamic_cast<const ProjDataInfoCylindricalArcCorr*>(sino.get_proj_data_info_sptr());
  ...
  if ( proj_data_cyl_ptr==NULL)
  {
    ...
    const float planes_per_virtual_ring = 
      sino.get_proj_data_info_sptr()->get_scanner_ptr()->ring_spacing / image.get_voxel_size().z();
    ...
    // CL&KT 05/11/98 use scan_info
    const int nviews = sino.get_proj_data_info_sptr()->get_num_views();
    ...
    assert(image.get_voxel_size().x == sino.get_proj_data_info_sptr()->get_num_tangential_poss()); 
    ...
    int  D, tang_pos_num;
    // CL&KT 05/11/98 use scan_info
    const float R = sino.get_proj_data_info_sptr()->get_scanner_ptr()->ring_radius;
...
void
ForwardProjectorByBinUsingRayTracing::forward_project_view_2D(Viewgram<float>& pos_view,
                                                              const VoxelsOnCartesianGrid<float>& image,
                                                              const int min_axial_pos_num, const int max_axial_pos_num,
                                                              const int min_tangential_pos_num,
                                                              const int max_tangential_pos_num) const {
  ...
  forward_project_all_symmetries_2D(pos_view, dummy, dummy, dummy, image, min_axial_pos_num, max_axial_pos_num,
                                    min_tangential_pos_num, max_tangential_pos_num);
}

void
ForwardProjectorByBinUsingRayTracing::forward_project_view_plus_90_2D(Viewgram<float>& pos_view, Viewgram<float>& pos_plus90,
                                                                      const VoxelsOnCartesianGrid<float>& image,
                                                                      const int min_axial_pos_num, const int max_axial_pos_num,
                                                                      const int min_tangential_pos_num,
                                                                      const int max_tangential_pos_num) const {
  ...
  forward_project_all_symmetries_2D(pos_view, pos_plus90, dummy, dummy, image, min_axial_pos_num, max_axial_pos_num,
                                    min_tangential_pos_num, max_tangential_pos_num);
}

void
ForwardProjectorByBinUsingRayTracing::forward_project_view_min_180_2D(Viewgram<float>& pos_view, Viewgram<float>& pos_min180,
                                                                      const VoxelsOnCartesianGrid<float>& image,
                                                                      const int min_axial_pos_num, const int max_axial_pos_num,
                                                                      const int min_tangential_pos_num,
                                                                      const int max_tangential_pos_num) const {
  ...
  forward_project_all_symmetries_2D(pos_view, dummy, pos_min180, dummy, image, min_axial_pos_num, max_axial_pos_num,
                                    min_tangential_pos_num, max_tangential_pos_num);
}

void
ForwardProjectorByBinUsingRayTracing::forward_project_all_symmetries_2D(Viewgram<float>& pos_view, Viewgram<float>& pos_plus90,
                                                                        Viewgram<float>& pos_min180, Viewgram<float>& pos_min90,
                                                                        const VoxelsOnCartesianGrid<float>& image,
                                                                        const int min_axial_pos_num, const int max_axial_pos_num,
                                                                        const int min_tangential_pos_num,
                                                                        const int max_tangential_pos_num) const {
      ...
      if (min_abs_tangential_pos_num == 0) {
        /* Here tang_pos_num=0 and phi=0 or 45*/
        {
          if (proj_Siddon
              ...
              (2,
#endif
                  Projall, image, proj_data_info_sptr, cphi, sphi, delta + D, 0, R, min_axial_pos_num, max_axial_pos_num,
                  0.F /*==offset*/, num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / num_lors_per_virtual_ring,
                  restrict_to_cylindrical_FOV))
        ...
        if (num_planes_per_axial_pos == 2) {
          if (proj_Siddon
              ...
              (2,
#endif
                  Projall2, image, proj_data_info_sptr, cphi, sphi, delta + D, 0, R, min_axial_pos_num, max_axial_pos_num + 1,
                  -0.5F /*==offset*/, num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / 4, restrict_to_cylindrical_FOV))
            for (int ax_pos0 = min_axial_pos_num; ax_pos0 <= max_axial_pos_num; ax_pos0++) {
              my_ax_pos0 = C * ax_pos0 + D;
      ...
      for (tang_pos_num = min_tang_pos_num_in_loop; tang_pos_num <= max_abs_tangential_pos_num; tang_pos_num++) {
...

        {
          if (proj_Siddon
              ...
              (1,
#endif
                  Projall, image, proj_data_info_sptr, cphi, sphi, delta + D, s_in_mm, R, min_axial_pos_num, max_axial_pos_num,
                  0.F, num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / num_lors_per_virtual_ring,
                  restrict_to_cylindrical_FOV))
            for (int ax_pos0 = min_axial_pos_num; ax_pos0 <= max_axial_pos_num; ax_pos0++) {
              my_ax_pos0 = C * ax_pos0 + D;
              if (tang_pos_num <= max_tangential_pos_num) {
        ...
        if (num_planes_per_axial_pos == 2) {
          if (proj_Siddon
              ...
              (1,
#endif
                  Projall2, image, proj_data_info_sptr, cphi, sphi, delta + D, s_in_mm, R, min_axial_pos_num,
                  max_axial_pos_num + 1, -0.5F, num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / 4,
                  restrict_to_cylindrical_FOV))
            for (int ax_pos0 = min_axial_pos_num; ax_pos0 <= max_axial_pos_num; ax_pos0++) {
              my_ax_pos0 = C * ax_pos0 + D;
              if (tang_pos_num <= max_tangential_pos_num) {
      ...
      if (min_abs_tangential_pos_num == 0) {
        /* Here tang_pos_num==0 and phi!=k*45 */
        {
          if (proj_Siddon
              ...
              (4,
#endif
                  Projall, image, proj_data_info_sptr, cphi, sphi, delta + D, 0, R, min_axial_pos_num, max_axial_pos_num, 0.F,
                  num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / num_lors_per_virtual_ring, restrict_to_cylindrical_FOV))
            for (int ax_pos0 = min_axial_pos_num; ax_pos0 <= max_axial_pos_num; ax_pos0++) {
              my_ax_pos0 = C * ax_pos0 + D;
              pos_view[my_ax_pos0][0] += Projall[ax_pos0][0][0][0];
        ...
        if (num_planes_per_axial_pos == 2) {
          if (proj_Siddon
              ...
              (4,
#endif
                  Projall2, image, proj_data_info_sptr, cphi, sphi, delta + D, 0, R, min_axial_pos_num, max_axial_pos_num, -0.5F,
                  num_planes_per_axial_pos, axial_pos_to_z_offset, 1.F / 4, restrict_to_cylindrical_FOV))
            for (int ax_pos0 = min_axial_pos_num; ax_pos0 <= max_axial_pos_num; ax_pos0++) {
              my_ax_pos0 = C * ax_pos0 + D;

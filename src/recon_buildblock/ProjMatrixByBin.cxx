...
ProjMatrixByBin::enable_tof(const shared_ptr<const ProjDataInfo>& _proj_data_info_sptr, const bool v)
{
  if (v)
    {
      if (proj_data_info_sptr->get_num_tof_poss() == 1 || proj_data_info_sptr->get_scanner_ptr()->get_timing_resolution() == 0 ) // This is a special case that we do a nonTOF backprojection without coincidence window.
        {
          tof_enabled = false;
        }
      else
...
void
ProjMatrixByBin::store_only_basic_bins_in_cache(const bool v)
{
  cache_stores_only_basic_bins = v;
}

bool
ProjMatrixByBin::is_cache_enabled() const
{
  return !cache_disabled;
}

bool
ProjMatrixByBin::does_cache_store_only_basic_bins() const
{
  return cache_stores_only_basic_bins;
}

void
ProjMatrixByBin::clear_cache() const
{
#ifdef STIR_OPENMP
#  pragma omp critical(PROJMATRIXBYBINCLEARCACHE)
#endif
  for (int i = this->cache_collection.get_min_index(); i <= this->cache_collection.get_max_index(); ++i)
    {
      for (int j = this->cache_collection[i].get_min_index(); j <= this->cache_collection[i].get_max_index(); ++j)
        {
...
void
ProjMatrixByBin::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_sptr_v,
...
)
{
  ...
  this->image_info_sptr.reset(dynamic_cast<const VoxelsOnCartesianGrid<float>*>(density_info_sptr_v->clone()));
  if (is_cache_enabled())
    {
      ...
      const int max_abs_timing_pos_num
          = std::max(-proj_data_info_sptr->get_min_tof_pos_num(), proj_data_info_sptr->get_max_tof_pos_num());
      ...
      if ((static_cast<CacheKey>(max_abs_axial_pos_num) >= (static_cast<CacheKey>(1) << axial_pos_bits))
        ...
        error("ProjMatrixByBin: not enough bits reserved for this data in the caching mechanism. You will have to switch caching "
              "off. Sorry.");
    }
...
void
ProjMatrixByBin::cache_proj_matrix_elems_for_one_bin(const ProjMatrixElemsForOneBin& probabilities) const
{
  if (cache_disabled)
    return;
...
#endif
  cache_collection[bin.view_num()][bin.segment_num()].insert(
      MapProjMatrixElemsForOneBin::value_type(cache_key(bin), probabilities));
#ifdef STIR_OPENMP
...
Succeeded
ProjMatrixByBin::get_cached_proj_matrix_elems_for_one_bin(ProjMatrixElemsForOneBin& probabilities) const
{
  if (cache_disabled)
    return Succeeded::no;
  ...
  {
    const_MapProjMatrixElemsForOneBinIterator pos = cache_collection[bin.view_num()][bin.segment_num()].find(cache_key(bin));

...
ChainedBinNormalisation::ChainedBinNormalisation()
{
  set_defaults();
}
...
void
ChainedBinNormalisation::undo(RelatedViewgrams<float>& viewgrams) const
{
  if (!is_null_ptr(apply_first))
...
void
ChainedBinNormalisation::undo_only_first(RelatedViewgrams<float>& viewgrams) const
{
  if (!is_null_ptr(apply_first))
    apply_first->undo(viewgrams);
}
...
void
ChainedBinNormalisation::undo_only_first(ProjData& proj_data) const
{
  if (!is_null_ptr(apply_first))
    apply_first->undo(proj_data);
}
...
void
ChainedBinNormalisation::undo_only_second(RelatedViewgrams<float>& viewgrams) const
{
  if (!is_null_ptr(apply_second))
    apply_second->undo(viewgrams);
}
...
void
ChainedBinNormalisation::undo_only_second(ProjData& proj_data) const
{
  if (!is_null_ptr(apply_second))
    apply_second->undo(proj_data);
}
...
{
  return (!is_null_ptr(apply_first) ? apply_first->get_bin_efficiency(bin) : 1)
         * (!is_null_ptr(apply_second) ? apply_second->get_bin_efficiency(bin) : 1);
}

...
const char* const ChainedBinNormalisation::registered_name = "Chained";
...
void
ChainedBinNormalisation::initialise_keymap() {
...
bool
ChainedBinNormalisation::post_processing() {
...
ChainedBinNormalisation::ChainedBinNormalisation() { set_defaults(); }

ChainedBinNormalisation::ChainedBinNormalisation(shared_ptr<BinNormalisation> const& apply_first_v,
                                                 shared_ptr<BinNormalisation> const& apply_second_v)
    : apply_first(apply_first_v), apply_second(apply_second_v) {
  post_processing();
}
...
void
ChainedBinNormalisation::apply_only_first(RelatedViewgrams<float>& viewgrams, const double start_time,
...
void
ChainedBinNormalisation::apply_only_second(RelatedViewgrams<float>& viewgrams, const double start_time,
                                           const double end_time) const {
  if (!is_null_ptr(apply_second))
    apply_second->apply(viewgrams, start_time, end_time);
}
...
void
ChainedBinNormalisation::undo(RelatedViewgrams<float>& viewgrams, const double start_time, const double end_time) const {
  if (!is_null_ptr(apply_first))
...
void
ChainedBinNormalisation::undo(ProjData& proj_data, const double start_time, const double end_time) const {
  if (!is_null_ptr(apply_first))
...
void
ChainedBinNormalisation::undo_only_first(RelatedViewgrams<float>& viewgrams, const double start_time,
                                         const double end_time) const {
  if (!is_null_ptr(apply_first))
    apply_first->undo(viewgrams, start_time, end_time);
}
...
void
ChainedBinNormalisation::undo_only_first(ProjData& proj_data, const double start_time, const double end_time) const {
  if (!is_null_ptr(apply_first))
    apply_first->undo(proj_data, start_time, end_time);
}

void
ChainedBinNormalisation::undo_only_second(RelatedViewgrams<float>& viewgrams, const double start_time,
                                          const double end_time) const {
  if (!is_null_ptr(apply_second))
    apply_second->undo(viewgrams, start_time, end_time);
}
...
void
ChainedBinNormalisation::undo_only_second(ProjData& proj_data, const double start_time, const double end_time) const {
  if (!is_null_ptr(apply_second))
    apply_second->undo(proj_data, start_time, end_time);
}
...
ChainedBinNormalisation::get_bin_efficiency(const Bin& bin, const double start_time, const double end_time) const {
  return (!is_null_ptr(apply_first) ? apply_first->get_bin_efficiency(bin, start_time, end_time) : 1) *
         (!is_null_ptr(apply_second) ? apply_second->get_bin_efficiency(bin, start_time, end_time) : 1);
}

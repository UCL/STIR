...
const char* const BinNormalisationSPECT::registered_name = "SPECT";
...
void
BinNormalisationSPECT::initialise_keymap() {
...
bool
BinNormalisationSPECT::post_processing() {
...
BinNormalisationSPECT::BinNormalisationSPECT() { set_defaults(); }

Succeeded
BinNormalisationSPECT::set_up(const shared_ptr<const ExamInfo>& exam_info_sptr,
...
BinNormalisationSPECT::BinNormalisationSPECT(const std::string& filename) { read_norm_data(filename); }

void
BinNormalisationSPECT::read_norm_data(const std::string& filename) { // to think about this
}

float
BinNormalisationSPECT::get_uncalibrated_bin_efficiency(const Bin& bin, const double start_time, const double end_time) const {
  ...
  double rel_time;
  rel_time = (this->view_time_interval) * (bin.view_num() + 1 - head_num * (num_views / num_detector_heads));
  /*####################################################################################################
   *####################################   uniformity factors  #########################################*/
  ...
  if (use_decay_correction_factors()) {
    normalisation = normalisation / decay_correction_factor(half_life, rel_time);
  }
  // std::cout<<"value"<<uniformity[head_num][bin.axial_pos_num()][bin.tangential_pos_num()+max_tang+1]<<"
  // "<<normalisation<<std::endl;
  return normalisation;
}

void
BinNormalisationSPECT::apply(RelatedViewgrams<float>& viewgrams, const double start_time, const double end_time) const {
  ...
  double rel_time;
  rel_time = (this->view_time_interval) * (view_num + 1 - head_num * (num_views / num_detector_heads));
    ...
    for (bin.axial_pos_num() = iter->get_min_axial_pos_num(); bin.axial_pos_num() <= iter->get_max_axial_pos_num();
      ...
      for (bin.tangential_pos_num() = iter->get_min_tangential_pos_num();
        ...
        (*iter)[bin.axial_pos_num()][bin.tangential_pos_num()] /=
            (std::max(1.E-20F, get_uncalibrated_bin_efficiency(bin, start_time, end_time)) * normalisation);
        normalisation = 1;
      }
  }
}
...
void
BinNormalisationSPECT::undo(RelatedViewgrams<float>& viewgrams, const double start_time, const double end_time) const {
...
void
BinNormalisationSPECT::read_uniformity_table(
    Array<3, float>& uniformity) const { // std::ofstream unif_table("uniformity.dat",std::ios::out);
...
void
BinNormalisationSPECT::resample_uniformity( // Array<3,float>& down_sampled_uniformity,
    Array<3, float> uniformity, const int max_tang, const int zoom) const {
      ...
      for (int j = 0; j <= 2 * max_tang + 1; j++) {
        for (int l = 0; l <= zoom - 1; l++) {
          for (int k = 0; k <= zoom - 1; k++) { // maybe resize uniformity
            ...
            down_sampled_uniformity[n][i][j] =
                down_sampled_uniformity[n][i][j] + uniformity[n][zoom * i + l][zoom * j + k] / square(zoom);
...
bool
BinNormalisationSPECT::use_detector_efficiencies() const {
  return this->_use_detector_efficiencies;
}

bool
BinNormalisationSPECT::use_decay_correction_factors() const {
  return this->_use_decay_correction;
}

bool
BinNormalisationSPECT::use_dead_time() const {
  return this->_use_dead_time;
}

bool
BinNormalisationSPECT::use_uniformity_factors() const {
  return this->_use_uniformity_factors;
}

double
BinNormalisationSPECT::get_half_life() const {

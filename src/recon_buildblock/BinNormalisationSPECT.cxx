...
BinNormalisationSPECT::BinNormalisationSPECT()
{
  set_defaults();
}
...
BinNormalisationSPECT::BinNormalisationSPECT(const std::string& filename)
{
  read_norm_data(filename);
}

void
BinNormalisationSPECT::read_norm_data(const std::string& filename)
{ // to think about this
}

float
BinNormalisationSPECT::get_uncalibrated_bin_efficiency(const Bin& bin) const
{
  ...
  if (zoom != 1 && !resampled && use_uniformity_factors())
    {
      ...
      resample_uniformity(uniformity, max_tang, zoom);
    }
...
void
BinNormalisationSPECT::apply(RelatedViewgrams<float>& viewgrams) const
{
        ...
        for (bin.tangential_pos_num() = iter->get_min_tangential_pos_num();
             ...
             ++bin.tangential_pos_num())
          {
            ...
            if (use_uniformity_factors())
              {
                ...
                if (zoom != 1)
                  normalisation
                      = normalisation
                        * down_sampled_uniformity[head_num][bin.axial_pos_num()][bin.tangential_pos_num() + max_tang + 1];
                else
                  normalisation
            ...
            (*iter)[bin.axial_pos_num()][bin.tangential_pos_num()]
                /= (std::max(1.E-20F, get_uncalibrated_bin_efficiency(bin)) * normalisation);
            normalisation = 1;
          }
    }
}
...
void
BinNormalisationSPECT::undo(RelatedViewgrams<float>& viewgrams) const
{
...
void
BinNormalisationSPECT::read_uniformity_table(Array<3, float>& uniformity) const
{
...
void
BinNormalisationSPECT::resample_uniformity(Array<3, float> uniformity, const int max_tang, const int zoom) const
{

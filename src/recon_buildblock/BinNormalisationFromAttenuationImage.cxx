...
BinNormalisationFromAttenuationImage::BinNormalisationFromAttenuationImage() { set_defaults(); }
...
BinNormalisationFromAttenuationImage::BinNormalisationFromAttenuationImage(
    ...
    : forward_projector_ptr(forward_projector_ptr), attenuation_image_filename(filename) {
  attenuation_image_ptr.reset();
  post_processing();
}
...
void
BinNormalisationFromAttenuationImage::apply(RelatedViewgrams<float>& viewgrams, const double start_time,
  ...
  for (RelatedViewgrams<float>::iterator viewgrams_iter = attenuation_viewgrams.begin();
       viewgrams_iter != attenuation_viewgrams.end(); ++viewgrams_iter) {
...
void
BinNormalisationFromAttenuationImage::undo(RelatedViewgrams<float>& viewgrams, const double start_time,
                                           const double end_time) const {
  this->check(*viewgrams.get_proj_data_info_sptr());
  RelatedViewgrams<float> attenuation_viewgrams = viewgrams.get_empty_copy();
  forward_projector_ptr->forward_project(attenuation_viewgrams);

...
void
wm_calculation(const int kOS, const angle_type* const ang, voxel_type vox, bin_type bin, const volume_type& vol,
               const proj_type& prj, const float* attmap, const bool* msk_3d, const bool* msk_2d, const int maxszb,
               const discrf_type* const gaussdens, const int* const NITEMS) {
  ...
  for (vox.irow = 0; vox.irow < vol.Nrow; vox.irow++) {
    ...
    for (vox.icol = 0; vox.icol < vol.Ncol; vox.icol++) {
      ...
      if (wmh.do_msk) {
        ...
        if (!msk_2d[vox.ip])
          continue; // to skip voxel if it is outside the 2d_mask
      }
      ...
      for (int k = 0; k < prj.NangOS; k++) {
        ...
        if (vox.dv2dp <= 0.)
          continue; // skipping voxel if it is beyond the detection plane (corner voxels)
        ...
        if (!wmh.do_psf)
          fill_psf_no(&psf, &psf1d_h, vox, &ang[ka], bin.szdx);

        else {

          if (wmh.do_psf_3d)
            fill_psf_3d(&psf, &psf1d_h, &psf1d_v, vox, gaussdens, bin.szdx, bin.thdx, bin.thcmd2);

          else
            fill_psf_2d(&psf, &psf1d_h, vox, gaussdens, bin.szdx);
        }
        ...
        if (wmh.do_att) {
          ...
          if (!wmh.do_full_att) { // simple correction for attenuation
            ...
            bin.x = ang[ka].xbin0 +
                    vox.xd0 * ang[ka].cos; // x coord of the projection of the center of the voxel in the detection line
        ...
        for (vox.islc = vol.first_sl; vox.islc < vol.last_sl; vox.islc++) {
          ...
          if (wmh.do_att && !wmh.do_full_att)
            coeff_att = calc_att(&attpth[0], attmap, vox.islc);
          ...
          for (int ie = 0; ie < psf.Nib; ie++) {
            ...
            if (wmh.do_full_att)
              coeff_att = calc_att(&attpth[ie], attmap, vox.islc);
            ...
            if (wm.do_save_STIR) {
              ...
              wm.nx[vox.iv] = (short int)invert.invert_axis_index((vox.icol - (int)floor(vol.Ncold2)), vol.Ncold2 * 2,
                                                                  "x");       // centered index for STIR format
...
void
wm_size_estimation(int kOS, const angle_type* const ang, voxel_type vox, bin_type bin, const volume_type& vol,
                   const proj_type& prj, const bool* const msk_3d, const bool* const msk_2d, const int maxszb,
                   const discrf_type* const gaussdens, int* NITEMS) {
  int jp;
  float eff;
...
void
calc_gauss(discrf_type* gaussdens) {
...
void
calc_vxprj(angle_type* ang) {
  //... initialization to zero .....................................

  for (int j = 0; j < ang->vxprj.lng; j++)
    ang->vxprj.acu[j] = ang->vxprj.val[j] = (float)0.;
...
void
voxel_projection(voxel_type* vox, float* eff, float lngcmd2) {

  if (wmh.COL.do_fb) { // fan_beam

    //... angle between voxel-focal line and center-focal line and distance from voxel projection to detection line relevant
    // points........
    ...
    if (wmh.do_psf)
      *eff = vox->costhe * vox->costhe * (wmh.COL.F - REF_DIST) / (wmh.COL.F - vox->dv2dp);
    else
      *eff = (float)1.;
...
void
fill_psf_no(psf2da_type* psf, psf1d_type* psf1d_h, const voxel_type& vox, angle_type const* const ang, float szdx) {
  psf1d_h->sgmcm = vox.szcm;

  if (wmh.COL.do_fb) {
    if (fabs(vox.x1) > EPSILON)
      psf1d_h->sgmcm *= vox.xdc / vox.x1; // fb expanded projection of the voxel
  }
...
void
fill_psf_2d(psf2da_type* psf, psf1d_type* psf1d_h, const voxel_type& vox, discrf_type const* const gaussdens, float szdx) {
...
void
fill_psf_3d(psf2da_type* psf, psf1d_type* psf1d_h, psf1d_type* psf1d_v, const voxel_type& vox, discrf_type const* const gaussdens,
  ...
  for (int i = 0; i < psf1d_h->Nib; i++) {
    ...
    for (int j = 0; j < psf1d_v->Nib; j++) {
      ...
      if ((a + (float)(psf1d_v->ind[j] * psf1d_v->ind[j]) / Nib_vp2) > (float)1.)
        continue;
...
void
calc_psf_bin(float center_psf, float binszcm, discrf_type const* const vxprj, psf1d_type* psf) {
  float weight, preval;
...
void
calc_att_path(const bin_type& bin, const voxel_type& vox, const volume_type& vol, attpth_type* attpth) {
  float dx, dy, dz;
...
float
calc_att(const attpth_type* const attpth, const float* const attmap, int nsli) {

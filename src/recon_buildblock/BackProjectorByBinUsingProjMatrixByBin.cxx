...
const char* const BackProjectorByBinUsingProjMatrixByBin::registered_name = "Matrix";
...
void
BackProjectorByBinUsingProjMatrixByBin::initialise_keymap() {
...
bool
BackProjectorByBinUsingProjMatrixByBin::post_processing() {
...
BackProjectorByBinUsingProjMatrixByBin::BackProjectorByBinUsingProjMatrixByBin() { set_defaults(); }

BackProjectorByBinUsingProjMatrixByBin::BackProjectorByBinUsingProjMatrixByBin(const shared_ptr<ProjMatrixByBin>& proj_matrix_ptr)
...
void
BackProjectorByBinUsingProjMatrixByBin::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_ptr,
...
void
BackProjectorByBinUsingProjMatrixByBin::actual_back_project(DiscretisedDensity<3, float>& image,
  ...
  if (proj_matrix_ptr->is_cache_enabled()/* &&
    ...
    Array<2, int> already_processed(
        IndexRange2D(min_axial_pos_num, max_axial_pos_num, min_tangential_pos_num, max_tangential_pos_num));
    ...
    for (int tang_pos = min_tangential_pos_num; tang_pos <= max_tangential_pos_num; ++tang_pos)
      for (int ax_pos = min_axial_pos_num; ax_pos <= max_axial_pos_num; ++ax_pos) {
        ...
        symmetries->get_related_bins_factorised(related_ax_tang_poss, basic_bin, min_axial_pos_num, max_axial_pos_num,
        ...
        for (
            ...
            r_ax_tang_poss_iter != related_ax_tang_poss.end(); ++r_ax_tang_poss_iter) {
          ...
          for (RelatedViewgrams<float>::const_iterator viewgram_iter = viewgrams.begin(); viewgram_iter != viewgrams.end();
               ++viewgram_iter) {
            // KT 21/02/2002 added check on 0

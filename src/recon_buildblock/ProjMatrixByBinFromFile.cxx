...
ProjMatrixByBinFromFile::ProjMatrixByBinFromFile() { set_defaults(); }
...
ProjMatrixByBinFromFile::post_processing() {
  ...
  if (this->symmetries_type == standardise_interfile_keyword("PET_CartesianGrid")) {
    symmetries_sptr.reset(new DataSymmetriesForBins_PET_CartesianGrid(
        proj_data_info_ptr, density_info_sptr, do_symmetry_90degrees_min_phi, do_symmetry_180degrees_min_phi,
...
Succeeded
ProjMatrixByBinFromFile::write_to_file(const std::string& output_filename_prefix, const ProjMatrixByBin& proj_matrix,
  ...
  {
    if (OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(template_density_filename,
  ...
  // projection matrix.
  {
    ...
    for (int segment_num = proj_data_info_sptr->get_min_segment_num(); segment_num <= proj_data_info_sptr->get_max_segment_num();
      ...
      for (int axial_pos_num = proj_data_info_sptr->get_min_axial_pos_num(segment_num);
           axial_pos_num <= proj_data_info_sptr->get_max_axial_pos_num(segment_num); ++axial_pos_num)
        for (int view_num = proj_data_info_sptr->get_min_view_num(); view_num <= proj_data_info_sptr->get_max_view_num();
          ...
          for (int tang_pos_num = proj_data_info_sptr->get_min_tangential_pos_num();
               tang_pos_num <= proj_data_info_sptr->get_max_tangential_pos_num(); ++tang_pos_num) {

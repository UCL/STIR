...
const char* const BinNormalisationFromGEHDF5::registered_name = "From GE HDF5";
...
set_detection_tangential_coords(shared_ptr<ProjDataInfoCylindricalNoArcCorr> proj_data_cyl_uncomp, const Bin& uncomp_bin,
  ...
  proj_data_cyl_uncomp->get_det_num_pair_for_view_tangential_pos_num(det1_num, det2_num, uncomp_bin.view_num(),
...
static int
set_detection_axial_coords(const ProjDataInfoCylindricalNoArcCorr* proj_data_info_cyl, int ring1_plus_ring2,
                           const Bin& uncomp_bin, DetectionPositionPair<>& detection_position_pair) {
...
static std::vector<int> // Prefered since C++11
...
static std::vector<unsigned int> // Prefered since C++11
create_ax_pos_offset(shared_ptr<ProjDataInfo> const& proj_data_info_ptr, std::vector<int>& segment_sequence) {
  ...
  for (int i_seg = 1; i_seg < proj_data_info_ptr->get_num_segments(); ++i_seg) {
    ...
    seg_ax_offset[i_seg] = static_cast<unsigned int>(proj_data_info_ptr->get_num_axial_poss(segment_num)) + previous_value;
...
void
BinNormalisationFromGEHDF5::initialise_keymap() {
...
bool
BinNormalisationFromGEHDF5::post_processing() {
  read_norm_data(normalisation_GEHDF5_filename);
  this->set_calibration_factor(1); // TODO: read actual factor somewhere
  return false;
}

BinNormalisationFromGEHDF5::BinNormalisationFromGEHDF5() { set_defaults(); }

BinNormalisationFromGEHDF5::BinNormalisationFromGEHDF5(const string& filename) { read_norm_data(filename); }
...
// This function will take a filename, that can be either a GE norm file, or a GE geo file. If you wan to do geo and norm
// corrections then you want the norm file as input, and if you only want geo files, then just the geo file is enough. The
// function will read from these files and fill the atributes with a full sinogram of efficiency factors and geometry factors.
void
BinNormalisationFromGEHDF5::read_norm_data(const string& filename) {
  ...
  if (!this->use_detector_efficiencies() && !this->use_geometric_factors())
    return;
  // Build the HDF5 wrapper. This opens the file and makes sure its the correct type, plus loads all information about the
  // scanner.
  m_input_hdf5_sptr.reset(new GEHDF5Wrapper(filename));

  // We need the norm file to correct for geometry and efficiecies (the geometric correcction is contained inside the norm file
  // too!) But if we are not correcting for efficiencies, then we dont require the file to be a norm file, it can be geo.
  ...
  if (this->use_geometric_factors()) {
    // Construct a proper ProjDataInfo to initialize geometry factors array and use it to know the boudns of the iteratios to load
    // it.
    shared_ptr<ProjDataInfo> projInfo =
        ProjDataInfo::construct_proj_data_info(scanner_ptr,
                                               ...
                                               /* num_tangential_poss */ scanner_ptr->get_max_num_non_arccorrected_bins(),
                                               /* arc_corrected */ false);
    geo_eff_factors_sptr.reset(new ProjDataInMemory(m_input_hdf5_sptr->get_exam_info_sptr(), projInfo,
                                                    true)); // Initialize with zeroes (always true internally...)

    // TODO: remove all these loops and "duplication", and load the entire geometric factors file. Then modify the function
    // get_geometric_factors() so that when accessed, re-indexes the bin number to the correct geometric factor. Doing this would
    // save lots of RAM, as there are lots of symetries that are exploited in the geo file, but we are here undoing all that and
    // duplicating data.
    ...
    // Geometric factors are related to geometry (ovbiously). This means that as the scanner has several geometric symetries
    // itself, there is no need to store all of them in a big file. This is what GE does in RDF9 files. The following loops undo
    // that. They go selecting different data pieces in the initialise_geo_factors() and reading different parts of it in
    // read_geo_factors(), all to create a complete sinogram with all the geo factors loaded.
      ...
      for (int i_view = 0; i_view < scanner_ptr->get_max_num_views(); ++i_view) {
        ...
        case 3: {
          ...
          std::array<hsize_t, 2> offset = {
              segment_axial_position_offset[detail::find_segment_index_in_sequence(segment_sequence, i_seg)], 0};
          ...
          std::transform(buffer.begin(), buffer.end(), viewgram.begin_all(),
        ...
        case 2: {
          ...
          // copy data back
          // AB TODO: Hardcoded magic number, remove somehow (when magic is discovered)
          std::transform(repeat_buffer.begin(), repeat_buffer.end(), viewgram.begin_all(),
                         [](const float f) { return 1 / (f * 2.2110049e-4); });
          break;
        }
        default:
...
bool
BinNormalisationFromGEHDF5::use_detector_efficiencies() const {
  return this->_use_detector_efficiencies;
}

bool
BinNormalisationFromGEHDF5::use_dead_time() const {
  return this->_use_dead_time;
}

bool
BinNormalisationFromGEHDF5::use_geometric_factors() const {
  ...
  for (uncompressed_bin.view_num() = start_view; uncompressed_bin.view_num() < end_view; ++uncompressed_bin.view_num()) {
    ...
    */
    for (uncompressed_bin.segment_num() = min_ring_diff + (min_ring_diff + ring1_plus_ring2) % 2;
         uncompressed_bin.segment_num() <= max_ring_diff; uncompressed_bin.segment_num() += 2) {

      // Make sure we are within the range. Just some error checking.
      int geo_plane_num =
          detail::set_detection_axial_coords(proj_data_info_cyl_ptr, ring1_plus_ring2, uncompressed_bin, detection_position_pair);
      if (geo_plane_num < 0) {

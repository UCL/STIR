...
BinNormalisationFromGEHDF5::BinNormalisationFromGEHDF5()
{
  set_defaults();
}

BinNormalisationFromGEHDF5::BinNormalisationFromGEHDF5(const string& filename)
{
...
BinNormalisationFromGEHDF5::read_norm_data(const string& filename)
{
  ...
  if (this->use_detector_efficiencies())
    {
      // Allocate efficiency factor data from an "uncompressed scanner" (i.e. span = 1, all bins are physical bins in the
      // scanner).
      efficiency_factors
  ...
  if (this->use_geometric_factors())
    {
      // Construct a proper ProjDataInfo to initialize geometry factors array and use it to know the bounds of the iteratios to
      // load it.
      ...
      // get_geometric_factors() so that when accessed, re-indexes the bin number to the correct geometric factor. Doing this
      // would save lots of RAM, as there are lots of symetries that are exploited in the geo file, but we are here undoing all
      // that and duplicating data.
              ...
              switch (m_input_hdf5_sptr->get_geo_dims())
                {
                  case 3: {
                    ...
                    std::transform(
                        buffer.begin(), buffer.end(), viewgram.begin_all(), [](const float f) { return 1 / (f * 2.2110049e-4); });
                    break;
                  }
                  case 2: {
                    ...
                    // copy data back
                    // AB TODO: Hardcoded magic number, remove somehow (when magic is discovered)
                    std::transform(repeat_buffer.begin(), repeat_buffer.end(), viewgram.begin_all(), [](const float f) {
                      return 1 / (f * 2.2110049e-4);
                    });
                    break;
                  }
                default:

...
static Succeeded find_start_values(const shared_ptr<const ProjDataInfoCylindricalArcCorr> proj_data_info_sptr, const float delta,
                                   const double cphi, const double sphi, const int s, const int ring0, const float image_rad,
                                   const double d_sl, int& X1, int& Y1, int& Z1, double& ds, double& dz, double& dzhor,
                                   double& dzvert, const float num_planes_per_axial_pos, const float axial_pos_to_z_offset);

inline void
check_values(const shared_ptr<const ProjDataInfoCylindricalArcCorr> proj_data_info_sptr, const float delta, const double cphi,
             const double sphi, const int s, const int ring0, const int X1, const int Y1, const int Z1, const double ds,
             const double dz, const float num_planes_per_axial_pos, const float axial_pos_to_z_offset) {
#ifdef BPINT_CHECK
...
#endif
    (Array<4, float> const& Projptr, VoxelsOnCartesianGrid<float>& image,
     const shared_ptr<const ProjDataInfoCylindricalArcCorr> proj_data_info_sptr, float delta, const double cphi,
     const double sphi, int s, int ring0, const int num_planes_per_axial_pos, const float axial_pos_to_z_offset) {
...
#  error times sqrt(2)/iets met t
  ...
  */
  ...
  if (find_start_values(proj_data_info_sptr, delta, cphi, sphi, s, ring0, image_rad, image.get_voxel_size().z(), X, Y, Z, ds, dz,
                        dzhor, dzvert, num_planes_per_axial_pos, axial_pos_to_z_offset) == Succeeded::no) {
...
#  endif // ALTERNATIVE
    ...
    check_values(proj_data_info_sptr, delta, cphi, sphi, s, ring0, X, Y, Z, ds, dz, num_planes_per_axial_pos,
...
static Succeeded
find_start_values(const shared_ptr<const ProjDataInfoCylindricalArcCorr> proj_data_info_sptr, const float delta,
                  const double cphi, const double sphi, const int s, const int ring0, const float image_rad, const double d_sl,
                  int& X1, int& Y1, int& Z1, double& ds, double& dz, double& dzhor, double& dzvert,
                  const float num_planes_per_axial_pos, const float axial_pos_to_z_offset) {

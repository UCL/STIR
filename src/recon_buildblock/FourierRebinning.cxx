...
const char* const FourierRebinning::registered_name = "FORE";

void
FourierRebinning::initialise_keymap() {
...
bool
FourierRebinning::post_processing() {
...
FourierRebinning::FourierRebinning() { set_defaults(); }

Succeeded
FourierRebinning::rebin() {

  if (proj_data_sptr->get_proj_data_info_sptr()->is_tof_data()) {
  ...
  int num_views_pow2;
  for (num_views_pow2 = 1; num_views_pow2 < 2 * proj_data_sptr->get_num_views() && num_views_pow2 < (1 << 15);
       num_views_pow2 *= 2)
    ;
  int num_tang_poss_pow2;
  for (num_tang_poss_pow2 = 1; num_tang_poss_pow2 < proj_data_sptr->get_num_tangential_poss() && num_tang_poss_pow2 < (1 << 15);
  ...
  Array<3, std::complex<float>> FT_rebinned_data(
      IndexRange3D(0, num_planes - 1, 0, num_views_pow2 - 1, 0, num_tang_poss_pow2 - 1));
  ...
  shared_ptr<ProjDataInfo> rebinned_proj_data_info_sptr(proj_data_sptr->get_proj_data_info_sptr()->clone());
  ...
  const int max_delta = dynamic_cast<ProjDataInfoCylindrical const&>(*proj_data_sptr->get_proj_data_info_sptr())
  ...
  rebinned_proj_data_sptr.reset(
      new ProjDataInterfile(proj_data_sptr->get_exam_info_sptr(), rebinned_proj_data_info_sptr, output_filename_prefix));
  ...
  const float R_field_of_view_mm =
      ((int)(rebinned_proj_data_info_sptr->get_num_tangential_poss() / 2) - 1) * sampling_distance_in_s;
  ...
  for (int seg_num = 0; seg_num <= max_segment_num_to_process; seg_num++) {
    ...
    const ProjDataInfoCylindrical& proj_data_info_cylindrical =
        dynamic_cast<const ProjDataInfoCylindrical&>(*segment.get_proj_data_info_sptr());
    const float average_ring_difference_in_segment =
        proj_data_info_cylindrical.get_average_ring_difference(segment.get_segment_num());
    ...
    segment.grow(IndexRange3D(segment.get_min_axial_pos_num(), segment.get_max_axial_pos_num(), 0,
                              2 * segment.get_num_views() - 1, segment.get_min_tangential_pos_num(),
                              segment.get_max_tangential_pos_num()));

    // CON merge the two segments to form "360 degrees" sinograms
    const int min_tangential_pos_num = std::max(segment_neg.get_min_tangential_pos_num(), -segment.get_max_tangential_pos_num());

    const int max_tangential_pos_num = std::min(segment_neg.get_max_tangential_pos_num(), -segment.get_min_tangential_pos_num());
    ...
    do_rebinning(FT_rebinned_data, Weights_for_FT_rebinned_data, num_rebinned, segment, num_tang_poss_pow2, num_views_pow2,
                 num_planes, average_ring_difference_in_segment, half_distance_between_rings, sampling_distance_in_s,
                 radial_sampling_freq_w, R_field_of_view_mm, ratio_ring_spacing_to_ring_radius);
    ...
    for (int j = 0; j < num_tang_poss_pow2; j++) {
      for (int i = 0; i <= num_views_pow2 / 2; i++) {
        const float Actual_Weight =
            (Weights_for_FT_rebinned_data[plane][i][j] == 0) ? 0 : 1.F / (Weights_for_FT_rebinned_data[plane][i][j]);
...
void
FourierRebinning::do_rebinning(Array<3, std::complex<float>>& FT_rebinned_data, Array<3, float>& Weights_for_FT_rebinned_data,
                               PETCount_rebinned& count_rebinned, const SegmentBySinogram<float>& segment,
                               const int num_tang_poss_pow2, const int num_views_pow2, const int num_planes,
                               const float average_ring_difference_in_segment, const float half_distance_between_rings,
                               const float sampling_distance_in_s, const float radial_sampling_freq_w,
...

{
  ...
  for (int axial_pos_num = segment.get_min_axial_pos_num(); axial_pos_num <= segment.get_max_axial_pos_num(); axial_pos_num++) {
    ...
    const float z_in_mm =
        proj_data_info.get_m(Bin(segment.get_segment_num(), 0, axial_pos_num, 0)) - proj_data_info.get_m(Bin(0, 0, 0, 0));

    // CON Call the rebinning kernel.
    rebinning(FT_rebinned_data, Weights_for_FT_rebinned_data, count_rebinned, FT_current_sinogram, z_in_mm,
              average_ring_difference_in_segment, num_views_pow2, num_tang_poss_pow2, half_distance_between_rings,
              sampling_distance_in_s, radial_sampling_freq_w, R_field_of_view_mm, ratio_ring_spacing_to_ring_radius);
...
void
FourierRebinning::rebinning(Array<3, std::complex<float>>& FT_rebinned_data, Array<3, float>& Weights_for_FT_rebinned_data,
                            PETCount_rebinned& num_rebinned, const Array<2, std::complex<float>>& FT_current_sinogram,
                            const float z_in_mm, const float delta, const int num_views_pow2, const int num_tang_poss_pow2,
...
void
FourierRebinning::do_log_file() { // CL Saving time details and write them to log file
  char file[200];
...
void
FourierRebinning::do_display_count(PETCount_rebinned& count) { // Display rebinning statistics
  std::cout << "FORE Rebinning :: Total rebinned: " << count.total << '\n'
            << "                  Total missed: " << count.miss << '\n';

  if (count.miss != 0)
    std::cout << "                  (" << 100. * count.miss / (count.miss + count.ssrb) << " percent)\n";
  std::cout << "FORE Rebinning :: Total rebinned SSRB: " << count.ssrb << std::endl;
}

void
FourierRebinning::do_adjust_nb_views_to_pow2(SegmentBySinogram<float>& segment) {
...
Succeeded
FourierRebinning::fore_check_parameters(int num_tang_poss_pow2, int num_views_pow2, int max_segment_num_to_process) {
  ...
  if (wmin < 0 || kmin < 0) {
    warning("FORE initialisation :: The parameters wmin and kmin must be >=0\n"
            "                       Negative frequencies are not allowed. Due to symmetry reasons they are implicitly defined by "
            "the positive wmin/kmin cut off values");
    return Succeeded::no;
  }
  ...
  if (kc >= num_views_pow2 / 2) {
    warning(boost::format("FORE initialisation :: Your parameter kc is larger than the highest frequency component in w (FTT of "
                          "radial coordinate s)\n"
                          "                       Choose an value smaller than the largest frequency\n"

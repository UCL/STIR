...
warp_imageTests::run_tests() {
  ...
  mvtest.warp_image(accumulated_image, gated_image);
  {
    ...
    check_if_equal((int)gated_image.get_time_gate_definitions().get_num_gates(), 2,
                   "testing gate_defs of gated_image are set correctly");

    // test if motion vectors have been set correctly
    check_if_equal((reverse_gated_motion_z.get_density(2))[indices], -1 * grid_spacing[1],
                   "testing the input to set the motion in z");
    check_if_equal((mvtest.get_spatial_transformation_z().get_density(2))[indices], -1 * grid_spacing[1],
                   "testing GatedSpatialTransformation class get the motion vector z correctly");
    check_if_equal((mvtest.get_spatial_transformation_z().get_density(2))[new_indices], -1 * grid_spacing[1],
                   "testing GatedSpatialTransformation class get the motion vector z correctly");
    check_if_equal((mvtest.get_spatial_transformation_y().get_density(2))[indices], -2 * grid_spacing[2],
                   "testing GatedSpatialTransformation class get the motion vector y correctly");
    check_if_equal((mvtest.get_spatial_transformation_y().get_density(2))[new_indices], -2 * grid_spacing[2],
                   "testing GatedSpatialTransformation class get the motion vector y correctly");
    check_if_equal((mvtest.get_spatial_transformation_x().get_density(2))[indices], -3 * grid_spacing[3],
                   "testing GatedSpatialTransformation class get the motion vector x correctly");
    check_if_equal((mvtest.get_spatial_transformation_x().get_density(2))[new_indices], -3 * grid_spacing[3],
                   "testing GatedSpatialTransformation class get the motion vector x correctly");
    check_if_equal((int)gate_defs.get_num_gates(), 2, "testing gate_defs are set correctly");
    check_if_equal((int)reverse_gated_motion_z.get_time_gate_definitions().get_num_gates(), 2,
                   "testing GatedSpatialTransformation class get the motion vector x correctly");
    check_if_equal((int)(mvtest.get_time_gate_definitions()).get_num_gates(), 2,
    ...
    check_if_equal(accumulated_image[new_indices], 0.F,
                   "testing the accumulated image at the location where the non-zero point had moved");
  }
}
END_NAMESPACE_STIR

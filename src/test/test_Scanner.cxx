  ...
  */
  ...
  if (scanner.get_type() <= Scanner::E966) // TODO relies on ordering of enum
  {
    ...
    if (scanner.get_type() != Scanner::E925) // ART is a partial ring tomograph
      check_if_equal(scanner.get_num_axial_buckets() * scanner.get_num_transaxial_buckets(), ecat_scanner_info->nbuckets,
    ...
    check_if_equal(scanner.get_num_axial_blocks_per_bucket(), ecat_scanner_info->axialBlocksPerBucket, "axial blocks per bucket");
    check_if_equal(scanner.get_num_transaxial_blocks_per_bucket() * scanner.get_num_axial_blocks_per_bucket(),
                   ecat_scanner_info->blocks, "total number of blocks");
    ...
    check_if_equal(scanner.get_default_bin_size(), ecat_scanner_info->binsize * 10, "bin size (spacing of transaxial elements)");
  }
#endif
}

...

  {
    ...
    for (cur_iter_1 = file_plasma_data.begin(), cur_iter_2 = testing_plasma_data.begin();
         ...
         ++cur_iter_1, ++cur_iter_2)
      {
        ...
        check_if_equal((*cur_iter_1).get_plasma_counts_in_kBq(),
                       (*cur_iter_2).get_plasma_counts_in_kBq(),
                       "Check Reading Plasma of PlasmaData ");
        check_if_equal((*cur_iter_1).get_blood_counts_in_kBq(),
                       (*cur_iter_2).get_blood_counts_in_kBq(),
...

  {
    ...
    for (unsigned int param_num = 1; param_num <= 2; ++param_num)
      for (unsigned int frame_num = 23; frame_num <= 28; ++frame_num)
        {
          check_if_equal(file_model_array[param_num][frame_num],
                         get_correct_model_array[param_num][frame_num],
                         "Check ModelMatrix reading. ");
          check_if_equal(
              file_model_array[param_num][frame_num], correct_model_array[param_num][frame_num], "Check ModelMatrix reading. ");
        }
  }
  {
    ...
    for (unsigned int frame_num = 17; frame_num <= time_frame_def.get_num_frames() && frame_num <= plasma_fdef.get_num_frames();
         ++frame_num)
      {
        check_if_equal(time_frame_def.get_start_time(frame_num),
                       plasma_fdef.get_start_time(frame_num),
                       "Check start time when sampling PlasmaData into frames");
        check_if_equal(time_frame_def.get_duration(frame_num),
                       plasma_fdef.get_duration(frame_num),
                       "Check duration when sampling PlasmaData into frames");
        check_if_equal(time_frame_def.get_end_time(frame_num),
                       plasma_fdef.get_end_time(frame_num),
                       "Check duration when sampling PlasmaData into frames");
      }

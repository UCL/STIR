  ...
  // images
  {
      ...
      if (run_stir_math(cmd_args))
        {
          ...
          calc_data += data1 + (data2_thresholded * data2_thresholded) * 3.2F + 3.1F
                       + (data3_thresholded * data3_thresholded) * 3.2F + 3.1F;
    ...
    if (run_stir_math(
            "--accumulate --power 2 --times-scalar 3.2  --add-scalar 3.1 --divide-scalar 2.1 --mult STIRtmp1.hv STIRtmp3.hv"))
      {
        ...
        check_if_equal(
            calc_data, *out_data_ptr, "test with power and scalar multiplication, division and addition with --accumulate");
      }
    ...
    if (run_stir_math("--accumulate --power 2 --times-scalar 3.2 --including-first STIRtmp2.hv STIRtmp3.hv"))
      {
        ...
        check_if_equal(
            calc_data, *out_data_ptr, "test with power and scalar multiplication with --including-first and --accumulate");
      }
      ...
      if (run_stir_math(cmd_args))
        {
          ...
          check_if_equal(calc_data,
                         out_data_ptr->get_segment_by_view(0),
    ...
    if (run_stir_math("-s --power 2 --times-scalar 3.2 --including-first STIRtmpout.hs STIRtmp1.hs STIRtmp2.hs STIRtmp3.hs"))
      {
        ...
        calc_data += (data1 * data1 + data2 * data2 + data3 * data3) * 3.2F;
        check_if_equal(
            calc_data, out_data_ptr->get_segment_by_view(0), "test with power and scalar multiplication with --including-first");
      }
    // add with power etc and accumulate
    if (run_stir_math(

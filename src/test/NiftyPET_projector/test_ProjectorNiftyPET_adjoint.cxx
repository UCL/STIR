...
get_rand_point(const CartesianCoordinate3D<float>& min, const CartesianCoordinate3D<float>& max)
{
  return CartesianCoordinate3D<float>(
      get_rand(min.at(1), max.at(1)), get_rand(min.at(2), max.at(2)), get_rand(min.at(3), max.at(3)));
}
...
TestGPUProjectors::set_up_image()
{
  ...
  _image_sptr = MAKE_SHARED<VoxelsOnCartesianGrid<float>>(_sino_sptr->get_exam_info_sptr(),
                                                          range,
...
static float
test_inner_product(const VoxelsOnCartesianGrid<float>& im,
                   const ProjData& sino,
                   const VoxelsOnCartesianGrid<float>& im_proj,
      ...
      if (adjoint_test > 0.f)
        {
          ...
          std::cout << "\tAvg. test result = " << std::accumulate(_results.begin(), _results.end(), 0.0) / double(i + 1)
                    << " (number of tests = " << i + 1 << "), avg. time forward projecting = " << _time_fwrd / double(i + 1)
                    << " s, "

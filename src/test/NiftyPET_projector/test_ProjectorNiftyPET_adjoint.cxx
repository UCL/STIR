...
public:
  //! Constructor
  explicit TestGPUProjectors(const unsigned num_attempts) : _num_attempts(num_attempts), _time_fwrd(0), _time_back(0) {}
...
static int
get_rand(const int lower, const int upper) {
  return rand() % upper + lower;
}

static float
get_rand(const float lower, const float upper) {
  return lower + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX / (upper - lower)));
}

static CartesianCoordinate3D<float>
get_rand_point(const CartesianCoordinate3D<float>& min, const CartesianCoordinate3D<float>& max) {
  return CartesianCoordinate3D<float>(get_rand(min.at(1), max.at(1)), get_rand(min.at(2), max.at(2)),
                                      get_rand(min.at(3), max.at(3)));
}

static CartesianCoordinate3D<float>
get_rand_point(const float min, const float max) {
  return CartesianCoordinate3D<float>(get_rand(min, max), get_rand(min, max), get_rand(min, max));
}

static float
find_max(const ProjDataInMemory& prj) {
...
void
TestGPUProjectors::set_up_image() {
  ...
  _image_sptr = MAKE_SHARED<VoxelsOnCartesianGrid<float>>(_sino_sptr->get_exam_info_sptr(), range,
...
TestGPUProjectors::set_up_sino() {
  ...
  shared_ptr<ProjDataInfo> proj_data_info_sptr(
      ProjDataInfo::construct_proj_data_info(scanner_sptr,
                                             11, // span
                                             /* mMR needs maxDelta of */ 60, scanner_sptr->get_num_detectors_per_ring() / 2,
...
static float
get_inner_product(const VoxelsOnCartesianGrid<float>& im1, const VoxelsOnCartesianGrid<float>& im2) {
  return std::inner_product(im1.begin_all(), im1.end_all(), im2.begin_all(), 0.f);
}

static float
get_inner_product(const ProjDataInMemory& proj1, const ProjDataInMemory& proj2) {
  // Get number of elements
...
static float
test_inner_product(const VoxelsOnCartesianGrid<float>& im, const ProjData& sino, const VoxelsOnCartesianGrid<float>& im_proj,
                   const ProjData& sino_proj) {
  ...
  float adjoint_test = std::abs(inner_product_images - inner_product_sinos) /
...
void
TestGPUProjectors::test_adjoints() {
  set_up();
  ...
  while (_results.size() < _num_attempts) {
    ...
    const float adjoint_test = test_inner_product(*_image_sptr, *_sino_sptr, *_projected_image_sptr, *_projected_sino_sptr);
    if (adjoint_test > 0.f) {

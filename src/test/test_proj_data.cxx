...
ProjDataTests::run_tests_on_proj_data(ProjData& proj_data) {
  ...
  timer.start();
  {
    shared_ptr<ProjDataInfo> proj_data_info_sptr2(
        ProjDataInfo::ProjDataInfoCTI(proj_data.get_proj_data_info_sptr()->get_scanner_sptr(),
                                      /*span*/ 1, 8, proj_data.get_num_views(), proj_data.get_num_tangential_poss(),
                                      /*arc_corrected*/ true));
  ...
  timer.start();
  {
    shared_ptr<ProjDataInfo> proj_data_info_sptr2(
        ProjDataInfo::ProjDataInfoCTI(proj_data.get_proj_data_info_sptr()->get_scanner_sptr(),
                                      /*span*/ 1, 12, proj_data.get_num_views(), proj_data.get_num_tangential_poss(),
                                      /*arc_corrected*/ true));
...
ProjDataTests::run_tests_in_memory_only(ProjDataInMemory& proj_data) {
  ...
  std::cerr << "test if copy_to is consistent with iterators\n";
  {
    Array<3, float> test_array(
        IndexRange3D(proj_data.get_num_sinograms(), proj_data.get_num_views(), proj_data.get_num_tangential_poss()));
    // copy to the array
...
ProjDataTests::run_tests() {
  ...
  shared_ptr<ProjDataInfo> proj_data_info_sptr(ProjDataInfo::ProjDataInfoCTI(scanner_sptr,
                                                                             /*span*/ 1, 10, /*views*/ 2096, /*tang_pos*/ 132,
                                                                             /*arc_corrected*/ true));

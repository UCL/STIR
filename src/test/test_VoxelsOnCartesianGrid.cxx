...

{
  ...
  }
  {
    ...
    check_if_equal(indices,
                   ob3.get_indices_closest_to_relative_coordinates(rel_coord),
                   "test on get_indices_closest_to_relative_coordinates");
    check_if_equal(
        indices, ob3.get_indices_closest_to_physical_coordinates(coord), "test on get_indices_closest_to_relative_coordinates");
    check_if_equal(indices,
                   ob3.get_indices_closest_to_relative_coordinates(rel_coord + grid_spacing / 3),
    ...
    if (is_arccorrected)
      {
        ...
        check_if_equal(low_bound,
                       CartesianCoordinate3D<int>(0, -(diameter_int / 2), -(diameter_int / 2)),
                       "test on index range: lower bounds");
        check_if_equal(high_bound,
                       CartesianCoordinate3D<int>(30, +(diameter_int / 2), +(diameter_int / 2)),
...

  {
    ...
    check_if_equal(indices,
                   hfs_image.get_indices_closest_to_LPS_coordinates(hfs_image.get_LPS_coordinates_for_indices(indices)),
                   "HFS inverse consistency");
    check_if_equal(indices,
                   ffs_image.get_indices_closest_to_LPS_coordinates(ffs_image.get_LPS_coordinates_for_indices(indices)),
                   "FFS inverse consistency");
    check_if_equal(indices,
                   hfp_image.get_indices_closest_to_LPS_coordinates(hfp_image.get_LPS_coordinates_for_indices(indices)),
                   "HFP inverse consistency");
    check_if_equal(indices,
                   ffp_image.get_indices_closest_to_LPS_coordinates(ffp_image.get_LPS_coordinates_for_indices(indices)),

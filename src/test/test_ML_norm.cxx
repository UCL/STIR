...
test_proj_data_info(shared_ptr<const ProjDataInfoCylindricalNoArcCorr> proj_data_info_sptr)
{
  ...
  const int num_virtual_axial_crystals_per_block =
    proj_data_info_sptr->get_scanner_sptr()->
  ...
  make_fan_data_remove_gaps(fan_data, proj_data);
  {
    ...
    set_fan_data_add_gaps(proj_data2, fan_data, /*gap_value=*/ 1.F);
    {
      ...
      check_if_zero(norm_squared(proj_data2.begin(), proj_data2.end())/square(static_cast<double>(proj_data_info_sptr->size_all())), "projdata <-> fandata with gap filled");
    }
            ...
            for (int tangential_pos_num = 0; tangential_pos_num<=proj_data2.get_max_tangential_pos_num(); ++tangential_pos_num)
              {
                ...
                const bool in_gap = (tangential_pos_num%num_transaxial_crystals_per_block) >= num_physical_transaxial_crystals_per_block;
                if (in_gap)

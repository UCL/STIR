...
ScatterSimulationTests::test_downsampling_ProjDataInfo()
{
  ...
  // Downsample the scanner 50%
  {
    ...
    auto sss_projdata = dynamic_cast<const ProjDataInfoCylindricalNoArcCorr*>(sss->get_template_proj_data_info_sptr().get());
    check_if_equal(original_projdata->get_scanner_ptr()->get_num_rings(),
                   2 * sss_projdata->get_scanner_ptr()->get_num_rings(),
                   "Check the number of rings is correct");
    check_if_equal(original_projdata->get_scanner_ptr()->get_num_detectors_per_ring(),
                   2 * sss_projdata->get_scanner_ptr()->get_num_detectors_per_ring(),
    ...
    Bin b1(original_projdata->get_min_segment_num(),
           0,
           original_projdata->get_max_axial_pos_num(original_projdata->get_min_segment_num()) / 2,
           0);
    Bin b2(
        sss_projdata->get_min_segment_num(), 0, sss_projdata->get_max_axial_pos_num(sss_projdata->get_min_segment_num()) / 2, 0);
...
ScatterSimulationTests::test_symmetric(ScatterSimulation& ss, const std::string& name)
{
  ...
  // check(seg.find_max()>0.F, "Check Scatter Simulation output not zero. test " + name);
  {
    ...
    set_tolerance(.5);
    check_if_equal(seg.find_max() * 1000,
                   approx_max * 1000,

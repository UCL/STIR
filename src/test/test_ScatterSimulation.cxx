...
void
ScatterSimulationTests::test_downsampling_ProjDataInfo() {
  ...
  shared_ptr<ProjDataInfoCylindricalNoArcCorr> original_projdata(dynamic_cast<ProjDataInfoCylindricalNoArcCorr*>(
      ProjDataInfo::ProjDataInfoCTI(test_scanner, 1, test_scanner->get_num_rings() - 1,
                                    test_scanner->get_num_detectors_per_ring() / 2,
                                    test_scanner->get_max_num_non_arccorrected_bins(), false)));
  ...
  // Downsample the scanner 50%
  {
    ...
    auto sss_projdata(sss->get_template_proj_data_info_sptr());
    check_if_equal(original_projdata->get_scanner_ptr()->get_num_rings(), 2 * sss_projdata->get_scanner_ptr()->get_num_rings(),
                   "Check the number of rings is correct");
    ...
    check_if_equal(2.f * original_projdata->get_axial_sampling(0), sss_projdata->get_axial_sampling(0),
                   "Check axial samping. Seg 0");
    ...
    Bin b2(sss_projdata->get_min_segment_num(), 0, sss_projdata->get_max_axial_pos_num(sss_projdata->get_min_segment_num()) / 2,
           0);
...
void
ScatterSimulationTests::test_downsampling_DiscretisedDensity() {
  Scanner::Type type = Scanner::E931;
  shared_ptr<Scanner> test_scanner(new Scanner(type));
  ...
  Box3D phantom(tmpl_density->get_x_size() * tmpl_density->get_voxel_size().x() * 0.25,
                ...
                tmpl_density->get_z_size() * tmpl_density->get_voxel_size().z() * 2, tmpl_density->get_origin());
  ...
  *atten_density += *water_density;
  ...
  CartesianCoordinate3D<float> cog_downed =
      find_centre_of_gravity_in_mm(*dynamic_cast<const VoxelsOnCartesianGrid<float>*>(downed_image.get()));
...
ScatterSimulationTests::test_output_is_symmetric(const ProjData& proj_data, const std::string& name) {
  ...
  for (int first = seg.get_min_axial_pos_num(), last = seg.get_max_axial_pos_num(); first < last; ++first, --last)
  ...
  // test views. Need to reduce tolerance due to discretisation artefacts
  {
    ...
    for (int view = seg.get_min_view_num() + 1; view <= seg.get_max_view_num(); ++view) {
      ...
      row.resize(-5, 5);
      check_if_equal(row_first, row, "check if symmetric along views. test " + name);
    }
    set_tolerance(old_tolerance);
  }
#if 0
...
ScatterSimulationTests::test_scatter_simulation() {
  ...
  shared_ptr<ProjDataInfoCylindricalNoArcCorr> original_projdata_info(dynamic_cast<ProjDataInfoCylindricalNoArcCorr*>(
      ProjDataInfo::ProjDataInfoCTI(test_scanner, 1, 0, test_scanner->get_num_detectors_per_ring() / 2,
                                    test_scanner->get_max_num_non_arccorrected_bins(), false)));
  ...
  CartesianCoordinate3D<float> centre((tmpl_density->get_physical_coordinates_for_indices(min_ind) +
                                       tmpl_density->get_physical_coordinates_for_indices(max_ind)) /
                                      2.F);
  ...
  *water_density *= 9.687E-02;
  ...
  // testing with zooming (but currently fails)
  {
    ...
    shared_ptr<VoxelsOnCartesianGrid<float>> zoomed_act_sptr(new VoxelsOnCartesianGrid<float>(
        zoom_image(*act_density, zooms, CartesianCoordinate3D<float>(0.F, 0.F, 0.F) /* offset */,
...
void
ScatterSimulationTests::run_tests() {

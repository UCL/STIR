...
NestedIteratorTests::run_tests()
{
  ...
  std::cerr << " full iterator coord\n";
  {
    ...
    {
      float value = 1.2F;
      for (Array<2, elemT>::full_iterator iter = test2.begin_all(); iter != test2.end_all();)
        {
    ...
    typedef NestedIterator<Array<2, elemT>::iterator> FullIter;
    FullIter fiter1;
    FullIter fiter(test2.begin(), test2.end());
    // fiter1=fiter;
    const FullIter fiter_end(test2.end(), test2.end());
  ...
  std::cerr << " full iterator coord\n";
  {
    ...
    {
      float value = 1.2F;
      for (Array<2, elemT>::full_iterator iter = test.begin_all(); iter != test.end_all();)
        {
          *iter++ = make_coordinate(value, value + .3F);
          ++value;
        }
    ...
    typedef NestedIterator<Array<2, elemT>::const_iterator, constbeginendfunction_type> FullIter;
    FullIter fiter1;
    FullIter fiter(test2.begin(), test2.end());
    // fiter1=fiter;
    ...
    {
      for (Array<2, elemT>::const_full_iterator iter = test2.begin_all_const(); iter != test2.end_all_const();)
        {
          check(fiter != fiter_end, "fiter");
          check(*fiter++ == *iter++, "fiter==");
        }
  ...
  std::cerr << " full iterator coord full\n";
  {
    ...
    check(test2.begin()->begin() == BeginEndFunction<Array<2, elemT>::iterator>().begin(test2.begin()), "begin");
    check((test2.begin() + 1)->begin() == BeginEndFunction<Array<2, elemT>::iterator>().begin(test2.begin() + 1), "begin");
    typedef NestedIterator<Array<2, elemT>::full_iterator, BeginEndFunction<Array<2, elemT>::full_iterator>> FullIter;
    FullIter fiter1;
  ...
  std::cerr << " full iterator coord full const\n";
  {
    ...
    check(test2.begin_all_const()->begin()
              == ConstBeginEndFunction<Array<2, elemT>::const_full_iterator>().begin(test2.begin_all_const()),
          "begin");
    typedef NestedIterator<Array<2, elemT>::const_full_iterator, ConstBeginEndFunction<Array<2, elemT>::const_full_iterator>>
        FullIter;
    FullIter fiter1;
  ...
  std::cerr << " NestedIterator vector< shared_ptr<Array<2,...> >\n";
  {
    ...
    int count = 1;
    for (C2::full_iterator fullarrayiter = c[0]->begin_all(); fullarrayiter != c[0]->end_all(); ++fullarrayiter, ++count)
      *fullarrayiter = count;
    for (C2::full_iterator fullarrayiter = c[1]->begin_all(); fullarrayiter != c[1]->end_all(); ++fullarrayiter, ++count)
      *fullarrayiter = count;
    ...
    // normal iterator
    {
      ...
      check_if_equal(count,
                     static_cast<int>(c[0]->size_all() + c[1]->size_all() + 1),

...
private:
  ...
  template <class ValueT, class BoundaryT>
  Succeeded test_case(const ValueT& invalues, const BoundaryT& inboundaries, const ValueT& outvalues,
                      const BoundaryT& outboundaries, const char* const description) {
    ValueT my_outvalues = outvalues;
    overlap_interpolate(my_outvalues.begin(), my_outvalues.end(), outboundaries.begin(), outboundaries.end(), invalues.begin(),
                        invalues.end(), inboundaries.begin(), inboundaries.end());
#ifndef STIR_OVERLAP_NORMALISATION
  ...
  Succeeded uniform_test_case(const ValueT& invalues, const float zoom, const float offset, const ValueT& outvalues,

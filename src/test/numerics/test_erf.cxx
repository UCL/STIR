...
erfTests::run_tests() {
...

  {
    ...
    std::vector<double> input_vector(10), output_correct_vector(10), output_correct_vector_c(10), STIR_vector(10),
        STIR_vector_c(10);
    ...
    std::vector<double>::iterator cur_iter_cor = output_correct_vector.begin(), cur_iter_cor_c = output_correct_vector_c.begin(),
                                  cur_iter_STIR = STIR_vector.begin(), cur_iter_STIR_c = STIR_vector_c.begin();

    for (cur_iter_STIR = STIR_vector.begin(); cur_iter_STIR != STIR_vector.end() && cur_iter_cor != output_correct_vector.end();
         ++cur_iter_STIR, ++cur_iter_cor)
      check_if_equal(*cur_iter_cor, *cur_iter_STIR, "check erf() implementation");
    for (; cur_iter_STIR_c != STIR_vector_c.end() && cur_iter_cor_c != output_correct_vector_c.end();
         ++cur_iter_STIR_c, ++cur_iter_cor_c)
      check_if_equal(*cur_iter_cor_c, *cur_iter_STIR_c, "check erfc() implementation");

    for (cur_iter_STIR_c = STIR_vector_c.begin(), cur_iter_STIR = STIR_vector.begin();
         cur_iter_STIR != STIR_vector.end() && cur_iter_STIR_c != STIR_vector_c.end(); ++cur_iter_STIR, ++cur_iter_STIR_c)

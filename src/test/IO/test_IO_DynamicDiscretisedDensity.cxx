...
{
public:
  explicit IOTests_DynamicDiscretisedDensity(istream& in)
      : IOTests(in)
  {}
...
IOTests_DynamicDiscretisedDensity::create_image()
{
  ...
  _image_to_write_sptr.reset(new DynamicDiscretisedDensity(
      tdefs, dummy_im_sptr->get_exam_info().start_time_in_secs_since_1970, scanner_sptr, dummy_im_sptr));
...
IOTests_DynamicDiscretisedDensity::check_result()
{
  ...
  check_if_equal(
      _image_to_read_sptr->get_densities().size(), _image_to_write_sptr->get_densities().size(), "test number of dynamic images");

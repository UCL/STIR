...
private:
  ...
  void compare_results_1arg(const ArrayFunctionObject<num_dimensions, float>& filter1,
                            const ArrayFunctionObject<num_dimensions, float>& filter2, const Array<num_dimensions, float>& test) {
    {
...
ArrayFilterTests::run_tests() {
    ...
    std::cerr << "Testing boundary conditions\n";
    {
      ...
      const ArrayFilter1DUsingConvolution<float> conv_filter_cst_BC(kernel, BoundaryConditions::constant);
      {
        ...
        // test if internal array elements are the same
        {
          ...
          check_if_equal(out_cst_BCs_small, out_zero_BCs_small,
                         "comparing 1D with different boundary conditions: internal values");
        }
        // edge
  ...
  std::cerr << "\nTesting 3D\n";
  {
    ...
    }
    {
      ...
      Array<3, float> kernel_for_conv(IndexRange3D(-(kernel_half_length / 2), kernel_half_length / 2, -kernel_half_length,
                                                   kernel_half_length, -kernel_half_length, kernel_half_length));
      for (int i = -(kernel_half_length / 2); i < kernel_half_length / 2; ++i)
        for (int j = -kernel_half_length; j < kernel_half_length; ++j)

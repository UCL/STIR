...
VectorWithOffsetTests::run_tests()
{
  ...
  // iterator tests
  {
    {
      int value = -3;
      for (VectorWithOffset<int>::iterator iter = v.begin(); iter != v.end(); iter++, value++)
        *iter = value;
  ...
  {   // tests on reserve() with 0 length
    { // check reserve 0,max
      {
        ...
        check_if_equal(test.capacity(), size_t(0), "check capacity after default constructor");
        test.reserve(2);
        check_if_equal(test.size(), size_t(0), "check reserve of empty vector (0,max) (size)");
        check_if_equal(test.capacity(), size_t(2), "check reserve of empty vector (0,max) (capacity)");
        check_if_equal(test.get_capacity_min_index(), 0, "check reserve of empty vector (0,max) (capacity_min_index)");
        check_if_equal(test.get_capacity_max_index(), 1, "check reserve of empty vector (0,max) (capacity_max_index)");
      }
      // check reserve -1,2
      {
        VectorWithOffset<int> test;
        test.reserve(-1, 2);
        check_if_equal(test.size(), size_t(0), "check reserve of empty vector (-1,2) (size)");
        check_if_equal(test.capacity(), size_t(4), "check reserve of empty vector (-1,2) (capacity)");
        // note: for length 0 vectors, get_capacity_min_index() is always 0
        check_if_equal(test.get_capacity_min_index(), 0, "check reserve of empty vector (-1,2) (capacity_min_index)");
        check_if_equal(test.get_capacity_max_index(), 3, "check reserve of empty vector (-1,2) (capacity_max_index)");
      }
      // check reserve -1,2 and then 1,6
      {
        ...
        test.reserve(-1, 2);
        test.reserve(1, 6);
        check_if_equal(test.size(), size_t(0), "check reserve of empty vector (-1,2 and then 1,6) (size)");
        check_if_equal(test.capacity(), size_t(6), "check reserve of empty vector (-1,2 and then 1,6) (capacity)");
        // note: for length 0 vectors, get_capacity_min_index() is always 0
        check_if_equal(
            test.get_capacity_min_index(), 0, "check reserve of empty vector (-1,2 and then 1,6) (capacity_min_index)");
        check_if_equal(
            test.get_capacity_max_index(), 5, "check reserve of empty vector (-1,2 and then 1,6) (capacity_max_index)");
      }
    ...
    // tests of reserve() with non-zero length
    {
      ...
      test.reserve(0, 1);
      check_if_equal(test.size(), ref.size(), "check reserve within range (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index(), "check reserve within range (get_min_index)");
      check_if_equal(test, ref, "check reserve within range (values)");
      check_if_equal(test.capacity(), ref.size(), "check reserve within range (capacity)");
      check_if_equal(test.get_capacity_min_index(), ref.get_min_index(), "check reserve within range (capacity_min_index)");
      check_if_equal(test.get_capacity_max_index(), ref.get_max_index(), "check reserve within range (capacity_max_index)");
      // check reserve within range on low index (should reserve space at higher indices only)
      test.reserve(0, test.get_max_index() + 5);
      check_if_equal(test.size(), ref.size(), "check reserve within range on low index (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index(), "check reserve within range on low index (get_min_index)");
      check_if_equal(test, ref, "check reserve within range on low index (values)");
      check_if_equal(test.capacity(), ref.size() + 5, "check reserve within range on low index (capacity)");
      check_if_equal(
          test.get_capacity_min_index(), ref.get_min_index(), "check reserve within range on low index (capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index() + 5, "check reserve within range on low index (capacity_max_index)");
      ...
      check_if_equal(test.size(), ref.size(), "check reserve within range on high index (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index(), "check reserve within range on high index (get_min_index)");
      check_if_equal(test, ref, "check reserve within range on high index (values)");
      check_if_equal(test.capacity(), ref.size() + 5, "check reserve within range on high index (capacity)");
      check_if_equal(test.get_capacity_min_index(),
                     ref.get_min_index() - 5,
                     "check reserve within range on high index (capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index(), "check reserve within range on high index (capacity_max_index)");
      // check reserve for both ranges
      test.recycle();
      check_if_equal(test.capacity(), size_t(0), "test recycle");
      test = ref;
      test.reserve(test.get_min_index() - 5, test.get_max_index() + 4);
      check_if_equal(test.size(), ref.size(), "check reserve (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index(), "check reserve (get_min_index)");
      check_if_equal(test, ref, "check reserve (values)");
      check_if_equal(test.capacity(), ref.size() + 9, "check reserve (capacity)");
      check_if_equal(test.get_capacity_min_index(), ref.get_min_index() - 5, "check reserve (capacity_min_index)");
      check_if_equal(test.get_capacity_max_index(), ref.get_max_index() + 4, "check reserve (capacity_max_index)");
    }
  }
  /**********************************************************************/
  // tests on resize()
  ...
  {   // tests on resize() with 0 length
    { // check resize 0,max
      {
        ...
        check_if_equal(test.capacity(), size_t(0), "check capacity after default constructor");
        test.resize(2);
        check_if_equal(test.size(), size_t(2), "check resize of empty vector (0,max) (size)");
        check_if_equal(test.capacity(), size_t(2), "check resize of empty vector (0,max) (capacity)");
        check_if_equal(test.get_min_index(), 0, "check resize of empty vector (0,max) (min_index)");
        check_if_equal(test.get_max_index(), 1, "check resize of empty vector (0,max) (max_index)");
      }
      // check resize -1,2
      {
        VectorWithOffset<int> test;
        test.resize(-1, 2);
        check_if_equal(test.size(), size_t(4), "check resize of empty vector (-1,2) (size)");
        check_if_equal(test.capacity(), size_t(4), "check resize of empty vector (-1,2) (capacity)");
        check_if_equal(test.get_min_index(), -1, "check resize of empty vector (-1,2) (min_index)");
        check_if_equal(test.get_max_index(), 2, "check resize of empty vector (-1,2) (max_index)");
      }
      // check resize -1,2 and then 1,6
      {
        ...
        test.resize(-1, 2);
        test.resize(1, 6);
        check_if_equal(test.size(), size_t(6), "check resize of empty vector (-1,2 and then 1,6) (size)");
        check_if_equal(test.capacity(), size_t(8), "check resize of empty vector (-1,2 and then 1,6) (capacity)");
        // note: for length 0 vectors, get_min_index() is always 0
        check_if_equal(test.get_min_index(), 1, "check resize of empty vector (-1,2 and then 1,6) (min_index)");
        check_if_equal(test.get_max_index(), 6, "check resize of empty vector (-1,2 and then 1,6) (max_index)");
      }
    } // end of tests length 0
    ...
    // tests of resize() with non-zero length
    {
      ...
      check_if_equal(test.size(), ref.size(), "check resize with identical range (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index(), "check resize with identical range (get_min_index)");
      check_if_equal(test, ref, "check resize with identical range (values)");
      check_if_equal(test.capacity(), ref.size(), "check resize with identical range (capacity)");
      check_if_equal(
          test.get_capacity_min_index(), ref.get_min_index(), "check resize with identical range (capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index(), "check resize with identical range (capacity_max_index)");
      ...
      check_if_equal(test.size(), ref.size() + 5, "check resize with grow on high index (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index(), "check resize with grow on high index (get_min_index)");
      for (int i = ref.get_min_index(); i <= ref.get_max_index(); ++i)
        check_if_equal(test[i], ref[i], "check resize with grow on high index (values)");
      check_if_equal(test.capacity(), ref.size() + 5, "check resize with grow on high index (capacity)");
      check_if_equal(
          test.get_capacity_min_index(), ref.get_min_index(), "check resize with grow on high index (capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index() + 5, "check resize with grow on high index (capacity_max_index)");
      ...
      check_if_equal(test.size(), ref.size() + 5, "check resize with grow on low index (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index() - 5, "check resize with grow on low index (get_min_index)");
      for (int i = ref.get_min_index(); i <= ref.get_max_index(); ++i)
        check_if_equal(test[i], ref[i], "check resize with grow on low index (values)");
      check_if_equal(test.capacity(), ref.size() + 5, "check resize with grow on low index (capacity)");
      check_if_equal(
          test.get_capacity_min_index(), ref.get_min_index() - 5, "check resize with grow on low index (capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index(), "check resize with grow on low index (capacity_max_index)");
      // check grow for both ranges
      test.recycle();
      check_if_equal(test.capacity(), size_t(0), "test recycle");
      test = ref;
      test.resize(test.get_min_index() - 5, test.get_max_index() + 4);
      check_if_equal(test.size(), ref.size() + 9, "check resize with grow at both ends (size)");
      check_if_equal(test.get_min_index(), ref.get_min_index() - 5, "check resize with grow at both ends (get_min_index)");
      for (int i = ref.get_min_index(); i <= ref.get_max_index(); ++i)
        check_if_equal(test[i], ref[i], "check resize with grow at both ends (values)");
      check_if_equal(test.capacity(), ref.size() + 9, "check resize with grow at both ends (capacity)");
      check_if_equal(
          test.get_capacity_min_index(), ref.get_min_index() - 5, "check resize with grow at both ends (capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index() + 4, "check resize with grow at both ends (capacity_max_index)");
      // check resize with shrink for both ranges
      test.recycle();
      check_if_equal(test.capacity(), size_t(0), "test recycle");
      test = ref;
      test.resize(test.get_min_index() + 5, test.get_max_index() - 4);
      check_if_equal(test.size(), ref.size() - 9, "check resize with shrink at both ends(size)");
      check_if_equal(test.get_min_index(), ref.get_min_index() + 5, "check resize with shrink at both ends(get_min_index)");
      for (int i = test.get_min_index(); i <= test.get_max_index(); ++i)
        check_if_equal(test[i], ref[i], "check resize with shrink at both ends (values)");
      check_if_equal(test.capacity(), ref.size(), "check resize with shrink at both ends(capacity)");
      check_if_equal(
          test.get_capacity_min_index(), ref.get_min_index(), "check resize with shrink at both ends(capacity_min_index)");
      check_if_equal(
          test.get_capacity_max_index(), ref.get_max_index(), "check resize with shrink at both ends(capacity_max_index)");
      // check resize with shrink at left and grow at right
      test.recycle();
      check_if_equal(test.capacity(), size_t(0), "test recycle");
      test = ref;
      test.resize(test.get_min_index() + 5, test.get_max_index() + 4);
      check_if_equal(test.size(), ref.size() - 1, "check resize with shrink at left and grow at right (size)");
      check_if_equal(
          test.get_min_index(), ref.get_min_index() + 5, "check resize with shrink at left and grow at right (get_min_index)");
      for (int i = test.get_min_index(); i <= ref.get_max_index(); ++i)
        check_if_equal(test[i], ref[i], "check resize with shrink at left and grow at right (values)");
      check_if_equal(test.capacity(), ref.size() + 4, "check resize with shrink at left and grow at right (capacity)");
      check_if_equal(test.get_capacity_min_index(),
                     ref.get_min_index(),
                     "check resize with shrink at left and grow at right (capacity_min_index)");
      check_if_equal(test.get_capacity_max_index(),
                     ref.get_max_index() + 4,
      ...
      check_if_equal(test.size(), size_t(ref.size() - 2), "check resize with resize non-overlapping to right (size)");
      check_if_equal(
          test.get_min_index(), ref.get_max_index() + 20, "check resize with resize non-overlapping to right (get_min_index)");
      check_if_equal(test.capacity(), ref.size(), "check resize with resize non-overlapping to right (capacity)");
      check_if_equal(test.get_capacity_min_index(),
                     test.get_min_index(),
                     "check resize with resize non-overlapping to right (capacity_min_index)");
      check_if_equal(test.get_capacity_max_index(),
                     test.get_min_index() + static_cast<int>(test.capacity()) - 1,
      ...
      check_if_equal(test.size(), size_t(ref.size() + 5), "check resize with resize non-overlapping to left (size)");
      check_if_equal(
          test.get_min_index(), ref.get_min_index() - 300, "check resize with resize non-overlapping to left (get_min_index)");
      check_if_equal(test.capacity(), test.size(), "check resize with resize non-overlapping to left (capacity)");
      check_if_equal(test.get_capacity_min_index(),
                     test.get_min_index(),
  ...
  /**********************************************************************/
  {
    ...
    const int ensure_non_zero = minimum <= 0 ? -minimum + 1 : 0;
  ...
  // checks on using existing data_ptr with constructor indices starting at 0
  {
    ...
    check_if_equal(
        std::accumulate(v.begin(), v.end(), 0), 6 + v[2] * (v.get_length() - 1), "test resizing vector using data_ptr");
  ...
  // checks on using existing data_ptr with constructor indices starting at -3
  {
    ...
    check(!v.owns_memory_for_data(),
          "test vector using data_ptr (negative min_index): assignment should not allocate new memory");
    v[4] = 4;
    ...
    check_if_equal(data_ptr[4 - v.get_min_index()],
                   4,
                   "test vector using data_ptr (negative min_index): data at 4-min_index after assignment and setting");
    // another assignment that does not reallocate
    v = VectorWithOffset<int>(static_cast<int>(size - (data_ptr - data)));
    check(!v.owns_memory_for_data(),
          "test vector using data_ptr (negative min_index): 2nd assignment should not allocate new memory");

...
void
ExportArrayTests ::check_if_equal_projdata(const ProjData& test_proj_data, const ProjData& check_proj_data,
                                           const std::string& test_name) {
  for (int segment_num = test_proj_data.get_min_segment_num(); segment_num <= test_proj_data.get_max_segment_num();
       ++segment_num) {
...
ExportArrayTests::test_dynamic_data() {
  ...
  shared_ptr<ProjDataInfo> tmp_proj_data_info_sptr(ProjDataInfo::ProjDataInfoCTI(
      test_scanner_sptr, 1, 1, /* Reduce the number of segments */
      test_scanner_sptr->get_max_num_views(), test_scanner_sptr->get_max_num_non_arccorrected_bins(), false));
  ...
  for (int i_gate = 1; i_gate <= num_of_gates; i_gate++) {
    ...
    for (int segment_num = test_proj_data_gate_ptr->get_min_segment_num();
         segment_num <= test_proj_data_gate_ptr->get_max_segment_num(); ++segment_num) {
  ...
  for (int i_gate = 1; i_gate <= num_of_gates; i_gate++) {
    ...
    for (int segment_num = _test_projdata_sptr->get_min_segment_num(); segment_num <= _test_projdata_sptr->get_max_segment_num();
         ++segment_num) {
      ...
      for (int view_num = _test_segment_by_view_data.get_min_view_num();
           view_num <= _test_segment_by_view_data.get_max_view_num(); ++view_num) {
        ...
        for (int axial = _test_view.get_min_axial_pos_num(); axial <= _test_view.get_max_axial_pos_num(); ++axial) {
          for (int s = _test_view.get_min_tangential_pos_num(); s <= _test_view.get_max_tangential_pos_num(); ++s) {
            ...
            check_if_equal(_check_view[axial][s], (float)(segment_num + (i_gate * 1000)),
...
ExportArrayTests ::test_static_data() {
  ...
  shared_ptr<ProjDataInfo> tmp_proj_data_info_sptr(
      ProjDataInfo::ProjDataInfoCTI(test_scanner_sptr, 1, 1, test_scanner_sptr->get_max_num_views(),
                                    test_scanner_sptr->get_max_num_non_arccorrected_bins(), false));

  {
    ...
    {
      ProjDataInterfile check_proj_data(test_exam_info_sptr, tmp_proj_data_info_sptr, "test_proj_data_export_check.hs",
                                        std::ios::out | std::ios::trunc | std::ios::in);
      run_static_test(test_proj_data, check_proj_data, "static test in-memory/interfile");
    }
  }
  {
    ProjDataInterfile test_proj_data(test_exam_info_sptr, tmp_proj_data_info_sptr, "test_proj_data_export.hs",
                                     std::ios::out | std::ios::trunc | std::ios::in);
    {
      ProjDataInMemory check_proj_data(test_exam_info_sptr, tmp_proj_data_info_sptr);
      run_static_test(test_proj_data, check_proj_data, "static test in-memory");

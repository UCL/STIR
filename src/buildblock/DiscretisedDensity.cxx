...
DiscretisedDensity<num_dimensions, elemT>::read_from_file(const string& filename) {
...
#  endif
    DiscretisedDensity<num_dimensions, elemT>* density_ptr = read_interfile_image(filename);
...
#  ifdef STIR_USE_GE_IO
  // NIFF file format (a simple almost raw format from GE)
  {
    ...
    if (Niff::isReadableNiffFile(filename)) {
      ...
      return new VoxelsOnCartesianGrid<float>(data, origin, voxel_size);
    }
  }
#  endif

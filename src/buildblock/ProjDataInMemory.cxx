...
ProjDataInMemory::ProjDataInMemory(shared_ptr<const ExamInfo> const& exam_info_sptr,
    ...
    : ProjDataFromStream(
          exam_info_sptr, proj_data_info_ptr, shared_ptr<iostream>(), // trick: first initialise sino_stream_ptr to 0
          std::streamoff(0), ProjData::standard_segment_sequence(*proj_data_info_ptr), Segment_AxialPos_View_TangPos) {
...
void
ProjDataInMemory::create_stream() {
  shared_ptr<std::iostream> output_stream_sptr;
#ifdef STIR_USE_OLD_STRSTREAM
  output_stream_sptr.reset(
      new strstream(buffer.get_data_ptr(), this->get_size_of_buffer_in_bytes(), ios::in | ios::out | ios::binary));
#else
  ...
  output_stream_sptr.reset(new boost::interprocess::bufferstream(reinterpret_cast<char*>(this->buffer.get_data_ptr()),
                                                                 this->get_size_of_buffer_in_bytes(),
...
ProjDataInMemory::ProjDataInMemory(const ProjData& proj_data)
    : ProjDataFromStream(proj_data.get_exam_info_sptr(), proj_data.get_proj_data_info_sptr()->create_shared_clone(),
                         shared_ptr<iostream>(), std::streamoff(0),
...
size_t
ProjDataInMemory::get_size_of_buffer_in_bytes() const {

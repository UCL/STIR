  ...
  for (int out_segment_num = proj_data_4D.get_min_segment_num(); out_segment_num <= proj_data_4D.get_max_segment_num();
    ...
    for (int out_ax_pos_num = proj_data_4D.get_min_axial_pos_num(out_segment_num);
      ...
      for (int in_ax_pos_num = proj_data_3D.get_min_axial_pos_num(0); in_ax_pos_num <= proj_data_3D.get_max_axial_pos_num(0);
        ...
        const float in_m = proj_data_3D_info_sptr->get_m(Bin(0, 0, in_ax_pos_num, 0));
        ...
        const float in_m_next = in_ax_pos_num == proj_data_3D.get_max_axial_pos_num(0)
                                    ? -1000000.F
                                    : proj_data_3D_info_sptr->get_m(Bin(0, 0, in_ax_pos_num + 1, 0));
      ...
      if (num_contributing_sinos == 0)
        warning("inverse_SSRB: no sinogram contributes to segment %d, axial_pos_num %d", out_segment_num, out_ax_pos_num);
    }
  }

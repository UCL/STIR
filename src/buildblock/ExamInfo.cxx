...
ExamInfo::operator == (const ExamInfo &p1) const {      
    const bool equal_without_en =
      this->num_windows==p1.num_windows &&
...
//              this->branching_ratio==p1.branching_ratio &&
    ...
    if (!equal_without_en)
      return false;
    for (int en=0; en < this->num_windows; ++en)
      {
        if (abs(this->up_energy_thres[en] - p1.up_energy_thres[en] ) > 1) // keV
          return false;
        if (abs(this->low_energy_thres[en] - p1.low_energy_thres[en]) > 1) // keV
          return false;
      }
    return this->en_win_pair[0] == p1.en_win_pair[0] &&
      this->en_win_pair[1] == p1.en_win_pair[1];
}

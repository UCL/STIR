...
ArrayFilter3DUsingConvolution<elemT>::ArrayFilter3DUsingConvolution() : filter_coefficients() {}

template <typename elemT>
ArrayFilter3DUsingConvolution<elemT>::ArrayFilter3DUsingConvolution(const Array<3, float>& filter_coefficients_v)
...
template <typename elemT>
bool
ArrayFilter3DUsingConvolution<elemT>::is_trivial() const {
  return filter_coefficients.get_length() == 0 ||
         (filter_coefficients.get_length() == 1 && filter_coefficients.get_min_index() == 0 && filter_coefficients[0][0][0] == 1);
}
...
template <typename elemT>
Succeeded
ArrayFilter3DUsingConvolution<elemT>::get_influenced_indices(IndexRange<1>& influenced_index_range,
...
template <typename elemT>
void
ArrayFilter3DUsingConvolution<elemT>::do_it(Array<3, elemT>& out_array, const Array<3, elemT>& in_array) const {
  ...
  if (is_trivial()) {
    for (int z = out_min_z; z <= out_max_z; z++)
      for (int y = out_min_y; y <= out_max_y; y++)
        ...
        {
          out_array[z][y][x] =
              ((z >= in_min_z && z <= in_max_z) && (y >= in_min_y && y <= in_max_y) && (x >= in_min_x && x <= in_max_x)
                   ? in_array[z][y][x]
                   : 0);
        }
    return;
...
template <typename elemT>
void
ArrayFilter3DUsingConvolution<elemT>::do_it_2d(Array<2, elemT>& out_array, const Array<2, elemT>& in_array) const {

...
void
zoom_viewgrams(RelatedViewgrams<float>& in_viewgrams, const float zoom, const int min_tang_pos_num, const int max_tang_pos_num,
               const float x_offset_in_mm, const float y_offset_in_mm) {
  if (min_tang_pos_num == in_viewgrams.get_min_tangential_pos_num() &&
      max_tang_pos_num == in_viewgrams.get_max_tangential_pos_num() && zoom == 1.0 && x_offset_in_mm == 0.0 &&
      y_offset_in_mm == 0.0)
    return;

  shared_ptr<ProjDataInfo> new_proj_data_info_sptr(in_viewgrams.get_proj_data_info_sptr()->clone());
...

  {
    ...
    for (; out_iter != out_viewgrams.end(); ++out_iter, ++in_iter)
      zoom_viewgram(*out_iter, *in_iter, x_offset_in_mm, y_offset_in_mm);
  }
...
void
zoom_viewgram(Viewgram<float>& in_view, const float zoom, const int min_tang_pos_num, const int max_tang_pos_num,
              const float x_offset_in_mm, const float y_offset_in_mm) {
  if (min_tang_pos_num == in_view.get_min_tangential_pos_num() && max_tang_pos_num == in_view.get_max_tangential_pos_num() &&
      zoom == 1.0 && x_offset_in_mm == 0.0 && y_offset_in_mm == 0.0)
    return;

  shared_ptr<ProjDataInfo> new_proj_data_info_sptr(in_view.get_proj_data_info_sptr()->clone());
  ProjDataInfoCylindricalArcCorr* new_proj_data_info_arccorr_sptr =
      dynamic_cast<ProjDataInfoCylindricalArcCorr*>(new_proj_data_info_sptr.get());
...
void
zoom_viewgram(Viewgram<float>& out_view, const Viewgram<float>& in_view, const float x_offset_in_mm, const float y_offset_in_mm) {
  // minimal checks on compatibility
  assert(in_view.get_proj_data_info_sptr()->get_num_views() == out_view.get_proj_data_info_sptr()->get_num_views());
  ...
  const float in_bin_size = in_proj_data_info_arccorr_sptr->get_tangential_sampling();
  const float out_bin_size = out_proj_data_info_arccorr_sptr->get_tangential_sampling();

  const float zoom = in_bin_size / out_bin_size;

  if (out_view.get_min_tangential_pos_num() == in_view.get_min_tangential_pos_num() &&
      out_view.get_max_tangential_pos_num() == in_view.get_max_tangential_pos_num() && zoom == 1.0F && x_offset_in_mm == 0.0F &&
      y_offset_in_mm == 0.0F)
    return;

  const float phi = in_proj_data_info_arccorr_sptr->get_phi(Bin(in_view.get_segment_num(), in_view.get_view_num(), 0, 0));

  // compute offset in tangential_sampling_in units
  const float offset = (x_offset_in_mm * cos(phi) + y_offset_in_mm * sin(phi)) / in_bin_size;
...
construct_new_image_from_zoom_parameters(const VoxelsOnCartesianGrid<float>& image, const CartesianCoordinate3D<float>& zooms,
  ...
  CartesianCoordinate3D<float> voxel_size = image.get_grid_spacing() / zooms;

  // first set origin to 0
  CartesianCoordinate3D<float> origin(0.F, 0.F, 0.F);

  VoxelsOnCartesianGrid<float> new_image(image.get_exam_info_sptr(),
                                         IndexRange3D(0, new_sizes.z() - 1, -new_sizes.y() / 2,
                                                      -new_sizes.y() / 2 + new_sizes.y() - 1, -new_sizes.x() / 2,
                                                      -new_sizes.x() / 2 + new_sizes.x() - 1),
                                         origin, voxel_size);
  ...
  const BasicCoordinate<3, float> middle =
      (image.get_physical_coordinates_for_indices(min_indices) + image.get_physical_coordinates_for_indices(max_indices)) / 2;
  const BasicCoordinate<3, float> new_middle = (new_image.get_physical_coordinates_for_indices(new_min_indices) +
                                                new_image.get_physical_coordinates_for_indices(new_max_indices)) /
                                               2;
  // now make sure that these are shifted as required
  new_image.set_origin(offsets_in_mm + middle - new_middle);
  // check
  {
    const BasicCoordinate<3, float> final_middle = (new_image.get_physical_coordinates_for_indices(new_min_indices) +
                                                    new_image.get_physical_coordinates_for_indices(new_max_indices)) /
                                                   2;
...
void
zoom_image_in_place(VoxelsOnCartesianGrid<float>& image, const float zoom, const float x_offset_in_mm, const float y_offset_in_mm,
                    const int new_size, const ZoomOptions zoom_options) {
  VoxelsOnCartesianGrid<float> new_image = zoom_image(image, zoom, x_offset_in_mm, y_offset_in_mm, new_size, zoom_options);
  image = new_image;
}

VoxelsOnCartesianGrid<float>
zoom_image(const VoxelsOnCartesianGrid<float>& image, const float zoom, const float x_offset_in_mm, const float y_offset_in_mm,
           const int new_size, const ZoomOptions zoom_options) {
  assert(new_size >= 0);
  ...
  const BasicCoordinate<3, int> new_sizes = make_coordinate(image.get_length(), new_size, new_size);

  VoxelsOnCartesianGrid<float> new_image = construct_new_image_from_zoom_parameters(image, zooms, offsets_in_mm, new_sizes);

  PixelsOnCartesianGrid<float> new_image2D = new_image.get_plane(new_image.get_min_z());
...
void
zoom_image_in_place(VoxelsOnCartesianGrid<float>& image, const CartesianCoordinate3D<float>& zooms,
                    const CartesianCoordinate3D<float>& offsets_in_mm, const BasicCoordinate<3, int>& new_sizes,
                    const ZoomOptions zoom_options) {
  const VoxelsOnCartesianGrid<float> new_image = zoom_image(image, zooms, offsets_in_mm, new_sizes, zoom_options);
  image = new_image;
}

VoxelsOnCartesianGrid<float>
zoom_image(const VoxelsOnCartesianGrid<float>& image, const CartesianCoordinate3D<float>& zooms,
           const CartesianCoordinate3D<float>& offsets_in_mm, const BasicCoordinate<3, int>& new_sizes,
           const ZoomOptions zoom_options) {
...
void
zoom_image(VoxelsOnCartesianGrid<float>& image_out, const VoxelsOnCartesianGrid<float>& image_in,
           const ZoomOptions zoom_options) {
  ...
  // check relation between indices and physical coordinates
  {
    ...
    if (norm(image_in.get_physical_coordinates_for_indices(indices) -
             (image_in.get_voxel_size() * BasicCoordinate<3, float>(indices) + image_in.get_origin())) > 2.F)
      error("zoom_image is confused about the relation between indices and physical coordinates");
  }
  const float zoom_x = image_in.get_voxel_size().x() / image_out.get_voxel_size().x();
  const float zoom_y = image_in.get_voxel_size().y() / image_out.get_voxel_size().y();
  const float zoom_z = image_in.get_voxel_size().z() / image_out.get_voxel_size().z();
  const float x_offset = (image_out.get_origin().x() - image_in.get_origin().x()) / image_in.get_voxel_size().x();
  const float y_offset = (image_out.get_origin().y() - image_in.get_origin().y()) / image_in.get_voxel_size().y();
  const float z_offset = (image_out.get_origin().z() - image_in.get_origin().z()) / image_in.get_voxel_size().z();

  if (zoom_x == 1.0F && zoom_y == 1.0F && zoom_z == 1.0F && x_offset == 0.F && y_offset == 0.F && z_offset == 0.F &&
  ...
  Array<3, float> temp(IndexRange3D(image_in.get_min_z(), image_in.get_max_z(), image_in.get_min_y(), image_in.get_max_y(),
  ...
  Array<3, float> temp2(IndexRange3D(image_in.get_min_z(), image_in.get_max_z(), image_out.get_min_y(), image_out.get_max_y(),
                                     image_out.get_min_x(), image_out.get_max_x()));
...
void
zoom_image(PixelsOnCartesianGrid<float>& image2D_out, const PixelsOnCartesianGrid<float>& image2D_in,
           const ZoomOptions zoom_options) {
  ...
  */
  ...
  const float zoom_x = image2D_in.get_pixel_size().x() / image2D_out.get_pixel_size().x();
  const float zoom_y = image2D_in.get_pixel_size().y() / image2D_out.get_pixel_size().y();
  const float x_offset = ((image2D_out.get_origin().x() - image2D_in.get_origin().x()) / image2D_in.get_pixel_size().x());
  const float y_offset = ((image2D_out.get_origin().y() - image2D_in.get_origin().y()) / image2D_in.get_pixel_size().y());

  if (zoom_x == 1.0F && zoom_y == 1.0F && x_offset == 0.F && y_offset == 0.F &&
  ...
  Array<2, float> temp(
      IndexRange2D(image2D_in.get_min_y(), image2D_in.get_max_y(), image2D_out.get_min_x(), image2D_out.get_max_x()));

...
ProjDataGEAdvance::ProjDataGEAdvance(iostream* s)
    : sino_stream(s), offset(0), on_disk_data_type(NumericType::SHORT), on_disk_byte_order(ByteOrder::big_endian) {
  // TODO find from file
  const int max_delta = 11;
  shared_ptr<Scanner> scanner_sptr(new Scanner(Scanner::Advance));
  proj_data_info_sptr.reset(ProjDataInfo::ProjDataInfoGE(scanner_sptr, max_delta, scanner_sptr->get_max_num_views(),
  ...
  // reads the views_scaling_factors
  {
    ...
    if (read_data(*sino_stream, view_scaling_factor, NumericType::FLOAT, scale, on_disk_byte_order) == Succeeded::no ||
        scale != 1)
...
ProjDataGEAdvance::get_viewgram(const int view_num, const int segment_num, const bool make_num_tangential_poss_odd,
  ...
  const int index_orig = static_cast<int>(find(segment_sequence_orig.begin(), segment_sequence_orig.end(), segment_num_orig) -
  ...
  num_rings_offset = accumulate(num_rings_orig.begin(), num_rings_orig.begin() + index_orig, 0) + odd_ring_segment_offset;

  //      cout << "num_rings_offset " << num_rings_offset << endl;

  // offset to the first data in the first view_num of the segment chosen

  segment_offset = num_rings_offset * get_num_tangential_poss() * on_disk_data_type.size_in_bytes();

  //      cout << "segment_offset " << segment_offset << endl;

  // size of the jump to the first record of the following view_num
  if (segment_num == 0) {
    jump_size = (accumulate(num_rings_orig.begin(), num_rings_orig.end(), 0) - num_rings_orig[index_orig] +
  ...
  streamoff jump_ini = (view_num - get_min_view_num()) *
                       (ring_offset * get_num_axial_poss(segment_num) + jump_ring * get_num_axial_poss(segment_num) + jump_size);
  ...
  for (int ring = get_min_axial_pos_num(segment_num); ring <= get_max_axial_pos_num(segment_num); ring++) {
    {
      float scale = float(1);
      if (read_data(*sino_stream, data[ring], on_disk_data_type, scale, on_disk_byte_order) == Succeeded::no || scale != 1)
  ...
  if (make_num_tangential_poss_odd && get_num_tangential_poss() % 2 == 0) {
    ...
    data.grow(IndexRange2D(get_min_axial_pos_num(segment_num), get_max_axial_pos_num(segment_num), get_min_tangential_pos_num(),
...
Succeeded
ProjDataGEAdvance::set_viewgram(const Viewgram<float>& v) {
  // TODO
...
Succeeded
ProjDataGEAdvance::set_sinogram(const Sinogram<float>& s) {
  // TODO

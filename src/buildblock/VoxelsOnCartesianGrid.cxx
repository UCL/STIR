...
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid() : DiscretisedDensityOnCartesianGrid<3, elemT>() {}

template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const Array<3, elemT>& v, const CartesianCoordinate3D<float>& origin,
...
template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const IndexRange<3>& range, const CartesianCoordinate3D<float>& origin,
                                                    const BasicCoordinate<3, float>& grid_spacing)
    : DiscretisedDensityOnCartesianGrid<3, elemT>(range, origin, grid_spacing) {}

template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const shared_ptr<const ExamInfo>& exam_info_sptr, const Array<3, elemT>& v,
...
template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const shared_ptr<const ExamInfo>& exam_info_sptr, const IndexRange<3>& range,
                                                    const CartesianCoordinate3D<float>& origin,
                                                    const BasicCoordinate<3, float>& grid_spacing)
...
template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const ProjDataInfo& proj_data_info, const float zoom,
                                                    const CartesianCoordinate3D<float>& origin,
                                                    const CartesianCoordinate3D<int>& sizes)

{
  shared_ptr<ExamInfo> exam_info_sptr_v(new ExamInfo);
  this->construct_from_projdata_info(exam_info_sptr_v, proj_data_info, CartesianCoordinate3D<float>(1.F, zoom, zoom), origin,
                                     sizes);
}

template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const shared_ptr<const ExamInfo>& exam_info_sptr_v,
                                                    const ProjDataInfo& proj_data_info, const float zoom,
                                                    const CartesianCoordinate3D<float>& origin,
                                                    const CartesianCoordinate3D<int>& sizes) {
  this->construct_from_projdata_info(exam_info_sptr_v, proj_data_info, CartesianCoordinate3D<float>(1.F, zoom, zoom), origin,
                                     sizes);
...
template <class elemT>
VoxelsOnCartesianGrid<elemT>::VoxelsOnCartesianGrid(const shared_ptr<const ExamInfo>& exam_info_sptr_v,
                                                    const ProjDataInfo& proj_data_info, const CartesianCoordinate3D<float>& zooms,
                                                    const CartesianCoordinate3D<float>& origin,
                                                    const CartesianCoordinate3D<int>& sizes) {
...
VoxelsOnCartesianGrid<elemT>::construct_from_projdata_info(const shared_ptr<const ExamInfo>& exam_info_sptr_v,
  ...
  if (sizes.x() == -1 || sizes.y() == -1) {
    ...
    const float FOVradius_in_mm = max(proj_data_info.get_s(Bin(0, 0, 0, proj_data_info.get_max_tangential_pos_num())),
  ...
  IndexRange3D range(0, z_size - 1, -(y_size_used / 2), -(y_size_used / 2) + y_size_used - 1, -(x_size_used / 2),
...
{
  return new VoxelsOnCartesianGrid(this->get_exam_info().create_shared_clone(), this->get_index_range(), this->get_origin(),
...
VoxelsOnCartesianGrid<elemT>::get_plane(const int z) const {
  PixelsOnCartesianGrid<elemT> plane(this->operator[](z), this->get_origin(),

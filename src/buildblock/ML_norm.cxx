...
DetPairData::DetPairData(const IndexRange<2>& range) : base_type(range), num_detectors(range.get_length()) {}
...
void
make_det_pair_data(DetPairData& det_pair_data, const ProjDataInfo& proj_data_info_general_type, const int segment_num,
  ...
  const int fan_size = 2 * max(proj_data_info.get_max_tangential_pos_num(), -proj_data_info.get_min_tangential_pos_num()) + 1;
...
GeoData3D::GeoData3D(const int num_axial_crystals_per_block, const int half_num_transaxial_crystals_per_block,
                     const int num_rings, const int num_detectors_per_ring)
    : num_axial_crystals_per_block(num_axial_crystals_per_block),
      half_num_transaxial_crystals_per_block(half_num_transaxial_crystals_per_block), num_rings(num_rings),
...
FanProjData::FanProjData(const int num_rings, const int num_detectors_per_ring, const int max_ring_diff, const int fan_size)
    : num_rings(num_rings), num_detectors_per_ring(num_detectors_per_ring), max_ring_diff(max_ring_diff),
...
get_fan_info(int& num_rings, int& num_detectors_per_ring, int& max_ring_diff, int& fan_size, const ProjDataInfo& proj_data_info) {
  ...
  shared_ptr<ProjDataInfoCylindricalNoArcCorr> ret_value(
      dynamic_cast<ProjDataInfoCylindricalNoArcCorr*>(proj_data_info_ptr->clone()));
  return ret_value;
}

void
make_fan_data(FanProjData& fan_data, const ProjData& proj_data) {
  int num_rings;
      ...
      for (bin.axial_pos_num() = proj_data.get_min_axial_pos_num(bin.segment_num());
        ...
        for (bin.view_num() = 0; bin.view_num() < num_detectors_per_ring / 2; bin.view_num()++)
          for (bin.tangential_pos_num() = -half_fan_size; bin.tangential_pos_num() <= half_fan_size; ++bin.tangential_pos_num()) {
            ...
            fan_data(ra, a, rb, b) = fan_data(rb, b, ra, a) =
...
void
set_fan_data(ProjData& proj_data, const FanProjData& fan_data) {
  int num_rings;
  int num_detectors_per_ring;
  int fan_size;
  int max_delta;
  ...
  for (bin.segment_num() = proj_data.get_min_segment_num(); bin.segment_num() <= proj_data.get_max_segment_num();
    ...
    {
      segment_ptr.reset(
          new SegmentBySinogram<float>(proj_data.get_empty_segment_by_sinogram(bin.segment_num() /*,bin.timing_pos_num()*/)));
      ...
      for (bin.axial_pos_num() = proj_data.get_min_axial_pos_num(bin.segment_num());
        ...
        for (bin.view_num() = 0; bin.view_num() < num_detectors_per_ring / 2; bin.view_num()++)
          for (bin.tangential_pos_num() = -half_fan_size; bin.tangential_pos_num() <= half_fan_size; ++bin.tangential_pos_num()) {
            ...
            (*segment_ptr)[bin.axial_pos_num()][bin.view_num()][bin.tangential_pos_num()] = fan_data(ra, a, rb, b);
          }
...
/// *** fan_data doesn't have gaps, proj_data has gaps *** ///
void
set_fan_data_add_gaps(ProjData& proj_data, const FanProjData& fan_data) {
  int num_rings;
  int num_detectors_per_ring;
  int fan_size;
  int max_delta;
  ...
  // const int num_transaxial_detectors = fan_data.get_num_detectors_per_ring();   // number of detector per ring in fan data w/o
  // gaps

  // const int num_axial_crystals_per_block = num_axial_detectors/num_axial_blocks;  // number of axial detector per block in
  // fan_data w/o gaps const int num_transaxial_crystals_per_block = num_transaxial_detectors/num_transaxial_blocks; // number of
  // transaxial detector per block in fan_data w/o gaps
    ...
    for (bin.axial_pos_num() = proj_data.get_min_axial_pos_num(bin.segment_num());
      ...
      for (bin.view_num() = 0; bin.view_num() < num_detectors_per_ring / 2; bin.view_num()++)
        for (bin.tangential_pos_num() = -half_fan_size; bin.tangential_pos_num() <= half_fan_size; ++bin.tangential_pos_num()) {
          ...
          if ((ra + 1) % num_axial_crystals_per_block == 0)
            continue;
          if ((rb + 1) % num_axial_crystals_per_block == 0)
            continue;
          if ((a + 1) % num_transaxial_crystals_per_block == 0)
            continue;
          if ((b + 1) % num_transaxial_crystals_per_block == 0)
            continue;
          ...
          (*segment_ptr)[bin.axial_pos_num()][bin.view_num()][bin.tangential_pos_num()] = fan_data(new_ra, new_a, new_rb, new_b);
        }
    proj_data.set_segment(*segment_ptr);
  }
}
      ...
      for (int rb = max(ra, fan_data.get_min_rb(ra)); rb <= fan_data.get_max_rb(ra); ++rb)
        for (int b = fan_data.get_min_b(a); b <= fan_data.get_max_b(a); ++b) {
          ...
          if (fan_data(ra, a, rb, b) == 0)
            continue;
          if (apply)
            fan_data(ra, a, rb, b) *= block_data(ra / num_axial_crystals_per_block, a / num_tangential_crystals_per_block,
...
void
make_fan_sum_data(Array<2, float>& data_fan_sums, const DetectorEfficiencies& efficiencies, const int max_ring_diff,
...
make_block_data(BlockData3D& block_data, const FanProjData& fan_data) {
  ...
  for (int ra = fan_data.get_min_ra(); ra <= fan_data.get_max_ra(); ++ra)
    for (int a = fan_data.get_min_a(); a <= fan_data.get_max_a(); ++a)
      ...
      for (int rb = max(ra, fan_data.get_min_rb(ra)); rb <= fan_data.get_max_rb(ra); ++rb)
        for (int b = fan_data.get_min_b(a); b <= fan_data.get_max_b(a); ++b) {
          block_data(ra / num_axial_crystals_per_block, a / num_transaxial_crystals_per_block, rb / num_axial_crystals_per_block,
                     b / num_transaxial_crystals_per_block) += fan_data(ra, a, rb, b);
        }
}
...
void
iterate_efficiencies(DetectorEfficiencies& efficiencies, const Array<2, float>& data_fan_sums, const int max_ring_diff,
                     const int half_fan_size) {
  const int num_rings = data_fan_sums.get_length();

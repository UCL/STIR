...
float&
DetPairData::operator()(const int a, const int b)
{
  return (*this)[a][b < get_min_index(a) ? b + num_detectors : b];
}

float
DetPairData::operator()(const int a, const int b) const
{
  return (*this)[a][b < get_min_index(a) ? b + num_detectors : b];
}

bool
DetPairData::is_in_data(const int a, const int b) const
{
...
void
DetPairData::fill(const float d)
{
  base_type::fill(d);
}

void
DetPairData::grow(const IndexRange<2>& range)
{
...
int
DetPairData::get_min_index() const
{
  return base_type::get_min_index();
}

int
DetPairData::get_max_index() const
{
  return base_type::get_max_index();
}

int
DetPairData::get_min_index(const int a) const
{
  return (*this)[a].get_min_index();
}

int
DetPairData::get_max_index(const int a) const
{
  return (*this)[a].get_max_index();
}

float
DetPairData::sum() const
{
  return base_type::sum();
}

float
DetPairData::sum(const int a) const
{
  return (*this)[a].sum();
}

float
DetPairData::find_max() const
{
  return base_type::find_max();
}

float
DetPairData::find_min() const
{
  return base_type::find_min();
}

int
DetPairData::get_num_detectors() const
{
...
static void
make_det_pair_data_help(DetPairData& det_pair_data,
                        const TProjDataInfo& proj_data_info_general_type,
                        const int segment_num,
                        const int ax_pos_num)
{
  const int num_detectors = proj_data_info_general_type.get_scanner_ptr()->get_num_detectors_per_ring();
  const int fan_size = 2
                           * max(proj_data_info_general_type.get_max_tangential_pos_num(),
                                 -proj_data_info_general_type.get_min_tangential_pos_num())
                       + 1;
  ...
  for (int a = 0; a < num_detectors; ++a)
    {
      fan_indices[a] = IndexRange<1>(a + num_detectors / 2 - half_fan_size, a + num_detectors / 2 + half_fan_size);
    }
...
void
make_det_pair_data(DetPairData& det_pair_data,
...
make_det_pair_data_help(DetPairData& det_pair_data,
                        ...
                        const int ax_pos_num)
{
  make_det_pair_data(det_pair_data, *proj_data.get_proj_data_info_sptr(), segment_num, ax_pos_num);
  const int num_detectors = det_pair_data.get_num_detectors();

  shared_ptr<Sinogram<float>> pos_sino_ptr(new Sinogram<float>(proj_data.get_sinogram(ax_pos_num, segment_num)));
  shared_ptr<Sinogram<float>> neg_sino_ptr;
  if (segment_num == 0)
    neg_sino_ptr = pos_sino_ptr;
  else
    neg_sino_ptr.reset(new Sinogram<float>(proj_data.get_sinogram(ax_pos_num, -segment_num)));

  for (int view_num = 0; view_num < num_detectors / 2; view_num++)
    for (int tang_pos_num = proj_data.get_min_tangential_pos_num(); tang_pos_num <= proj_data.get_max_tangential_pos_num();
         ++tang_pos_num)
      {
        int det_num_a = 0;
        int det_num_b = 0;
        ...
        det_pair_data(det_num_a, det_num_b) = (*pos_sino_ptr)[view_num][tang_pos_num];
        det_pair_data(det_num_b, det_num_a) = (*neg_sino_ptr)[view_num][tang_pos_num];
      }
}
...
void
apply_block_norm(DetPairData& det_pair_data, const BlockData& block_data, const bool apply)
{
  const int num_detectors = det_pair_data.get_num_detectors();
    ...
    for (int b = det_pair_data.get_min_index(a); b <= det_pair_data.get_max_index(a); ++b)
      {
        ...
        if (det_pair_data(a, b) == 0)
          continue;
        if (apply)
          det_pair_data(a, b) *= block_data[a / num_crystals_per_block][(b / num_crystals_per_block) % num_blocks];
        else
          det_pair_data(a, b) /= block_data[a / num_crystals_per_block][(b / num_crystals_per_block) % num_blocks];
      }
}
...
void
apply_geo_norm(DetPairData& det_pair_data, const GeoData& geo_data, const bool apply)
{
  const int num_detectors = det_pair_data.get_num_detectors();
    ...
    for (int b = det_pair_data.get_min_index(a); b <= det_pair_data.get_max_index(a); ++b)
      {
        ...
        // note: add 2*num_detectors to newb to avoid using mod with negative numbers
        if (apply)
          det_pair_data(a, b) *= geo_data[newa][(2 * num_detectors + newb) % num_detectors];
        else
          det_pair_data(a, b) /= geo_data[newa][(2 * num_detectors + newb) % num_detectors];
      }
}
...
void
apply_efficiencies(DetPairData& det_pair_data, const Array<1, float>& efficiencies, const bool apply)
{
  const int num_detectors = det_pair_data.get_num_detectors();
  for (int a = det_pair_data.get_min_index(); a <= det_pair_data.get_max_index(); ++a)
    for (int b = det_pair_data.get_min_index(a); b <= det_pair_data.get_max_index(a); ++b)
      {
        if (det_pair_data(a, b) == 0)
          continue;
        if (apply)
          det_pair_data(a, b) *= efficiencies[a] * efficiencies[b % num_detectors];
        else
          det_pair_data(a, b) /= efficiencies[a] * efficiencies[b % num_detectors];
      }
}
...
void
make_fan_sum_data(Array<1, float>& data_fan_sums, const DetPairData& det_pair_data)
{
  for (int a = det_pair_data.get_min_index(); a <= det_pair_data.get_max_index(); ++a)
...
void
make_geo_data(GeoData& geo_data, const DetPairData& det_pair_data)
{
  const int num_detectors = det_pair_data.get_num_detectors();
  const int num_crystals_per_block = geo_data.get_length() * 2;
  ...
  for (int a = det_pair_data.get_min_index(); a <= det_pair_data.get_max_index(); ++a)
    for (int b = det_pair_data.get_min_index(a); b <= det_pair_data.get_max_index(a); ++b)
      {
        // mirror symmetry
        work(a, b) = det_pair_data(a, b) + det_pair_data(num_detectors - 1 - a, (2 * num_detectors - 1 - b) % num_detectors);
      }

  geo_data.fill(0);

  for (int crystal_num_a = 0; crystal_num_a < num_crystals_per_block / 2; ++crystal_num_a)
    for (int det_num_b = det_pair_data.get_min_index(crystal_num_a); det_num_b <= det_pair_data.get_max_index(crystal_num_a);
         ++det_num_b)
      {
        for (int block_num = 0; block_num < num_blocks; ++block_num)
          {
            ...
            if (det_pair_data.is_in_data(new_det_num_a, new_det_num_b))
              geo_data[crystal_num_a][det_num_b % num_detectors] += work(new_det_num_a, new_det_num_b);
          }
      }
...
void
make_block_data(BlockData& block_data, const DetPairData& det_pair_data)
{
  const int num_detectors = det_pair_data.get_num_detectors();
  const int num_blocks = block_data.get_length();
  const int num_crystals_per_block = num_detectors / num_blocks;
  ...
  for (int a = det_pair_data.get_min_index(); a <= det_pair_data.get_max_index(); ++a)
    for (int b = det_pair_data.get_min_index(a); b <= det_pair_data.get_max_index(a); ++b)
      {
        block_data[a / num_crystals_per_block][(b / num_crystals_per_block) % num_blocks] += det_pair_data(a, b);
      }
...
void
iterate_efficiencies(Array<1, float>& efficiencies, const Array<1, float>& data_fan_sums, const DetPairData& model)
{
...
void
iterate_geo_norm(GeoData& norm_geo_data, const GeoData& measured_geo_data, const DetPairData& model)
{
  ...
  for (int a = 0; a < num_crystals_per_block / 2; ++a)
    for (int b = 0; b < num_detectors; ++b)
      {
        norm_geo_data[a][b] = (measured_geo_data[a][b] >= threshold || measured_geo_data[a][b] < 10000 * norm_geo_data[a][b])
...
void
iterate_block_norm(BlockData& norm_block_data, const BlockData& measured_block_data, const DetPairData& model)
{
  ...
  for (int a = 0; a < num_blocks; ++a)
    for (int b = 0; b < num_blocks; ++b)
      {
        norm_block_data[a][b]
            = (measured_block_data[a][b] >= threshold || measured_block_data[a][b] < 10000 * norm_block_data[a][b])
...
double
KL(const DetPairData& d1, const DetPairData& d2, const double threshold)
{
  double sum = 0;
...
GeoData3D::GeoData3D(const int num_axial_crystals_per_block,
                     const int half_num_transaxial_crystals_per_block,
                     const int num_rings,
                     const int num_detectors_per_ring)
    : num_axial_crystals_per_block(num_axial_crystals_per_block),
      half_num_transaxial_crystals_per_block(half_num_transaxial_crystals_per_block),
      num_rings(num_rings),
      num_detectors_per_ring(num_detectors_per_ring)
{
...
/*float & GeoData3D::operator()(const int ra, const int a, const int rb, const int b)
{
    ...
    : b < half_num_transaxial_crystals_per_block ? (*this)[rb][b%num_detectors_per_ring][ra][a<get_min_b(b%num_detectors_per_ring)
? a+num_detectors_per_ring: a] : (*this)[ra][a%num_detectors_per_ring][rb][b<get_min_b(a) ? b+num_detectors_per_ring: b];
}
...
float GeoData3D::operator()(const int ra, const int a, const int rb, const int b) const
{
   assert(a>=0);
    ...
    ra<rb
    ? (*this)[ra][a%num_detectors_per_ring][rb][b<get_min_b(a) ? b+num_detectors_per_ring: b]
    : b < half_num_transaxial_crystals_per_block  ?
(*this)[rb][b%num_detectors_per_ring][ra][a<get_min_b(b%num_detectors_per_ring) ? a+num_detectors_per_ring: a] :
(*this)[ra][a%num_detectors_per_ring][rb][b<get_min_b(a) ? b+num_detectors_per_ring: b];
} */

float&
GeoData3D::operator()(const int ra, const int a, const int rb, const int b)
{
  assert(a >= 0);
  assert(b >= 0);
  return
...
float
GeoData3D::operator()(const int ra, const int a, const int rb, const int b) const
{
  assert(a >= 0);
  assert(b >= 0);
  return
...
bool
GeoData3D::is_in_data(const int ra, const int a, const int rb, const int b) const
{
  assert(a >= 0);
  assert(b >= 0);
...
void
GeoData3D::fill(const float d)
{
  base_type::fill(d);
}

int
GeoData3D::get_min_ra() const
{
  return base_type::get_min_index();
}

int
GeoData3D::get_max_ra() const
{
  return base_type::get_max_index();
}

int
GeoData3D::get_min_a() const
{
  return (*this)[get_min_index()].get_min_index();
}

int
GeoData3D::get_max_a() const
{
  return (*this)[get_min_index()].get_max_index();
}

int
GeoData3D::get_min_rb(const int ra) const
{
  return 0;
...
float
GeoData3D::sum(const int ra, const int a) const
{
...
float
GeoData3D::find_max() const
{
  return base_type::find_max();
}

float
GeoData3D::find_min() const
{
  return base_type::find_min();
}

int
GeoData3D::get_num_axial_crystals_per_block() const
{
  return num_axial_crystals_per_block;
}

int
GeoData3D::get_half_num_transaxial_crystals_per_block() const
{
  return half_num_transaxial_crystals_per_block;
}

std::ostream&
operator<<(std::ostream& s, const GeoData3D& geo_data)
{
  return s << static_cast<GeoData3D::base_type>(geo_data);
}

std::istream&
operator>>(std::istream& s, GeoData3D& geo_data)
{
  ...
  const int num_detectors_per_ring = geo_data[0][0][0].get_length();
          ...
          if (geo_data[ra][a].get_length() != max_rb - max(ra, min_rb) + 1)
            {
              warning("Reading GeoData3D: inconsistent length %d for rb at ra=%d, a=%d, "
                      "Expected length %d\n",
                      geo_data[ra][a].get_length(),
                      ra,
                      a,
                      max_rb - max(ra, min_rb) + 1);
            }
...
FanProjData::FanProjData(const int num_rings, const int num_detectors_per_ring, const int max_ring_diff, const int fan_size)
    : num_rings(num_rings),
      num_detectors_per_ring(num_detectors_per_ring),
...
float&
FanProjData::operator()(const int ra, const int a, const int rb, const int b)
{
  assert(a >= 0);
  assert(b >= 0);
  return ra < rb ? (*this)[ra][a % num_detectors_per_ring][rb][b < get_min_b(a) ? b + num_detectors_per_ring : b]
                 : (*this)[rb][b % num_detectors_per_ring][ra]
                          [a < get_min_b(b % num_detectors_per_ring) ? a + num_detectors_per_ring : a];
}

float
FanProjData::operator()(const int ra, const int a, const int rb, const int b) const
{
  assert(a >= 0);
  assert(b >= 0);
  return ra < rb ? (*this)[ra][a % num_detectors_per_ring][rb][b < get_min_b(a) ? b + num_detectors_per_ring : b]
...
bool
FanProjData::is_in_data(const int ra, const int a, const int rb, const int b) const
{
  assert(a >= 0);
  assert(b >= 0);
  if (rb < (*this)[ra][a].get_min_index() || rb > (*this)[ra][a].get_max_index())
...
void
FanProjData::fill(const float d)
{
  base_type::fill(d);
}
...
int
FanProjData::get_min_ra() const
{
  return base_type::get_min_index();
}

int
FanProjData::get_max_ra() const
{
  return base_type::get_max_index();
}

int
FanProjData::get_max_delta() const
{
  return max_ring_diff;
}

int
FanProjData::get_min_a() const
{
  return (*this)[get_min_index()].get_min_index();
}

int
FanProjData::get_max_a() const
{
  return (*this)[get_min_index()].get_max_index();
}

int
FanProjData::get_min_rb(const int ra) const
{
...
int
FanProjData::get_max_rb(const int ra) const
{
  return (*this)[ra][(*this)[ra].get_min_index()].get_max_index();
}

int
FanProjData::get_min_b(const int a) const
{
  return (*this)[get_min_index()][a][(*this)[get_min_index()][a].get_min_index()].get_min_index();
}

int
FanProjData::get_max_b(const int a) const
{
  return (*this)[get_min_index()][a][(*this)[get_min_index()][a].get_min_index()].get_max_index();
}

float
FanProjData::sum() const
{
  // return base_type::sum();
  float sum = 0;
  for (int ra = get_min_ra(); ra <= get_max_ra(); ++ra)
...
float
FanProjData::sum(const int ra, const int a) const
{
  // return (*this)[ra][a].sum();
  float sum = 0;
  for (int rb = get_min_rb(ra); rb <= get_max_rb(ra); ++rb)
...
float
FanProjData::find_max() const
{
  return base_type::find_max();
}

float
FanProjData::find_min() const
{
  return base_type::find_min();
}

int
FanProjData::get_num_detectors_per_ring() const
{
  return num_detectors_per_ring;
}

int
FanProjData::get_num_rings() const
{
  return num_rings;
}

std::ostream&
operator<<(std::ostream& s, const FanProjData& fan_data)
{
  return s << static_cast<FanProjData::base_type>(fan_data);
}

std::istream&
operator>>(std::istream& s, FanProjData& fan_data)
{
  ...
  const int half_fan_size = fan_data[0][0][0].get_length() / 2;
...
{
  auto proj_data_info_ptr = dynamic_cast<const ProjDataInfoCylindrical* const>(&proj_data_info);
...
template <class TProjDataInfo>
static void
set_det_pair_data_help(ProjData& proj_data,
                       const TProjDataInfo proj_data_info,
                       ...
                       const int ax_pos_num)
{
  ...
  shared_ptr<Sinogram<float>> pos_sino_ptr(new Sinogram<float>(proj_data.get_empty_sinogram(ax_pos_num, segment_num)));
  shared_ptr<Sinogram<float>> neg_sino_ptr;
  if (segment_num != 0)
    neg_sino_ptr.reset(new Sinogram<float>(proj_data.get_empty_sinogram(ax_pos_num, -segment_num)));
  ...
  for (int view_num = 0; view_num < num_detectors / 2; view_num++)
    for (int tang_pos_num = proj_data.get_min_tangential_pos_num(); tang_pos_num <= proj_data.get_max_tangential_pos_num();
         ++tang_pos_num)
      {
        ...
        (*pos_sino_ptr)[view_num][tang_pos_num] = det_pair_data(det_num_a, det_num_b);
        if (segment_num != 0)
          (*neg_sino_ptr)[view_num][tang_pos_num] = det_pair_data(det_num_b, det_num_a);
      }
...
template <class TProjDataInfo>
static void
make_fan_data_remove_gaps_help(FanProjData& fan_data,
                               int num_rings,
...
//   std::cout<<"Returned the omp threads to " << omp_get_num_threads() << std::endl;
}
...
void
set_fan_data_add_gaps(ProjData& proj_data, const FanProjData& fan_data, const float gap_value)
{
  int num_rings;
  int num_detectors_per_ring;
  int fan_size;
...
void
apply_block_norm(FanProjData& fan_data, const BlockData3D& block_data, const bool apply)
{
        ...
        for (int b = fan_data.get_min_b(a); b <= fan_data.get_max_b(a); ++b)
          {
            ...
            if (fan_data(ra, a, rb, b) == 0)
              continue;
            if (apply)
              fan_data(ra, a, rb, b) *= block_data(ra / num_axial_crystals_per_block,
                                                   a / num_tangential_crystals_per_block,
                                                   rb / num_axial_crystals_per_block,
                                                   b / num_tangential_crystals_per_block);
            else
              fan_data(ra, a, rb, b) /= block_data(ra / num_axial_crystals_per_block,
                                                   a / num_tangential_crystals_per_block,
                                                   rb / num_axial_crystals_per_block,
...
#  pragma omp single
  {
    std::cout << "apply_geo_norm in parallel..." << std::endl;
    std::cout << "We will be using " << num_threads << " threads... If your machine does not support that please contact sb from STIR" << std::endl;

    local_work_sptrs.resize(num_threads, shared_ptr<FanProjData>());

    for (int i = 0; i < num_threads; i++)
      {
        std::cout << "Allocated data to FanProjData for thread " << i << std::endl;
        local_work_sptrs[i].reset(new FanProjData(
            fan_data.size(), fan_data[0].size(),
            fan_data[0][0].size(), fan_data[0][0][0].size()));
        local_work_sptrs[i]->fill(0);
      }
  }
#endif

std::cout << "Starting  loops over geo... " << std::endl;
#ifdef STIR_OPENMP
#  if _OPENMP >= 201107
#    pragma omp parallel for schedule(dynamic) collapse(4) num_threads(num_threads)
#  endif
                ...
                for (int b = fan_data.get_min_b(a); b <= fan_data.get_max_b(a); ++b)
                  {
                    ...
                    if ( (*local_work_sptrs[thread_num]).is_in_data(new_ring_num_a, new_det_num_a, new_ring_num_b, new_det_num_b))
                      (*local_work_sptrs[thread_num])(new_ring_num_a, new_det_num_a, new_ring_num_b, new_det_num_b)
                          = geo_data(ra, a, rb, b % num_transaxial_detectors);

                    if ( (*local_work_sptrs[thread_num]).is_in_data(new_ring_num_a, ma, new_ring_num_b, mb))
                      (*local_work_sptrs[thread_num])(new_ring_num_a, ma, new_ring_num_b, mb) = geo_data(ra, a, rb, b % num_transaxial_detectors);

                    if ( (*local_work_sptrs[thread_num]).is_in_data(mra, new_det_num_a, mrb, new_det_num_b))
                      (*local_work_sptrs[thread_num])(mra, new_det_num_a, mrb, new_det_num_b) = geo_data(ra, a, rb, b % num_transaxial_detectors);

                    if ( (*local_work_sptrs[thread_num]).is_in_data(mra, ma, mrb, mb))
                      (*local_work_sptrs[thread_num])(mra, ma, mrb, mb) = geo_data(ra, a, rb, b % num_transaxial_detectors);
                  }
            }
      }
  }
  std::cout << " loops over geo finished! " << std::endl;

std::cout<<"Now flattening the FanProjData data ..." << std::endl;
#ifdef STIR_OPENMP
// flatten data constructed by threads
{
  for (int i = 1; i < static_cast<int>(local_work_sptrs.size()); ++i)
    if (!is_null_ptr(local_work_sptrs[i])) // only accumulate if a thread filled something in
...
template <class TProjDataInfo>
static void
make_fan_sum_data_help(Array<2, float>& data_fan_sums,
                       int num_rings,
                       int num_detectors_per_ring,
                       int max_ring_diff,
                       int fan_size,
...
void
make_fan_sum_data(Array<2, float>& data_fan_sums,
                  ...
                  const int half_fan_size)
{
  ...
  const int num_detectors_per_ring = data_fan_sums[0].get_length();
...
void
make_geo_data(GeoData3D& geo_data, const FanProjData& fan_data)
{
...
void
make_block_data(BlockData3D& block_data, const FanProjData& fan_data)
{
  const int num_axial_detectors = fan_data.get_num_rings();
  const int num_transaxial_detectors = fan_data.get_num_detectors_per_ring();
  const int num_axial_blocks = block_data.get_num_rings();
  ...
  for (int ra = fan_data.get_min_ra(); ra <= fan_data.get_max_ra(); ++ra)
    for (int a = fan_data.get_min_a(); a <= fan_data.get_max_a(); ++a)
      ...
      for (int rb = max(ra, fan_data.get_min_rb(ra)); rb <= fan_data.get_max_rb(ra); ++rb)
        for (int b = fan_data.get_min_b(a); b <= fan_data.get_max_b(a); ++b)
          {
            block_data(ra / num_axial_crystals_per_block,
                       a / num_transaxial_crystals_per_block,
                       rb / num_axial_crystals_per_block,
                       b / num_transaxial_crystals_per_block)
                += fan_data(ra, a, rb, b);
          }
}

void
iterate_efficiencies(DetectorEfficiencies& efficiencies, const Array<2, float>& data_fan_sums, const FanProjData& model)
{
...
void
iterate_efficiencies(DetectorEfficiencies& efficiencies,
...
void
iterate_geo_norm(GeoData3D& norm_geo_data, const GeoData3D& measured_geo_data, const FanProjData& model)
{
  make_geo_data(norm_geo_data, model);
  // norm_geo_data = measured_geo_data / norm_geo_data;
...
void
iterate_block_norm(BlockData3D& norm_block_data, const BlockData3D& measured_block_data, const FanProjData& model)
{
  make_block_data(norm_block_data, model);
  // norm_block_data = measured_block_data / norm_block_data;
  const float threshold = measured_block_data.find_max() / 10000.F;
    ...
    for (int a = norm_block_data.get_min_a(); a <= norm_block_data.get_max_a(); ++a)
      ...
      for (int rb = max(ra, norm_block_data.get_min_rb(ra)); rb <= norm_block_data.get_max_rb(ra); ++rb)
        for (int b = norm_block_data.get_min_b(a); b <= norm_block_data.get_max_b(a); ++b)
          {
            norm_block_data(ra, a, rb, b) = (measured_block_data(ra, a, rb, b) >= threshold
...
double
KL(const FanProjData& d1, const FanProjData& d2, const double threshold)
{
  double sum = 0;

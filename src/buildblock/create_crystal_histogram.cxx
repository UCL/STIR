/*!
  ...
  \brief Implementation of stir::create_crystal_histogram
...
#include "stir/create_crystal_histogram.h"
...
void
create_crystal_histogram_help(Array<2, float>& histogram, const ProjData& proj_data, const TProjDataInfo& proj_data_info)
{
...
#  endif
#endif
                      histogram[p1.axial_coord()][p1.tangential_coord()] += count;
#if defined(STIR_OPENMP)
#  if _OPENMP >= 201012
#    pragma omp atomic update
#  endif
#endif
                      histogram[p2.axial_coord()][p2.tangential_coord()] += count;
...
void
create_crystal_histogram(Array<2, float>& histogram, const ProjData& proj_data)
{
  if (proj_data.get_proj_data_info_sptr()->get_scanner_ptr()->get_scanner_geometry() == "Cylindrical")
    {
      ...
      create_crystal_histogram_help(histogram, proj_data, *proj_data_info_ptr);
    }
  else
    {
      auto proj_data_info_ptr
...
template void create_crystal_histogram_help(Array<2, float>&, const ProjData&, const ProjDataInfoCylindricalNoArcCorr&);
template void create_crystal_histogram_help(Array<2, float>&, const ProjData&, const ProjDataInfoBlocksOnCylindricalNoArcCorr&);

...
static void build_gauss(VectorWithOffset<elemT>& kernel, int res, float s2, float sampling_interval);

template <typename elemT>
static void build_metz(VectorWithOffset<elemT>& kernel, float N, float fwhm, float MmPerVoxel, int max_kernel_size);

template <int num_dimensions, typename elemT>
SeparableMetzArrayFilter<num_dimensions, elemT>::SeparableMetzArrayFilter(
    const VectorWithOffset<float>& fwhms_v, const VectorWithOffset<float>& metz_powers_v,
    const BasicCoordinate<num_dimensions, float>& sampling_distances_v, const VectorWithOffset<int>& max_kernel_sizes_v)
    : fwhms(fwhms_v), metz_powers(metz_powers_v), sampling_distances(sampling_distances_v), max_kernel_sizes(max_kernel_sizes_v) {
  ...
  for (int i = 1; i <= num_dimensions; ++i) {
    ...
    for (int j = 0; j < kernel.get_length(); j++)
      if (metz_powers[i] > 0.0)
        printf("%d-dir Metz[%d]=%f\n", i, j, kernel[j]);
      else
        printf("%d-dir Gauss[%d]=%f\n", i, j, kernel[j]);
...
template <typename elemT>
void
build_gauss(VectorWithOffset<elemT>& kernel, int res, float s2, float sampling_interval) {
  ...
  for (j = 1; (j < hres && !cutoff); j++) {
    kernel[hres - j - 1] =
        static_cast<elemT>(kernel[hres - 1] * exp(-0.5 * (j * sampling_interval) * (j * sampling_interval) / s2));
    kernel[hres + j - 1] = kernel[hres - j - 1];
    sum += 2.0F * kernel[hres - j - 1];
    if (kernel[hres - j - 1] < kernel[hres - 1] * ZERO_TOL)
      cutoff = 1;
  }
...
template <typename elemT>
void
build_metz(VectorWithOffset<elemT>& kernel, float N, float fwhm, float MmPerVox, int max_kernel_size) {
  ...
  if (fwhm > 0.0F) {
    ...
    for (int i = 0; i < Res; i++) {
      ...
      if (stretch > 0.0) {
        ...
        if (i > cutoff && Res - i > cutoff)
          zabs2 = 0;
      }

      if (zabs2 > 1)
        zabs2 = static_cast<elemT>(1 - ZERO_TOL);
      if (zabs2 > 0) {
    ...
    for (int i = Res - 1; i >= 0; i--) {
      if (fabs((double)filter[i]) >= (0.0001) * filter[0])
        break;
      else
        (kernel_length)--;
    }

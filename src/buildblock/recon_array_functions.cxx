...
// AZ 07/10/99: added
void
truncate_rim(Viewgram<float>& viewgram, const int rim_truncation_sino)
{
  ...
  if (viewgram.get_num_tangential_poss() % 2 != 0)
    upper_truncation_offset--;
...
void
truncate_rim(SegmentByView<float>& seg, const int rim_truncation_sino)
{
  ...
  int upper_truncation_offset = rim_truncation_sino;
  if (seg.get_num_tangential_poss() % 2 != 0)
    upper_truncation_offset--;
...
void
truncate_rim(SegmentBySinogram<float>& seg, const int rim_truncation_sino)
{
...
// MJ 18/9/98 new
void
truncate_rim(DiscretisedDensity<3, float>& input_image, const int rim_truncation_image, const bool strictly_less_than_radius)
{
...
void
divide_and_truncate(Viewgram<float>& numerator,
                    ...
                    double* log_likelihood_ptr /* = NULL */)
{
  ...
  const float small_value = max(numerator.find_max() * SMALL_NUM, 0.F);
...
#else
          if (b < bs + rim_truncation_sino || b > be - rim_truncation_sino)
            {
              numerator[r][b] = 0;
            }
          else
            {
              ...
              if (num
                  <= small_value) // KT Feb2011 was "num<small_value", resulting in a BUG if the whole numerator viewgram was zero
                {
...
void
divide_and_truncate(RelatedViewgrams<float>& numerator,
                    ...
                    double* log_likelihood_ptr)
{
  ...
  while (numerator_iter != numerator.end())
    {
      divide_and_truncate(*numerator_iter, *denominator_iter, rim_truncation_sino, count, count2, log_likelihood_ptr);
      ++numerator_iter;
      ++denominator_iter;
    }
}
...
void
divide_array(SegmentByView<float>& numerator, const SegmentByView<float>& denominator)
{
...
void
accumulate_loglikelihood(Viewgram<float>& projection_data,
                         ...
                         double* accum)
{
  ...
  */
  double result = 0;
  const float small_value = max(projection_data.find_max() * SMALL_NUM, 0.F);
  const float max_quotient = 10000.F;
...
void
accumulate_loglikelihood(DiscretisedDensity<3, float>& outer_loop_dyn_image_estimate,
                         ...
                         double* accum)
{
  ...
  */
  double result = 0;
  const float small_value = max(outer_loop_dyn_image_estimate.find_max() * SMALL_NUM, 0.F);
  const float max_quotient = 10000.F;
  ...
  for (int z = outer_loop_dyn_image_estimate.get_min_index(); z <= outer_loop_dyn_image_estimate.get_max_index(); z++)
    {
      ...
      for (int y = outer_loop_dyn_image_estimate[z].get_min_index(); y <= outer_loop_dyn_image_estimate[z].get_max_index(); y++)
        for (int x = outer_loop_dyn_image_estimate[z][y].get_min_index();
             x <= outer_loop_dyn_image_estimate[z][y].get_max_index();
             x++)
          {
            const float new_estimate
                = max(nested_loop_dyn_image_estimates[z][y][x], outer_loop_dyn_image_estimate[z][y][x] / max_quotient);

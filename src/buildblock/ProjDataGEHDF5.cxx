...
ProjDataGEHDF5::ProjDataGEHDF5(const std::string& input_filename) : ProjData() {
  this->m_input_hdf5_sptr.reset(new GEHDF5Wrapper(input_filename));
  this->initialise_from_wrapper();
}

ProjDataGEHDF5::ProjDataGEHDF5(shared_ptr<GEHDF5Wrapper> input_hdf5_sptr) : ProjData(), m_input_hdf5_sptr(input_hdf5_sptr) {
  this->initialise_from_wrapper();
}

void
ProjDataGEHDF5::initialise_from_wrapper() {
...
void
ProjDataGEHDF5::initialise_viewgram_buffer() {
...
void
ProjDataGEHDF5::initialise_segment_sequence() {
...
void
ProjDataGEHDF5::initialise_ax_pos_offset() {
  ...
  for (int i_seg = 1; i_seg < get_num_segments(); ++i_seg) {
    ...
    seg_ax_offset[i_seg] = static_cast<unsigned int>(get_num_axial_poss(segment_num)) + previous_value;
...
ProjDataGEHDF5::get_viewgram(const int view_num, const int segment_num, const bool make_num_tangential_poss_odd,
  ...
  for (int tang_pos = ret_viewgram.get_min_tangential_pos_num(), i_tang = 0;
       tang_pos <= ret_viewgram.get_max_tangential_pos_num(); ++tang_pos, ++i_tang)
    for (int i_axial = get_min_axial_pos_num(segment_num), axial_pos = seg_ax_offset[find_segment_index_in_sequence(segment_num)];
         i_axial <= get_max_axial_pos_num(segment_num); i_axial++, axial_pos++)
      for (int tof_poss = 0; tof_poss <= num_tof_poss - 1; tof_poss++) {
        ret_viewgram[i_axial][-tang_pos] +=
            static_cast<float>(tof_data[get_max_view_num() - view_num][i_tang][tof_poss][axial_pos]);
      }
...
Succeeded
ProjDataGEHDF5::set_viewgram(const Viewgram<float>& v) {
...
Succeeded
ProjDataGEHDF5::set_sinogram(const Sinogram<float>& s) {
...
#else
  vector<int>::const_iterator iter = find(segment_sequence.begin(), segment_sequence.end(), segment_num);
#endif

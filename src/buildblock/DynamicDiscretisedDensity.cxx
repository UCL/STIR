...
DynamicDiscretisedDensity::DynamicDiscretisedDensity(const DynamicDiscretisedDensity& argument) { (*this) = argument; }
...
void
DynamicDiscretisedDensity::set_isotope_halflife(const float isotope_halflife) {
  ...
  if (_is_decay_corrected == true)
    ...
    for (unsigned int frame_num = 1; frame_num <= get_time_frame_definitions().get_num_frames(); ++frame_num) {
      *(_densities[frame_num - 1]) *=
          static_cast<float>(decay_correction_factor(_isotope_halflife, get_time_frame_definitions().get_start_time(frame_num),

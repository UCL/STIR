...
SeparableConvolutionImageFilter<elemT>::post_processing() {
  ...
  typename VectorWithOffset<VectorWithOffset<elemT>>::iterator coefficients_iter = filter_coefficients.begin();
#ifndef STIR_NO_NAMESPACES
  std::
#endif
      vector<vector<double>>::const_iterator parsing_iter = filter_coefficients_for_parsing.begin();
  for (; parsing_iter != filter_coefficients_for_parsing.end(); ++parsing_iter, ++coefficients_iter) {
    ...
    *coefficients_iter = VectorWithOffset<elemT>(min_index, min_index + size - 1);
    ...
    typename VectorWithOffset<elemT>::iterator coefficients_elem_iter = coefficients_iter->begin();
#ifndef STIR_NO_NAMESPACES
    std::
#endif
        vector<double>::const_iterator parsing_elem_iter = parsing_iter->begin();
...
template <typename elemT>
SeparableConvolutionImageFilter<elemT>::SeparableConvolutionImageFilter() : filter_coefficients_for_parsing(3) {
  set_defaults();
}
...
SeparableConvolutionImageFilter<elemT>::SeparableConvolutionImageFilter(
    ...
    : filter_coefficients_for_parsing(filter_coefficients.get_length()), filter_coefficients(filter_coefficients) {
  ...
  std::
#endif
      vector<vector<double>>::iterator parsing_iter = filter_coefficients_for_parsing.begin();
  for (; parsing_iter != filter_coefficients_for_parsing.end(); ++parsing_iter, ++coefficients_iter) {

...
*/
inline static Array<2, float>
extend_sinogram_in_views(const Array<2, float>& sino_positive_segment, const Array<2, float>& sino_negative_segment,
                         const ProjDataInfo& proj_data_info, const int min_view_extension, const int max_view_extension) {
  ...
  const float sampling_phi = proj_data_info.get_phi(Bin(0, 1, 0, 0)) - min_phi;
  ...
  for (int view_num = min_in[1]; view_num <= max_in[1]; ++view_num) {
    ...
    if (!use_extension)
      input_extended_view[view_num] = sino_positive_segment[view_num];
    else {
      ...
      for (int tang_num = symmetric_min; tang_num <= symmetric_max; ++tang_num)
        input_extended_view[view_num][tang_num] = sino_negative_segment[symmetric_view_num][-tang_num];
      // now do extrapolation where we don't have data
      for (int tang_num = min_in[2]; tang_num < symmetric_min; ++tang_num)
        input_extended_view[view_num][tang_num] = input_extended_view[view_num][symmetric_min];
      for (int tang_num = symmetric_max + 1; tang_num <= max_in[2]; ++tang_num)
        input_extended_view[view_num][tang_num] = input_extended_view[view_num][symmetric_max];
    }
  } // loop over views
...
Array<3, float>
extend_segment_in_views(const SegmentBySinogram<float>& sino, const int min_view_extension, const int max_view_extension) {
  ...
  for (int ax_pos_num = min[1]; ax_pos_num <= max[1]; ++ax_pos_num) {
    out[ax_pos_num] = detail::extend_sinogram_in_views(sino[ax_pos_num], sino[ax_pos_num], *(sino.get_proj_data_info_sptr()),
...
Array<2, float>
extend_sinogram_in_views(const Sinogram<float>& sino, const int min_view_extension, const int max_view_extension) {
  if (sino.get_segment_num() != 0)
    error("extend_segment with single segment works only for segment 0");

  return detail::extend_sinogram_in_views(sino, sino, *(sino.get_proj_data_info_sptr()), min_view_extension, max_view_extension);
}

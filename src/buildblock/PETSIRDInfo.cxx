      ...
      forced_geometry(scanner_geometry)
{
  ...
  if (std::abs(expected_circle_area - polygon_area) / expected_circle_area < 0.05f || forced_geometry == "cylindrical")
    {
      info(fmt::format(
          "PETSIRDInfo: The cylindrical area {} is more than 95% matching the polygon area {}. We will presume a cylindrical configuration.",
          expected_circle_area,
          polygon_area));
    ...
    {
      info("PETSIRDInfo: The cylindrical area is less than 95% matching the polygon area. We will predsume a non-cylindrical configuration.");

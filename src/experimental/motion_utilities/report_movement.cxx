...
ReportMovement::ReportMovement(const char* const par_filename) {
  set_defaults();
  if (par_filename != 0) {
...
bool
ReportMovement::post_processing() {
...
ReportMovement::process_data() {
  ...
  const unsigned int min_frame_num = this->get_frame_num_to_process() == -1 ? 1 : this->get_frame_num_to_process();
  const unsigned int max_frame_num =
      this->get_frame_num_to_process() == -1 ? this->get_time_frame_defs().get_num_frames() : this->get_frame_num_to_process();
  ...
  for (unsigned int current_frame_num = min_frame_num; current_frame_num <= max_frame_num; ++current_frame_num) {
    ...
    const RigidObject3DTransformation frame_transformation_to_reference =
        compose(this->get_rigid_object_transformation_to_reference(),
                this->get_motion().compute_average_motion_in_scanner_coords_rel_time(start_time, end_time));
    ...
    // now go through tracker data for this frame
    {
      ...
      for (std::vector<double>::const_iterator iter = sample_times.begin(); iter != sample_times.end(); ++iter) {
        const RigidObject3DTransformation current_transformation_to_reference =
            compose(this->get_rigid_object_transformation_to_reference(),
                    this->get_motion().get_motion_in_scanner_coords_rel_time(*iter));

        const RigidObject3DTransformation current_transformation_to_frame_ref =
            compose(frame_transformation_to_reference, current_transformation_to_reference.inverse());

        const double RMSE_within_frame =
            RigidObject3DTransformation::RMSE(current_transformation_to_frame_ref, this->reference_points.begin(),
                                              this->reference_points.end(), this->reference_points.begin());

        const double RMSE_from_ref =
            RigidObject3DTransformation::RMSE(current_transformation_to_reference, this->reference_points.begin(),
                                              this->reference_points.end(), this->reference_points.begin());
  ...
  output_summary << "\n\nTotal RMSE all frames " << std::sqrt(MSE_within_frame_all_frames / num_samples_all_frames) << " "
...
main(int argc, char* argv[]) {
  ...
  Succeeded success = application.process_data();

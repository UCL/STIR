...
MoveProjData::MoveProjData(const char* const par_filename) {
  set_defaults();
  if (par_filename != 0) {
...
MoveProjData::post_processing() {
  ...
  in_proj_data_sptr = ProjData::read_from_file(input_filename);
  ...
  if (output_template_filename.size() != 0) {
    shared_ptr<ProjData> template_proj_data_sptr = ProjData::read_from_file(output_template_filename);
...
MoveProjData::process_data() {
  ...
  for (unsigned int current_frame_num = min_frame_num; current_frame_num <= max_frame_num; ++current_frame_num) {
    ...
    if (transform_3d_object(*out_proj_data_sptr, *in_proj_data_sptr, this->get_current_rigid_object_transformation()) ==
        Succeeded::no)
      return Succeeded::no;
  }

...
get_percentage_corrupted(const Sinogram<float>& sinogram) {
  ...
  for (int view_num = sinogram.get_min_view_num(); view_num <= sinogram.get_max_view_num(); view_num++)
    for (int tang_pos = sinogram.get_min_tangential_pos_num() + 20; tang_pos <= sinogram.get_max_tangential_pos_num() - 20;
         tang_pos++) {
...
void
check_for_corrupted_sinograms(int& axial_pos_to_remove_min, int& axial_pos_to_remove_max, const ProjData& proj_data,
                              const int segment_num, const float tolerance_of_corruption)

{
  axial_pos_to_remove_min = 0;
  axial_pos_to_remove_max = 0;
  for (int axial_pos_num = proj_data.get_min_axial_pos_num(segment_num);
       axial_pos_num <= proj_data.get_max_axial_pos_num(segment_num); ++axial_pos_num) {
    const float percentage_corrupted = get_percentage_corrupted(proj_data.get_sinogram(axial_pos_num, segment_num));
  ...
  for (int axial_pos_num = proj_data.get_max_axial_pos_num(segment_num);
       axial_pos_num > proj_data.get_min_axial_pos_num(segment_num) + axial_pos_to_remove_min; axial_pos_num--) {
    const float percentage_corrupted = get_percentage_corrupted(proj_data.get_sinogram(axial_pos_num, segment_num));
    cerr << "Percentage corrupted at plane " << axial_pos_num << " is " << percentage_corrupted << "\n";
    if (percentage_corrupted > tolerance_of_corruption)
  ...
  if (axial_pos_to_remove_max + axial_pos_to_remove_min >= proj_data.get_num_axial_poss(segment_num))
    return;
  ...
  */
  if ((axial_pos_to_remove_max + axial_pos_to_remove_min) % 2 != 0) {
    const float percentage_corrupted_min = get_percentage_corrupted(
        proj_data.get_sinogram(proj_data.get_min_axial_pos_num(segment_num) + axial_pos_to_remove_min, 0));
    const float percentage_corrupted_max = get_percentage_corrupted(
        proj_data.get_sinogram(proj_data.get_max_axial_pos_num(segment_num) - axial_pos_to_remove_min, 0));
...
main(int argc, char* argv[]) {
  ...
  shared_ptr<ProjDataInfo> proj_data_info_ptr = in_projdata_ptr->get_proj_data_info_sptr()->create_shared_clone();
...
#  if 1
    ...
    check_for_corrupted_sinograms(axial_pos_to_remove_min, axial_pos_to_remove_max, *eff_projdata_ptr, 0,
    ...
    new_num_axial_poss_per_segment[0] = in_projdata_ptr->get_num_axial_poss(0) - number_of_axial_pos_to_remove;
    ...
    for (int segment_num = proj_data_info_ptr->get_min_segment_num(); segment_num <= proj_data_info_ptr->get_max_segment_num();
      ...
      if (segment_num != 0) {
        ...
        new_num_axial_poss_per_segment[segment_num] =
            in_projdata_ptr->get_num_axial_poss(segment_num) - number_of_axial_pos_to_remove;
...
#  endif
    if (proj_data_info_ptr->get_max_segment_num() < max_segment_num_to_process)
      warning("%s WARNING: highest segments were corrupted too much. \n"
              ...
              argv[0], 2 * proj_data_info_ptr->get_max_segment_num() + 1, 2 * max_segment_num_to_process + 1);
  }
  ...
  // check if everything was done consistently
  {
    /* We do this by checking if all m-coordinates are shifted with the
       same amount.
    */
    const ProjDataInfo& in_proj_data_info = *(in_projdata_ptr->get_proj_data_info_sptr());

    const float m_difference_segment_0 = in_proj_data_info.get_m(Bin(0, 0, 0, 0)) - proj_data_info_ptr->get_m(Bin(0, 0, 0, 0));
    // variable used to scale differences in floating point comparison
    const float reference_sampling_in_m = in_proj_data_info.get_sampling_in_m(Bin(0, 0, 0, 0));

    for (int segment_num = proj_data_info_ptr->get_min_segment_num(); segment_num <= proj_data_info_ptr->get_max_segment_num();
         ++segment_num) {
      const float m_difference = in_proj_data_info.get_m(Bin(segment_num, 0, 0, 0)) -
                                 proj_data_info_ptr->get_m(Bin(segment_num, 0, 0, 0)) - m_difference_segment_0;
  ...
  std::vector<int>::iterator iter_record_of_number_of_planes_to_remove_min = record_of_number_of_planes_to_remove_min.begin();
  std::vector<int>::iterator iter_record_of_number_of_planes_to_remove_max = record_of_number_of_planes_to_remove_max.begin();

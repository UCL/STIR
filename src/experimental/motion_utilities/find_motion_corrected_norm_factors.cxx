...
static void allocate_segments(all_segments_type& segments, const int start_segment_index, const int end_segment_index,
                              const ProjDataInfo* proj_data_info_ptr);
/* last parameter only used if USE_SegmentByView
   first parameter only used when not USE_SegmentByView
 */
static void save_and_delete_segments(shared_ptr<iostream>& output, all_segments_type& segments, const int start_segment_index,
...
FindMCNormFactors::post_processing() {
  ...
  proj_data_info_uncompressed_ptr.reset(ProjDataInfo::ProjDataInfoCTI(scanner_ptr, 1, scanner_ptr->get_num_rings() - 1,
                                                                      scanner_ptr->get_num_detectors_per_ring() / 2,
                                                                      scanner_ptr->get_default_num_arccorrected_bins(), false));
  ...
  if (frame_num != -1 && (frame_num < 1 || unsigned(frame_num) > frame_defs.get_num_frames())) {
    warning("'time frame num (%u) should be either -1 or between 1 and the number of frames (%u)", frame_num,
...
FindMCNormFactors::process_data() {
  ...
  for (unsigned int current_frame_num = min_frame_num; current_frame_num <= max_frame_num; ++current_frame_num) {
    ...
    allocate_segments(segments, template_proj_data_info_ptr->get_min_segment_num(),
                      template_proj_data_info_ptr->get_max_segment_num(), template_proj_data_info_ptr.get());
    ...
    for (double current_time = start_time; current_time <= end_time; current_time += time_interval_this_frame) {
      ...
      for (int in_segment_num = proj_data_info_uncompressed_ptr->get_min_segment_num();
           in_segment_num <= proj_data_info_uncompressed_ptr->get_max_segment_num(); ++in_segment_num) {

        for (int in_ax_pos_num = proj_data_info_uncompressed_ptr->get_min_axial_pos_num(in_segment_num);
             in_ax_pos_num <= proj_data_info_uncompressed_ptr->get_max_axial_pos_num(in_segment_num); ++in_ax_pos_num) {

          for (int in_view_num = proj_data_info_uncompressed_ptr->get_min_view_num();
               in_view_num <= proj_data_info_uncompressed_ptr->get_max_view_num(); ++in_view_num) {
            ...
            for (int in_tangential_pos_num = proj_data_info_uncompressed_ptr->get_min_tangential_pos_num();
                ...
                if (bin.segment_num() >= start_segment_index && bin.segment_num() <= end_segment_index) {
                  ...
                  if (do_pre_normalisation) {
                    (*segments[bin.segment_num()])[bin.view_num()][bin.axial_pos_num()][bin.tangential_pos_num()] +=
                        1.F / (out_proj_data_info_ptr->get_num_ring_pairs_for_segment_axial_pos_num(bin.segment_num(),
                                                                                                    bin.axial_pos_num()) *
...
#else
              const float value =
                  do_pre_normalisation ? 1 : normalisation_ptr->get_bin_efficiency(original_bin, start_time, end_time);
              ...
              if (get_transformed_LOR(transformed_lor, ro3dtrans, original_bin, *proj_data_info_uncompressed_ptr) ==
                  Succeeded::yes)
                bin_interpolate(segments, transformed_lor, *out_proj_data_info_ptr, *proj_data_info_uncompressed_ptr, value);
#endif
            }
          }
        }
    ...
    if (current_num_time_intervals != num_time_intervals_this_frame)
      warning("\nUnexpected number of time intervals %d, should be %d", current_num_time_intervals,
              num_time_intervals_this_frame);
...
void
allocate_segments(all_segments_type& segments, const int start_segment_index, const int end_segment_index,
                  const ProjDataInfo* proj_data_info_ptr) {
  ...
  for (int seg = start_segment_index; seg <= end_segment_index; seg++) {
    ...
    segments[seg] = new Array<3, elem_type>(
        IndexRange3D(0, proj_data_info_ptr->get_num_views() - 1, 0, proj_data_info_ptr->get_num_axial_poss(seg) - 1,
...
void
save_and_delete_segments(shared_ptr<iostream>& output, all_segments_type& segments, const int start_segment_index,
                         const int end_segment_index, ProjData& proj_data) {
...
construct_proj_data(shared_ptr<iostream>& output, const string& output_filename,
                    const shared_ptr<const ProjDataInfo>& proj_data_info_ptr) {
...
#endif
        vector<int>::iterator current_segment_iter = segment_sequence_in_stream.begin();
  ...
  shared_ptr<ProjData> retvalue(new ProjDataInterfile(proj_data_info_ptr, output_filename, ios::out, segment_sequence_in_stream,
  ...
  shared_ptr<ProjDataFromStream> proj_data_ptr = new ProjDataFromStream(
      proj_data_info_ptr, output,
      /*offset=*/0, segment_sequence_in_stream, ProjDataFromStream::Segment_View_AxialPos_TangPos, OUTPUTNumericType);

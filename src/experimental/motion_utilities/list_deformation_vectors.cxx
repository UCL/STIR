...
MyApp::MyApp(const char* const par_filename)
{
  set_defaults();
  if (par_filename != 0)
    {
  ...
  while (true)
    {
      ...
      std::cout << this->transformation_sptr->transform_point(in_coord)
                << this->transformation_sptr->transform_point(in_coord) - in_coord << " Jacobian "
                << this->transformation_sptr->jacobian(in_coord) << std::endl;
    }

...
MoveImage::MoveImage(const char* const par_filename) {
  set_defaults();
  if (par_filename != 0) {
...
MoveImage::post_processing() {
  ...
  in_density_sptr = read_from_file<DiscretisedDensity<3, float>>(input_filename);
...
MoveImage::process_data() {
  ...
  for (unsigned int current_frame_num = min_frame_num; current_frame_num <= max_frame_num; ++current_frame_num) {
    ...
    transform_3d_object(*out_density_sptr, *in_density_sptr, this->get_current_rigid_object_transformation());

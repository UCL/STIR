...
void construct_scaled_filter_coefficients_2D(Array<2, float>& new_filter_coefficients_2D_array,
                                             VectorWithOffset<float> kernel_1d,
                                             const float kapa0_over_kapa1);
...

{
  ...
  Array<3, float> filter_coefficients(IndexRange3D(kernel_1d.get_min_index(),
                                                   kernel_1d.get_max_index(),
                                                   kernel_1d.get_min_index(),
                                                   kernel_1d.get_max_index(),
                                                   kernel_1d.get_min_index(),
                                                   kernel_1d.get_max_index()));
      ...
      while (true)
        {
          ...
          Array<1, float> fft_filter_1D_array(
              1,
              2 * fft_filter.get_length() * fft_filter[fft_filter.get_min_index()].get_length()
                  * fft_filter[fft_filter.get_min_index()][fft_filter.get_min_index()].get_length());
          ...
          Array<3, float> new_filter_coefficients_3D_array_tmp(IndexRange3D(1,
...
#  endif
            ...
            /*********************************************************************************/
            {
              Array<1, float> real_div_1D_array(
                  1,
          ...
          for (int i = new_filter_coefficients_3D_array_tmp[kx][jx].get_min_index();
               i <= filter_coefficients_padded[kx][jx].get_max_index() / 4;
               i++)
            {
          ...
          for (int j = new_filter_coefficients_3D_array_tmp[ky].get_min_index();
               j <= filter_coefficients_padded[ky].get_max_index() / 4;
               j++)
            {
  ...
  for (int k = new_filter_coefficients_3D_array.get_min_index(); k <= new_filter_coefficients_3D_array.get_max_index(); k++)
    for (int j = new_filter_coefficients_3D_array[k].get_min_index(); j <= new_filter_coefficients_3D_array[k].get_max_index();
      ...
      for (int i = new_filter_coefficients_3D_array[k][j].get_min_index();
           i <= new_filter_coefficients_3D_array[k][j].get_max_index();
           i++)
  ...
  if (kapa0_over_kapa1 != 0)
    {
      ...
      // in the case where sq_kappas=1 --- scaled_filter == original template filter
      Array<2, float> filter_coefficients(IndexRange2D(
          kernel_1d.get_min_index(), kernel_1d.get_max_index(), kernel_1d.get_min_index(), kernel_1d.get_max_index()));
          ...
          while (true)
            {
...

              {
                ...
                {
                  Array<1, float> real_div_1D_array(
                      1, fft_filter.get_length() * fft_filter[fft_filter.get_min_index()].get_length());
              ...
              for (int i = new_filter_coefficients_2D_array_tmp[jy].get_min_index();
                   i <= filter_coefficients_padded[jy].get_max_index() / 2;
                   i++)
                {
              ...
              for (int j = new_filter_coefficients_2D_array_tmp.get_min_index();
                   j <= filter_coefficients_padded.get_max_index() / 2;
                   j++)
                {
              ...
              if (kernel_length_x == filter_coefficients_padded.get_length() / 4)
                {
                  warning(
                      "ModifiedInverseAverigingArrayFilter3D: kernel_length_x reached maximum length %d. "
                      "First filter coefficient %g, last %g, kappa0_over_kappa1 was %g\n"
                      "Increasing length of FFT array to resolve this problem\n",
                      kernel_length_x,
                      new_filter_coefficients_2D_array_tmp[new_filter_coefficients_2D_array_tmp.get_min_index()]
                ...
                {
                  warning(
                      "ModifiedInverseAverigingArrayFilter3D: kernel_length_y reached maximum length %d. "
                      "First filter coefficient %g, last %g, kappa0_over_kappa1 was %g\n"
                      "Increasing length of FFT array to resolve this problem\n",
                      kernel_length_y,
                      new_filter_coefficients_2D_array_tmp[new_filter_coefficients_2D_array_tmp.get_min_index()]
      ...
      for (int j = new_filter_coefficients_2D_array.get_min_index(); j <= new_filter_coefficients_2D_array.get_max_index(); j++)
        for (int i = new_filter_coefficients_2D_array[j].get_min_index();
             i <= new_filter_coefficients_2D_array[j].get_max_index();
             i++)
    ...
    DiscretisedDensity<3, elemT>* in_density) const
{
  ...
  VoxelsOnCartesianGrid<float>* vox_image_ptr_1 = new VoxelsOnCartesianGrid<float>(IndexRange3D(in_density_cast->get_min_z(),
                                                                                                in_density_cast->get_max_z(),
                                                                                                in_density_cast->get_min_y(),
                                                                                                in_density_cast->get_max_y(),
                                                                                                in_density_cast->get_min_x(),
                                                                                                in_density_cast->get_max_x()),
                                                                                   in_density_cast->get_origin(),
                                                                                   in_density_cast->get_voxel_size());
  ...
  VoxelsOnCartesianGrid<float>* vox_image_ptr_kappa0
      = new VoxelsOnCartesianGrid<float>(IndexRange3D(in_density_cast->get_min_z(),
                                                      in_density_cast->get_max_z(),
                                                      in_density_cast->get_min_y(),
                                                      in_density_cast->get_max_y(),
                                                      in_density_cast->get_min_x(),
  ...
  VoxelsOnCartesianGrid<float>* kappa_coefficients = new VoxelsOnCartesianGrid<float>(IndexRange3D(in_density_cast->get_min_z(),
                                                                                                   in_density_cast->get_max_z(),
                                                                                                   in_density_cast->get_min_y(),
                                                                                                   in_density_cast->get_max_y(),
                                                                                                   in_density_cast->get_min_x(),
  ...
  find_inverse_and_bck_densels(*kappa1_ptr_bck,
                               ...
                               vox_image_ptr_kappa1->get_min_y(),
                               vox_image_ptr_kappa1->get_max_y(),
                               vox_image_ptr_kappa1->get_min_x(),
                               vox_image_ptr_kappa1->get_max_x(),
                               *proj_matrix_ptr,
                               do_attenuation,
                               threshold,
                               false); // true);
          ...
          if (true) // attenuation_proj_data_filename !="1")
            {
                  ...
                  for (int i_in = min_i; i_in <= max_i; i_in++)
                    {
                      (*in_density_cast_tmp)[k_in - k + (ceil(in_density_cast->get_max_z() - in_density_cast->get_min_z()) / 2)]
                                            [j_in - j + 6][i_in - i + 6]
                          = (*in_density_cast)[k_in][j_in][i_in];
              ...
              fwd_densels_all(all_segments_for_kappa0,
                              proj_matrix_ptr,
                              proj_data_ptr,
                              in_density_cast_tmp->get_min_z(),
                              in_density_cast_tmp->get_max_z(),
...
#  if 1
              shared_ptr<VoxelsOnCartesianGrid<float>> in_density_cast_tmp = new VoxelsOnCartesianGrid<float>(
                  IndexRange3D(k, k, -mask_size + 6, mask_size + 6, -mask_size + 6, mask_size + 6),
                  in_density_cast->get_origin(),
                  in_density_cast->get_voxel_size());
              ...
              for (int k = in_density_cast_0.get_min_z(); k <= in_density_cast_0.get_max_z(); k++)
                {
                  ...
                  for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++)
                    {
                      (all_filter_coefficients_nonseparable_2D[k])[j].grow(in_density_cast_0.get_min_x(),
                                                                           in_density_cast_0.get_max_x());
                    }
                }
              ...
              if (precomputed_coefficients_filename != "1")
                {
                  ...
                  for (int k = precomputed_coefficients_image_cast->get_min_z();
                       k <= precomputed_coefficients_image_cast->get_max_z();
                       k++)
                    for (int j = precomputed_coefficients_image_cast->get_min_y();
                         j <= precomputed_coefficients_image_cast->get_max_y();
                         j++)
                      for (int i = precomputed_coefficients_image_cast->get_min_x();
                           i <= precomputed_coefficients_image_cast->get_max_x();
                           i++)
                        {
                          Array<2, float> new_coeffs;
                          ...
                          if ((*precomputed_coefficients_image)[k][j][i] > 0.00001)
                            {
                              construct_scaled_filter_coefficients_2D(
                                  new_coeffs, filter_coefficients, 1 / (*precomputed_coefficients_image)[k][j][i]);
              ...
              for (int k = in_density_cast_0.get_min_z(); k <= in_density_cast_0.get_max_z(); k++)
                {
                  ...
                  for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++)
                    {
                      (all_filter_coefficients_separable[k])[j].grow(in_density_cast_0.get_min_x(),
                                                                     in_density_cast_0.get_max_x());
                    }
                }
              if (precomputed_coefficients_filename != "1")
      ...
      if ((count % 20) == 0 /*|| count == 1 */)
        {
          ...
          VoxelsOnCartesianGrid<float>* vox_image_ptr_1
              = new VoxelsOnCartesianGrid<float>(IndexRange3D(in_density_cast_0.get_min_z(),
                                                              ...
                                                              in_density_cast_0.get_min_y(),
                                                              in_density_cast_0.get_max_y(),
                                                              in_density_cast_0.get_min_x(),
                                                              in_density_cast_0.get_max_x()),
          ...
          for (int segment_num = start_segment_num; segment_num <= end_segment_num; ++segment_num)
            {
              ...
              if (attenuation_proj_data_filename != "1")
              ...
              else
                {
                  do_attenuation = false;
                  all_attenuation_segments[segment_num]
                      = new SegmentByView<float>(proj_data_ptr->get_empty_segment_by_view(segment_num));
                  (*all_attenuation_segments[segment_num]).fill(1);
                }
            }
          ...
          for (int segment_num = start_segment_num; segment_num <= end_segment_num; ++segment_num)
            all_segments_for_kappa0[segment_num]
                = new SegmentByView<float>(proj_data_ptr->get_empty_segment_by_view(segment_num));
                              ...
                              if (in_density_cast_0[k][j][i] > 0.00001)
                                {
                                  ...
                                  construct_scaled_filter_coefficients_2D(
                                      new_coeffs, filter_coefficients, 1 / (*newly_computed_coeff)[k][j][i]);
                                  all_filter_coefficients_nonseparable_2D[k][j][i]
                                      = new ArrayFilter2DUsingConvolution<float>(new_coeffs);

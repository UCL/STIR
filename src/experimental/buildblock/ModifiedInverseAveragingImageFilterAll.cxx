...
void construct_scaled_filter_coefficients_3D(
    ...
    VectorWithOffset<float> kernel_1d, const float kapa0_over_kapa1);

void construct_scaled_filter_coefficients_2D(Array<2, float>& new_filter_coefficients_2D_array, VectorWithOffset<float> kernel_1d,
...
void
construct_scaled_filter_coefficients_3D(Array<3, float>& new_filter_coefficients_3D_array, VectorWithOffset<float> kernel_1d,
                                        const float kapa0_over_kapa1)

{
  ...
  const int kapa0_over_kapa1_interval =
      min(static_cast<int>(floor(kapa0_over_kapa1 / kapa0_over_kapa1_interval_size)), length_of_size_array - 1);
  ...
  if (sq_kapas > 10000) {
    ...
    while (true) {
      ...
      // Array<1,float> fft_1_1D_array (1, 2*fft_filter.get_length()*fft_filter[fft_filter.get_min_index()].get_length()
      // *fft_filter[fft_filter.get_min_index()][fft_filter.get_min_index()].get_length());
      Array<1, float> fft_filter_1D_array(1, 2 * fft_filter.get_length() * fft_filter[fft_filter.get_min_index()].get_length() *
                                                 fft_filter[fft_filter.get_min_index()][fft_filter.get_min_index()].get_length());

      static shared_ptr<Array<1, float>> fft_filter_1D_array_64 = new Array<1, float>(1, 2 * 64 * 64 * 64);
      static shared_ptr<Array<1, float>> fft_filter_1D_array_128 = new Array<1, float>(1, 2 * 128 * 128 * 128);
      static shared_ptr<Array<1, float>> fft_filter_1D_array_256 = new Array<1, float>(1, 2 * 256 * 256 * 256);
      ...
      Array<3, float> new_filter_coefficients_3D_array_tmp(IndexRange3D(1, filter_coefficients_padded.get_length(), 1,
                                                                        filter_coefficients_padded.get_length(), 1,
                                                                        filter_coefficients_padded.get_length()));
      ...
      {
        Array<1, float> fft_filter_num_1D_array(
            1, 2 * fft_filter.get_length() * fft_filter[fft_filter.get_min_index()].get_length() *
                   fft_filter[fft_filter.get_min_index()][fft_filter.get_min_index()].get_length());
...
#  endif
        ...
        /*********************************************************************************/
        {
          Array<1, float> real_div_1D_array(1,
                                            fft_filter.get_length() * fft_filter[fft_filter.get_min_index()].get_length() *
                                                fft_filter[fft_filter.get_min_index()][fft_filter.get_min_index()].get_length());
      ...
      for (int i = new_filter_coefficients_3D_array_tmp[kx][jx].get_min_index();
           i <= filter_coefficients_padded[kx][jx].get_max_index() / 4; i++) {
        if (fabs((double)new_filter_coefficients_3D_array_tmp[kx][jx][i]) <=
            new_filter_coefficients_3D_array_tmp
                    ...
                    [new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()].get_min_index()]
                    [new_filter_coefficients_3D_array_tmp
                         [new_filter_coefficients_3D_array_tmp.get_min_index()]
      ...
      for (int j = new_filter_coefficients_3D_array_tmp[ky].get_min_index();
           j <= filter_coefficients_padded[ky].get_max_index() / 4; j++) {
        if (fabs((double)new_filter_coefficients_3D_array_tmp[ky][j][iy])
            //=
            // new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()][new_filter_coefficients_3D_array_tmp.get_min_index()][new_filter_coefficients_3D_array_tmp.get_min_index()]*1/100000000)
            // break;
            <=
            new_filter_coefficients_3D_array_tmp
                    [new_filter_coefficients_3D_array_tmp.get_min_index()]
      ...
      for (int k = new_filter_coefficients_3D_array_tmp.get_min_index(); k <= filter_coefficients_padded.get_max_index() / 4;
           k++) {
      ...
      if (kernel_length_x == filter_coefficients_padded.get_length() / 4) {
        warning("ModifiedInverseAverigingArrayFilter3D: kernel_length_x reached maximum length %d. "
                ...
                new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    ...
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()],
                new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()][kernel_length_x],
                kapa0_over_kapa1);
        ...
        for (int i = kapa0_over_kapa1_interval + 1; i < size_for_kapa0_over_kapa1.get_length(); ++i)
          size_for_kapa0_over_kapa1[i] = max(size_for_kapa0_over_kapa1[i], size_for_kapa0_over_kapa1[kapa0_over_kapa1_interval]);
        ...
        warning("ModifiedInverseAverigingArrayFilter3D: kernel_length_y reached maximum length %d. "
                ...
                new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()],
                new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()][kernel_length_y]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()],
                kapa0_over_kapa1);
        size_for_kapa0_over_kapa1[kapa0_over_kapa1_interval] *= 2;
        for (int i = kapa0_over_kapa1_interval + 1; i < size_for_kapa0_over_kapa1.get_length(); ++i)
        ...
        warning("ModifiedInverseAverigingArrayFilter3D: kernel_length_z reached maximum length %d. "
                ...
                new_filter_coefficients_3D_array_tmp[new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()],
                new_filter_coefficients_3D_array_tmp[kernel_length_z][new_filter_coefficients_3D_array_tmp.get_min_index()]
                                                    [new_filter_coefficients_3D_array_tmp.get_min_index()],
                kapa0_over_kapa1);
        size_for_kapa0_over_kapa1[kapa0_over_kapa1_interval] *= 2;
        for (int i = kapa0_over_kapa1_interval + 1; i < size_for_kapa0_over_kapa1.get_length(); ++i)
        ...
        new_filter_coefficients_3D_array.grow(IndexRange3D(-(kernel_length_z - 1), kernel_length_z, -(kernel_length_y - 1),
                                                           kernel_length_y, -(kernel_length_x - 1), kernel_length_x));

        new_filter_coefficients_3D_array[0][0][0] = new_filter_coefficients_3D_array_tmp[1][1][1];
        new_filter_coefficients_3D_array[kernel_length_z][kernel_length_y][kernel_length_x] =
            new_filter_coefficients_3D_array_tmp[kernel_length_z][kernel_length_y][kernel_length_x];
  ...
  for (int k = new_filter_coefficients_3D_array.get_min_index(); k <= new_filter_coefficients_3D_array.get_max_index(); k++)
    for (int j = new_filter_coefficients_3D_array[k].get_min_index(); j <= new_filter_coefficients_3D_array[k].get_max_index();
         j++)
      for (int i = new_filter_coefficients_3D_array[k][j].get_min_index();
           i <= new_filter_coefficients_3D_array[k][j].get_max_index(); i++)
...
void
construct_scaled_filter_coefficients_2D(Array<2, float>& new_filter_coefficients_2D_array, VectorWithOffset<float> kernel_1d,
                                        const float kapa0_over_kapa1)

{
  ...
  if (kapa0_over_kapa1 != 0) {
    ...
    Array<2, float> filter_coefficients(
        IndexRange2D(kernel_1d.get_min_index(), kernel_1d.get_max_index(), kernel_1d.get_min_index(), kernel_1d.get_max_index()));
    ...
    if (sq_kapas > 10000) {
      ...
      while (true) {
        ...
        static shared_ptr<Array<1, float>> fft_filter_1D_array_64 = new Array<1, float>(1, 2 * 64 * 64);
        static shared_ptr<Array<1, float>> fft_filter_1D_array_128 = new Array<1, float>(1, 2 * 128 * 128);
        static shared_ptr<Array<1, float>> fft_filter_1D_array_256 = new Array<1, float>(1, 2 * 256 * 256);
        static shared_ptr<Array<1, float>> fft_filter_1D_array_512 = new Array<1, float>(1, 2 * 512 * 512);
        static shared_ptr<Array<1, float>> fft_filter_1D_array_1024 = new Array<1, float>(1, 2 * 1024 * 1024);
        static shared_ptr<Array<1, float>> fft_filter_1D_array_2048 = new Array<1, float>(1, 2 * 2048 * 2048);
        ...
        // fft_1_1D_array /= sqrt(static_cast<double> (size *size*size));
        // fft_filter_1D_array /= sqrt(static_cast<double>(size *size*size));
        Array<2, float> new_filter_coefficients_2D_array_tmp(
            IndexRange2D(1, filter_coefficients_padded.get_length(), 1, filter_coefficients_padded.get_length()));

        {
          Array<1, float> fft_filter_num_1D_array(1, 2 * fft_filter.get_length() *
                                                         fft_filter[fft_filter.get_min_index()].get_length());
          // Array<1,float> div_1D_array(1, 2*fft_filter.get_length()*fft_filter[fft_filter.get_min_index()].get_length());
        ...
        for (int i = new_filter_coefficients_2D_array_tmp[jy].get_min_index();
             i <= filter_coefficients_padded[jy].get_max_index() / 2; i++) {
          if (fabs((double)new_filter_coefficients_2D_array_tmp[jy][i]) <=
              new_filter_coefficients_2D_array_tmp[new_filter_coefficients_2D_array_tmp.get_min_index()]
                                                  [new_filter_coefficients_2D_array_tmp.get_min_index()] *
                  1 / 1000000)
            break;
          else
            (kernel_length_x)++;
        ...
        for (int j = new_filter_coefficients_2D_array_tmp.get_min_index(); j <= filter_coefficients_padded.get_max_index() / 2;
             j++) {
        ...
        if (kernel_length_x == filter_coefficients_padded.get_length() / 4) {
          ...
          warning("ModifiedInverseAverigingArrayFilter3D: kernel_length_y reached maximum length %d. "
                  ...
                  new_filter_coefficients_2D_array_tmp[new_filter_coefficients_2D_array_tmp.get_min_index()]
                                                      [new_filter_coefficients_2D_array_tmp.get_min_index()],
                  new_filter_coefficients_2D_array_tmp[kernel_length_y][new_filter_coefficients_2D_array_tmp.get_min_index()],
                  kapa0_over_kapa1);
          size_for_kapa0_over_kapa1[kapa0_over_kapa1_interval] *= 2;
          for (int i = kapa0_over_kapa1_interval + 1; i < size_for_kapa0_over_kapa1.get_length(); ++i)
            size_for_kapa0_over_kapa1[i] =
          ...
          new_filter_coefficients_2D_array.grow(
              IndexRange2D(-(kernel_length_y - 1), kernel_length_y - 1, -(kernel_length_x - 1), kernel_length_x - 1));

          new_filter_coefficients_2D_array[0][0] = new_filter_coefficients_2D_array_tmp[1][1];

          // new_filter_coefficients_2D_array[kernel_length_y-1][kernel_length_x-1] =
          // new_filter_coefficients_2D_array_tmp[kernel_length_y][kernel_length_x];
    ...
    for (int j = new_filter_coefficients_2D_array.get_min_index(); j <= new_filter_coefficients_2D_array.get_max_index(); j++)
      for (int i = new_filter_coefficients_2D_array[j].get_min_index(); i <= new_filter_coefficients_2D_array[j].get_max_index();
           i++)
    ...
    /*	cerr << " now assigned symmetric coeff " << endl;
           for (int  j = new_filter_coefficients_2D_array.get_min_index();j<=
       new_filter_coefficients_2D_array.get_max_index();j++) for (int  i =
       new_filter_coefficients_2D_array[j].get_min_index();i<=new_filter_coefficients_2D_array[j].get_max_index();i++)
           {
...
ModifiedInverseAveragingImageFilterAll<elemT>::ModifiedInverseAveragingImageFilterAll() {
  set_defaults();
}

template <typename elemT>
ModifiedInverseAveragingImageFilterAll<elemT>::ModifiedInverseAveragingImageFilterAll(
    string proj_data_filename_v, string attenuation_proj_data_filename_v, const VectorWithOffset<elemT>& filter_coefficients_v,
    shared_ptr<ProjData> proj_data_ptr_v, shared_ptr<ProjData> attenuation_proj_data_ptr_v,
    DiscretisedDensity<3, float>* initial_image_v, DiscretisedDensity<3, float>* sensitivity_image_v,
...
{
  assert(filter_coefficients.get_length() == 0 || filter_coefficients.begin() == 0);
...
template <typename elemT>
Succeeded
ModifiedInverseAveragingImageFilterAll<elemT>::virtual_set_up(const DiscretisedDensity<3, elemT>& density) {
  proj_data_ptr = ProjData::read_from_file(proj_data_filename);

  if (attenuation_proj_data_filename != "1")
    attenuation_proj_data_ptr = ProjData::read_from_file(attenuation_proj_data_filename);
  else
    attenuation_proj_data_ptr = NULL;

  if (initial_image_filename != "1")
    initial_image = DiscretisedDensity<3, float>::read_from_file(initial_image_filename);
  else
    initial_image = NULL;

  if (sensitivity_image_filename != "1")
    sensitivity_image = DiscretisedDensity<3, float>::read_from_file(sensitivity_image_filename);
  else
    sensitivity_image = NULL;
  if (precomputed_coefficients_filename != "1")
    precomputed_coefficients_image = DiscretisedDensity<3, float>::read_from_file(precomputed_coefficients_filename);
  else
    precomputed_coefficients_image = NULL;

  if (normalised_bck_filename != "1")
    normalised_bck_image = DiscretisedDensity<3, float>::read_from_file(normalised_bck_filename);
  else
...
template <typename elemT>
void
ModifiedInverseAveragingImageFilterAll<elemT>::precalculate_filter_coefficients(
    VectorWithOffset<VectorWithOffset<VectorWithOffset<shared_ptr<ArrayFilter3DUsingConvolution<float>>>>>&
  ...
  VoxelsOnCartesianGrid<float>* in_density_cast = dynamic_cast<VoxelsOnCartesianGrid<float>*>(in_density);

  VoxelsOnCartesianGrid<float>* vox_image_ptr_1 = new VoxelsOnCartesianGrid<float>(
      IndexRange3D(in_density_cast->get_min_z(), in_density_cast->get_max_z(), in_density_cast->get_min_y(),
                   in_density_cast->get_max_y(), in_density_cast->get_min_x(), in_density_cast->get_max_x()),
  ...
  fwd_densels_all(all_segments, proj_matrix_ptr, proj_data_ptr, in_density_cast->get_min_z(), in_density_cast->get_max_z(),
                  in_density_cast->get_min_y(), in_density_cast->get_max_y(), in_density_cast->get_min_x(),
                  in_density_cast->get_max_x(), *in_density);

  VoxelsOnCartesianGrid<float>* vox_image_ptr_kappa0 = new VoxelsOnCartesianGrid<float>(
      IndexRange3D(in_density_cast->get_min_z(), in_density_cast->get_max_z(), in_density_cast->get_min_y(),
                   in_density_cast->get_max_y(), in_density_cast->get_min_x(), in_density_cast->get_max_x()),
      in_density_cast->get_origin(), in_density_cast->get_voxel_size());

  VoxelsOnCartesianGrid<float>* vox_image_ptr_kappa1 = new VoxelsOnCartesianGrid<float>(
      IndexRange3D(in_density_cast->get_min_z(), in_density_cast->get_max_z(), in_density_cast->get_min_y(),
                   in_density_cast->get_max_y(), in_density_cast->get_min_x(), in_density_cast->get_max_x()),
      in_density_cast->get_origin(), in_density_cast->get_voxel_size());

  VoxelsOnCartesianGrid<float>* kappa_coefficients = new VoxelsOnCartesianGrid<float>(
      IndexRange3D(in_density_cast->get_min_z(), in_density_cast->get_max_z(), in_density_cast->get_min_y(),
                   in_density_cast->get_max_y(), in_density_cast->get_min_x(), in_density_cast->get_max_x()),
      in_density_cast->get_origin(), in_density_cast->get_voxel_size());
  ...
  for (int segment_num = start_segment_num; segment_num <= end_segment_num; segment_num++) {
    SegmentByView<float> segment_by_view = proj_data_ptr->get_segment_by_view(segment_num);
  ...
  find_inverse_and_bck_densels(*kappa1_ptr_bck, all_segments, all_attenuation_segments, vox_image_ptr_kappa1->get_min_z(),
                               vox_image_ptr_kappa1->get_max_z(), vox_image_ptr_kappa1->get_min_y(),
                               vox_image_ptr_kappa1->get_max_y(), vox_image_ptr_kappa1->get_min_x(),
                               vox_image_ptr_kappa1->get_max_x(), *proj_matrix_ptr, do_attenuation, threshold, false); // true);
  ...
  for (int k = in_density_cast->get_min_z(); k <= in_density_cast->get_max_z(); k++)
    for (int j = in_density_cast->get_min_y(); j <= in_density_cast->get_max_y(); j++)
      for (int i = in_density_cast->get_min_x(); i <= in_density_cast->get_max_x(); i++) {
        ...
        {
          shared_ptr<VoxelsOnCartesianGrid<float>> in_density_cast_tmp = new VoxelsOnCartesianGrid<float>(
              IndexRange3D(-mask_size + (ceil(in_density_cast->get_max_z() - in_density_cast->get_min_z()) / 2),
                           mask_size + (ceil(in_density_cast->get_max_z() - in_density_cast->get_min_z()) / 2), -mask_size + 6,
                           mask_size + 6, -mask_size + 6, mask_size + 6),
              in_density_cast->get_origin(), in_density_cast->get_voxel_size());
          ...
          for (int k_in = min_k; k_in <= max_k; k_in++)
            for (int j_in = min_j; j_in <= max_j; j_in++)
              for (int i_in = min_i; i_in <= max_i; i_in++) {
                (*in_density_cast_tmp)[k_in - k + (ceil(in_density_cast->get_max_z() - in_density_cast->get_min_z()) / 2)]
                                      [j_in - j + 6][i_in - i + 6] = (*in_density_cast)[k_in][j_in][i_in];
                //(*in_density_cast_tmp)[k][j_in-j +6][i_in-i+6] = (*in_density_cast)[k][j_in][i_in];
              }

          fwd_densels_all(all_segments_for_kappa0, proj_matrix_ptr, proj_data_ptr, in_density_cast_tmp->get_min_z(),
                          in_density_cast_tmp->get_max_z(), in_density_cast_tmp->get_min_y(), in_density_cast_tmp->get_max_y(),
                          in_density_cast_tmp->get_min_x(), in_density_cast_tmp->get_max_x(), *in_density_cast_tmp);

          find_inverse_and_bck_densels(*kappa0_ptr_bck, all_segments_for_kappa0, all_attenuation_segments,
                                       // k,k,
                                       (ceil(vox_image_ptr_kappa1->get_max_z() - vox_image_ptr_kappa1->get_min_z())) / 2,
                                       ceil((vox_image_ptr_kappa1->get_max_z() - vox_image_ptr_kappa1->get_min_z()) / 2),
                                       // 0,0,0,0,
                                       6, 6, 6, 6, *proj_matrix_ptr, false, threshold, false); // true);
          (*kappa0_ptr_bck)[k][j][i] =
              (*kappa0_ptr_bck)[(ceil(vox_image_ptr_kappa1->get_max_z() - vox_image_ptr_kappa1->get_min_z())) / 2][6][6];
          ...
          fwd_densels_all(all_segments_for_kappa0, proj_matrix_ptr, proj_data_ptr, in_density_cast->get_min_z(),
                          in_density_cast->get_max_z(), min_j, max_j, min_i, max_i,
                          // j-2,j+2,
                          // i-2,i+2,
                          *in_density_cast);

          find_inverse_and_bck_densels(*kappa0_ptr_bck, all_segments_for_kappa0, all_attenuation_segments,
                                       vox_image_ptr_kappa1->get_min_z(), vox_image_ptr_kappa1->get_max_z(), j, j, i, i,
        ...
        if (fabs((double)(*kappa1_ptr_bck)[k][j][i]) > 0.00000000000001 &&
          ...
          // cerr << (*sensitivity_image)[k][j][i] << "   " <<
          // (*sensitivity_image)[ceil(in_density_cast->get_max_z()-in_density_cast->get_min_z())/2][6][6];

          /* multiply_with_sensitivity =
           (*sensitivity_image)[ceil(in_density_cast->get_max_z()-in_density_cast->get_min_z())/2][6][6]; if
           ((*sensitivity_image)[k][j][i] >0.0000001 || (*sensitivity_image)[k][j][i] <-0.0000001)
           {
...
template <typename elemT>
void
ModifiedInverseAveragingImageFilterAll<elemT>::precalculate_filter_coefficients_2D(
...
#  if 1
          shared_ptr<VoxelsOnCartesianGrid<float>> in_density_cast_tmp = new VoxelsOnCartesianGrid<float>(
              IndexRange3D(k, k,
                           ...
                           // mask_size+(ceil(in_density_cast->get_max_z()-in_density_cast->get_min_z())/2),
                           -mask_size + 6, mask_size + 6, -mask_size + 6, mask_size + 6),
              in_density_cast->get_origin(), in_density_cast->get_voxel_size());
          /*shared_ptr< VoxelsOnCartesianGrid<float> > in_density_cast_tmp =
             new VoxelsOnCartesianGrid<float>(IndexRange3D(k,k,
               //-mask_size+k,mask_size+k,
          ...
          find_inverse_and_bck_densels(
              *kappa0_ptr_bck, all_segments_for_kappa0, all_attenuation_segments, k, k,
...
#  endif
          ...
          all_filter_coefficients[k][j][i] = new ArrayFilter2DUsingConvolution<float>();
        }
      }
...
template <typename elemT>
void
ModifiedInverseAveragingImageFilterAll<elemT>::precalculate_filter_coefficients_separable(
    VectorWithOffset<VectorWithOffset<VectorWithOffset<shared_ptr<ModifiedInverseAverigingArrayFilter<3, float>>>>>&
        all_filter_coefficients,
    DiscretisedDensity<3, elemT>* in_density) const {
...
#  if 1
          shared_ptr<VoxelsOnCartesianGrid<float>> in_density_cast_tmp =
              new VoxelsOnCartesianGrid<float>(IndexRange3D(k, k, -mask_size + 6, mask_size + 6, -mask_size + 6, mask_size + 6),
                                               in_density_cast->get_origin(), in_density_cast->get_voxel_size());
          ...
          fwd_densels_all(all_segments_for_kappa0, proj_matrix_ptr, proj_data_ptr, in_density_cast->get_min_z(),
                          in_density_cast->get_max_z(), min_j, max_j, min_i, max_i, *in_density_cast);
...
template <typename elemT>
void
ModifiedInverseAveragingImageFilterAll<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& out_density,
  ...
  static VectorWithOffset<VectorWithOffset<VectorWithOffset<shared_ptr<ArrayFilter3DUsingConvolution<float>>>>>
      all_filter_coefficients;
  static VectorWithOffset<VectorWithOffset<VectorWithOffset<shared_ptr<ArrayFilter2DUsingConvolution<float>>>>>
      all_filter_coefficients_nonseparable_2D;
  static VectorWithOffset<VectorWithOffset<VectorWithOffset<shared_ptr<ModifiedInverseAverigingArrayFilter<3, float>>>>>
      all_filter_coefficients_separable;
    ...
    if (count == 1) {
      if (num_dim == 3) {
        ...
        if (precomputed_coefficients_filename != "1") {
          ...
          for (int k = precomputed_coefficients_image_cast->get_min_z(); k <= precomputed_coefficients_image_cast->get_max_z();
               k++)
            for (int j = precomputed_coefficients_image_cast->get_min_y(); j <= precomputed_coefficients_image_cast->get_max_y();
                 j++)
              for (int i = precomputed_coefficients_image_cast->get_min_x();
                   i <= precomputed_coefficients_image_cast->get_max_x(); i++) {
                Array<2, float> new_coeffs;
                // cerr << (*precomputed_coefficients_image)[k][j][i] << "     " << endl;
                if ((*precomputed_coefficients_image)[k][j][i] > 0.00001) {
                  construct_scaled_filter_coefficients_2D(new_coeffs, filter_coefficients,
                                                          1 / (*precomputed_coefficients_image)[k][j][i]);
                  all_filter_coefficients_nonseparable_2D[k][j][i] = new ArrayFilter2DUsingConvolution<float>(new_coeffs);
                } else {
                  all_filter_coefficients_nonseparable_2D[k][j][i] = new ArrayFilter2DUsingConvolution<float>();
                }
              }
        } else
              ...
              for (int i = -26; i <= 26; i++) {
                ...
                if ((*precomputed_coefficients_image)[k][j][i] > 0.00001) {
                  all_filter_coefficients_separable[k][j][i] = new ModifiedInverseAverigingArrayFilter<3, float>(
                      filter_coefficients, 1 / (*precomputed_coefficients_image)[k][j][i]);
                } else {
                  all_filter_coefficients_separable[k][j][i] = new ModifiedInverseAverigingArrayFilter<3, float>();
                }
  ...
  } else // for initial image
  {
      ...
      for (int k = in_density_cast_0.get_min_z(); k <= in_density_cast_0.get_max_z(); k++) {
        ...
        for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++) {
          ...
          for (int i = in_density_cast_0.get_min_x(); i <= in_density_cast_0.get_max_x(); i++) {
            ...
            all_filter_coefficients_separable[k][j][i] = new ModifiedInverseAverigingArrayFilter<3, elemT>;
          }
        }
      }
    }
    ...
    if ((count % 20) == 0 /*|| count == 1 */) {
      ...
      VoxelsOnCartesianGrid<float>* vox_image_ptr_1 = new VoxelsOnCartesianGrid<float>(
          IndexRange3D(in_density_cast_0.get_min_z(), in_density_cast_0.get_max_z(), in_density_cast_0.get_min_y(),
                       in_density_cast_0.get_max_y(), in_density_cast_0.get_min_x(), in_density_cast_0.get_max_x()),
      ...
      fwd_densels_all(all_segments, proj_matrix_ptr, proj_data_ptr, in_density_cast_0.get_min_z(), in_density_cast_0.get_max_z(),
                      in_density_cast_0.get_min_y(), in_density_cast_0.get_max_y(), in_density_cast_0.get_min_x(),
                      in_density_cast_0.get_max_x(), in_density_cast_0);
      ...
      for (int segment_num = 0; segment_num <= 0; segment_num++) {
        SegmentByView<float> segment_by_view = proj_data_ptr->get_segment_by_view(segment_num);
        const float current_max_in_viewgram = segment_by_view.find_max();
        if (current_max_in_viewgram >= max_in_viewgram)
          max_in_viewgram = current_max_in_viewgram;
      ...
      find_inverse_and_bck_densels(*kappa1_ptr_bck, all_segments, all_attenuation_segments, vox_image_ptr_kappa1->get_min_z(),
                                   vox_image_ptr_kappa1->get_max_z(), vox_image_ptr_kappa1->get_min_y(),
                                   vox_image_ptr_kappa1->get_max_y(), vox_image_ptr_kappa1->get_min_x(),
                                   vox_image_ptr_kappa1->get_max_x(), *proj_matrix_ptr, do_attenuation, threshold, true);
      ...
      shared_ptr<iostream> output = new fstream(filename.c_str(), ios::trunc | ios::in | ios::out | ios::binary);

      const string filename1 = "values_of_kapa0_and_kapa1_pf";
      shared_ptr<iostream> output1 = new fstream(filename1.c_str(), ios::trunc | ios::in | ios::out | ios::binary);
      ...
      *output << "Plane number " << endl;
      ...
      shared_ptr<iostream> output_att = new fstream(testing_kappas_att.c_str(), ios::trunc | ios::out | ios::binary);

      const string testing_kappas_noatt = "kappa_noatt";
      shared_ptr<iostream> output_noatt = new fstream(testing_kappas_noatt.c_str(), ios::trunc | ios::out | ios::binary);
        ...
        for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++)
          for (int i = in_density_cast_0.get_min_x(); i <= in_density_cast_0.get_max_x(); i++) {
            ...
            if (true) // attenuation_proj_data_filename !="1")
            {
              ...
              fwd_densels_all(all_segments_for_kappa0, proj_matrix_ptr, proj_data_ptr, in_density_cast_0.get_min_z(),
                              in_density_cast_0.get_max_z(), in_density_cast_tmp->get_min_y(), in_density_cast_tmp->get_max_y(),
                              in_density_cast_tmp->get_min_x(), in_density_cast_tmp->get_max_x(), *in_density_cast_tmp);

              find_inverse_and_bck_densels(*kappa0_ptr_bck, all_segments_for_kappa0, all_attenuation_segments,
                                           vox_image_ptr_kappa1->get_min_z(), vox_image_ptr_kappa1->get_max_z(),
                                           // 0,0,0,0,
                                           6, 6, 6, 6, *proj_matrix_ptr, false, threshold, true);
              (*kappa0_ptr_bck)[k][j][i] = (*kappa0_ptr_bck)[k][6][6];
              ...
              *output_att << k << "  " << j << "  " << i << "  " << (*kappa0_ptr_bck)[k][j][i] << endl;
              ...
              fwd_densels_all(all_segments_for_kappa0, proj_matrix_ptr, proj_data_ptr, in_density_cast_0.get_min_z(),
                              in_density_cast_0.get_max_z(), min_j, max_j, min_i, max_i,
                              // j-2,j+2,
                              // i-2,i+2,
                              in_density_cast_0);
      ...
      for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++)
        for (int i = in_density_cast_0.get_min_x(); i <= in_density_cast_0.get_max_x(); i++) {
          ...
          if (num_dim == 3) {
            ...
            if (k == in_density_cast_0.get_min_z() && j == in_density_cast_0.get_min_y() && i == in_density_cast_0.get_min_x() &&
                count == 300) {
                ...
                for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++)
                  for (int i = in_density_cast_0.get_min_x(); i <= in_density_cast_0.get_max_x(); i++) {
                    ...
                    if (in_density_cast_0[k][j][i] > 0.00001) {
                      VoxelsOnCartesianGrid<float>* newly_computed_coeff = new VoxelsOnCartesianGrid<float>(
                      ...
                      precompute_filter_coefficients_for_second_apporach(*newly_computed_coeff, in_density_cast_0,
                                                                         *sensitivity_image_cast, *normalised_bck_image_cast);
                      construct_scaled_filter_coefficients_2D(new_coeffs, filter_coefficients,
                                                              1 / (*newly_computed_coeff)[k][j][i]);
                      all_filter_coefficients_nonseparable_2D[k][j][i] = new ArrayFilter2DUsingConvolution<float>(new_coeffs);
...
template <typename elemT>
void
ModifiedInverseAveragingImageFilterAll<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& density) const {
  DiscretisedDensity<3, elemT>* tmp_density = density.clone();
...
template <typename elemT>
bool
ModifiedInverseAveragingImageFilterAll<elemT>::post_processing() {
  ...
  for (int i = min_index; i <= filter_coefficients.get_max_index(); ++i)
    filter_coefficients[i] = static_cast<float>(filter_coefficients_for_parsing[i - min_index]);
  return false;
}

const char* const ModifiedInverseAveragingImageFilterAll<float>::registered_name = "Modified Inverse Image Filter All";

...
static void construct_scaled_filter_coefficients(Array<3, float>& new_filter_coefficients_3D_array,
                                                 const VectorWithOffset<float>& kernel_1d, const bool z_direction_trivial,
...
static void
construct_scaled_filter_coefficients(Array<3, float>& new_filter_coefficients_3D_array, const VectorWithOffset<float>& kernel_1d,
                                     const bool z_direction_trivial, const float kapa0_over_kapa1)
...
//	cerr << "for kappa_0_over_kappa_1 " << kapa0_over_kapa1 ;
	...
	for (int i =1;i<=size_z *size_y*size_x;i++)
	{
      ...
      for (int j = new_filter_coefficients_3D_array_tmp[ky].get_min_index();
           j <= new_filter_coefficients_3D_array_tmp[ky].get_max_index() / 4; j++) {
      ...
      if (!z_direction_trivial) {
        ...
        for (int k = new_filter_coefficients_3D_array_tmp.get_min_index();
             k <= new_filter_coefficients_3D_array_tmp.get_max_index() / 4; k++) {
      ...
      if (kernel_length_x == size_x / 4) {
        ...
        new_filter_coefficients_3D_array.grow(IndexRange3D(-(kernel_length_z - 1), kernel_length_z - 1, -(kernel_length_y - 1),
                                                           kernel_length_y - 1, -(kernel_length_x - 1), kernel_length_x - 1));
          ...
          for (int k = 0; k <= kernel_length_z - 1; k++)
            for (int j = 0; j <= kernel_length_y - 1; j++)
              for (int i = 0; i <= kernel_length_x - 1; i++) {
                new_filter_coefficients_3D_array[k][j][i] = new_filter_coefficients_3D_array[k][j][-i] =
                    new_filter_coefficients_3D_array[k][-j][-i] = new_filter_coefficients_3D_array[k][-j][i] =
                        new_filter_coefficients_3D_array[-k][j][i] = new_filter_coefficients_3D_array[-k][-j][i] =
                            new_filter_coefficients_3D_array[-k][j][-i] = new_filter_coefficients_3D_array[-k][-j][-i] =
  ...
  } else // sq_kappas == 1
  {
    ...
    new_filter_coefficients_3D_array = Array<3, float>(
        IndexRange3D(z_direction_trivial ? 0 : kernel_1d.get_min_index(), z_direction_trivial ? 0 : kernel_1d.get_max_index(),
  ...
  /*for (int  k = new_filter_coefficients_3D_array.get_min_index();k<=new_filter_coefficients_3D_array.get_max_index() ;k++)
           for (int  j = new_filter_coefficients_3D_array[k].get_min_index();j<=
     new_filter_coefficients_3D_array[k].get_max_index();j++) for (int  i =
     new_filter_coefficients_3D_array[k][j].get_min_index();i<=new_filter_coefficients_3D_array[k][j].get_max_index();i++)
             {
               cerr << new_filter_coefficients_3D_array[k][j][i] << "   ";
...
ModifiedInverseAverigingImageFilter<elemT>::ModifiedInverseAverigingImageFilter() {
  set_defaults();
}
...
ModifiedInverseAverigingImageFilter<elemT>::ModifiedInverseAverigingImageFilter(
    ...
    DiscretisedDensity<3, float>* initial_image_v, DiscretisedDensity<3, float>* sensitivity_image_v, int mask_size_v,
    bool z_direction_trivial_v)
...
template <typename elemT>
Succeeded
ModifiedInverseAverigingImageFilter<elemT>::virtual_set_up(const DiscretisedDensity<3, elemT>& density) {
...
template <typename elemT>
void
ModifiedInverseAverigingImageFilter<elemT>::precalculate_filter_coefficients(
    VectorWithOffset<VectorWithOffset<VectorWithOffset<shared_ptr<ArrayFunctionObject<3, float>>>>>& all_filter_coefficients,
  ...
  do_segments_densels_fwd(*in_density_cast, *proj_data_ptr, all_segments, in_density_cast->get_min_z(),
                          in_density_cast->get_max_z(), in_density_cast->get_min_y(), in_density_cast->get_max_y(),
                          in_density_cast->get_min_x(), in_density_cast->get_max_x(), *proj_matrix_ptr);
    ...
    for (int j = in_density_cast->get_min_y(); j <= in_density_cast->get_max_y(); j++)
      for (int i = in_density_cast->get_min_x(); i <= in_density_cast->get_max_x(); i++) {
        ...
        if (true) // attenuation_proj_data_filename !="1")
        {
          ...
          const int mask_size_z = z_direction_trivial == 1 ? 0 : mask_size;

          shared_ptr<VoxelsOnCartesianGrid<float>> in_density_cast_tmp =
              new VoxelsOnCartesianGrid<float>(IndexRange3D(-mask_size_z + new_k_ctr, mask_size_z + new_k_ctr, -mask_size + 6,
          ...
          find_inverse_and_bck_densels(*kappa0_ptr_bck, all_segments_for_kappa0, all_attenuation_segments, new_k_ctr, new_k_ctr,
        ...
        if (fabs((double)(*kappa1_ptr_bck)[k][j][i]) > 0.00000000000001 &&
          ...
          if (filter_lookup[k_index] == NULL) { // first compute the filter and store in filter_loopup
...
template <typename elemT>
void
ModifiedInverseAverigingImageFilter<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& out_density,
      ...
      for (int k = in_density_cast_0.get_min_z(); k <= in_density_cast_0.get_max_z(); k++) {
        ...
        for (int j = in_density_cast_0.get_min_y(); j <= in_density_cast_0.get_max_y(); j++) {
          ...
          for (int i = in_density_cast_0.get_min_x(); i <= in_density_cast_0.get_max_x(); i++) {
            ...
            all_filter_coefficients[k][j][i] = new ModifiedInverseAverigingArrayFilter<3, elemT>;
          }
        }
      }
    }
          ...
          for (int i = in_density_cast_0.get_min_x(); i <= in_density_cast_0.get_max_x(); i++) {
            ...
            if (fabs((double)(*kappa1_ptr_bck)[k][j][i]) > 0.00000000000001 &&
              ...
              *output << endl;
              ...
              // sq_kapas = 10;
              all_filter_coefficients[k][j][i] = new ModifiedInverseAverigingArrayFilter<3, elemT>(filter_coefficients, sq_kapas);
...
template <typename elemT>
void
ModifiedInverseAverigingImageFilter<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& density) const {
...
template <typename elemT>
bool
ModifiedInverseAverigingImageFilter<elemT>::post_processing() {
...
const char* const ModifiedInverseAverigingImageFilter<float>::registered_name = "Modified Inverse Image Filter";

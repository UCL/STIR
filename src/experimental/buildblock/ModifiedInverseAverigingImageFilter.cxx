...
//	cerr << "for kappa_0_over_kappa_1 " << kapa0_over_kapa1 ;
	  ...
	  for (int  k = 1;k<= size_z;k++)
		...
		for (int  i = 1;i<= size_x;i++)
		{
          ...
          for (int i = new_filter_coefficients_3D_array_tmp[kx][jx].get_min_index();
               i <= new_filter_coefficients_3D_array_tmp[kx][jx].get_max_index() / 4;
               i++)
            {
          ...
          for (int j = new_filter_coefficients_3D_array_tmp[ky].get_min_index();
               j <= new_filter_coefficients_3D_array_tmp[ky].get_max_index() / 4;
               j++)
            {
          ...
          if (!z_direction_trivial)
            {
              ...
              for (int k = new_filter_coefficients_3D_array_tmp.get_min_index();
                   k <= new_filter_coefficients_3D_array_tmp.get_max_index() / 4;
                   k++)
                {
  ...
  else // sq_kappas == 1
    {
      ...
      new_filter_coefficients_3D_array = Array<3, float>(IndexRange3D(z_direction_trivial ? 0 : kernel_1d.get_min_index(),
                                                                      z_direction_trivial ? 0 : kernel_1d.get_max_index(),
    ...
    DiscretisedDensity<3, elemT>* in_density) const
{
  ...
  do_segments_densels_fwd(*in_density_cast,
                          *proj_data_ptr,
                          all_segments,
          ...
          if (true) // attenuation_proj_data_filename !="1")
            {
              ...
              shared_ptr<VoxelsOnCartesianGrid<float>> in_density_cast_tmp
                  = new VoxelsOnCartesianGrid<float>(IndexRange3D(-mask_size_z + new_k_ctr,
                                                                  mask_size_z + new_k_ctr,
                                                                  -mask_size + 6,
                                                                  mask_size + 6,
                                                                  -mask_size + 6,
                                                                  mask_size + 6),
              ...
              for (int k_in = min_k; k_in <= max_k; k_in++)
                for (int j_in = min_j; j_in <= max_j; j_in++)
                  for (int i_in = min_i; i_in <= max_i; i_in++)
                    {
                      (*in_density_cast_tmp)[k_in - k + new_k_ctr][j_in - j + 6][i_in - i + 6]

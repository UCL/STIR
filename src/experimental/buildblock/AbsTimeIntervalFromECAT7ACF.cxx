...
const char* const AbsTimeIntervalFromECAT7ACF::registered_name = "from ECAT7 ACF";

AbsTimeIntervalFromECAT7ACF::AbsTimeIntervalFromECAT7ACF() { set_defaults(); }

AbsTimeIntervalFromECAT7ACF::AbsTimeIntervalFromECAT7ACF(const std::string& filename, double duration_in_secs)
    : _attenuation_filename(filename), _transmission_duration(duration_in_secs) {
  if (set_times() == Succeeded::no)
    error("Exiting"); // TODO should throw exception
}

Succeeded
AbsTimeIntervalFromECAT7ACF::set_times() {
#ifdef HAVE_LLN_MATRIX
  if (_transmission_duration <= 0) {
    warning("AbsTimeIntervalFromECAT7ACF: duration should be > 0 but is %g%.", _transmission_duration);
    return Succeeded::no;
  }
  ...
  _end_time_in_secs_since_1970 = _start_time_in_secs_since_1970 + _transmission_duration;

  matrix_close(attn_file);

  return Succeeded::yes;
#else
  warning("Error opening attenuation file %s: compiled without ECAT7 support.", _attenuation_filename.c_str());
  return Succeeded::no;
#endif
}

void
AbsTimeIntervalFromECAT7ACF::set_defaults() {
  _transmission_duration = -1;
  _attenuation_filename = "";
}

void
AbsTimeIntervalFromECAT7ACF::initialise_keymap() {
...
bool
AbsTimeIntervalFromECAT7ACF::post_processing() {
  if (set_times() == Succeeded::no) {

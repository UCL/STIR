...
void construct_scaled_filter_coefficients_3D(Array<3, float>& new_filter_coefficients_2D_array, Array<3, float>& kernel_3d,
...
void
construct_scaled_filter_coefficients_3D(Array<3, float>& new_filter_coefficients_3D_array, Array<3, float>& kernel_3d,
                                        const float kapa0_over_kapa1) {
  ...
  if (kapa0_over_kapa1 != 0) {
    ...
    if (sq_kapas > 10000) {
      ...
      while (true) {
	    ...
	    for ( int k = 1; k<=size_z;k++)
	    {
          ...
          // cerr << fft_filter_1D_array[fft_filter_1D_array.get_min_index()]<< "   " <<
          // fft_filter_1D_array[fft_filter_1D_array.get_min_index()+1]<<"    ";
        ...
        for (int j = new_filter_coefficients_3D_array_tmp[ky].get_min_index();
             j <= new_filter_coefficients_3D_array_tmp[ky].get_max_index() / 2; j++) {
        ...
        if (size_z == 1)
          ...
          for (int k = new_filter_coefficients_3D_array_tmp.get_min_index();
               k <= new_filter_coefficients_3D_array_tmp.get_max_index() / 2; k++) {
    ...
    } else // sq_kappas == 1
    {
      ...
      new_filter_coefficients_3D_array.grow(IndexRange3D(min_index_z, min_index_z + size_z - 1, min_index_y,
                                                         min_index_y + size_y - 1, min_index_x, min_index_x + size_x - 1));
        ...
        for (int j = 0; j <= new_filter_coefficients_3D_array[oldk].get_max_index(); j++) {
          ...
          for (int i = 0; i <= new_filter_coefficients_3D_array[oldk][oldj].get_max_index(); i++) {
            ...
            new_filter_coefficients_3D_array[k][j][i] = new_filter_coefficients_3D_array[k][j][-i] =
                new_filter_coefficients_3D_array[k][-j][i] = new_filter_coefficients_3D_array[k][-j][-i] =
                    new_filter_coefficients_3D_array[-k][j][i] = new_filter_coefficients_3D_array[-k][-j][i] =
                        new_filter_coefficients_3D_array[-k][j][-i] = new_filter_coefficients_3D_array[-k][-j][-i] = tmp;
          }
        }
      }
    }
...
NonseparableSpatiallyVaryingFilters3D<elemT>::NonseparableSpatiallyVaryingFilters3D() {
  set_defaults();
}

template <typename elemT>
NonseparableSpatiallyVaryingFilters3D<elemT>::NonseparableSpatiallyVaryingFilters3D(
    string proj_data_filename_v, string attenuation_proj_data_filename_v, const Array<3, float>& filter_coefficients_v,
...
template <typename elemT>
Succeeded
NonseparableSpatiallyVaryingFilters3D<elemT>::virtual_set_up(const DiscretisedDensity<3, elemT>& density) {
...
template <typename elemT>
void
NonseparableSpatiallyVaryingFilters3D<elemT>::precalculate_filter_coefficients_3D(
...
template <typename elemT>
void
NonseparableSpatiallyVaryingFilters3D<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& out_density,
  ...
  if (recompute_every_num_subiterations > 0 && count > recompute_from_subiteration_num &&
      count % recompute_every_num_subiterations == 0) {
    recompute_filters = true;
    info("recompute coefficients now\n WARNING this is specific to approach 2!");

    VoxelsOnCartesianGrid<float>* normalised_bck_image_cast = dynamic_cast<VoxelsOnCartesianGrid<float>*>(normalised_bck_image);
    VoxelsOnCartesianGrid<float>* sensitivity_image_cast = dynamic_cast<VoxelsOnCartesianGrid<float>*>(sensitivity_image);

    // WARNING this is specific to approach 2!
    precompute_filter_coefficients_for_second_apporach(*precomputed_coefficients_image_cast, in_density_cast_0,
...
template <typename elemT>
void
NonseparableSpatiallyVaryingFilters3D<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& density) const {
...
template <typename elemT>
bool
NonseparableSpatiallyVaryingFilters3D<elemT>::post_processing() {
  ...
  filter_coefficients.grow(IndexRange3D(min_index_z, min_index_z + size_z - 1, min_index_y, min_index_y + size_y - 1, min_index_x,
                                        min_index_x + size_x - 1));
...
const char* const NonseparableSpatiallyVaryingFilters3D<float>::registered_name = "Nonseparable Spatially Varying Filters 3D";

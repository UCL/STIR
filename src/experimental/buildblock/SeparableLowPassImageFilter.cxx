...
SeparableLowPassImageFilter<elemT>::SeparableLowPassImageFilter() : filter_coefficients(VectorWithOffset<float>(-1, 1)) {
  set_defaults();
}

template <typename elemT>
VectorWithOffset<float>
SeparableLowPassImageFilter<elemT>::get_filter_coefficients() {
  return filter_coefficients;
}

template <typename elemT>
Succeeded
SeparableLowPassImageFilter<elemT>::virtual_set_up(const DiscretisedDensity<3, elemT>& density) {
  ...
  lowpass_filter = SeparableLowPassArrayFilter<3, elemT>(filter_coefficients, z_trivial);
...
template <typename elemT>
void
SeparableLowPassImageFilter<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& density) const
...
template <typename elemT>
void
SeparableLowPassImageFilter<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& out_density,
...
template <typename elemT>
void
SeparableLowPassImageFilter<elemT>::set_defaults() {
...
template <typename elemT>
void
SeparableLowPassImageFilter<elemT>::initialise_keymap() {
...
template <typename elemT>
bool
SeparableLowPassImageFilter<elemT>::post_processing() {
...
const char* const SeparableLowPassImageFilter<float>::registered_name = "Separable Lowpass Filter";

  ...
  if (kapa0_over_kapa1 != 0)
    {
      ...
      Array<2, float> filter_coefficients(IndexRange2D(
          kernel_2d.get_min_index(), kernel_2d.get_max_index(), kernel_2d[0].get_min_index(), kernel_2d[0].get_max_index()));
                ...
                for (int i = 0; i <= number_of_coefficients_before_padding - 1; i++)
                  {
                    ...
                    if (i != (number_of_coefficients_before_padding - 1))
                      {
                        filter_coefficients_padded_2D_array[j + 1][size - (i)]
                            = kernel_2d[j + min_kernel_2d_y][i + min_kernel_2d_x + 1];
                      }
                  }
...
#    endif
              ...
              if (kernel_length_x == filter_coefficients_padded.get_length() / 2)
                {
                  warning(
                      "NonseparableSpatiallyVaryingFilters: kernel_length_x reached maximum length %d. "
                ...
                {
                  warning(
                      "NonseparableSpatiallyVaryingFilters: kernel_length_y reached maximum length %d. "
                      "First filter coefficient %g, last %g, kappa0_over_kappa1 was %g\n"
                      "Increasing length of FFT array to resolve this problem\n",
                      kernel_length_y,
                      new_filter_coefficients_2D_array_tmp[new_filter_coefficients_2D_array_tmp.get_min_index()]
...
#    endif
      ...
      else // sq_kappas == 1
        {
          ...
          new_filter_coefficients_2D_array.grow(IndexRange2D(-number_of_coefficients_before_padding - 1,
                                                             number_of_coefficients_before_padding - 1,
                                                             -number_of_coefficients_before_padding - 1,
                                                             number_of_coefficients_before_padding - 1));
                  ...
                  for (int i = precomputed_coefficients_image_cast->get_min_x();
                       ...
                       i++)
                    {
                      ...
                      if ((*precomputed_coefficients_image)[k][j][i] > 0.00001)
                        {
                          construct_scaled_filter_coefficients_2D(new_coeffs,
                                                                  filter_coefficients,
                      ...
                      if (in_density_cast_0[k][j][i] > 0.00001)
                        {
                          ...
                          precompute_filter_coefficients_for_second_apporach(
                              *newly_computed_coeff, in_density_cast_0, *sensitivity_image_cast, *normalised_bck_image_cast);
                          construct_scaled_filter_coefficients_2D(new_coeffs,
                                                                  filter_coefficients,

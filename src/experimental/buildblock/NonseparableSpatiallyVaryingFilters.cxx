...
void construct_scaled_filter_coefficients_2D(Array<2, float>& new_filter_coefficients_2D_array, Array<2, float>& kernel_2d,
                                             const float kapa0_over_kapa1, int number_of_coefficients_before_padding);

/**********************************************************************************************/

void
construct_scaled_filter_coefficients_2D(Array<2, float>& new_filter_coefficients_2D_array, Array<2, float>& kernel_2d,
                                        const float kapa0_over_kapa1, int number_of_coefficients_before_padding)
...
#    endif
    ...
    } else // sq_kappas == 1
    {
      ...
      new_filter_coefficients_2D_array.grow(
          IndexRange2D(-number_of_coefficients_before_padding - 1, number_of_coefficients_before_padding - 1,
                       -number_of_coefficients_before_padding - 1, number_of_coefficients_before_padding - 1));

      for (int j = 0; j <= number_of_coefficients_before_padding - 1; j++)
        for (int i = 0; i <= number_of_coefficients_before_padding - 1; i++) {
          new_filter_coefficients_2D_array[j][i] =
              filter_coefficients[filter_coefficients.get_min_index() + j][filter_coefficients[1].get_min_index() + i];
          new_filter_coefficients_2D_array[-j][-i] =
              filter_coefficients[filter_coefficients.get_min_index() + j][filter_coefficients[1].get_min_index() + i];
...
NonseparableSpatiallyVaryingFilters<elemT>::NonseparableSpatiallyVaryingFilters() {
  set_defaults();
}

template <typename elemT>
NonseparableSpatiallyVaryingFilters<elemT>::NonseparableSpatiallyVaryingFilters(
    string proj_data_filename_v, string attenuation_proj_data_filename_v, const Array<2, float>& filter_coefficients_v,
...
template <typename elemT>
Succeeded
NonseparableSpatiallyVaryingFilters<elemT>::virtual_set_up(const DiscretisedDensity<3, elemT>& density) {
...
template <typename elemT>
void
NonseparableSpatiallyVaryingFilters<elemT>::precalculate_filter_coefficients_2D(
...
#  endif
          ...
          if (filter_lookup[k_index] == NULL) {
            ...
            construct_scaled_filter_coefficients_2D(new_coeffs, filter_coefficients, sq_kapas,
                                                    number_of_coefficients_before_padding);
...
template <typename elemT>
void
NonseparableSpatiallyVaryingFilters<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& out_density,
      ...
      if (precomputed_coefficients_filename != "1") {
        ...
        for (int k = precomputed_coefficients_image_cast->get_min_z(); k <= precomputed_coefficients_image_cast->get_max_z(); k++)
          for (int j = precomputed_coefficients_image_cast->get_min_y(); j <= precomputed_coefficients_image_cast->get_max_y();
               j++)
            for (int i = precomputed_coefficients_image_cast->get_min_x(); i <= precomputed_coefficients_image_cast->get_max_x();
                 i++) {
              Array<2, float> new_coeffs;
...
template <typename elemT>
void
NonseparableSpatiallyVaryingFilters<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& density) const {
...
template <typename elemT>
bool
NonseparableSpatiallyVaryingFilters<elemT>::post_processing() {
  ...
  filter_coefficients.grow(IndexRange2D(min_index_y, min_index_y + size_y - 1, min_index_x, min_index_x + size_x - 1));

  for (int j = min_index_y; j <= filter_coefficients.get_max_index(); ++j)
    for (int i = min_index_x; i <= filter_coefficients[j].get_max_index(); ++i) {
      filter_coefficients[j][i] = static_cast<float>(filter_coefficients_for_parsing[j - min_index_y][i - min_index_x]);
    }
  return false;
}

const char* const NonseparableSpatiallyVaryingFilters<float>::registered_name = "Nonseparable Spatially Varying Filters";

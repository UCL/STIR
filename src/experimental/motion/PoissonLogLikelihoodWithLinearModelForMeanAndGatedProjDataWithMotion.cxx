...
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::initialise_keymap() {
  ...
  this->parser.add_parsing_key("transformation type for gate 1", &this->_forward_transformations[1]);
  this->parser.add_parsing_key("transformation type for gate 2", &this->_forward_transformations[2]);
  this->parser.add_parsing_key("transformation type for gate 3", &this->_forward_transformations[3]);
  this->parser.add_parsing_key("transformation type for gate 4", &this->_forward_transformations[4]);
  this->parser.add_parsing_key("transformation type for gate 5", &this->_forward_transformations[5]);
  this->parser.add_parsing_key("transformation type for gate 6", &this->_forward_transformations[6]);
  this->parser.add_parsing_key("transformation type for gate 7", &this->_forward_transformations[7]);
  this->parser.add_parsing_key("transformation type for gate 8", &this->_forward_transformations[8]);
  this->parser.add_parsing_key("transformation type for gate 9", &this->_forward_transformations[9]);
  this->parser.add_parsing_key("transformation type for gate 10", &this->_forward_transformations[10]);
  this->parser.add_parsing_key("transformation type for gate 11", &this->_forward_transformations[11]);
  this->parser.add_parsing_key("transformation type for gate 12", &this->_forward_transformations[12]);
  this->parser.add_parsing_key("transformation type for gate 13", &this->_forward_transformations[13]);
  this->parser.add_parsing_key("transformation type for gate 14", &this->_forward_transformations[14]);
  this->parser.add_parsing_key("transformation type for gate 15", &this->_forward_transformations[15]);
  this->parser.add_parsing_key("transformation type for gate 16", &this->_forward_transformations[16]);
  this->parser.add_parsing_key("transformation type for gate 17", &this->_forward_transformations[17]);
  this->parser.add_parsing_key("transformation type for gate 18", &this->_forward_transformations[18]);
  this->parser.add_parsing_key("transformation type for gate 19", &this->_forward_transformations[19]);
  this->parser.add_parsing_key("transformation type for gate 20", &this->_forward_transformations[20]);
}
...
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::post_processing() {
  ...
  if (this->_additive_projection_data_filename != "0") {
    this->_gated_additive_proj_data_sptr = GatedProjData::read_from_file(this->_additive_projection_data_filename);
  };
...
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::construct_target_ptr() const {
  return new VoxelsOnCartesianGrid<float>(
      *this->_gated_proj_data_sptr->get_proj_data_info_sptr(), static_cast<float>(this->zoom),
      CartesianCoordinate3D<float>(static_cast<float>(this->Zoffset), static_cast<float>(this->Yoffset),
...
template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_proj_data_sptr(
    const shared_ptr<GatedProjData>& arg) {
  this->_gated_proj_data_sptr = arg;
}

template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_max_segment_num_to_process(const int arg) {
  this->max_segment_num_to_process = arg;
}

template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_zero_seg0_end_planes(const bool arg) {
...
template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_projector_pair_sptr(
...
template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_frame_num(const int arg) {
  this->frame_num = arg;
}

template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_frame_definitions(
...
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::set_up_before_sensitivity(
  ...
  shared_ptr<ProjDataInfo> proj_data_info_sptr(this->_gated_proj_data_sptr->get_proj_data_info_sptr()->clone());

  if (this->max_segment_num_to_process == -1)
    this->max_segment_num_to_process = proj_data_info_sptr->get_max_segment_num();
  ...
  // initialise the objective functions for each gate
  {
    ...
    for (unsigned int gate_num = 1; gate_num <= this->_gated_proj_data_sptr->get_num_gates(); ++gate_num) {

      PoissonLogLikelihoodWithLinearModelForMeanAndProjData<TargetT> objective_function;

      objective_function.set_proj_data_sptr(this->_gated_proj_data_sptr->get_proj_data_sptr(gate_num));
      objective_function.set_max_segment_num_to_process(this->max_segment_num_to_process);
      objective_function.set_zero_seg0_end_planes(this->zero_seg0_end_planes);

      {
        shared_ptr<ProjectorByBinPair> projector_pair_sptr_this_gate;

        if (is_null_ptr(this->_forward_transformations[gate_num])) {
          projector_pair_sptr_this_gate = this->projector_pair_ptr;
        } else {
          Transform3DObjectImageProcessor<float> const* forward_transformer_ptr =
              dynamic_cast<Transform3DObjectImageProcessor<float> const*>(this->_forward_transformations[gate_num].get());
          if (forward_transformer_ptr == 0) {
            warning("transformation type has to be Transform3DObjectImageProcessor");
            return Succeeded::no;
          }
          shared_ptr<ForwardProjectorByBin> forward_projector_sptr_this_gate(new PresmoothingForwardProjectorByBin(
              this->projector_pair_ptr->get_forward_projector_sptr(), this->_forward_transformations[gate_num]));
          ...
          transpose_transformer.set_do_transpose(!forward_transformer_ptr->get_do_transpose());
          shared_ptr<BackProjectorByBin> back_projector_sptr_this_gate(new PostsmoothingBackProjectorByBin(
              this->projector_pair_ptr->get_back_projector_sptr(), transpose_transformer_sptr));
          projector_pair_sptr_this_gate.reset(
              new ProjectorByBinPairUsingSeparateProjectors(forward_projector_sptr_this_gate, back_projector_sptr_this_gate));
        }
        objective_function.set_projector_pair_sptr(projector_pair_sptr_this_gate);
      }
      if (is_null_ptr(this->_gated_additive_proj_data_sptr)) {
        shared_ptr<ProjData> nullsptr;
        objective_function.set_additive_proj_data_sptr(nullsptr);
      } else {
        objective_function.set_additive_proj_data_sptr(this->_gated_additive_proj_data_sptr->get_proj_data_sptr(gate_num));
      }
      objective_function.set_frame_num(this->frame_num);
      objective_function.set_frame_definitions(this->frame_defs);
      objective_function.set_normalisation_sptr(this->_normalisation_sptrs[gate_num]);
      objective_function.set_num_subsets(this->num_subsets);
...
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<
    TargetT>::compute_sub_gradient_without_penalty_plus_sensitivity(TargetT& gradient, const TargetT& current_estimate,
  ...
  if (iter != end_iter) {
    iter->compute_sub_gradient_without_penalty_plus_sensitivity(gradient, current_estimate, subset_num);
  }
  ++iter;
  if (iter == end_iter)
    return;
  ...
  while (iter != end_iter) {
    iter->compute_sub_gradient_without_penalty_plus_sensitivity(*gradient_this_function_sptr, current_estimate, subset_num);
    // now add it to the total gradient
    typename TargetT::full_iterator gradient_iter = gradient.begin_all();
    typename TargetT::full_iterator gradient_end = gradient.end_all();
    typename TargetT::const_full_iterator gradient_this_function_iter = gradient_this_function_sptr->begin_all_const();
...
template <typename TargetT>
void
PoissonLogLikelihoodWithLinearModelForMeanAndGatedProjDataWithMotion<TargetT>::add_subset_sensitivity(
  ...
  while (iter != end_iter) {
    iter->add_subset_sensitivity(sensitivity, subset_num);

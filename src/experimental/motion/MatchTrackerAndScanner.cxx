...
MatchTrackerAndScanner::initialise_keymap() {
  ...
  parser.add_key("scan_start_time_secs_since_1970_UTC", &this->scan_start_time_secs_since_1970_UTC);
...
MatchTrackerAndScanner::MatchTrackerAndScanner(const char* const par_filename) {
  set_defaults();
  if (par_filename != 0) {
...
bool
MatchTrackerAndScanner::post_processing() {

  if (scan_start_time_secs_since_1970_UTC == -1) {
    warning("scan_start_time_secs_since_1970_UTC not set.\n"
            "Will use relative time (to RigidObjectMotion object, which for Polaris means relative to the start of the scan data "
            "you use for synchronisation).");
    scan_start_time = 0;
  } else {
    ...
    }
    {
      ...
      scan_start_time = _ro3d_sptr->secs_since_1970_to_rel_time(sec_time);
    }
  }
...
MatchTrackerAndScanner::get_time_frame_defs() const {
  return frame_defs;
}

Succeeded
MatchTrackerAndScanner::run() {
  ...
  for (unsigned current_frame_num = 1U; current_frame_num <= this->get_time_frame_defs().get_num_frames(); ++current_frame_num) {
	...
	location_of_image_max_in_mm =
      ...
      if (input_image_cartesian_ptr == 0) {
        error("Image '%s' should be voxels on a cartesian grid", input_filename.c_str());
      }
      const float threshold = input_image_sptr->find_max() * this->relative_threshold;
      threshold_lower(input_image_sptr->begin_all(), input_image_sptr->end_all(), threshold);
      (*input_image_sptr) -= threshold;
      location_of_image_max_in_mm = find_centre_of_gravity_in_mm(*input_image_cartesian_ptr);
#endif
    }

    // now go through tracker data for this frame
    {
      const double start_time = this->get_frame_start_time(current_frame_num);
      const double end_time = this->get_frame_end_time(current_frame_num);

      cerr << "\nDoing frame " << current_frame_num << ": from " << start_time << " to " << end_time << endl;

      const std::vector<double> sample_times = this->get_motion().get_rel_time_of_samples(start_time, end_time);

      if (sample_times.size() == 0)
        error("No tracker samples between %g and %g (relative to scan start)", start_time, end_time);
      ...
      CartesianCoordinate3D<float> first_location_in_tracker_coords = this->get_motion()
                                                                          .get_motion_in_tracker_coords_rel_time(sample_times[0])
                                                                          .inverse()
                                                                          .transform_point(CartesianCoordinate3D<float>(0, 0, 0));

      for (std::vector<double>::const_iterator iter = sample_times.begin(); iter != sample_times.end(); ++iter) {
        CartesianCoordinate3D<float> location_in_tracker_coords =
            this->get_motion().get_motion_in_tracker_coords_rel_time(*iter).inverse().transform_point(
        ...
        sum_location_in_tracker_coords += location_in_tracker_coords - first_location_in_tracker_coords;
        sum_square_location_in_tracker_coords += square(location_in_tracker_coords - first_location_in_tracker_coords);
      }
      // check if frame is uniform
      const unsigned num_samples = sample_times.size();
      const CartesianCoordinate3D<float> variance =
          (sum_square_location_in_tracker_coords - square(sum_location_in_tracker_coords) / num_samples) / (num_samples - 1);
      ...
      const double stddev = std::sqrt(std::max(0.F, (variance[1] + variance[2] + variance[3]) / 3));

      if (stddev > 2)
        warning("Intra-frame motion for frame %d is too large:  %g", current_frame_num, stddev);
      else
        std::cerr << "Intra-frame motion for frame " << current_frame_num << " : " << stddev;
    }
  ...
  if (RigidObject3DTransformation::find_closest_transformation(
          this->_transformation_from_scanner_coords, positions_in_scanner.begin(), positions_in_scanner.end(),
          polaris_points.begin(), Quaternion<float>(1, 0, 0, 0)) == Succeeded::no) {
    warning("Could not find match"); // note: find_closest_transformation writes some more info
    return Succeeded::no;
  }

  const double RMSE = RigidObject3DTransformation::RMSE(this->_transformation_from_scanner_coords, positions_in_scanner.begin(),
                                                        positions_in_scanner.end(), polaris_points.begin());

  ...
  if (scan_start_time_secs_since_1970_UTC == -1)
    {
      warning("scan_start_time_secs_since_1970_UTC not set.\n"
              "Will use relative time (to RigidObjectMotion object, which for Polaris means relative to the start of the scan "
              "data you use for synchronisation).");
      scan_start_time = 0;
    }
  else
    {
...
MatchTrackerAndScanner::run()
{
  ...
  if (RigidObject3DTransformation::find_closest_transformation(this->_transformation_from_scanner_coords,
                                                               positions_in_scanner.begin(),
                                                               positions_in_scanner.end(),
  ...
  const double RMSE = RigidObject3DTransformation::RMSE(this->_transformation_from_scanner_coords,
                                                        positions_in_scanner.begin(),
                                                        positions_in_scanner.end(),
                                                        polaris_points.begin());

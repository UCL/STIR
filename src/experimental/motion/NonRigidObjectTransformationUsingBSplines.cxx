...
template <>
const char* const NonRigidObjectTransformationUsingBSplines<3, float>::registered_name = "BSplines transformation";

//////////// functions for reading NCAT transformations ///////////////////////
// (shouldn't be in this file)
static Succeeded
parse_line(const std::string& deformation_field_from_NCAT_file, std::istream& ncat_file,
           CartesianCoordinate3D<int>& current_voxel, CartesianCoordinate3D<float>& current_displacement) {
  std::string line;
  ...
  const std::string::size_type position = line.find("FRAME");
  CartesianCoordinate3D<float> new_voxel_coords;
  CartesianCoordinate3D<float> current_voxel_coords;
  int frame_num1, frame_num2;
#if 1
  if (std::sscanf(line.c_str() + position, "FRAME%d %f %f %f FRAME%d %f %f %f", &frame_num1, &current_voxel_coords.x(),
                  &current_voxel_coords.y(), &current_voxel_coords.z(), &frame_num2, &new_voxel_coords.x(), &new_voxel_coords.y(),
                  &new_voxel_coords.z()) != 8) {
    warning("Error parsing line in NCAT file %s:\n\"%s\"\nstart position %d\ntext to parse:\n%s",
            deformation_field_from_NCAT_file.c_str(), line.c_str(), position, line.c_str() + position);
    return Succeeded::no;
  }
  ...
  if (std::sscanf(line.c_str() + position, "FRAME%d %f %f %f FRAME%d %f %f %f VECTOR %f %f %f", &frame_num1,
                  &current_voxel_coords.x(), &current_voxel_coords.y(), &current_voxel_coords.z(), &frame_num2,
                  &new_voxel_coords.x(), &new_voxel_coords.y(), &new_voxel_coords.z(), &current_displacement.x(),
                  &current_displacement.y(), &current_displacement.z()) != 11) {
    warning("Error parsing line in NCAT file %s:\n\"%s\"\nstart position %d\ntext to parse:\n%s",
            deformation_field_from_NCAT_file.c_str(), line.c_str(), position, line.c_str() + position);
    return Succeeded::no;
  }
  if (norm(new_voxel_coords - current_voxel_coords - current_displacement) > .1) {
    warning("Error in line in NCAT file %s: inconsistent coordinates\n\"%s\"", deformation_field_from_NCAT_file.c_str(),
            line.c_str());
  ...
  if (norm(BasicCoordinate<3, float>(current_voxel) - current_voxel_coords) > .01) {
    warning("Error in line in NCAT file %s: ORIG voxel coordinates are expected to be on the grid\n\"%s\"",
            deformation_field_from_NCAT_file.c_str(), line.c_str());
    return Succeeded::no;
  }
...
set_deformation_field_from_NCAT_file(DeformationFieldOnCartesianGrid<3, float>& deformation_field,
  ...
  while (ncat_file) {
    if (parse_line(deformation_field_from_NCAT_file, ncat_file, current_voxel, current_displacement) != Succeeded::yes)
      return Succeeded::no;
    const CartesianCoordinate3D<float> current_displacement_in_mm = current_displacement * grid_spacing;
    if (current_voxel[1] < deformation_field[1].get_min_index() || current_voxel[1] > deformation_field[1].get_max_index() ||
...
set_deformation_field_from_file(DeformationFieldOnCartesianGrid<3, float>& deformation_field,
                                CartesianCoordinate3D<float>& grid_spacing, CartesianCoordinate3D<float>& origin,
...
set_deformation_field_from_file(DeformationFieldOnCartesianGrid<3, float>& deformation_field,
  ...
  unique_ptr<VoxelsOnCartesianGrid<CartesianCoordinate3D<float>>> uptr =
      read_from_file<VoxelsOnCartesianGrid<CartesianCoordinate3D<float>>>(deformation_field_multicomponent_filename);
...
void
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::set_defaults() {
...
template <int num_dimensions, class elemT>
void
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::initialise_keymap() {
...
template <int num_dimensions, class elemT>
void
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::set_key_values() {
...
template <int num_dimensions, class elemT>
bool
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::post_processing() {
    ...
    if (this->_deformation_field_from_NCAT_file.size() == 0) {
      if (this->_deformation_field_from_file_z.size() == 0) {
        if (this->_deformation_field_multicomponent_filename.size() == 0) {
          ...
          if (set_deformation_field_from_file(*(this->deformation_field_sptr), this->_grid_spacing, this->_origin,
                                              this->_deformation_field_multicomponent_filename) == Succeeded::no)
            return true;
        }
      } else {
        if (set_deformation_field_from_file(*(this->deformation_field_sptr), this->_grid_spacing, this->_origin,
                                            this->_deformation_field_from_file_x, this->_deformation_field_from_file_y,
                                            this->_deformation_field_from_file_z) == Succeeded::no)
          return true;
      }

    } else {
      if (set_deformation_field_from_NCAT_file(*(this->deformation_field_sptr), this->_deformation_field_from_NCAT_file,
                                               this->_deformation_field_from_NCAT_size, this->_grid_spacing,
                                               this->_origin) == Succeeded::no)
        return true;
    }
  }
...
template <int num_dimensions, class elemT>
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::NonRigidObjectTransformationUsingBSplines() {
  this->set_defaults();
}

template <int num_dimensions, class elemT>
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::NonRigidObjectTransformationUsingBSplines(
    const std::string& filename_x, const std::string& filename_y, const std::string& filename_z, const int bspline_order) {
  this->set_defaults();
...
template <int num_dimensions, class elemT>
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::NonRigidObjectTransformationUsingBSplines(
    const std::string& filename, const int bspline_order) {
  this->set_defaults();
...
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::transform_point(
    const BasicCoordinate<num_dimensions, elemT>& point) const {
...
template <int num_dimensions, class elemT>
float
NonRigidObjectTransformationUsingBSplines<num_dimensions, elemT>::jacobian(
    const BasicCoordinate<num_dimensions, elemT>& point) const {
  // note: current Bspline needs double here
  const BasicCoordinate<num_dimensions, double> point_in_grid_coords =
      BasicCoordinate<num_dimensions, double>((point - this->_origin) / this->_grid_spacing);
  ...
  for (int i = 1; i <= num_dimensions; ++i) {
    BasicCoordinate<num_dimensions, elemT> gradient = this->interpolator[i].gradient(point_in_grid_coords) / this->_grid_spacing;
  ...
  return std::fabs(determinant(jacobian_matrix));
}

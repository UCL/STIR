...
template <>
const char* const Transform3DObjectImageProcessor<float>::registered_name =
#if XXX
...
void
Transform3DObjectImageProcessor<elemT>::initialise_keymap() {
...
template <typename elemT>
bool
Transform3DObjectImageProcessor<elemT>::post_processing() {
  ...
  if (this->transformation_as_string.size() > 0) {
    ...
    if (std::fabs(norm(this->transformation.get_quaternion()) - 1) > .01) {
      warning("Quaternion should have norm 1 but is %g", norm(this->transformation.get_quaternion()));
      return true;
    }
  }
...
template <typename elemT>
Transform3DObjectImageProcessor<elemT>::Transform3DObjectImageProcessor(const shared_ptr<ObjectTransformation<3, elemT>> transf) {
  set_defaults();
...
template <typename elemT>
Succeeded
Transform3DObjectImageProcessor<elemT>::virtual_set_up(const DiscretisedDensity<3, elemT>& density) {
  if (this->_cache_transformed_coords) {
    CPUTimer timer;
    timer.start();
    if (!this->_do_jacobian) {
      this->_transformed_coords = find_grid_coords_of_transformed_centres(density, density, *this->transformation_sptr);
    } else {
...
template <typename elemT>
void
Transform3DObjectImageProcessor<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& density) const
...
template <typename elemT>
void
Transform3DObjectImageProcessor<elemT>::virtual_apply(DiscretisedDensity<3, elemT>& out_density,
                                                      const DiscretisedDensity<3, elemT>& in_density) const {

#if XXX
  if (this->_do_transpose)
    transpose_of_transform_3d_object(out_density, in_density, this->transformation);
  else
    transform_3d_object(out_density, in_density, this->transformation);
#else
      ...
      for (int z = in_density.get_min_index(); z <= in_density.get_max_index(); ++z)
        for (int y = in_density[z].get_min_index(); y <= in_density[z].get_max_index(); ++y)
          for (int x = in_density[z][y].get_min_index(); x <= in_density[z][y].get_max_index(); ++x) {
            if (this->_do_jacobian) {
              const float jacobian = this->_transformed_coords_and_jacobian[z][y][x].second;
              if (jacobian < .01)
                error("jacobian too small : %g at z=%d,y=%d,x=%d", jacobian, z, y, x);
              interpolator.add_to(this->_transformed_coords_and_jacobian[z][y][x].first, in_density[z][y][x] * jacobian);
            } else
        ...
        for (int y = out_density[z].get_min_index(); y <= out_density[z].get_max_index(); ++y)
          for (int x = out_density[z][y].get_min_index(); x <= out_density[z][y].get_max_index(); ++x) {
            if (this->_do_jacobian) {
              ...
              out_density[z][y][x] = interpolator(this->_transformed_coords_and_jacobian[z][y][x].first) * jacobian;
            } else
              out_density[z][y][x] = interpolator(this->_transformed_coords[z][y][x]);
          }
    }
  } else // !_cache_transformed_coords
  {
    if (this->_do_transpose)
      transform_3d_object_push_interpolation(out_density, in_density, *this->transformation_sptr,
                                             PushTransposeLinearInterpolator<float>(),
                                             // PushNearestNeighbourInterpolator<float>(),
                                             this->_do_jacobian);
    else
      transform_3d_object_pull_interpolation(out_density, in_density, *this->transformation_sptr, PullLinearInterpolator<float>(),
...
template <typename elemT>
void
Transform3DObjectImageProcessor<elemT>::set_defaults() {
  base_type::set_defaults();
  this->_do_transpose = false;
  this->_do_jacobian = false;
  this->_cache_transformed_coords = false;
#if XXX
  this->transformation = RigidObject3DTransformation(Quaternion<float>(1, 0, 0, 0), CartesianCoordinate3D<float>(0, 0, 0));
#else
...
template <typename elemT>
bool
Transform3DObjectImageProcessor<elemT>::get_do_transpose() const {
  return this->_do_transpose;
}

template <typename elemT>
void
Transform3DObjectImageProcessor<elemT>::set_do_transpose(const bool value) {
  this->_do_transpose = value;
}

template <typename elemT>
bool
Transform3DObjectImageProcessor<elemT>::get_do_jacobian() const {
  return this->_do_jacobian;
}

template <typename elemT>
void
Transform3DObjectImageProcessor<elemT>::set_do_jacobian(const bool value) {
  this->_do_jacobian = value;
}

template <typename elemT>
bool
Transform3DObjectImageProcessor<elemT>::get_do_cache() const {
  return this->_cache_transformed_coords;
}

template <typename elemT>
void
Transform3DObjectImageProcessor<elemT>::set_do_cache(const bool value) {

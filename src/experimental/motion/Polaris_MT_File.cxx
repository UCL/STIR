...
Polaris_MT_File::Polaris_MT_File(const std::string& mt_filename) {
  ...
  if (!mt_stream.getline(DataStr, MAX_STRING_LENGTH)) {
    error("Polaris_MT_File: error reading Line 1 of file %s", mt_filename.c_str());
  }
...
void
Polaris_MT_File::read_Peter_Bloomfield_mt_file(const std::string& mt_filename, std::istream& mt_stream,
                                               const char* const first_line) {
  const unsigned int MAX_STRING_LENGTH = 512;
  // parse first line
  {
    int v1, v2;
    ...
    if (sscanf(first_line, "%d/%d/%d %d:%d:%d - %d %d %511s", &start_time_tm.tm_mday, &start_time_tm.tm_mon,
               &start_time_tm.tm_year, &start_time_tm.tm_hour, &start_time_tm.tm_min, &start_time_tm.tm_sec, &v1, &v2,
               toolkit) != 9)
      error("Polaris_MT_File: error parsing first line of file %s", mt_filename.c_str());

    start_time_tm.tm_mon -= 1;
    start_time_tm.tm_year -= 1900;
    start_time_tm.tm_isdst = -1;

    start_time_in_secs_since_1970 = mktime(&start_time_tm);

    if (start_time_in_secs_since_1970 == std::time_t(-1))
      error("Polaris_MT_File: error interpreting data/time in first line of mt file %s", mt_filename.c_str());

    std::cout << "\nPolaris .mt file info:"
              << "\n\tDate: " << asctime(&start_time_tm) << "\t\twhich is " << start_time_in_secs_since_1970
              << " secs since 1970 UTC"
              << "\n\tToolkit: " << toolkit << "\n\tVersion info (?): " << v1 << ' ' << v2 << std::endl;
  }
  Record record;
  char DataStr[MAX_STRING_LENGTH];
  while (!mt_stream.eof() && mt_stream.getline(DataStr, MAX_STRING_LENGTH)) {
    /* Extract elements from string */
    if (sscanf(DataStr, "%lf %u %c %f %f %f %f %f %f %f %f", &record.sample_time, &record.rand_num, &record.total_num,
               &record.quat[1], &record.quat[2], &record.quat[3], &record.quat[4], &record.trans.x(), &record.trans.y(),
               &record.trans.z(), &record.rms) == 11) {
    ...
    } else if (sscanf(DataStr, "%lf %u %c : ---- Missing ----", &record.sample_time, &record.rand_num, &record.total_num) == 3) {
      vector_of_tags.push_back(record);
    } else {
...
void
Polaris_MT_File::read_NDI_Toolviewer_mt_file(const std::string& mt_filename, std::istream& mt_stream,
                                             const char* const first_line) {
  const unsigned int MAX_STRING_LENGTH = 512;
  char DataStr[MAX_STRING_LENGTH];
  ...
  mt_stream.getline(DataStr, MAX_STRING_LENGTH);
  {
    ...
    if (strptime(DataStr, "[%b %d,%Y%I:%M%p]", &start_time_tm) == NULL)
      error("Polaris_MT_File: error parsing date line of file %s", mt_filename.c_str());
    ...
    if (start_time_in_secs_since_1970 == std::time_t(-1))
      error("Polaris_MT_File: error interpreting data/time in 3rd line of mt file %s", mt_filename.c_str());

    std::cout << "\nPolaris .mt file info:"
              << "\n\tDate: " << asctime(&start_time_tm) << "\t\twhich is " << start_time_in_secs_since_1970
              << " secs since 1970 UTC" << std::endl;
  }
  // skip empty line
  mt_stream.getline(DataStr, MAX_STRING_LENGTH);
  ...
  while (!mt_stream.eof() && mt_stream.getline(DataStr, MAX_STRING_LENGTH)) {
    ...
    if (sscanf(DataStr, "%u,%f,%f,%f,%f,%f,%f,%f,%f,%u", &record.frame_num, &record.quat[1], &record.quat[2], &record.quat[3],
               &record.quat[4], &record.trans.x(), &record.trans.y(), &record.trans.z(), &record.rms, &record.out_of_FOV) == 10) {
      ...
      vector_of_records.push_back(record);
      vector_of_tags.push_back(record);
    } else if (sscanf(DataStr, "%u,    MISSING", &record.frame_num) == 1) {
      vector_of_tags.push_back(record);
    } else {
      warning("\nPolaris_MT_File: skipping (as I cannot decode) the following line:\n"
              "'%s'\n",

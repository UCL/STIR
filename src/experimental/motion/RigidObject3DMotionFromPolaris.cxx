              ...
              if (num_matched_tags > 10)
                {
                  warning("Matching sequence of length %d (starting at MT time %g) breaks at MT time %g",
                          num_matched_tags,
                          start_MT_time_of_matching_sequence,
  ...
  if (this->listmode_data_start_time_in_secs != std::time_t(-1))
    {
      ...
      cout << "\nListmode file says that listmode start time is " << lm_start_time << " in secs after midnight local time"
           << endl;
...
RigidObject3DMotionFromPolaris::post_processing()
{
  ...
  if (norm(this->move_from_scanner_coords.get_translation() - invalid_translation) < 1)
    {
      warning("transformation from scanner coordinates invalid. \"transformation_from_scanner_coordinates_filename\" keyword not "
              "set?");
      return true;
    }

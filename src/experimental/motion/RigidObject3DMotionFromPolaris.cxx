...
*/
RigidObject3DTransformation
RigidObject3DMotionFromPolaris::make_transformation_from_polaris_data(Polaris_MT_File::Record const& record) {
  /* WARNING:
     ...
     return
        RigidObject3DTransformation(Quaternion<float>(record.quat[1],
                                ...
                                CartesianCoordinate3D<float>(record.trans.z(),
                                                             ...
                                                             record.trans.y()));
  */
  return RigidObject3DTransformation(Quaternion<float>(record.quat[1], record.quat[4], record.quat[2], record.quat[3]),
                                     CartesianCoordinate3D<float>(record.trans.z(), record.trans.x(), record.trans.y()));
}

// Find and store gating values in a vector from lm_file
static void find_and_store_gate_tag_values_from_lm(VectorWithOffset<unsigned long>& lm_times_in_millisecs,
                                                   VectorWithOffset<unsigned>& lm_random_number, CListModeData& listmode_data,
                                                   const unsigned int mask_for_tags);

const char* const RigidObject3DMotionFromPolaris::registered_name = "Motion From Polaris";

RigidObject3DMotionFromPolaris::RigidObject3DMotionFromPolaris() { set_defaults(); }

const RigidObject3DTransformation&
RigidObject3DMotionFromPolaris::get_transformation_to_scanner_coords() const {
  return move_to_scanner_coords;
}

const RigidObject3DTransformation&
RigidObject3DMotionFromPolaris::get_transformation_from_scanner_coords() const {
  return move_from_scanner_coords;
}

void
RigidObject3DMotionFromPolaris::set_transformation_from_scanner_coords(
    const RigidObject3DTransformation& new_move_from_scanner_coords) {
...
double
RigidObject3DMotionFromPolaris::rel_time_to_polaris_time(const double time) const {
  return time * time_drift + time_offset;
}

double
RigidObject3DMotionFromPolaris::polaris_time_to_rel_time(const double time) const {
  return (time - time_offset) / time_drift;
}

std::vector<double>
RigidObject3DMotionFromPolaris::get_rel_time_of_samples(const double start_time, const double end_time) const {
...
RigidObject3DTransformation
RigidObject3DMotionFromPolaris::compute_average_motion_polaris_time(const double start_time, const double end_time) const {
  ...
  while (iter != mt_file_ptr->end()) {
    ...
    if ((iter->sample_time >= start_time) && (iter->sample_time <= end_time)) {
      RigidObject3DTransformation transf = make_transformation_from_polaris_data(*iter);
...
RigidObject3DTransformation
RigidObject3DMotionFromPolaris::compute_average_motion_in_tracker_coords_rel_time(const double start_time,
                                                                                  const double end_time) const {
  return compute_average_motion_polaris_time(rel_time_to_polaris_time(start_time), rel_time_to_polaris_time(end_time));
}

RigidObject3DTransformation
RigidObject3DMotionFromPolaris::get_motion_in_tracker_coords_rel_time(const double time) const {
  const double polaris_time = rel_time_to_polaris_time(time);

  Polaris_MT_File::const_iterator iterator_for_record_just_after_this_time = mt_file_ptr->begin();
  ...
  if (iterator_for_record_just_after_this_time == mt_file_ptr->end()) {
    ...
    return make_transformation_from_polaris_data(*iterator_for_record_just_after_this_time);
  }
}

void
RigidObject3DMotionFromPolaris::do_synchronisation(CListModeData& listmode_data) {
  ...
  */
  ...
  for (long int mt_offset = 0; mt_offset + num_lm_tags <= num_mt_tags; ++mt_offset) {
    ...
    Polaris_MT_File::const_iterator iterator_for_random_num = mt_file_ptr->begin_all_tags() + mt_offset;
    ...
    while (iterator_for_random_num != mt_file_ptr->end_all_tags() && lm_tag_num < num_lm_tags) {
      if ((iterator_for_random_num->rand_num & this->_mask_for_tags) != lm_random_numbers[lm_tag_num]) {
        // no match
        if (num_matched_tags > 10) {
          warning("Matching sequence of length %d (starting at MT time %g) breaks at MT time %g", num_matched_tags,
                  start_MT_time_of_matching_sequence, (double)iterator_for_random_num->sample_time);
        }
        num_matched_tags = 0;
      ...
      ++lm_tag_num;
      while (lm_tag_num < num_lm_tags && ((lm_random_numbers[lm_tag_num] & this->_mask_for_tags) == current_lm_rand_num ||
    ...
    if (num_matched_tags != 0) {
      ...
      cout << "\tEntry " << mt_offset << " in .mt file (MT time " << start_MT_time_of_matching_sequence
           << ") corresponds to start of list mode data \n";
      ...
      // fit
      {
        ...
        linear_regression(constant, scale, chi_square, variance_of_constant, variance_of_scale, covariance_of_constant_with_scale,
                          mt_match_times.begin(), mt_match_times.end(), lm_match_times.begin(), weights.begin(),
                          /* use_estimated_variance = */ true);

        std::cout << "\tscale = " << scale << " +- " << sqrt(variance_of_scale) << ", cst = " << constant << " +- "
                  << sqrt(variance_of_constant) << "\n\tchi_square = " << chi_square
                  << "\n\tcovariance = " << covariance_of_constant_with_scale << endl;
        ...
        // report max-difference
        {
          ...
          std::cout << "\nDeviation between Polaris and listmode time is between " << diff.find_min() << " and "
                    << diff.find_max() << '\n';
        }
      } // end of fit
      ...
      // do some reporting of time discrepancies
      {
        ...
        const double expected_tag_period =
            ((mt_file_ptr->end_all_tags() - 1)->sample_time - mt_file_ptr->begin_all_tags()->sample_time) /
            ((mt_file_ptr->end_all_tags() - 1) - mt_file_ptr->begin_all_tags());

        Polaris_MT_File::const_iterator mt_iter = mt_file_ptr->begin_all_tags() + mt_offset;
        double previous_mt_tag_time = mt_iter->sample_time;
        while (mt_iter != mt_file_ptr->end_all_tags() && lm_tag_num < num_lm_tags) {
          const float mt_tag_time = mt_iter->sample_time;
          ++mt_iter;
          const float elapsed_mt_tag_time = (mt_tag_time - previous_mt_tag_time);
          if (elapsed_mt_tag_time > 1.3F * expected_tag_period) {
            warning("MT file contains a time interval (%g) that is larger than expected after time %g", elapsed_mt_tag_time,
  ...
  {
    const std::time_t listmode_data_start_time_in_secs = listmode_data.get_scan_start_time_in_secs_since_1970();
    if (listmode_data_start_time_in_secs != std::time_t(-1)) {
      std::cerr << "List mode data started at " << listmode_data_start_time_in_secs << " secs since 1970\n";
      ...
      const double lm_start_time =
          (lm_start_time_tm->tm_hour * 3600.) + (lm_start_time_tm->tm_min * 60.) + lm_start_time_tm->tm_sec;

      std::cerr << "Listmode file says that listmode start time is " << lm_start_time << " in secs after midnight local time"
                << endl;
    } else {
    ...
    std::cerr << "Polaris first and last tags are at " << mt_file_ptr->begin_all_tags()->sample_time << ", "
              << (mt_file_ptr->end_all_tags() - 1)->sample_time << " in secs after midnight local time\n";
...
void
RigidObject3DMotionFromPolaris::set_mask_for_tags(const unsigned int mask_for_tags) {
...
RigidObject3DMotionFromPolaris::synchronise() {
  ...
  try {
    shared_ptr<CListModeData> lm_data_ptr(read_from_file<CListModeData>(list_mode_filename));

    this->listmode_data_start_time_in_secs = lm_data_ptr->get_scan_start_time_in_secs_since_1970();
  ...
  {
    // TODO warning->info
    warning("Looking for synchronisation file: Assuming that listmode corresponds to  \"%s\".", this->list_mode_filename.c_str());

    const string sync_filename = this->list_mode_filename + "_" + get_filename(this->mt_filename) + ".sync";
    ...
    if (sync_file) {
      if (parser.parse(sync_file) == false || sync_version != current_sync_version ||
          this->time_offset == time_not_yet_determined) {
      ...
      // TODO warning-<info
      warning("Could not open synchronisation file %s  for reading.\nSynchronising...", sync_filename.c_str());

      shared_ptr<CListModeData> lm_data_ptr(read_from_file<CListModeData>(this->list_mode_filename));
  ...
  if (this->listmode_data_start_time_in_secs != std::time_t(-1)) {
    ...
    cout << "\nListmode file says that listmode start time is " << lm_start_time << " in secs after midnight local time" << endl;
    ...
    this->listmode_data_start_time_in_secs =
        mt_file_ptr->get_start_time_in_secs_since_1970() + round(time_offset - mt_file_ptr->begin_all_tags()->sample_time);
...
double
RigidObject3DMotionFromPolaris::secs_since_1970_to_rel_time(std::time_t secs) const {
  ...
  if (secs > this->listmode_data_start_time_in_secs)
    return static_cast<double>(secs - this->listmode_data_start_time_in_secs);
  else
    return -static_cast<double>(this->listmode_data_start_time_in_secs - secs);
}

void
find_and_store_gate_tag_values_from_lm(VectorWithOffset<unsigned long>& lm_time, VectorWithOffset<unsigned>& lm_random_number,
...
bool
RigidObject3DMotionFromPolaris::is_synchronised() const {
...
RigidObject3DMotionFromPolaris::set_defaults() {
  ...
  this->move_from_scanner_coords = RigidObject3DTransformation(Quaternion<float>(1.F, 0.F, 0.F, 0.F), invalid_translation);
...
RigidObject3DMotionFromPolaris::initialise_keymap() {
  ...
  parser.add_key("transformation_from_scanner_coordinates_filename", &transformation_from_scanner_coordinates_filename);
  parser.add_key("maximum time_drift deviation", &max_time_drift_deviation);
  parser.add_key("maximum time offset deviation", &max_time_offset_deviation);
  parser.add_key("mask for tags", &this->_mask_for_tags);
  parser.add_stop_key("End Rigid Object 3D Motion From Polaris");
}

bool
RigidObject3DMotionFromPolaris::post_processing() {
...
#else
    {
      ...
      if (conventions != "q0qzqyqx and left-handed") {
        warning("Error reading transformation_from_scanner_coordinates_filename:\n'%s'"
                ...
                "\nbut is '%s'", transformation_from_scanner_coordinates_filename.c_str(), conventions.c_str());
        return true;
      }
...
#  endif
        ...
        warning("Error reading transformation_from_scanner_coordinates_filename:\n'%s'"
                ...
                "\ntransformation read:\n%s",
                transformation_from_scanner_coordinates_filename.c_str(), s.str().c_str());
        return true;
      }
    }
#endif
  ...
  if (norm(this->move_from_scanner_coords.get_translation() - invalid_translation) < 1) {
    warning(
        "transformation from scanner coordinates invalid. \"transformation_from_scanner_coordinates_filename\" keyword not set?");
    return true;
  }
...
Succeeded
RigidObject3DMotionFromPolaris::set_mt_file(const string& mt_filename_v) {
...
Succeeded
RigidObject3DMotionFromPolaris::set_list_mode_data_file(const string& lm_filename) {

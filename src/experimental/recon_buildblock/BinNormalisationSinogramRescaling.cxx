...
const char* const BinNormalisationSinogramRescaling::registered_name = "Sinogram Rescaling";
...
void
BinNormalisationSinogramRescaling::initialise_keymap() {
...
bool
BinNormalisationSinogramRescaling::post_processing() {
...
BinNormalisationSinogramRescaling::BinNormalisationSinogramRescaling() { set_defaults(); }

BinNormalisationSinogramRescaling::BinNormalisationSinogramRescaling(const string& filename)
    : sinogram_rescaling_factors_filename(filename) {}

Succeeded
BinNormalisationSinogramRescaling::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_sptr_v) {
  ...
  bool something_wrong = rescaling_factors.get_max_index() != max_segment_num;
  ...
  for (int segment_num = min_segment_num; !something_wrong && segment_num <= max_segment_num; ++segment_num) {
    ...
    something_wrong = something_wrong || rescaling_factors[segment_num].get_max_index() != max_axial_pos_num;
...
float
BinNormalisationSinogramRescaling::get_bin_efficiency(const Bin& bin, const double /*start_time*/,
                                                      const double /*end_time*/) const {
  return rescaling_factors[bin.segment_num()][bin.axial_pos_num()][bin.view_num()];
}

void
BinNormalisationSinogramRescaling::apply(RelatedViewgrams<float>& viewgrams, const double start_time,
                                         const double end_time) const {
...
void
BinNormalisationSinogramRescaling::undo(RelatedViewgrams<float>& viewgrams, const double start_time,
                                        const double end_time) const {
  for (RelatedViewgrams<float>::iterator iter = viewgrams.begin(); iter != viewgrams.end(); ++iter)

...
const char* const ProjMatrixByBinWithPositronRange::registered_name = "With Positron Range";

ProjMatrixByBinWithPositronRange::ProjMatrixByBinWithPositronRange() { set_defaults(); }
...
ProjMatrixByBinWithPositronRange::initialise_keymap() {
  ...
  parser.add_parsing_key("matrix type to use after positron range blurring", &post_projmatrix_ptr);
...
void
ProjMatrixByBinWithPositronRange::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_ptr_v,
  ...
  const CartesianCoordinate3D<float> zoomed_voxel_size = voxel_size / zoom;
  const CartesianCoordinate3D<int> zoomed_max_index = max_index * zoom + (zoom - 1) / 2 + (positron_range_num_sample - 1) / 2;
  const CartesianCoordinate3D<int> zoomed_min_index = min_index * zoom - (zoom - 1) / 2 - (positron_range_num_sample - 1) / 2;

  shared_ptr<DiscretisedDensity<3, float>> zoomed_density_info_ptr =
      new VoxelsOnCartesianGrid<float>(IndexRange<3>(zoomed_min_index, zoomed_max_index), origin, zoomed_voxel_size);
  ...
  symmetries_ptr = new DataSymmetriesForBins_PET_CartesianGrid(proj_data_info_ptr, density_info_ptr);
};

//////////////////////////////////////

void
ProjMatrixByBinWithPositronRange::calculate_proj_matrix_elems_for_one_bin(ProjMatrixElemsForOneBin& lor) const {
  ...
  for (ProjMatrixElemsForOneBin::const_iterator iter = zoomed_lor.begin(); iter != zoomed_lor.end(); ++iter) {

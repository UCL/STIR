...
const char* const ProjMatrixByBinUsingSolidAngle::registered_name = "Solid Angle";

ProjMatrixByBinUsingSolidAngle::ProjMatrixByBinUsingSolidAngle() { set_defaults(); }
...
void
ProjMatrixByBinUsingSolidAngle::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_ptr_v,
  ...
  symmetries_ptr.reset(new DataSymmetriesForBins_PET_CartesianGrid(proj_data_info_ptr, density_info_ptr));
};
...
inline float
trapezoid_integral(const float s, const float m2, const float m3) {
...
// 0/0 when m2? == m3?
inline float
convolution_2_trapezoids(const float x, const float m21, const float m31, const float m22, const float m32) {
  return (cube(fabs(m21 - m22 - x)) + cube(fabs(m21 + m22 - x)) - cube(fabs(m22 - m31 - x)) - cube(fabs(m22 + m31 - x)) -
          cube(fabs(m21 - m32 - x)) + cube(fabs(m31 - m32 - x)) - cube(fabs(m21 + m32 - x)) + cube(fabs(m31 + m32 - x)) +
          cube(fabs(m21 - m22 + x)) + cube(fabs(m21 + m22 + x)) - cube(fabs(m22 - m31 + x)) - cube(fabs(m22 + m31 + x)) -
          cube(fabs(m21 - m32 + x)) + cube(fabs(m31 - m32 + x)) - cube(fabs(m21 + m32 + x)) + cube(fabs(m31 + m32 + x))) /
         (12 * (m21 - m31) * (m22 - m32));
}
...
inline float
VOI(const float s_voxel, const float half_bin_size, const float halfcosminsin, const float halfcosplussin) {
  return trapezoid_integral(half_bin_size - s_voxel, halfcosminsin, halfcosplussin) -
         trapezoid_integral(-half_bin_size - s_voxel, halfcosminsin, halfcosplussin);
}

inline float
SA_rotated_voxel(const float s_voxel, const float half_voxel_size, const float m2, const float m3) {
  return trapezoid_integral(half_voxel_size + s_voxel, m2, m3) - trapezoid_integral(-half_voxel_size + s_voxel, m2, m3);
}

inline float
SAapprox(const float s_voxel, const float half_voxel_size, const float m2, const float m3, const float halfcosminsin,
         const float halfcosplussin) {
  ...
  return
      ...
      SA_rotated_voxel(s_voxel-halfcosplussin/2, half_voxel_size, m2, m3);
      */
      SA_rotated_voxel(s_voxel + halfcosminsin / 2, .5, m2, m3) + SA_rotated_voxel(s_voxel - halfcosminsin / 2, .5, m2, m3) +
      SA_rotated_voxel(s_voxel + halfcosplussin / 2, .5, m2, m3) + SA_rotated_voxel(s_voxel - halfcosplussin / 2, .5, m2, m3);
}

inline float
SA(const float s_voxel, const float half_voxel_size, const float m2, const float m3, const float halfcosminsin,
   const float halfcosplussin) {
  return halfcosminsin == halfcosplussin ? SA_rotated_voxel(s_voxel, .5, m2, m3)
                                         : convolution_2_trapezoids(s_voxel, m2, m3, halfcosminsin, halfcosplussin);
}

void
ProjMatrixByBinUsingSolidAngle::calculate_proj_matrix_elems_for_one_bin(ProjMatrixElemsForOneBin& lor) const {
  ...
  const float fovrad_in_mm =
      min((min(max_index.x(), -min_index.x()) - 1) * voxel_size.x(), (min(max_index.y(), -min_index.y()) - 1) * voxel_size.y());
  ...
  assert(fabs(sampling_distance_of_adjacent_LORs_z - num_lors_per_axial_pos * voxel_size.z()) <= 1E-4);
  ...
  const float offset_in_z =
      (num_lors_per_axial_pos == 1 || tantheta == 0 ? 0.F : -sampling_distance_of_adjacent_LORs_z / 4) - origin.z();
  ...
  const float Z1f =
      (t_in_mm / costheta - TMP2f * tantheta + offset_in_z) / voxel_size.z() + (max_index.z() + min_index.z()) / 2.F;
  // const float Z2f =
  ...
  const float half_tube_length =
      sqrt(square(proj_data_info_ptr->get_scanner_ptr()->get_effective_ring_radius()) - square(s_in_mm)) / voxel_size.x();
  {
    ...
    do {
      ...
      while (s_voxel > min_s) {
        const float Pbv =
            ...
            SA(s_voxel, half_bin_size, m2, m3, halfcosminsin, halfcosplussin) * height_of_trapezoid;
        // assert(Pbv>0);

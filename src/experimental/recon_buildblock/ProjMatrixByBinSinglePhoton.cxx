...
const char* const ProjMatrixByBinSinglePhoton::registered_name = "Single Photon";
...
void
ProjMatrixByBinSinglePhoton::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_ptr_v,
...
void
ProjMatrixByBinSinglePhoton::calculate_proj_matrix_elems_for_one_bin(ProjMatrixElemsForOneBin& lor) const {
  const Bin bin = lor.get_bin();

  assert(lor.size() == 0);

  vector<DetectionPositionPair<>> det_pos_pairs;
  static_cast<ProjDataInfoCylindricalNoArcCorr&>(*proj_data_info_ptr).get_all_det_pos_pairs_for_bin(det_pos_pairs, bin);
  for (std::vector<DetectionPositionPair<>>::const_iterator det_pos_pair_iter = det_pos_pairs.begin();
    ...
    lor.push_back(
        ProjMatrixElemsForOneBin::value_type(Coordinate3D<int>(0, det_pos_pair_iter->pos1().axial_coord() + min_index[2],
                                                               det_pos_pair_iter->pos1().tangential_coord() + min_index[3]),
                                             1));
    lor.push_back(
        ProjMatrixElemsForOneBin::value_type(Coordinate3D<int>(0, det_pos_pair_iter->pos2().axial_coord() + min_index[2],
                                                               det_pos_pair_iter->pos2().tangential_coord() + min_index[3]),
                                             1));
  }
}

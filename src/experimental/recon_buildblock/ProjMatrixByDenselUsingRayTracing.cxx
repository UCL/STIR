...
ProjMatrixByDenselUsingRayTracing::ProjMatrixByDenselUsingRayTracing()
{
  set_defaults();
}
...
static inline float
projection_of_voxel_help(const float xctr,
                         const float yctr,
                         const float zctr,
                         const float xhalfsize,
                         const float yhalfsize,
                         const float zhalfsize,
                         const float ctheta,
...
static inline float
projection2D_of_voxel_help(const float xctr,
                           const float yctr,
                           const float zctr,
                           const float xhalfsize,
                           const float yhalfsize,
                    ...
                    const float s)
{
  ...
  // assert(sphi>0);
  return fabs(tantheta) < 1.E-4 ? projection2D_of_voxel_help(densel_ctr.x(),
                                                             densel_ctr.y(),
                                                             densel_ctr.z(),
                                                             sign(cphi) * xhalfsize,
                                                             sign(sphi) * yhalfsize,
                                                             ...
                                                             m,
                                                             s)
                                : projection_of_voxel_help(densel_ctr.x(),
                                                           densel_ctr.y(),
                                                           densel_ctr.z(),
                                                           sign(cphi) * xhalfsize,
                                                           sign(sphi) * yhalfsize,
                                                           sign(tantheta) * zhalfsize,
                                                           ctheta,
                                                           tantheta,
                                                           cphi,
                                                           sphi,
  ...
  if (num_tangential_LORs == 1)
  ...
  else
    {
      ...
      for (int s_LOR_num = 1; s_LOR_num <= num_tangential_LORs; ++s_LOR_num, current_s_in_mm += s_inc)
        {
          const float current_res = projection_of_voxel(
              densel_ctr, xhalfsize, yhalfsize, zhalfsize, costheta, tantheta, cphi, sphi, m, current_s_in_mm);
          if (current_res > 0)
            {
              found_non_zero = true;
              res += current_res;

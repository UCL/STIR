...
const char* const ProjMatrixByDenselUsingRayTracing::registered_name = "Ray Tracing";

ProjMatrixByDenselUsingRayTracing::ProjMatrixByDenselUsingRayTracing() { set_defaults(); }
...
void
ProjMatrixByDenselUsingRayTracing::set_up(
  ...
  symmetries_ptr.reset(new DataSymmetriesForDensels_PET_CartesianGrid(proj_data_info_ptr, density_info_ptr));
...
static inline float
projection_of_voxel_help(const float xctr, const float yctr, const float zctr, const float xhalfsize, const float yhalfsize,
                         const float zhalfsize, const float ctheta, const float tantheta, float cphi, float sphi, const float m,
                         const float s) {
  ...
  return max((-max((zs - zhalfsize) / tantheta, max((ys - yhalfsize) / sphi, (xs - xhalfsize) / cphi)) +
              min((zs + zhalfsize) / tantheta, min((ys + yhalfsize) / sphi, (xs + xhalfsize) / cphi))) /
                 ctheta,
             0.F);
}
...
static inline float
projection2D_of_voxel_help(const float xctr, const float yctr, const float zctr, const float xhalfsize, const float yhalfsize,
                           const float zhalfsize, float cphi, float sphi, const float m, const float s) {
  const float epsilon = 1.E-4F;
  ...
  const float ys = yctr - s * cphi;
  const float xs = xctr + s * sphi;
  return max(-max((ys - yhalfsize) / sphi, (xs - xhalfsize) / cphi) + min((ys + yhalfsize) / sphi, (xs + xhalfsize) / cphi), 0.F);
}

static inline float
projection_of_voxel(const CartesianCoordinate3D<float>& densel_ctr, const float xhalfsize, const float yhalfsize,
                    const float zhalfsize, const float ctheta, const float tantheta, const float cphi, const float sphi,
                    const float m, const float s) {
  // if you want to relax the next assertion, replace yhalfsize with sign(sphi)*yhalfsize below
  // assert(sphi>0);
  return fabs(tantheta) < 1.E-4
             ? projection2D_of_voxel_help(densel_ctr.x(), densel_ctr.y(), densel_ctr.z(), sign(cphi) * xhalfsize,
                                          sign(sphi) * yhalfsize, zhalfsize, cphi, sphi, m, s)
             : projection_of_voxel_help(densel_ctr.x(), densel_ctr.y(), densel_ctr.z(), sign(cphi) * xhalfsize,
                                        sign(sphi) * yhalfsize, sign(tantheta) * zhalfsize, ctheta, tantheta, cphi, sphi, m, s);
}
...
float
ProjMatrixByDenselUsingRayTracing::get_element(const Bin& bin, const CartesianCoordinate3D<float>& densel_ctr) const {
...
file.
  ...
  */
  ...
  if (num_tangential_LORs == 1) {
    res = projection_of_voxel(densel_ctr, xhalfsize, yhalfsize, zhalfsize, costheta, tantheta, cphi, sphi, m, s_in_mm);
  } else {
    ...
    for (int s_LOR_num = 1; s_LOR_num <= num_tangential_LORs; ++s_LOR_num, current_s_in_mm += s_inc) {
      const float current_res =
          projection_of_voxel(densel_ctr, xhalfsize, yhalfsize, zhalfsize, costheta, tantheta, cphi, sphi, m, current_s_in_mm);
      if (current_res > 0) {
        found_non_zero = true;
        res += current_res;

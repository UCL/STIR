...
*/
DataSymmetriesForDensels_PET_CartesianGrid::DataSymmetriesForDensels_PET_CartesianGrid(
    const shared_ptr<const ProjDataInfo>& proj_data_info_ptr_v, const shared_ptr<DiscretisedDensity<3, float>>& image_info_ptr)
...
bool
DataSymmetriesForDensels_PET_CartesianGrid::operator==(const DataSymmetriesForDensels_PET_CartesianGrid& that) const {
  if (!base_type::operator==(that))
    return false;

  return *this->proj_data_info_ptr == *that.proj_data_info_ptr && this->num_planes == that.num_planes &&
         this->num_independent_planes == that.num_independent_planes && this->num_views == that.num_views &&
...
bool
DataSymmetriesForDensels_PET_CartesianGrid::blindly_equals(const root_type* const sym_ptr) const {
  assert(dynamic_cast<const self_type* const>(sym_ptr) != 0);
  return this->operator==(static_cast<const self_type&>(*sym_ptr));
}

...
const char* const BinNormalisationUsingProfile::registered_name = "Using Profile";

void
BinNormalisationUsingProfile::set_defaults() {
  profile_filename = "";
}

void
BinNormalisationUsingProfile::initialise_keymap() {
...
bool
BinNormalisationUsingProfile::post_processing() {
#if 0
...
BinNormalisationUsingProfile::BinNormalisationUsingProfile() { set_defaults(); }

BinNormalisationUsingProfile::BinNormalisationUsingProfile(const string& filename) : profile_filename(filename) {
  if (post_processing() == true)
    error("Exiting\n");
}

void
BinNormalisationUsingProfile::apply(RelatedViewgrams<float>& viewgrams, const double start_time, const double end_time) const {
  RelatedViewgrams<float>::iterator viewgrams_iter = viewgrams.begin();
  for (; viewgrams_iter != viewgrams.end(); ++viewgrams_iter) {
    for (int ax_pos_num = viewgrams_iter->get_min_index(); ax_pos_num <= viewgrams_iter->get_max_index(); ++ax_pos_num) {
      for (int i = std::max(profile.get_min_index(), viewgrams.get_min_tangential_pos_num());
           i <= std::min(profile.get_max_index(), viewgrams.get_max_tangential_pos_num()); ++i)
...
void
BinNormalisationUsingProfile::undo(RelatedViewgrams<float>& viewgrams, const double start_time, const double end_time) const {
  /*

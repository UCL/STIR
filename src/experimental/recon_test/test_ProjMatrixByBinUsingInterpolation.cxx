...
ProjMatrixByBinUsingInterpolationTests::
ProjMatrixByBinUsingInterpolationTests(char const * template_proj_data_filename)
				...
				const Bin& bin)
{
  ...
  for (vector<Bin>::const_iterator bin_iter = related_bins.begin();
      ...
      if (!check(elems_no_sym == elems_with_sym, "comparing lors"))
	{
	  ...
	  cerr << "no sym ("<<elems_no_sym.size() 
	       << ") with sym (" << elems_with_sym.size() 
	       << ")\n";

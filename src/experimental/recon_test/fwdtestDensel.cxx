  ...
  switch (ask_num("Start image is cuboid (1) or cylinder (2) or on file (3)", 1, 3, 2))
    {
      case 1: {
        ...
        image_sptr = vox_image_ptr = new VoxelsOnCartesianGrid<float>(*(proj_data_ptr->get_proj_data_info_sptr()),
                                                                      zoom,
          ...
          for (int x = min_x; x <= max_x; ++x)
            {
              ...
              for (ProjMatrixElemsForOneDensel::const_iterator element_ptr = probs.begin(); element_ptr != probs.end();
                   ++element_ptr)
                {
...
fill_cylinder(VoxelsOnCartesianGrid<float>& image)
{
  ...
  const double zc = ask_num(
      "Centre Z coordinate", (double)image.get_min_z(), (double)image.get_max_z(), (image.get_min_z() + image.get_max_z()) / 2.);
  ...
  const double Lcyl = ask_num(
      "Length (planes)", 1., (image.get_max_z() - image.get_min_z()) + 1., (image.get_max_z() - image.get_min_z()) + 1.);

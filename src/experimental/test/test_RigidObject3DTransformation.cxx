...
RigidObject3DTransformationTests::run_tests() {
  ...
  std::cerr << "\tTesting inverse and check if it's a rigid transformation\n";
  {
    ...
    const CartesianCoordinate3D<float> transformed_origin = ro3dtrans.transform_point(CartesianCoordinate3D<float>(0, 0, 0));
    for (int i = 0; i < 100; ++i) {
      ...
      // Testing to see if inverse gets us back
      {
        ...
        check_if_zero(norm(point - transformed_back_point) / norm(point), "test on inverse transformation of transformed point");
      }
    }
  }
  ...
  std::cerr << "\n\tTesting compose " << std::endl;
  {
    ...
    compose_timer.reset();
#endif
    const RigidObject3DTransformation composed_ro3dtrans1 = compose(ro3dtrans_3, compose(ro3dtrans_2, ro3dtrans_1));
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      const CartesianCoordinate3D<float> transformed_point_3 =
          ro3dtrans_3.transform_point(ro3dtrans_2.transform_point(ro3dtrans_1.transform_point(point)));
#ifdef DO_TIMINGS
      timer.stop();

      compose_timer.start();
#endif
      const CartesianCoordinate3D<float> transformed_point_composed = composed_ro3dtrans1.transform_point(point);
#ifdef DO_TIMINGS
      compose_timer.stop();
#endif
      check_if_zero(norm(transformed_point_3 - transformed_point_composed) / norm(transformed_point_3), "test on compose");
    }
#ifdef DO_TIMINGS
...
void
RigidObject3DTransformationTests::test_transform_bin() {
  ...
  shared_ptr<ProjDataInfo> proj_data_info_sptr =
      ProjDataInfo::ProjDataInfoCTI(scanner_ptr,
                                    ...
                                    /*arc_corrected*/ false);
  ProjDataInfoCylindricalNoArcCorr& proj_data_info = dynamic_cast<ProjDataInfoCylindricalNoArcCorr&>(*proj_data_info_sptr);
  ...
  proj_data_info_sptr = ProjDataInfo::ProjDataInfoCTI(scanner_ptr,
...
swap_direction(const Bin& bin, const int num_views) {
  return Bin(-bin.segment_num(), bin.view_num() < num_views / 2 ? bin.view_num() + num_views : bin.view_num() - num_views,
...
bin_diff_no_reorder(const Bin& org_bin, const Bin& transformed_bin, const ProjDataInfo& proj_data_info) {
  ...
  diff[1] = (org_bin.segment_num() - transformed_bin.segment_num());
  diff[2] = (org_bin.view_num() - transformed_bin.view_num());
  diff[3] = (proj_data_info.get_m(org_bin) - proj_data_info.get_m(transformed_bin)) / proj_data_info.get_sampling_in_m(org_bin);
  diff[4] = (proj_data_info.get_s(org_bin) - proj_data_info.get_s(transformed_bin)) / proj_data_info.get_sampling_in_s(org_bin);
  return diff;
}

static float
sup_norm(const Coordinate4D<float>& c) {
  return std::max(std::fabs(c[1]), std::max(std::fabs(c[2]), std::max(std::fabs(c[3]), std::fabs(c[4]))));
}
...
bin_diff(const Bin& org_bin, const Bin& transformed_bin, const ProjDataInfo& proj_data_info) {
  ...
  const Coordinate4D<float> diff2 =
      bin_diff_no_reorder(org_bin, swap_direction(transformed_bin, proj_data_info.get_num_views()), proj_data_info);
  return sup_norm(diff1) < sup_norm(diff2) ? diff1 : diff2;
}
#endif

void
RigidObject3DTransformationTests::test_transform_bin_with_inverse(const ProjDataInfo& proj_data_info,
...
#ifdef NEW_ROT
  const float allowed_diff = is_uncompressed ? 1.1F : 1.1F;
#else
  const float allowed_diff = 2.1F;
#endif

  for (int segment_num = proj_data_info.get_min_segment_num(); segment_num <= proj_data_info.get_max_segment_num();
       ++segment_num) {
    for (int view_num = proj_data_info.get_min_view_num(); view_num <= proj_data_info.get_max_view_num(); view_num += 5) {
      ...
      for (int axial_pos_num = proj_data_info.get_min_axial_pos_num(segment_num) + 1;
           axial_pos_num <= proj_data_info.get_max_axial_pos_num(segment_num) - 1; axial_pos_num += 3) {
        for (int tangential_pos_num = proj_data_info.get_min_tangential_pos_num() + 1;
             tangential_pos_num <= proj_data_info.get_max_tangential_pos_num() - 1; tangential_pos_num += 17) {
          ++num_bins_checked;
          ...
          if (transformed_bin.get_bin_value() > 0) // only check when the transformed_bin is within the range
          {
            ...
            if (!check(org_bin.get_bin_value() == transformed_bin.get_bin_value(), "transform_bin_with_inverse: value") ||
              ...
              std::cerr << "\tProblem at  segment=" << org_bin.segment_num() << ", axial pos=" << org_bin.axial_pos_num()
                        << ", view=" << org_bin.view_num() << ", tangential_pos=" << org_bin.tangential_pos_num() << '\n';
              std::cerr << "\tround-trip to  segment=" << transformed_bin.segment_num()
                        << ", axial pos=" << transformed_bin.axial_pos_num() << ", view= " << transformed_bin.view_num()
                        << ", tangential_pos=" << transformed_bin.tangential_pos_num()
                        << " value=" << transformed_bin.get_bin_value() << " sup_norm(diff)=" << sup_norm(diff) << '\n';
            }
          }
        } // tangential_pos
      }   // axial_pos
...
void
RigidObject3DTransformationTests::test_transform_bin_vs_transform_point(const shared_ptr<ProjDataInfo>& proj_data_info_sptr) {
  std::cerr << "\n\t\ttesting consistency transform_point and transform_bin\n";

  shared_ptr<DiscretisedDensity<3, float>> density_sptr = new VoxelsOnCartesianGrid<float>(*proj_data_info_sptr);
  const CartesianCoordinate3D<float> origin = density_sptr->get_origin();
  ...
  double max_deviation_org = 0;
  {
    ...
    const CartesianCoordinate3D<float> densel_coord = BasicCoordinate<3, float>(densel) * voxel_size + origin;
    const CartesianCoordinate3D<float> transformed_densel_coord = ro3dtrans.transform_point(densel_coord);
    const CartesianCoordinate3D<float> transformed_densel_float = (transformed_densel_coord - origin) / voxel_size;
    const CartesianCoordinate3D<float> image_centre =
        CartesianCoordinate3D<float>((density_sptr->get_min_index() + density_sptr->get_max_index()) / 2.F, 0, 0) * voxel_size;
    ...
    const double ring_radius = static_cast<double>(proj_data_info_sptr->get_scanner_ptr()->get_effective_ring_radius());
    for (ProjMatrixElemsForOneDensel::const_iterator bin_iter = bins.begin(); bin_iter != bins.end(); ++bin_iter) {
      {
        ...
        const float deviation_org =
            distance_between_line_and_point(lor2pts, CartesianCoordinate3D<float>(densel_coord - image_centre));
      ...
      if (transformed_bin.get_bin_value() > 0) {
...

        {
          ...
          const float deviation_transformed =
              distance_between_line_and_point(lor2pts, CartesianCoordinate3D<float>(transformed_densel_coord - image_centre));
    ...
    check(num_contributing_bins > 30, "num_contributing_bins should be large enough for the back-projection test to make sense");
    std::cerr << "\t\tnum_contributing_bins " << num_contributing_bins << " out of " << bins.end() - bins.begin() << '\n';
    const CartesianCoordinate3D<int> densel_from_bins = indices_at_maximum(*density_sptr);

    const double deviation = norm(BasicCoordinate<3, float>(densel_from_bins) - transformed_densel_float);
    ...
    if (!check(deviation < allowed_deviation, "deviation determined via backprojection")) {
      ...
      (*density_sptr)[round(transformed_densel_float)] = density_sptr->find_max() * 2;
      OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file("STIRImage", *density_sptr);
...
void
RigidObject3DTransformationTests::test_find_closest_transformation() {
  ...
  for (std::vector<CartesianCoordinate3D<float>>::const_iterator iter = points.begin(); iter != points.end(); ++iter)
  ...
  if (!check(RigidObject3DTransformation::find_closest_transformation(result, points.begin(), points.end(),
                                                                      transformed_points.begin(),
                                                                      Quaternion<float>(1, 0, 0, 0)) == Succeeded::yes,

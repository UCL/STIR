...
void
RigidObject3DTransformationTests::test_transform_bin_with_inverse(const ProjDataInfo& proj_data_info, const bool is_uncompressed)
{
                      ...
                      if (!check(org_bin.get_bin_value() == transformed_bin.get_bin_value(), "transform_bin_with_inverse: value")
                          || !check(sup_norm(diff) <= allowed_diff, "transform_bin_with_inverse: different bin"))
                        {
                          std::cerr << "\tProblem at  segment=" << org_bin.segment_num()
                                    << ", axial pos=" << org_bin.axial_pos_num() << ", view=" << org_bin.view_num()
        ...
        if (transformed_bin.get_bin_value() > 0)
          {
...

            {
              ...
              lorNAC.get_intersections_with_cylinder(lor2pts, ring_radius);
              const float deviation_transformed = distance_between_line_and_point(
                  lor2pts, CartesianCoordinate3D<float>(transformed_densel_coord - image_centre));
...
RigidObject3DTransformationTests::test_find_closest_transformation()
{
  ...
  if (!check(RigidObject3DTransformation::find_closest_transformation(
                 result, points.begin(), points.end(), transformed_points.begin(), Quaternion<float>(1, 0, 0, 0))
                 == Succeeded::yes,
             "find_closest_transformation for exact match returned Succeeded::no"))
    return;
  if (!check_if_zero(norm(quat - result.get_quaternion()), "find_closest_transformation for exact match: non-matching quaternion")

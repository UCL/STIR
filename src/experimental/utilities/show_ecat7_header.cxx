...
void
dump_subheader(const Scan3D_subheader& scan3dsub) {
  /*
        short data_type;
...
#define STIR_DUMPIT(x) cout << "x = " << scan3dsub.x << '\n';
  ...
  for (int i = 0; i < 128; ++i)
    cout << "uncor_singles[" << i << "] = " << scan3dsub.uncor_singles[i] << '\n';
}

void
dump_subheader(MatrixFile* mptr, const int frame_num, const int plane_num, const int gate_num, const int data_num,
               const int bed_num) {
  int matnum;
...
main(int argc, char* argv[]) {
  ...
  MatrixFile* mptr = matrix_open(cti_name.c_str(), MAT_READ_ONLY, MAT_UNKNOWN_FTYPE);
  if (!mptr) {
    ...
    for (int frame_num = 1; frame_num <= num_frames; ++frame_num)
      for (int bed_num = 0; bed_num < num_bed_poss; ++bed_num)
        for (int gate_num = 1; gate_num <= num_gates; ++gate_num)
          dump_subheader(mptr, frame_num, 1, gate_num, data_num, bed_num);
  } else {

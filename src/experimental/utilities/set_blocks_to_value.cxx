        ...
        const int ring = ax_crystal + ax_det_offset;
        {
          for (int other_det = 0; other_det < num_detectors_per_ring; ++other_det)
            for (int other_ring = max(0, ring + min_ring_diff); other_ring <= min(num_rings - 1, ring + max_ring_diff);
                 ++other_ring)
              {
                // first check for impossible coincidence (because it cannot be handled by get_bin_for_det_ps_pair)
                if (det == other_det)
                  continue;
...
main(int argc, char** argv)
{
  ...
  ProjDataInfoPETScannerWithDiscreteDetectors* proj_data_info_ptr
        ...
        for (; bin_iter != list_of_bins.end(); ++bin_iter)
          {
            ...
            if (ax_pos_num <= max_ax_pos_num && ax_pos_num >= min_ax_pos_num && tang_pos_num <= max_tang_pos_num

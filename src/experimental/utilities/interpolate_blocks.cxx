          ...
          for (int other_det = 0; other_det < num_detectors_per_ring; ++other_det)
            for (int other_ring = max(0, ring + min_ring_diff); other_ring <= min(num_rings - 1, ring + max_ring_diff);
                 ++other_ring)
              {
                ...
                const DetectionPositionPair<> det_pos_pair(DetectionPosition<>(det, ring),
                                                           DetectionPosition<>(other_det, other_ring));
                const Succeeded success = proj_data_info.get_bin_for_det_pos_pair(bin, det_pos_pair);
  ...
  do
    {
      ...
      do_block(list_of_bins,
               axial_block_num,
               tangential_block_num,
               *proj_data_info_ptr,
               axial_num_crystals_in_block,
  ...
  for (int segment_num = out_projdata.get_min_segment_num(); segment_num <= out_projdata.get_max_segment_num(); ++segment_num)
	...
	cerr << "s "<< segment_num 
	     ...
	     << std::endl;
            ...
            for (int tang_pos_num = first_tang_pos_num; tang_pos_num <= last_tang_pos_num;

...
main(int argc, char** argv) {
  if (argc < 3 || argc > 7) {
    cerr << "Usage: " << argv[0]
         << " <output image filename> <input image filename> [xy-spacing [xy-size][z-spacing [z-size] ] ] ]\n"
  ...
  // write image
  Succeeded res = OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(output_filename, *density_sptr);

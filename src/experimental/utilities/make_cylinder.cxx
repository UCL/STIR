...
main(int argc, char** argv)
{
  ...
  shared_ptr<DiscretisedDensity<3, float>> template_density_ptr = DiscretisedDensity<3, float>::read_from_file(argv[2]);

  VoxelsOnCartesianGrid<float>* vox_template_image_ptr = dynamic_cast<VoxelsOnCartesianGrid<float>*>(template_density_ptr.get());
  ...
  */
  ...
  VoxelsOnCartesianGrid<float> image(
      IndexRange3D(min_plane, max_plane, min_y_dir, max_y_dir, min_x_dir, max_x_dir), origin, voxel_size);
  ...
  dir_y /= norm(dir_y);
  CartesianCoordinate3D<float> dir_x(-velocity_x * velocity_z, -velocity_x * velocity_y, square(velocity_y) + square(velocity_z));
  dir_x /= norm(dir_x);
  ...
  const float origin_z = ask_num("z of centre of cylinder (w.r.t.z of centre of image)", -centre_z, centre_z, 0.F);

  const CartesianCoordinate3D<float> cyl_origin(
      origin_z + centre_z, shift_y + origin_z * velocity_y, shift_x + origin_z * velocity_x);

  EllipsoidalCylinder cylinder(length, radius, radius, cyl_origin, dir_x, dir_y, dir_z);

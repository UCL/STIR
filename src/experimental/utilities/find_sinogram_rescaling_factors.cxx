...
main(int argc, char* argv[]) {
  ...
  if (*numerator_sptr->get_proj_data_info_sptr() != *denominator_sptr->get_proj_data_info_sptr()) {
  ...
  Array<3, float> scaling_factors;
  {
    ...
    for (int segment_num = numerator_sptr->get_min_segment_num(); segment_num <= numerator_sptr->get_max_segment_num();
         ++segment_num) {
      const int min_axial_pos_num = numerator_sptr->get_min_axial_pos_num(segment_num);
      const int max_axial_pos_num = numerator_sptr->get_max_axial_pos_num(segment_num);
      ...
      for (int axial_pos_num = numerator_sptr->get_min_axial_pos_num(segment_num);
           axial_pos_num <= numerator_sptr->get_max_axial_pos_num(segment_num); ++axial_pos_num) {
        scaling_factor_index_range[segment_num][axial_pos_num] =
            IndexRange<1>(numerator_sptr->get_min_view_num(), numerator_sptr->get_max_view_num());
      }
    }
  ...
  for (int segment_num = numerator_sptr->get_min_segment_num(); segment_num <= numerator_sptr->get_max_segment_num();
    ...
    for (int axial_pos = numerator_sptr->get_min_axial_pos_num(segment_num);
         axial_pos <= numerator_sptr->get_max_axial_pos_num(segment_num); ++axial_pos) {
      ...
      for (int view_num = numerator_sinogram.get_min_view_num(); view_num <= numerator_sinogram.get_max_view_num(); view_num++) {
        float numerator_sum = 0;
        float denominator_sum = 0;

        for (int tang_pos = numerator_sinogram.get_min_tangential_pos_num();
             tang_pos <= numerator_sinogram.get_max_tangential_pos_num(); tang_pos++) {
          numerator_sum += numerator_sinogram[view_num][tang_pos];
          denominator_sum += denominator_sinogram[view_num][tang_pos];
        }

        scaling_factors[segment_num][axial_pos][view_num] = numerator_sum / denominator_sum;
      }
    }
  }

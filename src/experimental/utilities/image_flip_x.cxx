...
main(int argc, char* argv[]) {
  ...
  for (DiscretisedDensity<3, float>::iterator z_iter = density_ptr->begin(); z_iter != density_ptr->end(); ++z_iter) {
    for (Array<2, float>::iterator y_iter = z_iter->begin(); y_iter != z_iter->end(); ++y_iter) {
      for (int left = y_iter->get_min_index(), right = y_iter->get_max_index(); left < right; ++left, --right)
        std::swap((*y_iter)[left], (*y_iter)[right]);
    }
  }

  Succeeded success =
      OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(output_filename, *density_ptr);

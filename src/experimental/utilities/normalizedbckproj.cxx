...
main(int argc, char* argv[]) {
  ...
  bool projector_type = ask(" Which projector do you want to use Ray Tracing (Y) or Solid Angle (N) ", true);

  const ProjDataInfo* proj_data_info_ptr = proj_data_ptr->get_proj_data_info_sptr();

  VoxelsOnCartesianGrid<float>* vox_image_ptr = new VoxelsOnCartesianGrid<float>(*proj_data_info_ptr);
  ...
  shared_ptr<BackProjectorByBin> bck_projector_ptr(new BackProjectorByBinUsingSquareProjMatrixByBin(PM));

  {
    const int max_segment_num = ask_num("Maximum absolute segment number to backproject", 0,
                                        proj_data_info_ptr->get_max_segment_num(), proj_data_info_ptr->get_max_segment_num());
    ...
    do_segments(*image_sptr, *proj_data_ptr, -max_segment_num, max_segment_num, proj_data_info_ptr->get_min_view_num(),
                proj_data_info_ptr->get_max_view_num(), *bck_projector_ptr, false);

    if (do_denominator) {
      shared_ptr<DiscretisedDensity<3, float>> denominator_ptr(image_sptr->get_empty_discretised_density());
      // set to non-zero value to avoid problems with division outside the FOV
      denominator_ptr->fill(image_sptr->find_max() * 1.E-10F);
      do_segments(*denominator_ptr, *proj_data_ptr, -max_segment_num, max_segment_num, proj_data_info_ptr->get_min_view_num(),
                  proj_data_info_ptr->get_max_view_num(), *bck_projector_ptr, true);
      *image_sptr /= *denominator_ptr;
    }
    if (do_sqrt) {
      // in_place_apply_function(*image_sptr, &sqrt);
      for (DiscretisedDensity<3, float>::full_iterator iter = image_sptr->begin_all(); iter != image_sptr->end_all(); ++iter)
        *iter = sqrt(*iter);
    }

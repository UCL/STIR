...
main(int argc, char** argv) {
  ...
  const int num_rings = measured_data->get_proj_data_info_sptr()->get_scanner_ptr()->get_num_rings();
  const int num_detectors_per_ring = measured_data->get_proj_data_info_sptr()->get_scanner_ptr()->get_num_detectors_per_ring();
  ...
  BlockData3D norm_block_data(num_axial_blocks, num_tangential_blocks, num_axial_blocks - 1, num_tangential_blocks - 1);
    ...
    // efficiencies
    {
      for (int ra = 0; ra < num_rings; ++ra)
        for (int a = 0; a < num_detectors_per_ring; ++a)
          efficiencies[ra][a] =
              static_cast<float>((2 + sin(2 * _PI * a / num_detectors_per_ring)) * exp(noise * ((2.F * rand()) / RAND_MAX - 1)));
      {
        ...
        for (int a = norm_block_data.get_min_a(); a <= norm_block_data.get_max_a(); ++a)
          ...
          for (int rb = std::max(ra, norm_block_data.get_min_rb(ra)); rb <= norm_block_data.get_max_rb(ra); ++rb)
            for (int b = norm_block_data.get_min_b(a); b <= norm_block_data.get_max_b(a); ++b) {
              norm_block_data(ra, a, rb, b) = exp(noise * ((1.F * rand()) / RAND_MAX - 0.5F));

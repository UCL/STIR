...
PatlakPlot::create_model_matrix() {
  if (_matrix_is_stored == false) {
    ...
    for (sample_num = 1; sample_num < this->_starting_frame; ++sample_num, ++cur_iter)
      sum_value +=
          cur_iter->get_plasma_counts_in_kBq() * this->_plasma_frame_data.get_time_frame_definitions().get_duration(sample_num);
    ...
    for (sample_num = this->_starting_frame; cur_iter != this->_plasma_frame_data.end(); ++sample_num, ++cur_iter) {
      ...
      if (this->_plasma_frame_data.get_is_decay_corrected()) {
        ...
        time_vector[sample_num] =
            static_cast<float>(0.5 * (this->_frame_defs.get_end_time(sample_num) + this->_frame_defs.get_start_time(sample_num)));
      }
    }
    if (this->_plasma_frame_data.get_is_decay_corrected())
...
void
PatlakPlot::apply_linear_regression(ParametricVoxelsOnCartesianGrid& par_image,
  ...
  if (!this->_in_correct_scale) {
    ...
    if (dyn_image.get_scanner_default_bin_size() <= 0)
      error("PatlakPlot: The dynamic image currently needs to know the Scanner's default_bin_size. Did you set the 'originating "
            "system'?");
    ...
    for (int k = min_k_index; k <= max_k_index; ++k) {
      ...
      for (int j = min_j_index; j <= max_j_index; ++j) {
        ...
        for (int i = min_i_index; i <= max_i_index; ++i) {
          ...
          linear_regression(y_intersection, slope, chi_square, variance_of_y_intersection, variance_of_slope,
                            covariance_of_y_intersection_with_slope, patlak_y, patlak_x, weights);

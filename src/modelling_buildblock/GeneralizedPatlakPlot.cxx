...
const char * const 
GeneralizedPatlakPlot::registered_name = "Generalized Patlak Plot";
...
ModelMatrix<2>
GeneralizedPatlakPlot::
get_initialization_model_matrix() const 
{    
...
void GeneralizedPatlakPlot::set_model_matrix(GeneralizedPatlakMatrix<2> model_matrix)
{
...
void GeneralizedPatlakPlot::set_initialization_model_matrix(ModelMatrix<2> initialization_model_matrix)
{
...
void GeneralizedPatlakPlot::set_Hfunction_matrix(GeneralizedPatlakMatrix<2> Hfunction_matrix)
{
...
GeneralizedPatlakMatrix<2>
GeneralizedPatlakPlot::
get_Hfunction_matrix() const 
{    
  return _Hfunction_matrix ; 
}

//! Simply set Ki matrix 
void GeneralizedPatlakPlot::set_Ki_matrix(GeneralizedPatlakMatrix<2> Ki_matrix)
{
...
GeneralizedPatlakMatrix<2>
GeneralizedPatlakPlot::
get_model_matrix(const PlasmaData& complete_plasma_data, const PlasmaData& plasma_frame_data,const TimeFrameDefinitions& time_frame_definitions,const unsigned int starting_frame)
{    
  ...
  if(_matrix_is_stored==false)
    {  
	  ...
	  float last_frame_time=floor(0.5*(time_frame_definitions.get_end_time(num_frames)+time_frame_definitions.get_start_time(num_frames)));
      ...
      max_range[1]=num_conv_params; max_range[2]=num_frames;
	  ...
	  for(frame_num=this->_starting_frame ; cur_iter!=this->_plasma_frame_data.end() ; ++frame_num, ++cur_iter )
		...
		for(conv_sample=1, actual_time_point=1 ; actual_time_point<=last_frame_mid_time ; ++conv_sample)
          ...
          if (actual_time_point<=cur_frame_mid_time)			  
		    patlak_array[conv_sample][frame_num]= complete_plasma_cur_iter->get_plasma_counts_in_kBq()*this->_conv_sample_interval;
		  else
		...
		if(this->_plasma_frame_data.get_if_decay_corrected())
		    ...
		    for(conv_sample=1, actual_time_point=1 ; actual_time_point<=last_frame_mid_time ; ++conv_sample)
              ...
              if (actual_time_point<=cur_frame_mid_time)
                ...
                plasma_sample_dec_fact[conv_sample]=
			       static_cast<float>(decay_correction_factor(this->_complete_plasma_data.get_isotope_halflife(),
			                                                  complete_plasma_cur_iter->get_time_in_s()));
			  ...
			  dec_fact[frame_num]=
			    static_cast<float>(decay_correction_factor(this->_plasma_frame_data.get_isotope_halflife(),this->_plasma_frame_data.get_time_frame_definitions().get_start_time(frame_num),
			  ...
			  time_vector[frame_num]= static_cast<float>(0.5*(this->_frame_defs.get_end_time(frame_num)+this->_frame_defs.get_start_time(frame_num)));
			}		
		}
		std::cout << endl << endl;
...
//! Create initialization (standard Patlak) model matrix from plasma data (has to be in appropriate frames: i.e. plasma_frame_data)
ModelMatrix<2>
GeneralizedPatlakPlot::
get_initialization_model_matrix(const PlasmaData& plasma_frame_data,const TimeFrameDefinitions& time_frame_definitions,const unsigned int starting_frame)
{    
  assert(starting_frame>0);
  ...
  if(_matrix_is_stored==false)
    {
      ...
      max_range[1]=2;  max_range[2]=plasma_frame_data.size();
      IndexRange<2> data_range(min_range,max_range);
      Array<2,float> patlak_array(data_range);
      VectorWithOffset<float> time_vector(min_range[2],max_range[2]);
      ...
      for(sample_num=1 ; sample_num<starting_frame; ++sample_num, ++cur_iter )
        {
          sum_value+=cur_iter->get_plasma_counts_in_kBq()*plasma_frame_data.get_time_frame_definitions().get_duration(sample_num);
        }
      ...
      for(sample_num=starting_frame ; cur_iter!=plasma_frame_data.end() ; ++sample_num, ++cur_iter )
        {
         integral_step=cur_iter->get_plasma_counts_in_kBq()*plasma_frame_data.get_time_frame_definitions().get_duration(sample_num);
         ...
         if(plasma_frame_data.get_if_decay_corrected())
           {
             const float dec_fact=
                static_cast<float>(decay_correction_factor(plasma_frame_data.get_isotope_halflife(),plasma_frame_data.get_time_frame_definitions().get_start_time(sample_num),
...
void
GeneralizedPatlakPlot::
create_Hfunction_matrix()
{  
  BasicCoordinate<2,int> min_range;
  BasicCoordinate<2,int> max_range;
  min_range[1]=1;  min_range[2]=1;
  max_range[1]=2;  max_range[2]=this->_kloss_num_samples;
  ...
  if(_matrix_is_stored==false)
    {
      ...
      BasicCoordinate<2,int> max_range;
      min_range[1]=1;  min_range[2]=this->_starting_frame;
      max_range[1]=this->_num_conv_params;  max_range[2]=this->_num_frames;
      IndexRange<2> data_range(min_range,max_range);
      Array<2,float> patlak_array(data_range);
      VectorWithOffset<float> time_vector(min_range[2],max_range[2]);
	  VectorWithOffset<float> plasma_sample_dec_fact(min_range[1],max_range[1]);
...
void
GeneralizedPatlakPlot::
create_initialization_model_matrix()
{    
  if(_initialization_matrix_is_stored==false)
    {
      BasicCoordinate<2,int> min_range;
...
void
GeneralizedPatlakPlot::multiply_dynamic_image_with_model_gradient_and_add_to_input(DynamicDiscretisedDensity & impulse_response,
																				   const DynamicDiscretisedDensity & dyn_image) const
{
  if (!this->_in_correct_scale)
    {
															      ...
															      const GeneralizedPatlakVoxelsOnCartesianGrid & par_image) const
{
  ...
  this->_model_matrix.synthesize_impulse_response_from_parametric_image(impulse_response_image,par_image,this->_num_conv_params); 
}
...
void 
GeneralizedPatlakPlot::get_generalized_patlak_parameters_from_impulse_response(GeneralizedPatlakVoxelsOnCartesianGrid & par_image,
																			   ...
																			   const DynamicDiscretisedDensity & impulse_response) const
{
  ...
  this->_model_matrix.estimate_generalized_patlak_parameters_with_impulse_response(par_image,impulse_response,this->_num_conv_params); 
}


void
GeneralizedPatlakPlot::multiply_dynamic_image_with_initialization_model_gradient(GeneralizedPatlakVoxelsOnCartesianGrid & par_image,
                                                                                 const DynamicDiscretisedDensity & dyn_image) const
{
  if (!this->_in_correct_scale)
    {
...
void
GeneralizedPatlakPlot::multiply_dynamic_image_with_initialization_model_gradient_and_add_to_input(GeneralizedPatlakVoxelsOnCartesianGrid & par_image,
                                                                                                  const DynamicDiscretisedDensity & dyn_image) const
{
  if (!this->_in_correct_scale)
    {
...
// Should be a virtual function declared in the KineticModels or better to the LinearModels
void
GeneralizedPatlakPlot::get_dynamic_image_from_initialization_parametric_image(DynamicDiscretisedDensity & dyn_image,
                                                                              const GeneralizedPatlakVoxelsOnCartesianGrid & par_image) const
{
  if (!this->_in_correct_scale)
    {
...
#endif //NDEBUG
      const DiscretisedDensityOnCartesianGrid <3,float>*  image_cartesian_ptr = 
        dynamic_cast< DiscretisedDensityOnCartesianGrid<3,float>*  > (((dynamic_image_nested_loop_estimate.get_densities())[0]).get());
      const BasicCoordinate<3,float> this_grid_spacing = image_cartesian_ptr->get_grid_spacing();
      this->_model_matrix.scale_model_matrix(this_grid_spacing[2]/dynamic_image_nested_loop_estimate.get_scanner_default_bin_size());
#ifndef NDEBUG
      this->_model_matrix.write_to_file("patlak_matrix_in_correct_scale.txt",this->_num_conv_params);
#endif //NDEBUG
    }
...
post_processing()
{
  ...
  if(this->_blood_data_filename=="0")
  ...
  else
    {
	   ...
	   float _last_frame_time=floor(0.5*(this->_frame_defs.get_end_time(this->_num_frames)+this->_frame_defs.get_start_time(this->_num_frames)));

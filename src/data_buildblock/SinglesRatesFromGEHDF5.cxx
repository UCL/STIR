...
const char* const SinglesRatesFromGEHDF5::registered_name = "Singles From GE HDF5 listmode File";
...
FrameSinglesRates
SinglesRatesFromGEHDF5::get_rates_for_frame(double start_time, double end_time) const {
  ...
  FrameSinglesRates frame_rates(average_singles_rates, frame_start_time, frame_end_time, SinglesRates::scanner_sptr);
...
int
SinglesRatesFromGEHDF5::get_end_time_slice_index(double t) const {
  ...
  if (_times[slice_index] < t) {
    ...
    // Check forwards.
    while (slice_index < m_num_time_slices - 1 && _times[slice_index] < t) {
      slice_index++;
    }
...
int
SinglesRatesFromGEHDF5::get_start_time_slice_index(double t) const {
...
int
SinglesRatesFromGEHDF5::get_singles_rate(int singles_bin_index, int time_slice) const {
...
void
SinglesRatesFromGEHDF5::set_singles_rate(int singles_bin_index, int time_slice, int new_rate) {
...
unsigned int
SinglesRatesFromGEHDF5::rebin(std::vector<double>& new_end_times) {
  ...
  Array<2, unsigned int> new_singles = Array<2, unsigned int>(IndexRange2D(0, num_new_slices - 1, 0, total_singles_units - 1));
...
unsigned int
SinglesRatesFromGEHDF5::get_num_time_slices() const {
  return (m_num_time_slices);
}

double
SinglesRatesFromGEHDF5::get_singles_time_interval() const {
  return (_singles_time_interval);
}

unsigned int
SinglesRatesFromGEHDF5::read_singles_from_listmode_file(const std::string& _listmode_filename) {
...
float
SinglesRatesFromGEHDF5::get_singles_rate(const int singles_bin_index, const double start_time, const double end_time) const {
...
float
SinglesRatesFromGEHDF5::get_singles_rate(const DetectionPosition<>& det_pos, const double start_time,
...
void
SinglesRatesFromGEHDF5::set_time_interval() {
...
double
SinglesRatesFromGEHDF5::get_slice_start(int slice_index) const {

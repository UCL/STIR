...
const unsigned SinglesRatesFromSglFile::SIZE_OF_SINGLES_RECORD = 4 * 128;

const char* const SinglesRatesFromSglFile::registered_name = "Singles From Sgl File";
...
// Constructor
SinglesRatesFromSglFile::SinglesRatesFromSglFile() {}

// Generate a FramesSinglesRate - containing the average rates
// for a frame begining at start_time and ending at end_time.
FrameSinglesRates
SinglesRatesFromSglFile::get_rates_for_frame(double start_time, double end_time) const {
...
int
SinglesRatesFromSglFile::get_end_time_slice_index(double t) const {
...
int
SinglesRatesFromSglFile::get_start_time_slice_index(double t) const {
...
int
SinglesRatesFromSglFile::get_singles_rate(int singles_bin_index, int time_slice) const {
...
void
SinglesRatesFromSglFile::set_singles_rate(int singles_bin_index, int time_slice, int new_rate) {
...
int
SinglesRatesFromSglFile::rebin(std::vector<double>& new_end_times) {
  ...
  for (unsigned int new_slice = 0; new_slice < new_end_times.size(); ++new_slice) {
    ...
    if (start_time > _times[_num_time_slices - 1]) {
      ...
      for (int singles_bin = 0; singles_bin < total_singles_units; ++singles_bin) {
        new_singles[new_slice][singles_bin] = round(get_singles_rate(singles_bin, start_time, end_time));
      }
    }
...
int
SinglesRatesFromSglFile::get_num_time_slices() const {
  return (_num_time_slices);
}

double
SinglesRatesFromSglFile::get_singles_time_interval() const {
  return (_singles_time_interval);
}

int
SinglesRatesFromSglFile::read_singles_from_sgl_file(const std::string& sgl_filename) {
  ...
  _num_time_slices = static_cast<int>((end_stream_position - static_cast<streampos>(512)) / SIZE_OF_SINGLES_RECORD);
  ...
  while (singles_file && slice < _num_time_slices) {
...

    {
      ...
      if (!singles_file) {
        ...
        if (!singles_file.eof()) {
          warning("Error reading singles file record %d. Stopped reading from this point.", slice);
        }
    ...
    if (singles_str.num_sgl != total_singles_units) {
      error("Number of singles units should be %d, but is %d in singles file", total_singles_units, singles_str.num_sgl);
    }
...
float
SinglesRatesFromSglFile::get_singles_rate(const int singles_bin_index, const double start_time, const double end_time) const {
...
void
SinglesRatesFromSglFile::set_time_interval() {
...
double
SinglesRatesFromSglFile::get_slice_start(int slice_index) const {
...
void
SinglesRatesFromSglFile::initialise_keymap() {
...
bool
SinglesRatesFromSglFile::post_processing() {

...
const char* const SinglesRatesFromECAT7::registered_name = "Singles From ECAT7";

SinglesRatesFromECAT7::SinglesRatesFromECAT7() {}

int
SinglesRatesFromECAT7::read_singles_from_file(const std::string& ECAT7_filename, const std::ios::openmode open_mode)

{
  ...
  if (total_singles_units > 0) {
    ...
    this->_singles = Array<2, float>(IndexRange2D(1, main_header->num_frames, 0, total_singles_units - 1));
  }
  ...
  for (int mat_frame = 1; mat_frame <= num_frames; mat_frame++) {
    ...
    Scan3D_subheader* scan_subheader_ptr = reinterpret_cast<Scan3D_subheader*>(matrix->shptr);
    time_frames[mat_frame - 1].first = scan_subheader_ptr->frame_start_time / 1000.;
    time_frames[mat_frame - 1].second = time_frames[mat_frame - 1].first + scan_subheader_ptr->frame_duration / 1000.;
...
void
SinglesRatesFromECAT7::initialise_keymap() {
...
bool
SinglesRatesFromECAT7::post_processing() {

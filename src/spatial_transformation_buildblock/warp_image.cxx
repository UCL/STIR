...
warp_image(const shared_ptr<DiscretisedDensity<3, float>>& density_sptr,
           ...
           const shared_ptr<DiscretisedDensity<3, float>>& motion_z_sptr, const BSpline::BSplineType spline_type,
  ...
  for (c[1] = min[1]; c[1] <= max[1]; ++c[1])
    for (c[2] = min[2]; c[2] <= max[2]; ++c[2])
      for (c[3] = min[3]; c[3] <= max[3]; ++c[3]) {
        ...
        if ((d[1] <= static_cast<double>(min[1])) ||
            (d[1] >= static_cast<double>(max[1])) || // I'm not considering the last plane if linear
            (d[2] <= static_cast<double>(min[2])) ||
            (d[2] >= static_cast<double>(max[2])) || // because it's going to use extrapolated data

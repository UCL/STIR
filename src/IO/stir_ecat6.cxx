...
/*
  ...
  */
static void read_sinogram(Sinogram<float>& sino_2D, char* buffer, FILE* fptr, int mat_index, int frame, int gate, int data,
                          int bed);
...
void
make_ECAT6_Main_header(ECAT6_Main_header& mhead, Scanner const& scanner, const std::string& orig_name,
...
ECAT6_to_VoxelsOnCartesianGrid(const int frame_num, const int gate_num, const int data_num, const int bed_num, FILE* cti_fptr,
  ...
  for (int z = 0; z < mhead.num_planes; z++) { // loop upon planes
...
#endif // STIR_ORIGINAL_ECAT6
    {
      if (norm(image_ptr->get_origin() - sub_head_origin) > .01F) {
        warning(
            "ECAT6_to_VoxelsOnCartesianGrid: x,y offset in subheader of plane %d does not agree with plane 1. Ignoring it...\n",
            z + 1);
      }
    }
...
void
ECAT6_to_PDFS(const int frame_num, const int gate_num, const int data_num, const int bed_num, int max_ring_diff,
              bool arccorrected, const std::string& data_name, FILE* cti_fptr, const ECAT6_Main_header& mhead) {
  ...
  // write to proj_data
  {
    ...
    if (is_3D_file) {
      for (int w = 0; w <= max_ring_diff; w++) { // loop on segment number
...
Succeeded
DiscretisedDensity_to_ECAT6(FILE* fptr, DiscretisedDensity<3, float> const& density, const ECAT6_Main_header& mhead,
...
Succeeded
DiscretisedDensity_to_ECAT6(DiscretisedDensity<3, float> const& density, std::string const& cti_name,
                            std::string const& orig_name, const Scanner& scanner, const int frame_num, const int gate_num,
...
Succeeded
ProjData_to_ECAT6(FILE* fptr, ProjData const& proj_data, const ECAT6_Main_header& mhead, const int frame_num, const int gate_num,
                  const int data_num, const int bed_num, const bool write_2D_sinograms) {
  ...
  for (int segment_num = min_segment_num; segment_num <= max_segment_num; ++segment_num) {
    ...
    for (int z = 0; z < num_axial_poss; z++) { // loop on planes
      ...
      const int indexcod = write_2D_sinograms ? z + 1 : cti_rings2plane(num_rings, ring1, ring2); // change indexation into CTI
...
Succeeded
ProjData_to_ECAT6(ProjData const& proj_data, std::string const& cti_name, std::string const& orig_name, const int frame_num,
                  const int gate_num, const int data_num, const int bed_num, const bool write_2D_sinograms) {
  ECAT6_Main_header mhead;

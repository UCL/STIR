...
create_image_and_header_from(InterfileImageHeader& hdr,
  ...
  CartesianCoordinate3D<float> voxel_size(static_cast<float>(hdr.pixel_sizes[2]), static_cast<float>(hdr.pixel_sizes[1]),
  ...
  if (hdr.first_pixel_offsets[2] != InterfileHeader::double_value_not_set) {
    ...
    origin =
        make_coordinate(float(hdr.first_pixel_offsets[2]), float(hdr.first_pixel_offsets[1]), float(hdr.first_pixel_offsets[0])) -
...
Succeeded
write_basic_interfile_image_header(const string& header_file_name, const string& image_file_name, const ExamInfo& exam_info,
                                   const IndexRange<3>& index_range, const CartesianCoordinate3D<float>& voxel_size,
  ...
  if (!exam_info.get_radionuclide().empty())
    output_header << "isotope name := " << exam_info.get_radionuclide() << endl;
...
#endif
Succeeded
write_basic_interfile(const string& filename, const Array<3, elemT>& image, const NumericType output_type, const float scale,
...
Succeeded
write_basic_interfile(const string& filename, const ExamInfo& exam_info, const Array<3, NUMBER>& image,
                      const CartesianCoordinate3D<float>& voxel_size, const CartesianCoordinate3D<float>& origin,
                      const NumericType output_type, const float scale, const ByteOrder byte_order) {
  ...
  const Succeeded success =
      write_basic_interfile_image_header(header_name, data_name, exam_info, image.get_index_range(), voxel_size, origin,
                                         output_type, byte_order, scaling_factors, file_offsets);
#if 0
    delete[] header_name;
    delete[] data_name;
#endif
...
template <class NUMBER>
Succeeded
write_basic_interfile(const string& filename, const Array<3, NUMBER>& image, const CartesianCoordinate3D<float>& voxel_size,
                      const CartesianCoordinate3D<float>& origin, const NumericType output_type, const float scale,
                      const ByteOrder byte_order) {
...
Succeeded
write_basic_interfile(const string& filename, const VoxelsOnCartesianGrid<float>& image, const NumericType output_type,
                      const float scale, const ByteOrder byte_order) {
  return write_basic_interfile(filename, image.get_exam_info(),
                               image, // use automatic reference to base class
                               image.get_grid_spacing(), image.get_origin(), output_type, scale, byte_order);
}

Succeeded
write_basic_interfile(const string& filename, const DiscretisedDensity<3, float>& image, const NumericType output_type,
                      const float scale, const ByteOrder byte_order) {
  // dynamic_cast will throw an exception when it's not valid
  return write_basic_interfile(filename, dynamic_cast<const VoxelsOnCartesianGrid<float>&>(image), output_type, scale,
                               byte_order);
}

Succeeded
write_basic_interfile(const string& filename, const ParametricVoxelsOnCartesianGrid& image, const NumericType output_type,
                      const float scale, const ByteOrder byte_order) {
  ...
  const Succeeded success = write_basic_interfile_image_header(
      header_name, data_name, image.get_exam_info(), image.get_index_range(), image.get_voxel_size(), image.get_origin(),
      output_type, byte_order, scaling_factors, file_offsets, data_type_descriptions);
#if 0
    delete[] header_name;
    delete[] data_name;
#endif
...
Succeeded
write_basic_interfile(const string& filename, const DynamicDiscretisedDensity& image, const NumericType output_type,
                      const float scale, const ByteOrder byte_order) {
  ...
  const Succeeded success = write_basic_interfile_image_header(
      header_name, data_name, image.get_exam_info(), image.get_density(1).get_index_range(),
      dynamic_cast<const VoxelsOnCartesianGrid<float>&>(image.get_density(1)).get_grid_spacing(),
      image.get_density(1).get_origin(), output_type, byte_order, scaling_factors, file_offsets);
#if 0
    delete[] header_name;
    delete[] data_name;
#endif
...
read_interfile_PDFS_SPECT(istream& input, const string& directory_for_data, const ios::openmode open_mode) {
  ...
  return new ProjDataFromStream(hdr.get_exam_info_sptr(), hdr.data_info_sptr, data_in, hdr.data_offset_each_dataset[0],
                                segment_sequence, hdr.storage_order, hdr.type_of_numbers, hdr.file_byte_order,
...
read_interfile_PDFS_Siemens(istream& input, const string& directory_for_data, const ios::openmode open_mode) {
  ...
  return new ProjDataFromStream(hdr.get_exam_info_sptr(), hdr.data_info_ptr->create_shared_clone(), data_in,
                                hdr.data_offset_each_dataset[0], hdr.segment_sequence, hdr.storage_order, hdr.type_of_numbers,
                                hdr.file_byte_order, 1.);
}
...
read_interfile_PDFS(istream& input, const string& directory_for_data, const ios::openmode open_mode) {
  ...
  return new ProjDataFromStream(hdr.get_exam_info_sptr(), hdr.data_info_sptr->create_shared_clone(), data_in,
                                hdr.data_offset_each_dataset[0], hdr.segment_sequence, hdr.storage_order, hdr.type_of_numbers,
                                hdr.file_byte_order, static_cast<float>(hdr.image_scaling_factors[0][0]));
}
  ...
  if (!is_null_ptr(proj_data_info_sptr)) {
    ...
    if (fabs(proj_data_info_sptr->get_ring_radius() - scanner.get_effective_ring_radius()) > .1)
      warning("write_basic_interfile_PDFS_header: inconsistent effective ring radius:\n"
              ...
              proj_data_info_sptr->get_ring_radius(), scanner.get_effective_ring_radius());
    if (fabs(proj_data_info_sptr->get_ring_spacing() - scanner.get_ring_spacing()) > .1)
      warning("write_basic_interfile_PDFS_header: inconsistent ring spacing:\n"
              ...
              "\tThis really should not happen and signifies a bug.\n"
              "\tYou will have a problem reading this data back in.",
              proj_data_info_sptr->get_ring_spacing(), scanner.get_ring_spacing());
...
template Succeeded write_basic_interfile<>(const string& filename, const Array<3, signed short>&,
                                           const CartesianCoordinate3D<float>& voxel_size,
                                           const CartesianCoordinate3D<float>& origin, const NumericType output_type,
                                           const float scale, const ByteOrder byte_order);
template Succeeded write_basic_interfile<>(const string& filename, const Array<3, unsigned short>&,
                                           const CartesianCoordinate3D<float>& voxel_size,
                                           const CartesianCoordinate3D<float>& origin, const NumericType output_type,
                                           const float scale, const ByteOrder byte_order);

template Succeeded write_basic_interfile<>(const string& filename, const Array<3, float>&,
                                           const CartesianCoordinate3D<float>& voxel_size,
                                           const CartesianCoordinate3D<float>& origin, const NumericType output_type,
                                           const float scale, const ByteOrder byte_order);
...
template Succeeded write_basic_interfile<>(const string& filename, const Array<3, signed short>& image,
                                           const NumericType output_type, const float scale, const ByteOrder byte_order);

template Succeeded write_basic_interfile<>(const string& filename, const Array<3, unsigned short>& image,
                                           const NumericType output_type, const float scale, const ByteOrder byte_order);

template Succeeded write_basic_interfile<>(const string& filename, const Array<3, float>& image, const NumericType output_type,
                                           const float scale, const ByteOrder byte_order);
#endif

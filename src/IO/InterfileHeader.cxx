...
MinimalInterfileHeader::MinimalInterfileHeader() : KeyParser() {
  ...
  add_key("imaging modality", KeyArgument::ASCII, (KeywordProcessor)&MinimalInterfileHeader::set_imaging_modality,
          &imaging_modality_as_string);

  add_key("version of keys", KeyArgument::ASCII, (KeywordProcessor)&MinimalInterfileHeader::set_version_specific_keys,
          &version_of_keys);
...
InterfileHeader::InterfileHeader() : MinimalInterfileHeader() {
  ...
  add_key("type of data", KeyArgument::ASCIIlist, (KeywordProcessor)&InterfileHeader::set_type_of_data, &type_of_data_index,
          &type_of_data_values);
  ...
  add_key("number of energy windows", KeyArgument::INT, (KeywordProcessor)&InterfileHeader::read_num_energy_windows,
          &num_energy_windows);
...
/***********************************************************************/
InterfileImageHeader::InterfileImageHeader() : InterfileHeader() {
  ...
  add_key("number of image data types", KeyArgument::INT, (KeywordProcessor)&InterfileImageHeader::read_image_data_types,
...
InterfilePDFSHeader::InterfilePDFSHeader() : InterfileHeader() {
  ...
  add_key("minimum ring difference per segment", KeyArgument::LIST_OF_INTS,
          (KeywordProcessor)&InterfilePDFSHeader::resize_segments_and_set, &min_ring_difference);
  add_key("maximum ring difference per segment", KeyArgument::LIST_OF_INTS,
          (KeywordProcessor)&InterfilePDFSHeader::resize_segments_and_set, &max_ring_difference);
  ...
  add_key("Number of TOF time bins", &max_num_timing_poss);
  size_of_timing_pos = -1.f;
  add_key("Size of timing bin (ps)", &size_of_timing_pos);
  timing_resolution = -1.f;
  add_key("Timing resolution (ps)", &timing_resolution);
...
class compare_first : public binary_function<T1, T1, bool> {
public:
...
void
find_segment_sequence(vector<int>& segment_sequence, VectorWithOffset<int>& sorted_num_rings_per_segment,
                      VectorWithOffset<int>& sorted_min_ring_diff, VectorWithOffset<int>& sorted_max_ring_diff,
                      vector<int>& num_rings_per_segment, const vector<int>& min_ring_difference,
...
InterfilePDFSHeader::post_processing() {
  ...
  if (applied_corrections.size() == 0) {
    ...
    for (
        ...
        iter != applied_corrections.end(); ++iter) {
  ...
  find_segment_sequence(segment_sequence, sorted_num_rings_per_segment, sorted_min_ring_diff, sorted_max_ring_diff,
                        num_rings_per_segment, min_ring_difference, max_ring_difference);
#if 0  
...
#  endif
  ...
  if (!originating_system_was_recognised) {
    ...
    case 336 * 2:
      ...
      break;
    case 288 * 2:
      if (num_rings == 104) { // added by Dylan Togane
  ...
  if (guessed_scanner_ptr->get_type() != Scanner::Unknown_scanner &&
    ...
    if (max_num_non_arccorrected_bins - guessed_scanner_ptr->get_max_num_non_arccorrected_bins()) {
      warning("Interfile warning: 'max_num_non_arccorrected_bins' (%d) is expected to be %d", max_num_non_arccorrected_bins,
    ...
    if (default_num_arccorrected_bins - guessed_scanner_ptr->get_default_num_arccorrected_bins()) {
      warning("Interfile warning: 'default_num_arccorrected_bins' (%d) is expected to be %d", default_num_arccorrected_bins,
    ...
    if (guessed_scanner_ptr->is_tof_ready()) {
      if (max_num_timing_poss != guessed_scanner_ptr->get_max_num_timing_poss()) {
        warning("Interfile warning: 'Number of TOF time bins' (%d) is expected to be %d.", max_num_timing_poss,
                guessed_scanner_ptr->get_max_num_timing_poss());
        mismatch_between_header_and_guess = true;
      }
      if (size_of_timing_pos != guessed_scanner_ptr->get_size_of_timing_pos()) {
        warning("Interfile warning: 'Size of timing bin (ps)' (%f) is expected to be %f.", size_of_timing_pos,
                guessed_scanner_ptr->get_size_of_timing_pos());
        mismatch_between_header_and_guess = true;
      }
      if (timing_resolution != guessed_scanner_ptr->get_timing_resolution()) {
        warning("Interfile warning: 'Timing resolution (ps)' (%f) is expected to be %f.", timing_resolution,
  ...
  scanner_sptr_from_file.reset(new Scanner(
      ...
      num_detector_layers, energy_resolution, reference_energy, max_num_timing_poss, size_of_timing_pos, timing_resolution));
  ...
  if (is_arccorrected) {
    ...
    data_info_sptr.reset(new ProjDataInfoCylindricalArcCorr(scanner_sptr_from_file, float(effective_central_bin_size_in_cm * 10.),
                                                            sorted_num_rings_per_segment, sorted_min_ring_diff,
                                                            sorted_max_ring_diff, num_views, num_bins, tof_mash_factor));
  } else {
    data_info_sptr.reset(new ProjDataInfoCylindricalNoArcCorr(scanner_sptr_from_file, sorted_num_rings_per_segment,

...
bool GEHDF5Wrapper::check_GE_signature(const std::string& filename)
{
    ...
    try
    {
        return (check_current_signature(file));
    }
    catch(...)
    {
        return false;
...
// AB Cheks if its a GE Scanner file, and returns if it is a GE scanner or not.
bool GEHDF5Wrapper::check_current_signature(H5::H5File& file)
{
    if(file.getId() == -1)
...
GEHDF5Wrapper::check_file()
{
    ...
    if(!check_current_signature(file))
...
Succeeded GEHDF5Wrapper::initialise_exam_info()
{
    ...
    std::vector<std::pair<double, double> >tf{{frameStartTime,frameStartTime+frameDuration/1000}};
...
// Initializes data for reading projections. Sets up reading addresses and inspect sizes. 
// view_num is 1-based indexing
// in RDF9, this fucntion has to be called for each projection/view_num

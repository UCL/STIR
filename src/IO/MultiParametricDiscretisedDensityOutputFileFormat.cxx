...
TEMPLATE
const char* const ParamDiscDensityOutputFileFormat::registered_name = "Multi";

TEMPLATE
ParamDiscDensityOutputFileFormat::MultiParametricDiscretisedDensityOutputFileFormat(const NumericType& type,
...
TEMPLATE
void
ParamDiscDensityOutputFileFormat::set_defaults()
{
...
TEMPLATE
void
ParamDiscDensityOutputFileFormat::initialise_keymap()
{
...
TEMPLATE
bool
ParamDiscDensityOutputFileFormat::post_processing()
{
...
TEMPLATE
ByteOrder
ParamDiscDensityOutputFileFormat::set_byte_order(const ByteOrder& new_byte_order, const bool warn)
{
...
TEMPLATE
Succeeded
ParamDiscDensityOutputFileFormat::actual_write_to_file(std::string& filename,
                                                       const ParametricDiscretisedDensity<DiscDensityT>& density) const
{
  ...
  for (int i = 1; i <= int(density.get_num_params()); i++)
    {
      Succeeded success = this->individual_output_type_sptr->write_to_file(individual_filenames[i],
                                                                           density.construct_single_density(unsigned(i)));

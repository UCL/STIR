...
*/
bool
InterfileDynamicDiscretisedDensityInputFileFormat::actual_can_read(const FileSignature& signature, std::istream& input) const
{
...
InterfileDynamicDiscretisedDensityInputFileFormat::read_from_file(std::istream& input) const
{
...
std::auto_ptr<InterfileDynamicDiscretisedDensityInputFileFormat::data_type>
InterfileDynamicDiscretisedDensityInputFileFormat::read_from_file(const std::string& filename) const
{
  ...
  // read signature
  {
    ...
    if (!input)
      {
        error("InterfileDynamicDiscretisedDensityInputFileFormat::read_from_file: error opening file '%s'. Does it exist?",
              filename.c_str());
  ...
  if (is_interfile_signature(signature))
    {
#ifndef NDEBUG
      info(boost::format("InterfileDynamicDiscretisedDensityInputFileFormat::read_from_file trying to read %s as Interfile")
           % filename);
#endif
      ...
      if (!is_null_ptr(dyn_ptr))
        ...
        {
          error(boost::format("InterfileDynamicDiscretisedDensityInputFileFormat::read_from_file failed to read %s as Interfile")
                % filename);
          return std::auto_ptr<data_type>(0);
        }
    }
    ...
    {
      error(boost::format("InterfileDynamicDiscretisedDensityInputFileFormat::read_from_file could not identify an Interfile "
                          "signature for the file: %s \n")
            % filename);
      return std::auto_ptr<data_type>(0);
    }
}
...
std::auto_ptr<InterfileDynamicDiscretisedDensityInputFileFormat::data_type>
InterfileDynamicDiscretisedDensityInputFileFormat::read_interfile_dyn_image(std::istream& input,
                                                                            const string& directory_for_data,
                                                                            const std::ios::openmode open_mode) const
{

  info(boost::format(
           "InterfileDynamicDiscretisedDensityInputFileFormat:: Reading the Interfile image set located in directory\n%1%  ...\n")
  ...
  if (!hdr.parse(input))
    {
      error("InterfileDynamicDiscretisedDensityInputFileFormat::Failed to properly parse the Interfile header file of the "
  ...
  if (is_null_ptr(dyn_image_ptr))
    {
      error(boost::format(
                "InterfileDynamicDiscretisedDensityInputFileFormat: error allocating memory for new object (for file \"%1%\")")
            % full_data_file_name);
      return std::auto_ptr<data_type>(0);
    }
  ...
  for (unsigned int frame_num = 1; frame_num <= nframes; ++frame_num)
    {
      ...
      std::cout << "The setting of the buffer of image " << frame_num
                << " to its proper position in the dynamic buffer was successful\n";
          ...
          if (hdr.data_offset_each_dataset[frame_num] > 0)
            {
              ...
              data_offset = hdr.data_offset_each_dataset[frame_num]; // in that case the current offset is provided by the header
                                                                     // file and may change between different frames/images
            }
          else
            data_offset += data_offset_increment; // in that case (when current offset is <=0) we assume increment of offset is
                                                  // the same for all frames/images
        }
...
// local helper function to read concatenated dynamic image data from Interfile
std::auto_ptr<InterfileDynamicDiscretisedDensityInputFileFormat::data_type>
InterfileDynamicDiscretisedDensityInputFileFormat::read_interfile_dyn_image(const string& filename,
                                                                            const std::ios::openmode open_mode) const
{

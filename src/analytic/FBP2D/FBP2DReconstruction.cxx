...
FBP2DReconstruction::FBP2DReconstruction() { set_defaults(); }

FBP2DReconstruction::FBP2DReconstruction(const shared_ptr<ProjData>& proj_data_ptr_v, const double alpha_ramp_v,
                                         const double fc_ramp_v, const int pad_in_s_v, const int num_segments_to_combine_v) {
  set_defaults();
...
FBP2DReconstruction::actual_reconstruct(shared_ptr<DiscretisedDensity<3, float>> const& density_ptr) {
  ...
  if (!is_null_ptr(dynamic_pointer_cast<const ProjDataInfoCylindricalArcCorr>(proj_data_ptr->get_proj_data_info_sptr()))) {
    ...
    tangential_sampling =
        dynamic_cast<const ProjDataInfoCylindricalArcCorr&>(*proj_data_ptr->get_proj_data_info_sptr()).get_tangential_sampling();
  } else {
...
#endif
      { viewgrams = proj_data_ptr->get_related_viewgrams(vs_num, symmetries_sptr); }

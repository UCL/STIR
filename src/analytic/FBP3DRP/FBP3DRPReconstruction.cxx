...
static void
find_rmin_rmax(int& rmin, int& rmax, const ProjDataInfoCylindrical& proj_data_info_cyl, const int seg_num,
...
FBP3DRPReconstruction::FBP3DRPReconstruction(const std::string& parameter_filename) { initialise(parameter_filename); }

FBP3DRPReconstruction::FBP3DRPReconstruction() { set_defaults(); }
...
FBP3DRPReconstruction::actual_reconstruct(shared_ptr<DiscretisedDensity<3, float>> const& target_image_ptr) {
  ...
  if (max_segment_num_to_process < 0)
    ...
    warning("max_segment_num_to_process was too large (%d) for this data, setting it to %d", max_segment_num_to_process,
  ...
  colsher_filter = ColsherFilter(theta_max, static_cast<float>(alpha_colsher_axial), static_cast<float>(fc_colsher_axial),
                                 static_cast<float>(alpha_colsher_planar), static_cast<float>(fc_colsher_planar),
                                 colsher_stretch_factor_planar, colsher_stretch_factor_axial);
#else
  warning("Using NRFFT");
#endif
  ...
  // and initialise proj_data_info_with_missing_data_sptr accordingly
  {
    if (!is_null_ptr(dynamic_pointer_cast<const ProjDataInfoCylindricalArcCorr>(proj_data_ptr->get_proj_data_info_sptr()))) {
      ...
      proj_data_info_with_missing_data_sptr = arc_correction_sptr->get_arc_corrected_proj_data_info_sptr()->create_shared_clone();
    }
  }
  ...
  // make space for missing data
  {
    ...
    for (int segment_num = proj_data_info_with_missing_data_sptr->get_min_segment_num();
         segment_num <= proj_data_info_with_missing_data_sptr->get_max_segment_num(); ++segment_num) {
...
void
FBP3DRPReconstruction::do_2D_reconstruction() { // SSRB+2D FBP with ramp filter
...
FBP3DRPReconstruction::do_3D_Reconstruction(VoxelsOnCartesianGrid<float>& image) {
  ...
  for (int seg_num = -max_segment_num_to_process; seg_num <= max_segment_num_to_process; seg_num++) {
    ...
    for (int view_num = proj_data_ptr->get_min_view_num(); view_num <= proj_data_ptr->get_max_view_num(); ++view_num) {
      ...
      do_process_viewgrams(viewgrams, new_min_axial_pos_num, new_max_axial_pos_num, orig_min_axial_pos_num,
...
FBP3DRPReconstruction::do_best_fit(const Sinogram<float>& sino_measured, const Sinogram<float>& sino_calculated) {
  ...
  for (int view = sino_measured.get_min_view_num(); view <= sino_measured.get_max_view_num(); view++) {
    for (int bin = sino_measured.get_min_tangential_pos_num(); bin <= sino_measured.get_max_tangential_pos_num(); bin++) {
      ...
      sigma_square +=
          ((sino_measured[view][bin] - sino_calculated[view][bin]) * (sino_measured[view][bin] - sino_calculated[view][bin]));
    }
  }
...
void
FBP3DRPReconstruction::do_grow3D_viewgram(RelatedViewgrams<float>& viewgrams, int new_min_axial_pos_num,
  ...
  const IndexRange2D new_range(new_min_axial_pos_num_grow, new_max_axial_pos_num_grow, viewgrams.get_min_tangential_pos_num(),
                               viewgrams.get_max_tangential_pos_num());
  viewgrams.grow(new_range);
}

void
FBP3DRPReconstruction::do_forward_project_view(RelatedViewgrams<float>& viewgrams, int new_min_axial_pos_num,
                                               int new_max_axial_pos_num, int orig_min_axial_pos_num,
...
#ifdef NRFFT
    colsher_filter = ColsherFilter(height, width, _PI / 2 - theta, theta_max, sampling_in_s, sampling_in_t, alpha_colsher_axial,
                                   fc_colsher_axial, alpha_colsher_planar, fc_colsher_planar);
#else
...
void
FBP3DRPReconstruction::do_3D_backprojection_view(const RelatedViewgrams<float>& viewgrams, int new_min_axial_pos_num,
                                                 int new_max_axial_pos_num) {
...
void
FBP3DRPReconstruction::do_process_viewgrams(RelatedViewgrams<float>& viewgrams, int new_min_axial_pos_num,
                                            int new_max_axial_pos_num, int orig_min_axial_pos_num, int orig_max_axial_pos_num) {
  do_arc_correction(viewgrams);

  do_grow3D_viewgram(viewgrams, new_min_axial_pos_num, new_max_axial_pos_num);

  do_forward_project_view(viewgrams, new_min_axial_pos_num, new_max_axial_pos_num, orig_min_axial_pos_num,
                          orig_max_axial_pos_num);

...
OSMAPOSLReconstruction<TargetT>::set_up(shared_ptr<TargetT> const& target_image_ptr) {
  ...
  if (is_null_ptr(
          dynamic_cast<PoissonLogLikelihoodWithLinearModelForMean<TargetT> const*>(this->objective_function_sptr.get()))) {
...
OSMAPOSLReconstruction<TargetT>::update_estimate(TargetT& current_image_estimate) {
  ...
  this->compute_sub_gradient_without_penalty_plus_sensitivity(*multiplicative_update_image_ptr, current_image_estimate,
                                                              subset_num);
  ...
  // divide by subset sensitivity
  {
    ...
    if (this->objective_function_sptr->prior_is_zero()) {
      divide(multiplicative_update_image_ptr->begin_all(), multiplicative_update_image_ptr->end_all(), sensitivity.begin_all(),
             small_num);
    } else {
      ...
      divide(multiplicative_update_image_ptr->begin_all(), multiplicative_update_image_ptr->end_all(),
             denominator_ptr->begin_all(), small_num);
    }
  ...
  if (this->subiteration_num != 1) {
    ...
    threshold_upper_lower(multiplicative_update_image_ptr->begin_all(), multiplicative_update_image_ptr->end_all(), new_min,
                          new_max);
  }

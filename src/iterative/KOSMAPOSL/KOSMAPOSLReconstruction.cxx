  ...
  for (int z = min_dz; z <= max_dz; ++z) {
    for (int y = min_dy; y <= max_dy; ++y) {
      for (int x = min_dx; x <= max_dx; ++x) {
        distance[z][y][x] =
            sqrt(square(x * grid_spacing.x()) + square(y * grid_spacing.y()) + square(z * grid_spacing.z())) / grid_spacing.x();
      }
    }
  }
}
...
template <typename TargetT>
void
KOSMAPOSLReconstruction<TargetT>::calculate_norm_matrix(TargetT& normp, const int dimf_row, const int dimf_col,
...
template <typename TargetT>
void
KOSMAPOSLReconstruction<TargetT>::calculate_norm_const_matrix(std::vector<shared_ptr<TargetT>>& normm, const int dimf_row,
                                                              const int dimf_col) {
  for (unsigned int i = 0; i < this->anatomical_prior_sptrs.size(); i++) {
...
#  endif
    ...
    for (int z = min_z; z <= max_z; z++) {
      for (int y = min_y; y <= max_y; y++) {
        for (int x = min_x; x <= max_x; x++) { // no break allowed inside a parallel for
...
template <typename TargetT>
void
KOSMAPOSLReconstruction<TargetT>::compute_kernelised_image(TargetT& kernelised_image_out, const TargetT& image_to_kernelise,
            ...
            for (int dx = min_dx; dx <= max_dx; ++dx) {
              ...
              if (get_hybrid()) {
                ...
                emission_kernel = calc_emission_kernel(current_alpha_estimate[z][y][x],
                                                       current_alpha_estimate[z + dz][y + dy][x + dx], distance[dz][dy][dx],
                                                       use_compact_implementation, current_ravelled_idx, delta_ravelled_idx);
              } else {
                emission_kernel = 1;
              }
              ...
              for (unsigned int i = 0; i < this->anatomical_prior_sptrs.size(); i++) {
                anatomical_kernel =
                    anatomical_kernel * calc_anatomical_kernel((*anatomical_prior_sptrs[i])[z][y][x],
                                                               (*anatomical_prior_sptrs[i])[z + dz][y + dy][x + dx],
                                                               distance[dz][dy][dx], use_compact_implementation,
                                                               current_ravelled_idx, delta_ravelled_idx, i);
              }
...
KOSMAPOSLReconstruction<TargetT>::calc_emission_kernel(const double current_alpha_estimate_zyx,
                                                       const double current_alpha_estimate_zyx_dr, const double distance_dzdydx,
                                                       const bool use_compact_implementation, const int l, const int m) {

  const double emission_kernel =
      use_compact_implementation ? calc_kernel_compact(current_alpha_estimate_zyx - current_alpha_estimate_zyx_dr,
                                                       sigma_p * sigma_p, sigma_dp * sigma_dp, distance_dzdydx * distance_dzdydx,
                                                       current_alpha_estimate_zyx * current_alpha_estimate_zyx)
                                 : calc_kernel_from_precalculated((*kpnorm_sptr)[0][l][m], sigma_p * sigma_p, sigma_dp * sigma_dp,
...
template <typename TargetT>
double
KOSMAPOSLReconstruction<TargetT>::calc_kernel_from_precalculated(const double precalculated_norm_zxy, const double sq_sigma_int,
                                                                 const double sq_sigma_dist, const double sq_distance_dzdydx,
...
template <typename TargetT>
double
KOSMAPOSLReconstruction<TargetT>::calc_anatomical_kernel(const double anatomical_prior_zyx, const double anatomical_prior_zyx_dr,
                                                         const double distance_dzdydx, const bool use_compact_implementation,
                                                         const int l, const int m, const int index) {

  const double anatomical_kernel =
      use_compact_implementation
          ? calc_kernel_compact(anatomical_prior_zyx - anatomical_prior_zyx_dr, sigma_m[index] * sigma_m[index],
                                sigma_dm * sigma_dm, distance_dzdydx * distance_dzdydx,
                                anatomical_sd[index] * anatomical_sd[index])
          : calc_kernel_from_precalculated((*kmnorm_sptrs[index])[0][l][m], sigma_m[index] * sigma_m[index], sigma_dm * sigma_dm,
                                           distance_dzdydx * distance_dzdydx, anatomical_sd[index] * anatomical_sd[index]);
  return anatomical_kernel;
}

template <typename TargetT>
double
KOSMAPOSLReconstruction<TargetT>::calc_kernel_compact(const double prior_image_zyx_diff, const double sq_sigma_int,
                                                      const double sq_sigma_dist, const double sq_distance_dzdydx,
                                                      const double sq_precalc_denom) {
...
KOSMAPOSLReconstruction<TargetT>::update_estimate(TargetT& current_alpha_coefficent_image) {
  ...
  base_type::compute_sub_gradient_without_penalty_plus_sensitivity(*multiplicative_update_image_ptr, *current_update_image_ptr,
                                                                   subset_num);
  ...
  if (this->objective_function_sptr->prior_is_zero()) {
    ...
    divide(kmultiplicative_update_ptr->begin_all(), kmultiplicative_update_ptr->end_all(), denominator_ptr->begin_all(),
           small_num);
  }

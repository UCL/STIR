...
class MedisoSPECTDICOMData
{
public:
  MedisoSPECTDICOMData(const std::string& DICOM_filename){ dicom_filename = DICOM_filename; };
...
stir::Succeeded MedisoSPECTDICOMData::open_dicom_file()
{

  stir::info(boost::format("MedisoSPECTDICOMData: opening file %1%") % dicom_filename);
  ...
  try {
    if (!DICOM_reader->Read()) {
      stir::error(boost::format("MedisoSPECTDICOMData: cannot read file %1%") % dicom_filename);
      //return stir::Succeeded::no;
    }
  ...
  if (GetDICOMTagInfo(file, gdcm::Tag(0x0018,0x1142), radius_as_string) == stir::Succeeded::yes){
    ...
    std::cout << "Radius: " << radius_as_string << std::endl;
  }
  ...
  if (GetDICOMTagInfo(file, gdcm::Tag(0x0018,0x1242), actual_frame_duration_as_string) == stir::Succeeded::yes){
    ...
    std::cout << "Projection frame duration (msec): " << actual_frame_duration << std::endl;
  }
...
stir::Succeeded MedisoSPECTDICOMData::get_interfile_header(std::string &output_header) const{
...
stir::Succeeded MedisoSPECTDICOMData::get_proj_data(const std::string &output_file) const{
...
int main(int argc, char * argv[])
{
  ...
  std::cout << "Running mediso_extract" << std::endl;

  std::unique_ptr<MedisoSPECTDICOMData>
      spect(new MedisoSPECTDICOMData(argv[1]));

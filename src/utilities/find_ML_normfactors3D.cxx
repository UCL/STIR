...
main(int argc, char** argv) {
  ...
  const int num_rings = measured_data->get_proj_data_info_sptr()->get_scanner_sptr()->get_num_rings();
  const int num_detectors_per_ring = measured_data->get_proj_data_info_sptr()->get_scanner_sptr()->get_num_detectors_per_ring();
  ...
  const int num_axial_crystals_per_block =
      measured_data->get_proj_data_info_sptr()->get_scanner_sptr()->get_num_axial_crystals_per_block();
  ...
  GeoData3D measured_geo_data(num_axial_crystals_per_block, num_transaxial_crystals_per_block / 2, num_rings,
                              num_detectors_per_ring); // inputes have to be modified
  GeoData3D norm_geo_data(num_axial_crystals_per_block, num_transaxial_crystals_per_block / 2, num_rings,
                          num_detectors_per_ring); // inputes have to be modified
  ...
  make_fan_data(model_fan_data, *model_data);
  {
    ...
    for (int iter_num = 1; iter_num <= std::max(num_iterations, 1); ++iter_num) {
      ...
      if (do_KL) {
        ...
        GeoData3D geo_data(num_axial_crystals_per_block, num_transaxial_crystals_per_block / 2, num_rings,
                           num_detectors_per_ring); // inputes have to be modified
        ...
        std::cerr << "KL on fans: " << KL(measured_fan_data, fan_data, 0) << ", " << KL(measured_geo_data, geo_data, 0)
                  << std::endl;
      }
    }
  }

...
void
process_data(const string& output_file_name, const int num_files, char** argv, const bool no_math_on_data,
             const bool except_first, const bool verbose, const bool do_add, const FunctionObjectT& pow_times_add_object,
  ...
  for (int i = 1; i < num_files; ++i) {
    ...
    if (do_add) {
      ...
      std::transform(image_ptr->begin_all(), image_ptr->end_all(), current_image_ptr->begin_all(), image_ptr->begin_all(),
                     std::plus<float>());
    } else {
      // *image_ptr *= *current_image_ptr;
      std::transform(image_ptr->begin_all(), image_ptr->end_all(), current_image_ptr->begin_all(), image_ptr->begin_all(),
...
process_data(const string& output_file_name, const int num_files, char** argv, const bool no_math_on_data,
  ...
  unique_ptr<DynamicDiscretisedDensity> dyn_image_sptr = read_from_file<DynamicDiscretisedDensity>(*argv);
  ...
  for (int i = 1; i < num_files; ++i) {
    if (verbose)
      cout << "Reading image " << argv[i] << endl;
    dyn_current_image_sptr = read_from_file<DynamicDiscretisedDensity>(argv[i]);
    ...
    for (unsigned int frame_num = 1; frame_num <= (dyn_image_sptr->get_time_frame_definitions()).get_num_frames(); ++frame_num) {
      ...
      if (do_add) {
        ...
        // also, it would be better to be able to call += on each element
        //*image_ptr += *current_image_ptr;
        std::transform(dyn_image[frame_num].begin_all(), dyn_image[frame_num].end_all(), dyn_current_image[frame_num].begin_all(),
                       dyn_image[frame_num].begin_all(), std::plus<float>());
      } else {
        // *image_ptr *= *current_image_ptr;
        std::transform(dyn_image[frame_num].begin_all(), dyn_image[frame_num].end_all(), dyn_current_image[frame_num].begin_all(),
                       dyn_image[frame_num].begin_all(), std::multiplies<float>());
      }
    }
  }

...
main (int argc, char **argv)
{
  ...
  const TimeFrameDefinitions time_def =
    singles.get_time_frame_definitions();
  ...
  Array<3,float> singles_arr(IndexRange3D(time_def.get_num_time_frames(),
                                          scanner.get_num_axial_singles_units(),
          ...
          for (int transax=0; transax<scanner.get_num_transaxial_singles_units(); ++transax)
            {
              ...
              singles_arr[time_frame_num-1][ax][transax] =
                singles.get_singles(singles_bin_index,
                                    time_def.get_start_time(time_frame_num),
                                    time_def.get_end_time(time_frame_num));
            }
        }
    }

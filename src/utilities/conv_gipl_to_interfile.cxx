  ...
  if (orientation == 1)
    {
      ...
      min_range[1] = 0;
      max_range[1] = image->m_dim[2] - 1;
      min_range[2] = -static_cast<int>(floor(image->m_dim[1] / 2.F));
      max_range[2] = min_range[2] + image->m_dim[1] - 1;
      min_range[3] = -static_cast<int>(floor(image->m_dim[0] / 2.F));
      max_range[3] = min_range[3] + image->m_dim[0] - 1;
      ...
      const CartesianCoordinate3D<float> grid_spacing(image->m_pixdim[2], image->m_pixdim[1], image->m_pixdim[0]);
      ...
      OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(output_filename, new_image);
    }
  else if (orientation == 2)
    {
      ...
      BasicCoordinate<3, int> max_range;
      min_range[1] = 0;
      max_range[1] = image->m_dim[1] - 1;
      min_range[2] = -static_cast<int>(floor(image->m_dim[2] / 2.F));
      max_range[2] = min_range[2] + image->m_dim[2] - 1;
      min_range[3] = -static_cast<int>(floor(image->m_dim[0] / 2.F));
      max_range[3] = min_range[3] + image->m_dim[0] - 1;
      ...
      const CartesianCoordinate3D<float> grid_spacing(image->m_pixdim[1], image->m_pixdim[2], image->m_pixdim[0]);
      CartesianCoordinate3D<float> origin(0.F, 0.F, 0.F);
  ...
  else if (orientation == 3)
    {
      ...
      const CartesianCoordinate3D<float> grid_spacing(image->m_pixdim[1], image->m_pixdim[0], image->m_pixdim[2]);
      CartesianCoordinate3D<float> origin(0.F, 0.F, 0.F);

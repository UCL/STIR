...
compute_num_bins(const int num_rings, const int num_detectors_per_ring, const int max_ring_diff, const int half_fan_size) {
  unsigned long num = 0;
...
main(int argc, char** argv) {
  ...
  const int do_display_interval = ask_num("Display iterations which are a multiple of ", 0, num_eff_iterations, 0);
  ...
  if (argc == 4) {
    ...
    // write fan sums to file
    {
      ...
      fan_sum_name += argv[2];
      fan_sum_name.erase(fan_sum_name.begin() + fan_sum_name.rfind('.'), fan_sum_name.end());
      fan_sum_name += ".dat";
    ...
    if (num_rings < max_ring_diff || num_detectors_per_ring < fan_size) {
      warning("input file %s is a matrix with sizes %dx%d, but this is "
              ...
              argv[3], num_rings, num_detectors_per_ring, max_ring_diff, fan_size);
      exit(EXIT_FAILURE);
    }
  }
  ...
  DetectorEfficiencies efficiencies(IndexRange2D(num_rings, num_detectors_per_ring));
  {
    ...
    const int iter_num = 1;
    {
      if (iter_num == 1) {
        efficiencies.fill(
            sqrt(data_fan_sums.sum() / compute_num_bins(num_rings, num_detectors_per_ring, max_ring_diff, half_fan_size)));
      }
      // efficiencies
      {

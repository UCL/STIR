...
{
    cerr<<"\nUsage: apply_RL_deconvolution <output filename> <input filename> <forward motion vectors prefix> <reverse motion vectors prefix> <number of EM iterations> <save iteration interval>\n"
		...
		<<"\t--<forward motion vectors prefix>: Filename prefix indicating the files of the motion vector fields used to introduce motion contamination to the original motion-less image "
		<<"\t--<reverse motion vectors prefix>: Filename prefix indicating the files of the motion vector fields used to reverse the motion contamination (flipped motion kernel) to the motion-contaminated image "
		<<"\t--<save iteration interval>: Integer designating the interval of Richardson-Lucy EM iterations at which to save images every time";
    exit(EXIT_FAILURE);
}
				 ...
				 const char *output_filename)											 
{
  ...
  std::fill(model_sensitivity_image.begin_all(),
            model_sensitivity_image.end_all(),
  	        1.F); 

  shared_ptr<DiscretisedDensity<3,float> > conv_image_of_all_ones(conv_image_reference.get_empty_copy());
  std::fill(conv_image_of_all_ones->begin_all(),
	        conv_image_of_all_ones->end_all(),
	        1.F);
  ...
  *std::min_element(model_sensitivity_image.begin_all(),
   				    model_sensitivity_image.end_all()); 
  const float current_max_model_sensitivity = 
  *std::max_element(model_sensitivity_image.begin_all(),
				    model_sensitivity_image.end_all());
  ...
  const float current_min_reference =
	*std::min_element(conv_image_reference.begin_all(),
					  conv_image_reference.end_all()); 
  const float current_max_reference = 
	*std::max_element(conv_image_reference.begin_all(),
					  conv_image_reference.end_all());
  cerr << "Reference convolved image , (min, max): (" 
	   << current_min_reference << ", " << current_max_reference << ")" << endl << endl;

  //Then, enter EM loop of Richardson-Lucy deconvolution of motion contaminated input image (conv_image_reference) utilizing the previous sensitivity image
  cerr << endl << "Entering EM update loop (" << num_iterations << " Richardson-Lucy EM deconvolution iterations for motion correction)." << endl;
  ...
  for(unsigned int iterations_num=1; iterations_num<=num_iterations; iterations_num++)
  {
    ...
    cerr << "Richardson-Lucy iteration: " << iterations_num 
	     << " Initial deconvolved image estimate , (min, max): (" 
    ...
    // from deconvolved (motion-corrected) image space to convolved (motion-contaminated) image space using a motion-corrected image estimate as an input
	motion_vectors.average_warp_image(conv_image_RL_estimate,
	                                  current_estimate);
	

	//Print out the min and max values of the current convolved image estimate after the forward convolution
    const float conv_min_RL_estimate =
	  *std::min_element(conv_image_RL_estimate.begin_all(),
					    conv_image_RL_estimate.end_all()); 
    const float conv_max_RL_estimate = 
	  *std::max_element(conv_image_RL_estimate.begin_all(),
					    conv_image_RL_estimate.end_all());
    cerr << "Richardson-Lucy iteration: " << iterations_num 
	     << " Current convolved image estimate after forward convolution , (min, max): (" 
		 << conv_min_RL_estimate << ", " << conv_max_RL_estimate << ")" << endl;
	
    conv_image_estimate = conv_image_reference;
	
	// Division of the reference with the estimate in the convolved space
	divide(conv_image_estimate.begin_all(),
	       conv_image_estimate.end_all(),
		   conv_image_RL_estimate.begin_all(),
		   small_num);

	//Print out the min and max values of the convolved image gradient after the division of the reference with the estimate in the convolved space
    const float conv_min_estimate =
	  *std::min_element(conv_image_estimate.begin_all(),
					    conv_image_estimate.end_all()); 
    const float conv_max_estimate = 
	  *std::max_element(conv_image_estimate.begin_all(),
					    conv_image_estimate.end_all());
    cerr << "Richardson-Lucy iteration: " << iterations_num 
	     << " Current convolved image gradient after division of reference with the estimate in the convolved space , (min, max): (" 
    ...
    // from convolved (motion-contaminated) image space to deconvolved (motion-corrected) image space using the convolved image estimate as an input	
    reverse_motion_vectors.average_warp_image(gradient,
	                                          conv_image_estimate);	

    											  
	//Print out the min and max values of the deconvolved image gradient after the backward convolution of the previous gradient from the convolved space
    const float unnormalized_gradient_min_estimate =
	  *std::min_element(gradient.begin_all(),
					    gradient.end_all()); 
    const float unnormalized_gradient_max_estimate = 
	  *std::max_element(gradient.begin_all(),
					    gradient.end_all());
    cerr << "Richardson-Lucy iteration: " << iterations_num 
	     << " Current deconvolved image gradient after the backward convolution of the previous image gradient from the convolved space , (min, max): (" 
	...
	*/
	...
	//Print out the min and max values of the normalized deconvolved image gradient after the devision to the model sensitivity image
	const float current_min_gradient =
	  *std::min_element(gradient.begin_all(),
	 			        gradient.end_all()); 
	const float current_max_gradient = 
        *std::max_element(gradient.begin_all(),
						  gradient.end_all()); 					  
	cerr << "Richardson-Lucy iteration: " << iterations_num 
		 << " Gradient after sensitivity image division: old value (min, max): (" 
		 << current_min_gradient << ", " << current_max_gradient
		 << "), new value (min, max) (" 
		 << max(current_min_gradient, min_update_factor) << ", " 
		 << min(current_max_gradient, max_update_factor) << ")"
		 << "), threshold limits (min, max) (" 
		 << min_update_factor << ", " << max_update_factor << ")" << endl;

	zero_threshold_upper_lower(gradient.begin_all(),
						       gradient.end_all(), 
						       min_update_factor, max_update_factor);      
	...
	const float current_min_updated_image =
	*std::min_element(current_estimate.begin_all(),
					  current_estimate.end_all()); 
	const float current_max_updated_image = 
	*std::max_element(current_estimate.begin_all(),
					  current_estimate.end_all());
	cerr << "Richardson-Lucy iteration: " << iterations_num 
	     << " Updated deconvolved (motion corrected) image value (min, max) ("
		 << current_min_updated_image << ", " << current_max_updated_image << ")" << endl << endl;
    ...
    if ( (iterations_num % save_iterations_interval)==0 )
      {
	   ...
	   Succeeded write_result = OutputFileFormat<DiscretisedDensity<3,float> >::
                                  default_sptr()-> write_to_file(iter_output_filename, current_estimate);

	   if (write_result==Succeeded::yes)
	      cerr << "Richardson-Lucy iteration: " << iterations_num
		       << " Writing of current deconvolved image estimate (header file " << iter_output_filename
			   << ") was successful." << endl << endl << endl;
	   else
	     {
	      cerr << "Richardson-Lucy iteration: " << iterations_num
		       << " Writing of current deconvolved image estimate (header file " << iter_output_filename
			   << ") did not succeed." << endl << endl << endl;
		  exit(EXIT_FAILURE);
		  }
  ...
  cerr << "End of EM deconvolution process to compute motion-corrected estimates (after " 
       << num_iterations << " Richardson-Lucy iterations)" << endl << endl;
...
int main(int argc, char **argv)
{
  ...
  cerr << "\nPost-reconstruction application of Richardson-Lucy EM deconvolution algorithm to correct for intra-frame/gate motion" << endl << endl;
  
  const shared_ptr<DiscretisedDensity<3,float> > convolved_density_sptr(read_from_file<DiscretisedDensity<3,float> >(input_filename));
  ...
  cerr << "\nNumber of Richardson-Lucy deconvolution iterations: " << num_iterations << endl
       ...
       << "Upper threshold for the update factors: " << max_update_factor << endl << endl;
  ...
  std::fill(corrected_density_sptr->begin_all(),
	        corrected_density_sptr->end_all(),
	        1.F);
  ...
  OutputFileFormat<DiscretisedDensity<3,float> >::default_sptr()-> write_to_file(sensitivity_image_filename, *model_sensitivity_image_sptr);
  const Succeeded res = OutputFileFormat<DiscretisedDensity<3,float> >::
                        default_sptr()-> write_to_file(final_iter_image_filename, *corrected_density_sptr);						 

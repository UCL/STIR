...
void do_math(enum options operation, SegmentByView<float>& sino1, SegmentByView<float>& sino2, float& accum_max, float& accum_min,
             float& accum_sum, bool accumulators_initialized);

// display, operations with a scalar, others
void do_math(enum options operation, SegmentByView<float>& sino1, SegmentBySinogram<float>& seg_sinogram, float& accum_max,
             float& accum_min, float& accum_sum, bool accumulators_initialized, float scalar = 0.0);

void make_buffer_header(const char* data_filename, const char* header_filename, ProjData& input_sino, int limit_segments,
...
void
do_math(enum options operation, SegmentByView<float>& sino1, SegmentByView<float>& sino2, float& accum_max, float& accum_min,
        float& accum_sum, bool accumulators_initialized) {
  switch (operation) {
...
void
do_math(enum options operation, SegmentByView<float>& sino1, SegmentBySinogram<float>& seg_sinogram, float& accum_max,
...
main(int argc, char* argv[]) {
  ...
  do { //(re)start from here
    ...
    int limit_segments = ask_num("Maximum absolute segment number to process: ", 0, first_operand->get_max_segment_num(),
    ...
    do { // math operations loop
      ...
      if (operation == _add_scalar || operation == _mult_scalar || operation == _div_scalar || operation == _trim ||
          operation == _zero_ends) { // requiring scalar operand
        bool need_int = false;
        ...
        do
          scalar = new float(ask_num("Scalar Operand: ", (need_int) ? (int)lower_bound : lower_bound,

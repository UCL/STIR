...
enum options { _quit,_display, _absdiff, _add_sino, _subtract_sino, _mult_sino,_div_sino, _add_scalar, _mult_scalar,_div_scalar,_stats,_pos_ind,_trunc_neg,_trim,_zero_ends,_restart,_menu};


void do_math(enum options operation, PETSegmentByView& sino1,PETSegmentByView &sino2, float &accum_max, float &accum_min, float &accum_sum, bool accumulators_initialized);

void do_math(enum options operation, PETSegmentByView& sino1, float &accum_max, float &accum_min, float &accum_sum, bool accumulators_initialized,float scalar=0.0);

void make_buffer_header(const char *data_filename,const char *header_filename, PETSinogramOfVolume& input_sino, int limit_segments/*,bool asfloats=true*/);
...
int main(int argc, char *argv[]){
  ...
  if (reload) {
    ...
    first_operand= new PETSinogramOfVolume(read_interfile_PSOV(argv[1]));
  else first_operand= new PETSinogramOfVolume(ask_interfile_PSOV("Input sinogram: "));
  ...
  operation= (enum options) ask_num("Choose Operation: ",0,16,16);
  ...
  if (operation!= _display  && operation!= _stats  &&!buffer_opened)  //operation result is a sinogram
    {

   ask_filename_with_extension(output_buffer_root,
      ...
      make_buffer_header(output_buffer_filename, output_buffer_header, *first_operand, limit_segments /* first_operand->get_max_segment() */);
  ...
  if(operation==_absdiff   ||  operation==_add_sino ||  operation==_subtract_sino   || operation==_mult_sino   || operation==_div_sino  ){ //requiring 2nd sinogram operand
  ...
  if(operation==_add_scalar   ||  operation==_mult_scalar ||
     operation==_div_scalar   || operation==_trim || operation==_zero_ends )  //requiring scalar operand
   {
	...
	scalar= new float (ask_num("Scalar Operand: ",(need_int)?(int)lower_bound:lower_bound ,(need_int)?(int)upper_bound: upper_bound,(need_int)?(int)deflt:deflt));
 ...
 if(second_operand != NULL)// seg2= new PETSegmentByView(second_operand->get_segment_view_copy(0));
   {
 ...
 else if(scalar != NULL)
   {
     ...
     else  do_math(operation,seg1,accum_max,accum_min,accum_sum,false,*scalar);
 ...
 else
   {
     ...
     do_math(operation,seg1,accum_max,accum_min,accum_sum,false);
 ...
 if(operation!= _display  && operation!= _stats && buffer_opened) seg1.write_data(new_sino);
  ...
  if(limit_segments>0 /*&& operation != _zero_ends */)
      for (int segment_num = 1; segment_num <= limit_segments ; segment_num++){

	//MJ not sure if this is the best place for this
	if(operation==_display && ask("Abort display",false)) break;
	  ...
	  //  PETSegmentByView *seg2_pos =NULL;
	  //  PETSegmentByView *seg2_neg =NULL;
	  ...
	  do_math(operation,seg1_neg,seg2_neg,accum_max,accum_min,accum_sum,true);
	    ...
	    {
	      do_math(operation,seg1_pos,accum_max,accum_min,accum_sum,true,*scalar);
	      do_math(operation,seg1_neg,accum_max,accum_min,accum_sum,true,*scalar);
	  ...
	  else
	    {
	      ...
	      do_math(operation,seg1_pos,accum_max,accum_min,accum_sum,true);
	
	      if(operation==_display && ask("Abort display",false)) break;
	
	
	      do_math(operation,seg1_neg,accum_max,accum_min,accum_sum,true);

	

	      if(operation==_display && segment_num<limit_segments  && ask("Abort display",false)) break; 
	  ...
	  //Write sinogram result to file
	  if(operation!= _display  && operation!= _stats && buffer_opened){
	...
	if(!accumulators_initialized)
	  {
	    ...
	    //we don't need this I guess, but anyway...
...
void do_math(enum options operation, PETSegmentByView& sino1, float &accum_max, float &accum_min, float &accum_sum, bool accumulators_initialized,float scalar){
 ...
 case _stats:
   {//global min&max + number of counts
  ...
  case _pos_ind:
    { //positive indicator
      ...
      in_place_apply_function(sino1,pos_indicate);
...
void show_math_menu(){
  cerr<<"\n\
    ...
    9. Divide scalar \n\
    10. Min/Max & counts \n\
    11. Positive indicator \n\
    12. Truncate negatives \n\
    13. Trim end bins\n\
    14. Zero end planes of segment 0 \n\
    15. Restart\n\
    16. Redisplay menu"<<endl;
   
}


void make_buffer_header(const char *data_filename,const char *header_filename, PETSinogramOfVolume& input_sino, int limit_segments /*,bool asfloats*/){
  ...
  ofstream header;
  //  open_write_binary(header, header_filename);
  header.open(header_filename, ios::out);
  if(!header) {cerr<<endl<<"Could not create header"<<endl; exit(1);}
  ...
  if(input_sino.scan_info.get_scanner().type==PETScannerInfo::RPT)
    header<<"PRT-1 \n";

  else if(input_sino.scan_info.get_scanner().type==PETScannerInfo::Advance)
     header<<"Advance \n";

  else if(input_sino.scan_info.get_scanner().type==PETScannerInfo::E953)
    header<<"ECAT 953 \n";
  
  else if(input_sino.scan_info.get_scanner().type==PETScannerInfo::E951)
    header<<"ECAT 951 \n";

  else if(input_sino.scan_info.get_scanner().type==PETScannerInfo::E966)
    header<<"EXACT3D \n";
  //MJ Are we absolutely sure about this name? 


  else
    { 
      PETerror("Tried to create header for unsupported scanner type"); Abort();
    }
  ...
  if(input_sino.scan_info.get_scanner().type==PETScannerInfo::Advance)
   {
     ...
     header<<2*nrings-1;
     for (int i=1; i<=limit_segments; ++i)
	header<<", "<<nrings-(i+1)<<", "<<nrings-(i+1);
   }
...
float pos_indicate(float x){return (x>0.0)?1.0:0.0;}

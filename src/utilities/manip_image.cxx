...
transpose_13(const VoxelsOnCartesianGrid<float>& image) {
  ...
  VoxelsOnCartesianGrid<float> out(IndexRange3D(image.get_min_x(), image.get_max_x(), image.get_min_y(), image.get_max_y(),
...
transpose_12(const VoxelsOnCartesianGrid<float>& image) {
  ...
  VoxelsOnCartesianGrid<float> out(IndexRange3D(image.get_min_y(), image.get_max_y(), image.get_min_z(), image.get_max_z(),
  ...
  do { // start main mode
    ...
    case 4: // min-max, counts plane-wise
    {
      ...
      while (plane > 0 && plane <= ze - zs + 1) {
        ...
        cerr << "Min and Max in plane " << main_buffer[zs + plane - 1].find_min() << " " << main_buffer[zs + plane - 1].find_max()
             << endl;
...
math_mode(VoxelsOnCartesianGrid<float>& main_buffer, int& quit_from_math) {
  ...
  do {
    ...
    case 10: // zoom
    {
      ...
      const float offset_x = ask_num("Offset x (in mm)", -math_buffer.get_x_size() * math_buffer.get_voxel_size().x(),
                                     math_buffer.get_x_size() * math_buffer.get_voxel_size().x(), 0.F);
      const float offset_y = ask_num("Offset y (in mm)", -math_buffer.get_y_size() * math_buffer.get_voxel_size().y(),
                                     math_buffer.get_y_size() * math_buffer.get_voxel_size().y(), 0.F);
      const float offset_z = ask_num("Offset z (in mm)", -math_buffer.get_z_size() * math_buffer.get_voxel_size().z(),
                                     math_buffer.get_z_size() * math_buffer.get_voxel_size().z(), 0.F);
      const int new_size_x = ask_num("New x size (pixels)", 1, static_cast<int>(math_buffer.get_x_size() * zoom_x * 2),
      ...
      const int new_size_z = ask_num("New z size (pixels)", 1, static_cast<int>(math_buffer.get_z_size() * zoom_z * 2),
                                     static_cast<int>(math_buffer.get_z_size() * zoom_z));
      zoom_image_in_place(math_buffer, CartesianCoordinate3D<float>(zoom_z, zoom_y, zoom_x),

...
main(int argc, char** argv) {
  ...
  ProjDataInterfile proj_data(template_projdata_ptr->get_exam_info_sptr(),
                              template_projdata_ptr->get_proj_data_info_sptr()->create_shared_clone(), output_file_name);
...

  {
    ...
    const int max_ring_diff = proj_data_info_sptr->get_max_ring_difference(proj_data_info_sptr->get_max_segment_num());

    const int mashing_factor = proj_data_info_sptr->get_view_mashing_factor();

    shared_ptr<Scanner> scanner_sptr(new Scanner(*proj_data_info_sptr->get_scanner_ptr()));
    unique_ptr<ProjDataInfo> uncompressed_proj_data_info_uptr(ProjDataInfo::construct_proj_data_info(
        scanner_sptr,
        /*span=*/1, max_ring_diff,
        /*num_views=*/num_detectors_per_ring / 2, scanner_sptr->get_max_num_non_arccorrected_bins(),
        /*arccorrection=*/false));
    const ProjDataInfoCylindricalNoArcCorr* const uncompressed_proj_data_info_sptr =
        dynamic_cast<const ProjDataInfoCylindricalNoArcCorr* const>(uncompressed_proj_data_info_uptr.get());
    ...
    for (bin.segment_num() = proj_data.get_min_segment_num(); bin.segment_num() <= proj_data.get_max_segment_num();
      ...
      for (bin.axial_pos_num() = proj_data.get_min_axial_pos_num(bin.segment_num());
        ...
        Sinogram<float> sinogram = proj_data_info_sptr->get_empty_sinogram(bin.axial_pos_num(), bin.segment_num());
        const float out_m = proj_data_info_sptr->get_m(bin);
        const int in_min_segment_num = proj_data_info_sptr->get_min_ring_difference(bin.segment_num());
        const int in_max_segment_num = proj_data_info_sptr->get_max_ring_difference(bin.segment_num());
...

        {
          for (uncompressed_bin.segment_num() = in_min_segment_num; uncompressed_bin.segment_num() <= in_max_segment_num;
            ...
            for (uncompressed_bin.axial_pos_num() =
                     uncompressed_proj_data_info_sptr->get_min_axial_pos_num(uncompressed_bin.segment_num());
                 uncompressed_bin.axial_pos_num() <=
                 uncompressed_proj_data_info_sptr->get_max_axial_pos_num(uncompressed_bin.segment_num());
                 ++uncompressed_bin.axial_pos_num()) {
              ...
              for (bin.view_num() = proj_data.get_min_view_num(); bin.view_num() <= proj_data.get_max_view_num();
                ...
                for (bin.tangential_pos_num() = proj_data_info_sptr->get_min_tangential_pos_num();
                     bin.tangential_pos_num() <= proj_data_info_sptr->get_max_tangential_pos_num(); ++bin.tangential_pos_num()) {
                  ...
                  for (uncompressed_bin.view_num() = bin.view_num() * mashing_factor;
                    ...
                    uncompressed_proj_data_info_sptr->get_det_pair_for_bin(a, ra, b, rb, uncompressed_bin);

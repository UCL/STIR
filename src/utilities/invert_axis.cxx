...
main(int argc, char** argv) {
  if (argc != 4) {
    std::cerr << "Usage: " << argv[0] << " <axis name>  <output filename> <input filename>\n"
  ...
  invert.invert_axis(out_image, image, name);

  const Succeeded res =
      OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(output_filename_prefix, out_image);

...
main(int argc, char* argv[]) {
  ...
  if ((its_an_image == false && argc - arg_index >= 1) || argc - arg_index >= 2) {
    ...
    ask_filename_with_extension(cti_name, "Name of the ECAT7 file? ", its_an_image ? ".img" : ".scn");
  }
  ...
  if (its_an_image) {
    ...
    while (1) {
      if (DiscretisedDensity_to_ECAT7(mptr, *density_ptr, frame_num) == Succeeded::no) {
        matrix_close(mptr);
        return EXIT_FAILURE;
      }
    ...
    make_ECAT7_main_header(mhead, filenames[0], proj_data_ptr->get_exam_info(), *proj_data_ptr->get_proj_data_info_sptr(),
                           write_as_attenuation, NumericType::SHORT);
    // fix time frame info
    mhead.num_frames = filenames.size();
    ...
    while (1) {
      if (ProjData_to_ECAT7(mptr, *proj_data_ptr, frame_num, 1, 0, 0, scale_factor) == Succeeded::no) {
        matrix_close(mptr);
        return EXIT_FAILURE;
      }
      if (++frame_num > filenames.size()) {

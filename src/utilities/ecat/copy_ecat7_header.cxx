...
copy_subheader(MatrixData* data_out, const MatrixData* data_in) {
  ...
  case PetVolume: {
    ...
    case Float3dSinogram:
      copy_subheader(*reinterpret_cast<Image_subheader*>(data_out->shptr), *reinterpret_cast<Scan3D_subheader*>(data_in->shptr));
      break;
    case PetImage:
    case ByteVolume:
    case PetVolume:
      copy_subheader(*reinterpret_cast<Image_subheader*>(data_out->shptr), *reinterpret_cast<Image_subheader*>(data_in->shptr));
      break;
    default:
...
ECAT_dataset_spec::ECAT_dataset_spec() { set_defaults(); }

ECAT_dataset_spec::ECAT_dataset_spec(const char* const spec) { decode_spec(spec); }

ECAT_dataset_spec::ECAT_dataset_spec(const string& spec) { decode_spec(spec.c_str()); }
...
mat_write_any_subheader(MatrixData* data) {
  ...
  case Short3dSinogram:
  case Float3dSinogram:
    return_value = mat_write_Scan3D_subheader(mptr->fptr, mptr->mhptr, strtblk, reinterpret_cast<Scan3D_subheader*>(data->shptr));

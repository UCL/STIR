...
main(int argc, char** argv) {
  ...
  static const char* const options[] = {"argv[1]  GATE file\n",         "argv[2]  Angles in GATE file\n",
  ...
  if ((GATE_file = fopen(GATE_filename, "rb")) == NULL)
    ...
    if (GATE_file_size % num_bins_per_sino != 0)
      error("Expected Input Data should be multiple of the number of bins per sinogram. Check input for bins and angles or GATE "
            "file.\n");
  }
  ...
  ProjDataInterfile proj_data(exam_info_sptr, proj_data_info_sptr, STIR_output_filename, std::ios::out);
  ...
  for (int seg_num = 0; seg_num <= max_ring_difference; seg_num = (seg_num <= 0) ? 1 + seg_num * -1 : -1 * seg_num) {
    ...
    for (int axial_pos_num = proj_data.get_min_axial_pos_num(segment_num);
      ...
      float scale = 1;
      if (read_data(GATE_file, sino, NumericType::SHORT, scale) != Succeeded::yes) {

...
main(int argc, const char* argv[])
{
  ...
  const stir::shared_ptr<const ProjData> template_projdata_sptr = ProjData::read_from_file(template_filename);
  ...
  if (addgaps)
    output_projdata_sptr.reset(new ProjDataInterfile(
        gtemplate_projdata_sptr->get_exam_info_sptr(), gtemplate_projdata_sptr->get_proj_data_info_sptr(), output_filename));
  else
    output_projdata_sptr.reset(new ProjDataInterfile(
        template_projdata_sptr->get_exam_info_sptr(), template_projdata_sptr->get_proj_data_info_sptr(), output_filename));
  ...
  const shared_ptr<const ProjDataInfoCylindricalNoArcCorr> out_projdata_info_sptr
      = dynamic_pointer_cast<ProjDataInfoCylindricalNoArcCorr>(
          output_projdata_sptr->get_proj_data_info_sptr()->create_shared_clone());
...
#endif
  for (int iseg = out_projdata_info_sptr->get_min_segment_num(); iseg <= out_projdata_info_sptr->get_max_segment_num(); ++iseg)
    {
...
#  endif
#endif
  for (int iSlice = 0; iSlice < sinoMap.nslice(); ++iSlice)
    {
...
#  pragma omp critical(LISTMODEIO)
      ...
      out_projdata_info_sptr->get_segment_axial_pos_num_for_ring_pair(cur_seg, cur_axial, dr1, dr2);
      ...
      for (int i_phi = 0; i_phi <= template_projdata_sptr->get_max_view_num(); ++i_phi)
        {
          for (int i_tang = 0; i_tang < template_projdata_sptr->get_num_tangential_poss(); ++i_tang)
            {
              int d1, d2;
              int _d1, _d2;

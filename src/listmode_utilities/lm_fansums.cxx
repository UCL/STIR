...
LmFansums::post_processing()
{
  ...
  const int num_rings = lm_data_ptr->get_scanner().get_num_rings();
  ...
  const int max_fan_size = lm_data_ptr->get_scanner().get_max_num_non_arccorrected_bins();
  if (fan_size == -1)
...
LmFansums::compute()
{
  ...
  //*********** get Scanner details
  const int num_rings = lm_data_ptr->get_scanner().get_num_rings();
  const int num_detectors_per_ring = lm_data_ptr->get_scanner().get_num_detectors_per_ring();

...
private:
  ...
  void write_fan_sums(const Array<2, float>& data_fan_sums, const unsigned current_frame_num) const;
};

void
LmFansums::set_defaults() {
...
LmFansums::post_processing() {
  lm_data_ptr = read_from_file<ListModeData>(input_filename);

  const int num_rings = lm_data_ptr->get_scanner_ptr()->get_num_rings();
  ...
  const int max_fan_size = lm_data_ptr->get_scanner_ptr()->get_max_num_non_arccorrected_bins();
  if (fan_size == -1)
    fan_size = max_fan_size;
  else
    fan_size = min(fan_size, max_fan_size);

  frame_defs = TimeFrameDefinitions(frame_definition_filename);
  return false;
}

LmFansums::LmFansums(const char* const par_filename) {
  set_defaults();
  if (par_filename != 0)
    parse(par_filename);
  else
...
LmFansums::compute() {
  ...
  //*********** get Scanner details
  const int num_rings = lm_data_ptr->get_scanner_ptr()->get_num_rings();
  const int num_detectors_per_ring = lm_data_ptr->get_scanner_ptr()->get_num_detectors_per_ring();
    ...
    while (true) {
      ...
      if (record.is_time()) {
        ...
        if (new_time >= frame_defs.get_end_time(current_frame_num)) {
          while (current_frame_num <= frame_defs.get_num_frames() && new_time >= frame_defs.get_end_time(current_frame_num)) {
            write_fan_sums(data_fan_sums, current_frame_num++);
        ...
        dynamic_cast<const CListEventCylindricalScannerWithDiscreteDetectors&>(record.event()).get_detection_position(det_pos);
        ...
        if (abs(ra - rb) <= max_segment_num_to_process) {
          const int det_num_diff = (a - b + 3 * num_detectors_per_ring / 2) % num_detectors_per_ring;
          if (det_num_diff <= fan_size / 2 || det_num_diff >= num_detectors_per_ring - fan_size / 2) {

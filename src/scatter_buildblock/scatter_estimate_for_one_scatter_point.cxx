...
float
SingleScatterSimulation::simulate_for_one_scatter_point(const std::size_t scatter_point_num, const unsigned det_num_A,
                                                        const unsigned det_num_B) {
  if (this->max_single_scatter_cos_angle <= 0.F) // set to negative value by set_up(), so recompute
  {
    this->max_single_scatter_cos_angle =
        max_cos_angle(this->template_exam_info_sptr->get_low_energy_thres(), 2.f,

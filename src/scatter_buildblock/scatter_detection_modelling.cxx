...
detection_efficiency(const float energy) const
{
  ...
  const float sigma_times_sqrt2= 
    sqrt(2.*energy*this->proj_data_info_sptr->get_scanner_ptr()->get_reference_energy())*
          this->proj_data_info_sptr->get_scanner_ptr()->get_energy_resolution()/2.35482f;  // 2.35482=2 * sqrt( 2 * ( log(2) )

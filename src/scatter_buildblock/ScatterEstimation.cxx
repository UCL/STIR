...
ScatterEstimation::ScatterEstimation() { this->set_defaults(); }
...
ScatterEstimation::set_up() {
  ...
  if (input_projdata_sptr->get_num_segments() > 1) {
    ...
    shared_ptr<ProjDataInfo> proj_data_info_2d_sptr(
        SSRB(*this->input_projdata_sptr->get_proj_data_info_sptr(), this->input_projdata_sptr->get_num_segments(), 1, false));

    this->input_projdata_2d_sptr.reset(
        new ProjDataInMemory(this->input_projdata_sptr->get_exam_info_sptr(), proj_data_info_2d_sptr));

    SSRB(*this->input_projdata_2d_sptr, *input_projdata_sptr, false);
  } else {
  ...
  if (this->override_density_image) {
    info(
        "ScatterEstimation: Over-riding attenuation image! (The file and settings set in the simulation par file are discarded)");
...
ScatterEstimation::set_up_iterative(shared_ptr<IterativeReconstruction<DiscretisedDensity<3, float>>> iterative_object) {
  ...
  if (tmp_atten_projdata_sptr->get_num_segments() > 1) {
    ...
    SSRB(*atten_projdata_2d_sptr, *tmp_atten_projdata_sptr, true);
  } else {
    ...
    if (run_in_2d_projdata) {
      ...
      if (input_projdata_sptr->get_num_segments() > 1) {
        ...
        shared_ptr<ProjData> inv_projdata_3d_sptr =
            create_new_proj_data(out_filename, this->input_projdata_sptr->get_exam_info_sptr(),
        ...
        shared_ptr<ProjData> norm_projdata_2d_sptr =
            create_new_proj_data(out_filename, this->input_projdata_2d_sptr->get_exam_info_sptr(),
                                 this->input_projdata_2d_sptr->get_proj_data_info_sptr()->create_shared_clone());
        ...
        SSRB(*norm_projdata_2d_sptr, *inv_projdata_3d_sptr, false);
  ...
  if (!is_null_ptr(this->back_projdata_sptr)) {
    if (back_projdata_sptr->get_num_segments() > 1) {
      ...
      this->back_projdata_2d_sptr.reset(
          new ProjDataInMemory(this->input_projdata_2d_sptr->get_exam_info_sptr(),
                               this->input_projdata_2d_sptr->get_proj_data_info_sptr()->create_shared_clone()));

      SSRB(*this->back_projdata_2d_sptr, *this->back_projdata_sptr, false);
    } else {
  ...
  if (run_in_2d_projdata) {
    ...
    add_projdata_2d_sptr = create_new_proj_data(out_filename, this->input_projdata_2d_sptr->get_exam_info_sptr(),
                                                this->input_projdata_2d_sptr->get_proj_data_info_sptr()->create_shared_clone());
    ...
    add_projdata_sptr = create_new_proj_data(out_filename, this->input_projdata_sptr->get_exam_info_sptr(),
                                             this->input_projdata_sptr->get_proj_data_info_sptr()->create_shared_clone());
    ...
    data_to_fit_projdata_sptr = create_new_proj_data(out_filename, this->input_projdata_sptr->get_exam_info_sptr(),
                                                     this->input_projdata_sptr->get_proj_data_info_sptr()->create_shared_clone());
...
ScatterEstimation::process_data() {
  ...
  for (int i_scat_iter = 1; i_scat_iter <= this->num_scatter_iterations; i_scat_iter++) {
    ...
    upsample_and_fit_scatter_estimate(*scaled_est_projdata_sptr, *data_to_fit_projdata_sptr, *unscaled_est_projdata_sptr,
                                      *normalisation_factors_sptr, *this->mask_projdata_sptr, local_min_scale_value,
                                      local_max_scale_value, this->half_filter_width, spline_type, true);
    ...
    if (this->export_scatter_estimates_of_each_iteration || i_scat_iter == this->num_scatter_iterations) {
      ...
      if (run_in_2d_projdata) {
        ...
        if (!this->output_scatter_estimate_prefix.empty()) {
          ...
          scatter_estimate_sptr.reset(new ProjDataInterfile(
              this->input_projdata_sptr->get_exam_info_sptr(), this->input_projdata_sptr->get_proj_data_info_sptr(),
              output_scatter_filename, std::ios::in | std::ios::out | std::ios::trunc));
        } else {
        ...
        upsample_and_fit_scatter_estimate(*scatter_estimate_sptr, *this->input_projdata_sptr, *temp_projdata,
                                          *normalisation_factors_3d_sptr, *this->input_projdata_sptr, 1.0f, 1.0f, 1, spline_type,
                                          false);
      } else {
      ...
      if (!this->output_additive_estimate_prefix.empty()) {
        ...
        shared_ptr<ProjData> temp_additive_projdata(new ProjDataInterfile(
            this->input_projdata_sptr->get_exam_info_sptr(), this->input_projdata_sptr->get_proj_data_info_sptr(),
            output_additive_filename, std::ios::in | std::ios::out | std::ios::trunc));
    ...
    iterative_method ? reconstruct_iterative(i_scat_iter, this->current_activity_image_sptr)
...
ScatterEstimation::reconstruct_iterative(int _current_iter_num,
  ...
  if (this->run_debug_mode) {
    ...
    FilePath tmp(convert.str(), false);
    tmp.prepend_directory_name(extras_path.get_path());
    OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(tmp.get_string(), *_current_estimate_sptr);
  }
}

void
ScatterEstimation::reconstruct_analytic(int _current_iter_num, shared_ptr<DiscretisedDensity<3, float>>& _current_estimate_sptr) {
...
ScatterEstimation::project_mask_image() {
  ...
  for (int segment_num = mask_projdata->get_min_segment_num(); segment_num <= mask_projdata->get_max_segment_num();
       ++segment_num) {
  ...
  if (this->mask_projdata_filename.size() > 0)
    this->mask_projdata_sptr.reset(new ProjDataInterfile(mask_projdata->get_exam_info_sptr(),
                                                         mask_projdata->get_proj_data_info_sptr(), this->mask_projdata_filename,
...
shared_ptr<ProjData>
ScatterEstimation::create_new_proj_data(const std::string& filename, const shared_ptr<const ExamInfo> exam_info_sptr,
  ...
  if (run_debug_mode) {
    ...
    tmp.prepend_directory_name(extras_path.get_path());
    pd_sptr.reset(new ProjDataInterfile(exam_info_sptr, proj_data_info_sptr, tmp.get_string(),
                                        std::ios::in | std::ios::out | std::ios::trunc));
  } else {

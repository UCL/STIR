...
ScatterSimulation::ScatterSimulation() { this->set_defaults(); }

ScatterSimulation::~ScatterSimulation() {
  // Sometimes I get a segfault without this line.
  scatt_points_vector.clear();
}

Succeeded
ScatterSimulation::process_data() {
  ...
  {
    if ((*output_proj_data_sptr->get_proj_data_info_sptr()) != (*this->get_template_proj_data_info_sptr()))
  ...
  for (vs_num.segment_num() = this->proj_data_info_cyl_noarc_cor_sptr->get_min_segment_num();
       vs_num.segment_num() <= this->proj_data_info_cyl_noarc_cor_sptr->get_max_segment_num(); ++vs_num.segment_num())
    axial_bins += this->proj_data_info_cyl_noarc_cor_sptr->get_num_axial_poss(vs_num.segment_num());

  const int total_bins = this->proj_data_info_cyl_noarc_cor_sptr->get_num_views() * axial_bins *
  ...
  for (vs_num.segment_num() = this->proj_data_info_cyl_noarc_cor_sptr->get_min_segment_num();
    ...
    for (vs_num.view_num() = this->proj_data_info_cyl_noarc_cor_sptr->get_min_view_num();
         vs_num.view_num() <= this->proj_data_info_cyl_noarc_cor_sptr->get_max_view_num(); ++vs_num.view_num()) {
      total_scatter += this->process_data_for_view_segment_num(vs_num);
      bin_counter += this->proj_data_info_cyl_noarc_cor_sptr->get_num_axial_poss(vs_num.segment_num()) *
                     this->proj_data_info_cyl_noarc_cor_sptr->get_num_tangential_poss();
      /* ////////////////// SCATTER ESTIMATION TIME ////////////////*/
      {
        ...
        info(boost::format(
                 ...
                 ((wall_clock_timer.value() - previous_timer) * (total_bins - bin_counter) / (bin_counter - previous_bin_count) /
                  60),
             /* verbosity level*/ 3);
...
double
ScatterSimulation::process_data_for_view_segment_num(const ViewSegmentNumbers& vs_num) {
...
#  endif
#endif
    viewgram[bin.axial_pos_num()][bin.tangential_pos_num()] = static_cast<float>(scatter_ratio);
...
void
ScatterSimulation::ask_parameters() {
...
ScatterSimulation::initialise_keymap() {
  ...
  this->parser.add_key("template projdata filename", &this->template_proj_data_filename);
  this->parser.add_key("attenuation image filename", &this->density_image_filename);
  this->parser.add_key("attenuation image for scatter points filename", &this->density_image_for_scatter_points_filename);
  this->parser.add_key("zoom XY for attenuation image for scatter points", &this->zoom_xy);
  this->parser.add_key("zoom Z for attenuation image for scatter points", &this->zoom_z);
  this->parser.add_key("XY size of downsampled image for scatter points", &this->zoom_size_xy);
  this->parser.add_key("Z size of downsampled image for scatter points", &this->zoom_size_z);
  this->parser.add_key("attenuation image for scatter points output filename",
                       &this->density_image_for_scatter_points_output_filename);
  this->parser.add_key("downsampled scanner number of detectors per ring", &this->downsample_scanner_dets);
  this->parser.add_key("downsampled scanner number of rings", &this->downsample_scanner_rings);
  this->parser.add_key("activity image filename", &this->activity_image_filename);
  this->parser.add_key("attenuation threshold", &this->attenuation_threshold);
  this->parser.add_key("output filename prefix", &this->output_proj_data_filename);
  this->parser.add_key("downsample scanner", &this->downsample_scanner_bool);
...
bool
ScatterSimulation::post_processing() {
...
*/
#ifndef NDEBUG
  {
    ...
    this->proj_data_info_cyl_noarc_cor_sptr->find_cartesian_coordinates_of_detection(detector_coord_A, detector_coord_B,
                                                                                     Bin(0, 0, 0, 0));
    assert(detector_coord_A.z() == 0);
    assert(detector_coord_B.z() == 0);
    // check that get_m refers to the middle of the scanner
    const float m_first = this->proj_data_info_cyl_noarc_cor_sptr->get_m(
        Bin(0, 0, this->proj_data_info_cyl_noarc_cor_sptr->get_min_axial_pos_num(0), 0));
    const float m_last = this->proj_data_info_cyl_noarc_cor_sptr->get_m(
        Bin(0, 0, this->proj_data_info_cyl_noarc_cor_sptr->get_max_axial_pos_num(0), 0));
...
void
ScatterSimulation::set_activity_image_sptr(const shared_ptr<const DiscretisedDensity<3, float>> arg) {
  if (is_null_ptr(arg))
...
void
ScatterSimulation::set_activity_image(const std::string& filename) {
...
void
ScatterSimulation::set_density_image_sptr(const shared_ptr<const DiscretisedDensity<3, float>> arg) {
  if (is_null_ptr(arg))
...
void
ScatterSimulation::set_density_image(const std::string& filename) {
...
void
ScatterSimulation::set_density_image_for_scatter_points_sptr(shared_ptr<const DiscretisedDensity<3, float>> arg) {
  if (is_null_ptr(arg))
...
ScatterSimulation::get_activity_image() const {
  return *activity_image_sptr;
}

const DiscretisedDensity<3, float>&
ScatterSimulation::get_attenuation_image() const {
  return *density_image_sptr;
}

const DiscretisedDensity<3, float>&
ScatterSimulation::get_attenuation_image_for_scatter_points() const {
  return *density_image_for_scatter_points_sptr;
}

shared_ptr<const DiscretisedDensity<3, float>>
ScatterSimulation::get_density_image_for_scatter_points_sptr() const {
  return density_image_for_scatter_points_sptr;
}

void
ScatterSimulation::set_density_image_for_scatter_points(const std::string& filename) {
...
void
ScatterSimulation::set_image_downsample_factors(float _zoom_xy, float _zoom_z, int _size_zoom_xy, int _size_zoom_z) {
...
void
ScatterSimulation::downsample_density_image_for_scatter_points(float _zoom_xy, float _zoom_z, int _size_xy, int _size_z) {
  ...
  if (_zoom_xy < 0 || _zoom_z < 0) {
    ...
    const float z_length =
        std::max((old_z + 1) * tmp_att.get_voxel_size().z(), (tmpl_density.get_z_size() + 1) * tmpl_density.get_voxel_size().z());
    if (_zoom_z < 0) {
      if (_size_z < 0)
  ...
  shared_ptr<VoxelsOnCartesianGrid<float>> vox_sptr(new VoxelsOnCartesianGrid<float>(
      tmp_att.get_exam_info_sptr(),
      IndexRange3D(0, new_z - 1, -new_y / 2, -new_y / 2 + new_y - 1, -new_x / 2, -new_x / 2 + new_x - 1), tmp_att.get_origin(),
      new_voxel_size));
  ...
  if (this->density_image_for_scatter_points_output_filename.size() > 0)
    OutputFileFormat<DiscretisedDensity<3, float>>::default_sptr()->write_to_file(
        density_image_for_scatter_points_output_filename, *this->density_image_for_scatter_points_sptr);
...
void
ScatterSimulation::set_output_proj_data_sptr(const shared_ptr<const ExamInfo> _exam, const shared_ptr<const ProjDataInfo> _info,
                                             const std::string& filename) {
  if (filename.size() > 0)
    this->output_proj_data_sptr.reset(
        new ProjDataInterfile(_exam, _info, filename, std::ios::in | std::ios::out | std::ios::trunc));
  else
    this->output_proj_data_sptr.reset(new ProjDataInMemory(_exam, _info));
}

shared_ptr<ProjData>
ScatterSimulation::get_output_proj_data_sptr() const {
...
void
ScatterSimulation::set_output_proj_data(const std::string& filename) {
  ...
  if (is_null_ptr(this->template_exam_info_sptr)) {
    ...
    tmp_sptr.reset(new ProjDataInterfile(exam_info_sptr, this->proj_data_info_cyl_noarc_cor_sptr->create_shared_clone(),
...
void
ScatterSimulation::set_output_proj_data_sptr(shared_ptr<ProjData> arg) {
  this->output_proj_data_sptr = arg;
}

shared_ptr<const ProjDataInfoCylindricalNoArcCorr>
ScatterSimulation::get_template_proj_data_info_sptr() const {
...
void
ScatterSimulation::set_template_proj_data_info(const std::string& filename) {
...
ScatterSimulation::set_template_proj_data_info(const ProjDataInfo& arg) {
  ...
  this->total_detectors = this->proj_data_info_cyl_noarc_cor_sptr->get_scanner_ptr()->get_num_rings() *
...
void
ScatterSimulation::set_exam_info(const ExamInfo& arg) {
  this->_already_set_up = false;
  this->template_exam_info_sptr = arg.create_shared_clone();
}

void
ScatterSimulation::set_exam_info_sptr(const shared_ptr<const ExamInfo> arg) {
  this->_already_set_up = false;
  this->template_exam_info_sptr = arg->create_shared_clone();
}
...
ScatterSimulation::downsample_scanner(int new_num_rings, int new_num_dets) {
  ...
  const float approx_num_non_arccorrected_bins = old_scanner_ptr->get_max_num_non_arccorrected_bins() *
  ...
  new_scanner_sptr->set_default_bin_size(new_scanner_sptr->get_effective_ring_radius() * _PI /
                                         new_num_dets); // approx new detector size
  ...
  int delta_ring = proj_data_info_cyl_noarc_cor_sptr->get_num_segments() == 1 ? 0 : new_scanner_sptr->get_num_rings() - 1;

  shared_ptr<ProjDataInfo> templ_proj_data_info_sptr(
      ProjDataInfo::ProjDataInfoCTI(new_scanner_sptr, 1, delta_ring, new_scanner_sptr->get_num_detectors_per_ring() / 2,
                                    new_scanner_sptr->get_max_num_non_arccorrected_bins(), false));
...
void
ScatterSimulation::set_attenuation_threshold(const float arg) {
  attenuation_threshold = arg;
  this->_already_set_up = false;
}

void
ScatterSimulation::set_randomly_place_scatter_points(const bool arg) {
  randomly_place_scatter_points = arg;
  this->_already_set_up = false;
}

void
ScatterSimulation::set_cache_enabled(const bool arg) {
  use_cache = arg;
}

void
ScatterSimulation::write_log(const double simulation_time, const float total_scatter) {
  if (this->output_proj_data_filename.empty())
    return;

  std::string log_filename = this->output_proj_data_filename + ".log";
  ...
  for (int segment_num = this->output_proj_data_sptr->get_min_segment_num();
       segment_num <= this->output_proj_data_sptr->get_max_segment_num(); ++segment_num)
    axial_bins += this->output_proj_data_sptr->get_num_axial_poss(segment_num);

  const int total_bins =
      this->output_proj_data_sptr->get_num_views() * axial_bins * this->output_proj_data_sptr->get_num_tangential_poss();
  mystream << this->parameter_info() << "\nTotal simulation time elapsed: " << simulation_time / 60 << "min"
           ...
           << "\nActivity image SIZE: " << (*this->activity_image_sptr).size() << " * " << (*this->activity_image_sptr)[0].size()
           << " * " // TODO relies on 0 index
           << (*this->activity_image_sptr)[0][0].size()
           << "\nAttenuation image for scatter points SIZE: " << (*this->density_image_for_scatter_points_sptr).size() << " * "
           << (*this->density_image_for_scatter_points_sptr)[0].size() << " * "
           << (*this->density_image_for_scatter_points_sptr)[0][0].size() << "\nTotal bins : " << total_bins << " = "
           << this->output_proj_data_sptr->get_num_views() << " view_bins * " << axial_bins << " axial_bins * "
           << this->output_proj_data_sptr->get_num_tangential_poss() << " tangential_bins\n";
}

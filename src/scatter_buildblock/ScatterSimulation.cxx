...
bool
ScatterSimulation::get_use_cache() const
{
...
ScatterSimulation::process_data()
{
  ...
  const int total_bins
      = this->proj_data_info_sptr->get_num_views() * axial_bins * this->proj_data_info_sptr->get_num_tangential_poss();
...
ScatterSimulation::downsample_density_image_for_scatter_points(float _zoom_xy, float _zoom_z, int _size_xy, int _size_z)
{
  ...
  if (_zoom_xy < 0 || _zoom_z < 0)
    {
      ...
      const float z_length = std::max((old_z + 1) * tmp_att.get_voxel_size().z(),
  ...
  shared_ptr<VoxelsOnCartesianGrid<float>> vox_sptr(new VoxelsOnCartesianGrid<float>(
      ...
      IndexRange3D(0, new_z - 1, -new_y / 2, -new_y / 2 + new_y - 1, -new_x / 2, -new_x / 2 + new_x - 1),
      tmp_att.get_origin(),
      new_voxel_size));
...
void
ScatterSimulation::set_output_proj_data_sptr(const shared_ptr<const ExamInfo> _exam,
  ...
  if (is_null_ptr(this->template_exam_info_sptr))
    {
      ...
      if (filename.empty())
        {
          tmp_sptr.reset(new ProjDataInMemory(exam_info_sptr, this->proj_data_info_sptr->create_shared_clone()));
        }
      else
        {
  ...
  else
    {
      if (filename.empty())
        {
          tmp_sptr.reset(new ProjDataInMemory(this->template_exam_info_sptr, this->proj_data_info_sptr->create_shared_clone()));
        }
      else
        {
...
void ScatterSimulation::set_template_proj_data_info(const ProjDataInfo& arg)
{
...
ScatterSimulation::set_template_proj_data_info(shared_ptr<const ProjDataInfo> arg)
{
  ...
  this->total_detectors = this->proj_data_info_sptr->get_scanner_ptr()->get_num_rings()
...
bool
ScatterSimulation::get_downsample_scanner_bool() const
{
  return this->downsample_scanner_bool;
}

void
ScatterSimulation::set_downsample_scanner_bool(const bool arg)
{
...
int
ScatterSimulation::get_num_downsample_scanner_rings() const
{
  return this->downsample_scanner_rings;
}

void
ScatterSimulation::set_num_downsample_scanner_rings(const int arg)
{
...
int
ScatterSimulation::get_num_downsample_scanner_dets() const
{
  return this->downsample_scanner_dets;
}

void
ScatterSimulation::set_num_downsample_scanner_dets(const int arg)
{
...
ScatterSimulation::downsample_scanner(int new_num_rings, int new_num_dets)
{
  ...
  int delta_ring = proj_data_info_sptr->get_num_segments() == 1 ? 0 : new_scanner_sptr->get_num_rings() - 1;

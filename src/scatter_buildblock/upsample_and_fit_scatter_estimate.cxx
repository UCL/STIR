                                  ...
                                  const bool do_3D)
{
    ...
    shared_ptr<ProjDataInfo>
      interpolated_scatter_proj_data_info_sptr(emission_proj_data.get_proj_data_info_sptr()->clone());

    info("upsample_and_fit_scatter_estimate: Interpolating scatter estimate to size of emission data");
    shared_ptr<ProjDataInMemory> interpolated_scatter( new ProjDataInMemory(emission_proj_data.get_exam_info_sptr(),
                                                                            interpolated_scatter_proj_data_info_sptr));
    ...
    {
        warning("upsample_and_fit_scatter_estimate: forcing remove_interleaving to false as non-cylindrical projdata, or trying 3D scatter estimation");
    ...
    if (emission_proj_data.get_proj_data_info_sptr()->get_scanner_sptr()->get_scanner_geometry()=="Cylindrical" && !do_3D)
    {
        ...
        interpolate_projdata(*interpolated_scatter, scatter_proj_data,
                             spline_type, actual_remove_interleaving);
    }
    else
        interpolate_projdata_3d(interpolated_scatter, scatter_proj_data,
                              spline_type);
    ...
    if (min_scale_factor != 1 || max_scale_factor != 1 || !scatter_normalisation.is_trivial())
    {
        ...
        else
        {
            ...
            scale_factors = get_scale_factors_per_sinogram(
                        emission_proj_data,
                        *_interpolated_scatter,
                        weights_proj_data);

            info(format("upsample_and_fit_scatter_estimate: scale factors before thresholding: {}", scale_factors), 2);
            ...
            threshold_upper(scale_factors.begin_all(),
                            scale_factors.end_all(),
                            max_scale_factor);
            info(format("upsample_and_fit_scatter_estimate: scale factors after thresholding: {}", scale_factors), 2);
            ...
            std::for_each(scale_factors.begin(),
                          scale_factors.end(),
                          lowpass_filter);
            info(format("upsample_and_fit_scatter_estimate: scale factors after filtering: {}", scale_factors),2);
        }

...
\brief   
  ...
  \author Kris Thielemans
	...
	$Revision$
	  ...
	  \par Usage:
	  \code
	  estimate_single_scatter_splitted                [activity_image]
	                                                  [attenuation_image]
	                                                  [attenuation_image_low]
							  [proj_data_filename]
	  ...
	  \param resolution defaults to 22% (BGO)
*/
/*
...
{         
	...
	float attenuation_threshold = argc>=7 ? atof(argv[6]) : 0.05 ;
	const float lower_energy_threshold = argc>=8 ? atof(argv[7]) : 350 ;
	const float upper_energy_threshold = argc>=9 ? atof(argv[8]) : 650 ;
	int scatt_points = argc>=10 ? atoi(argv[9]) : 1000 ;
	bool random = true;
	...
	const DiscretisedDensityOnCartesianGrid<3,float>& density_image_low = 
		dynamic_cast<const DiscretisedDensityOnCartesianGrid<3,float>&  > 
		(*density_image_sptr_low.get());

    string output_proj_data_filename(argv[5]);
    string output_proj_data_filename2(argv[5]);
    output_proj_data_filename += "_0_2";
    output_proj_data_filename2 += "_1_1";

		
	ProjDataInterfile output_proj_data(proj_data_info_ptr->clone(),output_proj_data_filename);
	ProjDataInterfile output_proj_data2(proj_data_info_ptr->clone(),output_proj_data_filename2);	
	...
	scatter_viewgram_splitted(output_proj_data,output_proj_data2,
		activity_image, density_image, density_image_low,

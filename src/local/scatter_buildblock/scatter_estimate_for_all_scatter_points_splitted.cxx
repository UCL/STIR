...
{	
  ...
  const VoxelsOnCartesianGrid<float>& smooth_image =
    dynamic_cast<const VoxelsOnCartesianGrid<float>&>(smooth_image_as_density);
  const CartesianCoordinate3D<float> voxel_size = smooth_image.get_voxel_size();
  ...
  for(std::size_t scatter_point_num =0;
	...
	for(std::size_t scatter_point_2_num =0;
	    ...
	    ++scatter_point_2_num)			
	  {
		...
		scatter_estimate_for_two_scatter_points_splitted(
  ...
  return 
    ...
    (single_scatter_ratio+
     double_scatter_ratio*scatter_volume*4./(1000000.)
     /total_cross_section_511keV)/2./_PI

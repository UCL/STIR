...
{	
    ...
    emiss_integral_to_detA
	*exp(-atten_integral_to_detA)
	*exp(-atten_integral_to_detB) 
	*scatter_point_mue
	*dif_cross_section(scatter_point,detector_coord_A,detector_coord_B,511)
	/(total_cross_section(511)*rA*rA); 
	// ( in v2 ->* efficiencies, for the E')

	const float scatter_point_to_detB_ratio=
	emiss_integral_to_detB
	*exp(-atten_integral_to_detB)
	*exp(-atten_integral_to_detA) 
	*scatter_point_mue
	*dif_cross_section(scatter_point,detector_coord_B,detector_coord_A,511)

...
Array<2,float>
scale_factors_per_sinogram(const shared_ptr<ProjData> & emission_proj_data_sptr, 
	    ...
	    ) 
{
	...
	++bin.segment_num())		
			...
			++bin.axial_pos_num())
			{
				const Sinogram<float> scatter_sinogram = scatter_proj_data_sptr->get_sinogram(
					bin.axial_pos_num(),bin.segment_num());
				const Sinogram<float> emission_sinogram = emission_proj_data_sptr->get_sinogram(
					bin.axial_pos_num(),bin.segment_num());

...
float estimate_scale_factor(const shared_ptr<ProjData> & no_scatter_proj_data_sptr, 
		...
		const float attenuation_threshold) 
{
	...
	++bin.segment_num())
		...
		++bin.view_num())
		{
			...
			++bin.axial_pos_num())
				...
				{  	
					...
					{
						total_outside_scatter += scatter_viewgram[bin.axial_pos_num()][bin.tangential_pos_num()] ;					
					    total_outside_no_scatter += no_scatter_viewgram[bin.axial_pos_num()][bin.tangential_pos_num()] ;					
					}
		...
		bin_timer.stop();
		return total_outside_no_scatter/total_outside_scatter;

...
set_up_output_format(int argc, char* argv[])
{
  shared_ptr<OutputFileFormat<DynamicDiscretisedDensity>> output = OutputFileFormat<DynamicDiscretisedDensity>::default_sptr();
...
main(int argc, char* argv[])
{
  ...
  if (patlak_plot.set_up() == Succeeded::no)
    ...
    {
      shared_ptr<ParametricVoxelsOnCartesianGrid> par_image_sptr(ParametricVoxelsOnCartesianGrid::read_from_file(argv[2]));
      ...
      if (file)
        {
...
#if 1
          shared_ptr<DynamicDiscretisedDensity> dyn_image_sptr(read_from_file<DynamicDiscretisedDensity>(argv[1]));
          ...
          const shared_ptr<DiscretisedDensity<3, float>> density_template_sptr(
              (par_image_sptr->construct_single_density(1)).clone());
          DynamicDiscretisedDensity dyn_image
              = DynamicDiscretisedDensity(patlak_plot.get_time_frame_definitions(), scanner_sptr, density_template_sptr);
#endif
          // ToDo: Assertion for the dyn-par images, sizes I have to create from one to the other image, so then it should be
          // OK...
      ...
      // If the output file doesn't exist, get all the info we need
      else
        {
          ...
          dyn_image = DynamicDiscretisedDensity(tdefs, time_since_1970, scanner_sptr, voxels_sptr);
        }
      ...
      shared_ptr<OutputFileFormat<DynamicDiscretisedDensity>> output_file_format = set_up_output_format(argc, argv);

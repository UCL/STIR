...
to use for STIR. You will want to read {[}Thi12] and {[}Fus13] for SPECT additions
...
Note that as DICOM files often store only a single slice, DICOM is currently not very well supported by \textit{STIR}.
...
This is a flexible implementation specific to SPECT data. See [Fus13] for some information.
Actual parameters are currently described in a separate document. A sample is given below

\begin{verbatim}
ction Matrix By Bin SPECT UB Parameters:=

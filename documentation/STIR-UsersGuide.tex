...
\subsubsection{JSON}
The \texttt{HUToMu} functionality parses a file in JSON format. We use the
...
The \texttt{Radionuclide} functionality also parses files in JSON format. 
To use and install these files the variable texttt{STIR_CONFIG_DIR} needs to be set via cmake. 
This allows you to read JSON files that you may have modified and install them in the config folder.
The default value is set to \textt{STIR/src/config/} and will be installed in your install folder  in \textt{share/stir/config}
...
{\raggedright Contains the location of the \textit{STIR} configuration folder. There you can find for example a JSON file used for the estimation of attenuation factors from a CT image.}\\
\hline
\end{longtable}
...
(If you do not want this, set it to \textt{User_defined_scanner} and specify all values).
...
\URL{https://github.com/gschramm/parallelproj}. This project provides implementations

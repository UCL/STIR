...
\section{
  Python interface and utilities}

  \textit{STIR} can be built with a Python interface. This is automatically generated via
  \url{https://swig.org/}{SWIG}. This means that the interface has a bit of a C++ feeling
  unfortunately\footnote{This could be improved by using SWIG's typemaps.}. In particular, keyword arguments are (sadly) not supported. Documentation
  is also underwhelming. The docstrings are generated from the doxygen comments and therefore
  often refer to C++ classes/notation etc. Help welcome! Nevertheless, the Python interface
  is very useful, and just as powerful as the C++ functionality (by construction).

  There are some examples in the \texttt{examples/python} folder that should be helpful.
  We also have a separate repository \url{https://github.com/UCL/STIR-exercises}{STIR-exercises}
  which is gradually being updated (many of the exercises still use bash scripts).
  You could even explore the tests in \texttt{src/swig/test/python}, although they are not
  designed for readability\footnote{These test only basic functionality.  Of course, the underlying
  functionality is the same as in C++, which is tested quite heavily.}.

  Some of the examples are installed (since STIR 6.4) and feel like ``part of STIR". However,
  they can be a bit rough, and probably need reading of the python files before you can use them properly.
  Here is an example (\texttt{projdata\_visualisation} needs PyQt5).
  \begin{verbatim}
import os
import stir
...
# copy to numpy
proj_data_array = proj_data.as_array()
  \end{verbatim}
...
[Kyr2025]
Kyriakopoulou, D., A. Fokas, K. Thielemans \textbf{2025}, submitted to \texttt{Inverse Problems}.

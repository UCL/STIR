...
the rest of this document). Unfortunately, this library is no longer
...
At present, STIR uses parts of the Louvain la Neuve ecat library, see \ref{sec:ECAT7support}. 
...
We do have software taht read VOLPET headers for all GE scanners, but can currently
...
\textbf{Warning} Due to license problems, the ECAT6 code is no longer distributed
with STIR. This might be fixable by using the LLN library instead, but this
will require external contributions. Please check the STIR mailing lists.
...
The current implementation is stable, except maybe for the parameters 
which might change. A sample.par file can be found in the \textbf{samples/} 

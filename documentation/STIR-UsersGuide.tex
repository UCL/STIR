...
The distribution contains a sample file STIR/samples/local/config.mk, 
...
You need Visual C++ 6.0, service pack 5. Oliver Nix (Heidelberg) 
has done a few preliminary tests that indicate that 7.0 also 
works. Version 7.1 (i.e. 2003) seems to be fine as well. Note 
however that since release 1.3, certain features do not work 
with versions prior to 7.1.


There are two different ways to build using Visual C++. Both 
are somewhat fiddly and largely unsupported (but so is the rest 
of STIR).
...
When opening the workpackages the first time, Visual Studio will 
not find 2 projects called local\_buildblock and local\_recon\_buildblock. 
You will have to delete these manually from the workspace. In 
addition, if you want ECAT7 support, you will need to tell Visual 
...
{ \subsubsubsection{Sensitivity image pre-computation}
}
...
precorrected before-hand. In particular, only arc-corrected data 
are supported.\\
...
be found in the STIR/samples directory.
...
{ \subsubsubsection{Chained Image Processor}
}
...
different types of projectors should be available in STIR/samples.\\
...
\item
More algorithms: 3DRP, OSSPS (potentially ART, FORE [Def95], 
OSCB [Ben99b] )
\item
More projectors
\item

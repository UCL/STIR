...
3.0.1. The final release of \textit{STIR} 2.0 should work with gcc 4.1.
...
\textit{STIR} 2.0 contains somewhat preliminary code for running \texttt{OSMAPOSL} in parallel-mode
...
\textit{STIR} 2.0 contains preliminary code for running \texttt{FBP2D} in threaded mode
...
available via the Louvain la Neuve website. Please check the \textit{STIR}
...
directly, this is currently not supported by \textit{STIR}. 
...
in conjunction with the \textit{LMF} library.
...
sensitivity will be written to that filename. This parameter might be useful
to avoid recomputing the sensitivity if it does not change.

\item[use subset sensitivities]
Defaults to 0. Currently, changing it to 1 needs \textit{recompute sensitivity} to be 1 as well.
...
We therefore recommend to set this parameter to 1.
...
A parametric image class (ParametricDiscretisedDensity) has been implemented that at 
the moment can hold two parameters, but it could be extended 
to account for more cases. In this release Patlak plot [Pat83, Pat85], i.e. a 
linear kinetic model for tracers with irreversible kinetic behavior has been implemented. 
Moreover, a direct version of Patlak plot has been also implemented and evaluated [Tso08]. 
...
\end{verbatim}
\begin{description}
\item[blood data filename] 
        File of input function (e.g. plasma activity or reference tissue activity values)
\item[Calibration Factor] scale factor that sets the projection data to the same units with the input function
\item[Starting frame] from where the Patlak Plot is valid
\item[time shift]
        Delay between input function and PET measurement 
\item[In total counts]
        is 0 or 1 depending if the counts are total or mean over each frame 
\item[In correct scale] 
        should be set to 1 if STIR has already scaled correctly the images according to x voxel size
\end{description}

Current assumptions are that a tracer is used with the F-18 isotope and that the input
function is not decay corrected. 
...
\item
Better dynamic/gated scans support.
\end{itemize}
...
More priors
\item
Extending the parallelisation of OSMAPOSL to OSSPS, FBP3DRP etc, or using OPEMMP
\item
...
[Bei08] Tobias Beisel, Stefan Lietsch, and Kris Thielemans \textbf{(2008)}
...
{[}Tso07{]} Tsoumpas C, Turkheimer F, Thielemans K  \textbf{(2007)}, Convergence properties of algorithms for direct parametric estimation of linear models in dynamic PET. \textit{IEEE Nucl Sci Symp Med Imaging Conf} 3034-3037.
{[}Tso08{]} Tsoumpas C, Turkheimer F E, Thielemans K \textbf{(2008)}, Study of direct and indirect parametric estimation methods of linear models in dynamic positron emission tomography. \textit{Med. Phys.} 35(4): p. 1299-1309.
{[}Pat83] Patlak C S, Blasberg R G, Fenstermacher J D \textbf{(1985)}  Graphical evaluation of blood-to-brain transfer constants from multiple-time uptake data. \textit{J Cereb Blood Flow Metab} 3(1): p. 1-7.
{[}Pat85] Patlak C S, Blasberg R G \textbf{(1985)}  Experimental and Graphical evaluation of blood-to-brain transfer constant from multiple-time uptake data: Generalizations. \textit{J Cereb Blood Flow Metab} 5: p. 584-90. 
\end{document}

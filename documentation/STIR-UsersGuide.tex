...
3.0.1. The final release of \textit{STIR} 2.2 should work with gcc 4.2 up to 4.6.
...
version 11 and CLang (checked with 2.9.1).
We would love 
...
Since version 2.1, \textit{STIR} contains code for running \texttt{OSMAPOSL} and \texttt{OSSPS} in parallel-mode
...
Studio Express versions are free. Version 7.1 (i.e. 2003) might still be fine. \textit{STIR} 2.2
should normally compile with Visual Studio 2008 and 2010.

There are three different ways to build \textit{STIR} using Visual C++. The recommended way (since 2.2) is
to use \textit{CMake}, see section \ref{sec:UsingCMake}. If you do not want to use \textit{CMake}, we provide
two alternatives, but both 
are fiddly. Before we go there, we have to got a problem out-of-the-way.
...
\textbf{Currently there might be a linking problem with some of the utilities using
...
\subsubsection{
Alternative building process: using CMake}
\label{sec:UsingCMake}
Since version 2.2, \textit{STIR} contains files to build using the platform-independent
\R2Lurl{"http://www.cmake.org"}{CMake}. This is much better for configuration and finding 
system dependencies.
...
\subsubsection{
If you have used the distributed Visual Studio projects for building}


This is only partially supported. There is a \texttt{test} ``solution'' which you need to build first. You can run
the projects then by hand. We also provide \textbf{STIR/test/run\_tests.bat} for running the automatic 
tests. This will currently not run any tests in \textbf{STIR/recon\_test} though.

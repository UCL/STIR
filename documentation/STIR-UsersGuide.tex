...
scatter for the downsampled scanner, and needs to be followed by upsampling.
...
; Caching (of the line integrals) speeds up the estimation drastically.
   ...
   ; if the above is 1, let STIR do the downsampling.
   ; This is safer than doing it yourself, physical size will be preserved etc.
   ; These settings try to sensible defaults
...
template.
\item You can construct a ``subsampled" attenuation image using \texttt{zoom\_image}. 
 STIR SSS will select 1 scatter point per voxel in this attenuation image.\\
 Careful: \texttt{zoom\_image} will by default scale the voxel values.    
 For example
 \begin{verbatim}
  # zoom to larger voxels 
  zoom_image --scaling preserve-values zoomed.hv image_mu.hv 21 .25 0 0 5 .1626
 \end{verbatim}
...
scatter simulation parameters file := ${scatter_pardir}/scatter_simulation.par 
; next option is the default
use scanner downsampling in scatter simulation := 1

reconstruction parameter template file := ${scatter_pardir}/run_reconstruction.par 
...
tail fitting par filename := ${scatter_pardir}/tail_fitting.par
...
attenuation image). Alternatively, you can pass a file such that it will find the tails itself. Parameters are
similar to what'sdescribed below.
\cmdline{%
...
After tail-fitting, the scale factors can be thresholded (if there would be no out-of-FOV
nor multiple scatter, the scale factors should be around $1$).

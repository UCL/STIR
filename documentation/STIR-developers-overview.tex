...
using the \textit{doxygen} program (\R2Lurl{https://www.doxygen.org/ }{https://www.doxygen.org/}). 
...
STIR uses a ``gantry-based'' coordinate system, which is left-handed.

\textbf{WARNING:} This text is correct for PET data. For SPECT, we currently have problems with the image orientation, caused
but the fact that SPECT DICOM raw data follow the patient position (not the gantry). Therefore, LPS info etc is currently
\textbf{incorrect} for SPECT reconstructed images.
...
The ``highest" TOF bin corresponds to voxels closer to the ``first" detector
(as encoded for instance in \texttt{stir::DetectionPositionPair}).

The Python example \texttt{coordinates\_vs\_bins.py} could be useful to clarify these conventions.
...
\subsection{
Documentation}
Most documentation is generated via in-code comments using \textit{doxygen}. There are some overview \LaTeX
files in the \texttt{documentation} folder, such as this one.

Documentation of a class and its methods generally works as follows
\begin{verbatim}
/*!
...
\end{verbatim}
where both styles of ``brief'' description are acceptable.

Similar styles are used for functions, constants and preprocessor defines. There should also be
a header in each file assigning it to a doxygen group.

Doxygen groups have to be defined. This is currently done in \texttt{src/include/stir/doxygengroups.h}.

Doxygen comments are also parsed to generate the SWIG documentation. This is currently done via
\R2Lurl{https://github.com/m7thon/doxy2swig}{doxy2swig}, although we hope to transition to SWIG's
native doxygen support, once its doxygen parser improves.

\subsection{
Class definitions}

Classes should be defined in their onwn \textit{.h} file (except in implementation classes, which ideally
are in the \texttt{detail} namespace). They follow generally the following format (ignoring \textit{doxygen} 
comments).
...
\texttt{a()} calls inline member function \texttt{b()}, 
then \texttt{b} should be defined first. (Otherwise very 
few compilers are able to inline the call to \texttt{b}
from \texttt{a}.)
...
compiled when the \textit{NDEBUG} preprocessor macro is not defined (for instance,
...
# Declare dependencies on other STIR libraries
target_link_libraries($(dir) PUBLIC ${STIR_LIBRARIES})
# Alternatively, only depend on a specific STIR libraries (NOT recommended as their names will change).
# For instance to depend only on the "normal" STIR buildblock
target_link_libraries($(dir) PUBLIC ${STIR_BUILDBLOCK_LIB})
\end{verbatim}

...
typedef struct
{
    ...
    string shape;  // hole shape { rect, round }
...
typedef struct
{
    string model;      // cylindrical (cyl) or polygonal prism (pol)
    ...
    vector <hole_type> holes;  //array hole_type structure
...
typedef struct
{
    ...
    vector<int> who;  // array of indices of holes projecting to this detel
...
typedef struct  
{
	...
	string att_fn;       // attenuation map filename
	string msk_fn;       // explicit mask filename
    string detector_fn;  // ring parameter filename
    string collim_fn;    // collimator parameter filename
	
	volume_type vol;           // structure with information of volume
    vector<detel_type> detel;  // structure with detection elements information
...
typedef struct   
{
	...
	string fn;      // matrix name
	string fn_hdr;  // matrix header file name
...
void error_wmtools_SPECT_mph( int nerr, int ip, string txt );    // error messages in wm_SPECT
